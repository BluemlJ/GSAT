...
public class StringTools {
  ...
  /**
   * Takes an input string and appends spaces to the left. Ignores any existing leading whitespace
   * when counting the indent size.
   * 
  ...
  /**
   * Word-wraps a string into an array of lines of no more than the given width. The string is split
   * into chunks using the regex supplied to identify the points where it can be broken. If a word
   * is longer than the width required, it is broken mid-word, otherwise the string is always broken
   * between words.
   * 
   * @param input the string to format
   * @param sepRegex the regex identifying the break points in the string, to be compiled using
   *        Pattern.
    ...
    while (start < input.length()) {
      ...
      // hard case, have to split it!
        ...
        while (splitPoint >= start) {
          ...
          if (m.matches()) {
            splitPoint += 1;// splitpoint is index of separator - include on this line - assumes a
      ...
      // trailing blanks - which may include the separator - are not in genbank lines - so they are
      // removed
      // output chunk from start to splitPoint - do not include trailing newline - it will be added
      // by writeKeyValueLine
      lines.add(trimTrailingBlanks(newline == splitPoint - 1
          ? input.substring(start, splitPoint - 1)
          : input.substring(start, splitPoint)));
  ...
  /**
   * Writes some text to the output stream in the following format: key text continuation of text
   * where the key/wrappedKey column is keyWidth wide, and the total line width is lineWidth, and
   * the text is split over multiple lines at the nearest occurrence of whitespace.
   * 
   ...
   */
  public static void writeKeyValueLine(String key, String text, int keyWidth, int lineWidth,
      PrintStream os) {
   ...
   * Writes some text to the output stream in the following format: key text continuation of text
   * where the key/wrappedKey column is keyWidth wide, and the total line width is lineWidth, and
   * the text is split over multiple lines at the nearest occurrence of separator sep.
   * 
   * @param key the key to write on the first line only
   * @param text the text to write out
   * @param keyWidth the width to indent the text by (in which the key will be printed)
   ...
   */
  public static void writeKeyValueLine(String key, String text, int keyWidth, int lineWidth,
      String sep, PrintStream os) {
    writeKeyValueLine(key, text, keyWidth, lineWidth, sep, null, os);
  }

  /**
   * Writes some text to the output stream in the following format: key text wrappedKey continuation
   * of text where the key/wrappedKey column is keyWidth wide, and the total line width is
   * lineWidth, and the text is split over multiple lines at the nearest occurrence of separator
   * sep.
   * 
   * @param key the key to write on the first line only
   * @param text the text to write out
   ...
   */
  public static void writeKeyValueLine(String key, String text, int keyWidth, int lineWidth,
      String sep, String wrappedKey, PrintStream os) {
    ...
    // text = text.trim(); // trim leading/trailing whitespace from text - this deletes leading
    // blank lines from comments: e.g. AC140936
