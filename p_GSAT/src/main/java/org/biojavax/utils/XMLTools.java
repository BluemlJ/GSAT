...
public class XMLTools {
  ...
  /**
   * Attempts to read XML file in chunks, passing each chunk to a SAX parser. As each chunk is read
   * into memory in a buffer, you need to ensure that each chunk is small enough to fit into
   * available memory. Only one chunk is held in memory at any one time, and then only long enough
   * for it to be parsed. When checking for the presence of further chunks, it'll only read up to
   * 1000 chars further into the file, after which results will be unpredictable.
   * 
   ...
   *        &lt;chunkToken and will buffer all content, including the opening tag and up to and
   *        including the closing &lt;/chunkToken> tag. It will not currently handle
   *        &lt;chunkToken/> instances, nor instances where more than one tag appears per line, or
   *        extra spaces appear between the angle brackets, slashes, and tag name of the tag we are
   *        searching for.
   ...
   */
  public static boolean readXMLChunk(BufferedReader reader, DefaultHandler m_handler,
      String chunkToken) throws ParserConfigurationException, SAXException, IOException {
    ...
    while (!filledBuffer && (line = reader.readLine()) != null) {
      line = line.trim();
      if (!begunChunk && !start.matcher(line).matches())
        continue;
      else
        begunChunk = true;
      buffer.append(line + "\n");
