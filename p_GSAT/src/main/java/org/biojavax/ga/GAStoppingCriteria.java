...
/**
 * Used by a <code>GeneticAlgorithm.run()</code> method to determine when the algorithm should stop
 * </p>
 * 
...
public interface GAStoppingCriteria {
  ...
  /**
   * Simple Implementation of GAStoppingCriteria, signals a <code>GeneticAlgorithm</code> to stop
   * after n generations
   * </p>
   *
   * Useful for pausing and seeing what the state of the algorithm is at any particular time and
   * possibly changing parameters etc before calling the run() method of the
