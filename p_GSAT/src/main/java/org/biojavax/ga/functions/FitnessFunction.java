...
/**
 * Calculates the fitness of an <code>Organism</code> in a <code>Population</code> of
 * <code>Organisms</code>
 *
...
public interface FitnessFunction {
  ...
  /**
   * Calculates the fitness of <code>org</code>. This can be done independently of the Population
   * pop (by ignoring the argument in your implementation) or dependent on the other members of the
   * <code>Population pop</code>. Every implementation of this function should assign the fitness
   * value computed in this function to the given organism. This is important so that the organism
   * knows its current fitness. Note that for simple problems this fitness array will contain only
   * one single value. However, to enable multi-objective fitness functions this has to be an array.
   *
