...
/**
 * Crosses two chromosomes. The basic usage of the class would be something like choosing two
 * chromosomes that you want to cross over and setting these with the <code>setChromosomePair</code>
 * method. Next you would call one of the <code>performCrossOver</code> methods to do the crossing
 * and finally you would retreive the chromsome pair with the <code>getChromosomes</code> method.
 *
...
public interface CrossOverFunction extends Changeable {
  ...
  /**
   * Sets an upper limit on the number of crosses. Its up to the implementation to decide what to do
   * when the limit is reached although a good convention would be to keep only the first N crosses
   * from the left end (5' end) of the sequence.
   *
   * By convention the default upper limit is DEFAULT_MAX_CROSS (eg infinite). This value should be
   * used as the default by all implementations.
   *
  ...
  /**
   * Sets the probability of crossing at each base. Each position in the array corresponds to a
   * position in the sequences to be crossed.
   *
   * The probability of a cross occuring <em>after</em> position 1 in the <code>SymbolList</code> is
   * given by <code>crossOverProbs[1]</code>. <code>CrossOverProbs[0]</code> is effectively
   * redundant as the cross would occur before the 1st position in the <code>SymbolList</code>.
   *
   * By convention if the array is shorter than the SymbolList it is being applied to then the last
   * value in the array will be applied to every subsequent residue.
   *
   * The default value in all implementations should be <code>DEFAULT_CROSS_PROB</code>
   *
   * @param crossOverProbs an array of doubles giving the probability of a cross occuring at any
   *        place.
   *
  ...
  public final class NoCross implements CrossOverFunction {
    ...
    public GACrossResult performCrossOver(SymbolList chromA, SymbolList chromB) {
      return new SimpleGACrossResult(new PointLocation[] {}, new SymbolList[] {chromA, chromB});
    }
