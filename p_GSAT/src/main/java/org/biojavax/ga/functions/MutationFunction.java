...
/**
 * <p>
 * A class that mutates a <code>SymbolList</code>
 * </p>
 * 
...
public interface MutationFunction extends Changeable {
  ...
  /**
   * Produces a new SymbolList by mutation. Each position <em>i</em> in the SymbolList
   * <code>seq</code> is mutated with probability <code>getMutationProbs[i]</code>. The new residue
   * is selected at random from the <code>Distribution mutation</code>. The use of an array of
   * probabilities allows the modelling of mutational hotspots. Position 0 in the array corresponds
   * to the probability of the first residue of <code>seq</code> mutating.
   *
   * If the length of the array defined in <code>getMutationProbs()</code> is shorter than the
   * length of the sequence the default behaivour of implementations will be to apply the last
   * probability to each subsequence residue. A single member array will mutate all bases with equal
   * probability.
   *
  ...
  /**
   * Set the probability of a mutation occuring at a certain position Position 0 in the array
   * corresponds to the probability of the first residue of <code>seq</code> mutating.
   *
   * If the length of the array defined in <code>getMutationProbs()</code> is shorter than the
   * length of the sequence the default behaivour of implementations will be to apply the last
   * probability to each subsequence residue. A single member array will mutate all bases with equal
  ...
  /**
   * Sets the <code>Distribution</code> of <code>Symbols</code> that will be selected from when a
   * mutation occurs. An <code>OrderNDistribution</code> is used so that you can model a situation
   * where the identity of the 'mutant' <code>Symbol</code> is dependent on the original
   * <code>Symbol</code>. The primary use is to prevent <code>Symbols</code> mutating to themselves.
   * Another use would be to model transitions and transversions.
   *
   * @param mutationSpectrum the Distribution of 'mutant' bases to choose from.
   * @throws ChangeVetoException if a ChangeListener vetoes the change.
   */
  public void setMutationSpectrum(org.biojava.bio.dist.OrderNDistribution mutationSpectrum)
