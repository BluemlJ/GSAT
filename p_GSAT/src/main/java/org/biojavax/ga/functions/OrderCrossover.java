 ...
 * The method is commonly named OX - operator
 *
 * @author Susanne Merz
 */
public class OrderCrossover extends AbstractCrossOverFunction {
  ...
  /**
   * Sets the maximal number of crossover points to two and the crossover probability to 0.5 and
   ...
   *
   * @see
   * org.biojavax.ga.functions.CrossOverFunction#performCrossOver(org.biojava.bio.symbol.SymbolList,
  ...
  public GACrossResult performCrossOver(SymbolList chromA, SymbolList chromB)
    ...
    if (crosslocations[1] != 0.0) {
      ...
      PointLocation[] points =
          {new PointLocation(crosslocations[0]), new PointLocation(crosslocations[1])};
      GACrossResult result = new SimpleGACrossResult(points, new SymbolList[] {chromA, chromB});
  ...
  /**
   * generate a pair of locations where the crossover should be made do this in two steps for
   * retracabilities sake. the location array will always list the earlier position first.
   *
   * @param chromlength the lenghth of the chromosomes, needed to decide if the probabilityarray
