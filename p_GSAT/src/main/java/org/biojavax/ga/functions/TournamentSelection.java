 ...
 * Currently it assumes a maximization problem. Perhaps this could be selected depending on the
 * Genetic Algorithm utilized.
 *
 * @author Susanne Merz
 */
...
public class TournamentSelection implements SelectionFunction {
   ...
   *
   * @param numberOfIndividuals the number of Individuals the best is selected from, ranges from 1
  ...
  /**
   * Standard call to select organisms, will select a number of Organisms corresponding to 75 % of
   * the population.
   *
   * @see org.biojavax.ga.functions.SelectionFunction#select(org.biojavax.ga.Population,
  ...
  /**
   * This method selects n Organism from the population it is given, using the tournament selection
   * method
   *
   * @param pop the population to select from
   * @param ga the <code>GeneticAlgorithm</code> this selection belongs to
   * @param n number of individuals to be selected.
   * @return nextgen a <code>Population</code> containing the selected Organisms
   */
