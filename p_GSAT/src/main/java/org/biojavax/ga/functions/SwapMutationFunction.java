...
/**
 * This class does a sort of mutation by exchanging two positions on the chromosome. Thus it can be
 * used for implementations where a change of the amount of one symbol is undesired, e.g. some TSP
 * implementations
 *
 * @author Susanne Merz
 */
public class SwapMutationFunction extends AbstractMutationFunction {
   ...
   *
   * @param probabilities An array, which contains the mutation probabilities.
   */
  ...
  /**
   * generate an array of two positions on the chromosome that get the highest values in a random
   * choosing relative to their mutationProbability
   *
   * @param seqLength the legth of the sequence used
