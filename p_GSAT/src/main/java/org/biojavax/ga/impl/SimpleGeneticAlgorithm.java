...
/**
 * A simple implementation of the <code>GeneticAlgorithm</code> interface it is not intended that
 * this class be overidden, hence it is final. It is much better to overide
 * <code>AbstractGeneticAlgorithm</code>.
 *
...
public final class SimpleGeneticAlgorithm extends AbstractGeneticAlgorithm {
  ...
  public SimpleGeneticAlgorithm(Population pop, MutationFunction mutFunc, CrossOverFunction xFunc,
  ...
  /**
   * Get a List containing details of all the cross over events during the run. If
   * <code>run(GAStoppingCriteria stoppingCriteria) has not yet been called
   * the list will be empty</code>. This implementation only stores a buffer of the last 100 crosses
   * for memory reasons.
   *
        ...
        if (it.hasNext()) {
          ...
          try {
            for (int i = 0; i < a.getChromosomes().length && i < b.getChromosomes().length; i++) {
              ...
              crossResults.addLast(getCrossOverFunction().performCrossOver(a.getChromosomes()[i],
                  b.getChromosomes()[i]));
