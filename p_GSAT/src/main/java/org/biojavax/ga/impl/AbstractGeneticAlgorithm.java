...
public abstract class AbstractGeneticAlgorithm extends AbstractChangeable
  ...
  /**
   * Assigns a fitness value to each organism within the population according to the currently set
   * fitness function. If no population or no fitness function is set, nothing will happen.
   */
   ...
   *
   * @see
   * org.biojavax.ga.GeneticAlgorithm#setFitnessFunction(org.biojavax.ga.functions.FitnessFunction)
   */
  public final void setFitnessFunction(FitnessFunction func) throws ChangeVetoException {
    if (!hasListeners()) {
      ...
      ChangeEvent ce = new ChangeEvent(this, GeneticAlgorithm.FITNESS_FUNCTION, func, fit);
