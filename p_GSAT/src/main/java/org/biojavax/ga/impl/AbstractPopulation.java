...
/**
 * Most Population implementations will want to inherit from here. This class doesn't define how
 * Organims are stored or accessed so inheriting classes can define that themselves.
 *
...
public abstract class AbstractPopulation extends AbstractChangeable implements Population {
  ...
  public final void addOrganism(Organism org) throws ChangeVetoException, IllegalOrganismException {
    if (!hasListeners()) {
      ...
      ChangeEvent ce = new ChangeEvent(this, Population.ORGANISMS, org, getOrganisms());
      ChangeSupport changeSupport = super.getChangeSupport(GeneticAlgorithm.POPULATION);
      synchronized (changeSupport) {
        changeSupport.firePreChangeEvent(ce);
        addOrganismImpl(org);
  ...
  public final void addOrganisms(Organism[] orgs)
      throws ChangeVetoException, IllegalOrganismException {
  ...
  public final void removeAllOrganisms() throws ChangeVetoException {
    if (!hasListeners()) {
      ...
      ChangeEvent ce = new ChangeEvent(this, Population.ORGANISMS, null, getOrganisms());
      ChangeSupport changeSupport = super.getChangeSupport(GeneticAlgorithm.POPULATION);
      synchronized (changeSupport) {
        changeSupport.firePreChangeEvent(ce);
        removeAllOrganismsImpl();
