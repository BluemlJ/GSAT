...
public interface RankedCrossRefable extends Changeable {
  ...
  /**
   * Sets the ranked cross references associated with an object. Null will throw an exception but
   * the empty set is fine.
   * 
   * @param crossrefs a set of RankedCrossRef objects.
   * @throws ChangeVetoException if the set was null or any of its contents were not RankedCrossRef
   *         objects.
   */
  public void setRankedCrossRefs(Set<RankedCrossRef> crossrefs) throws ChangeVetoException;

  /**
   * Adds a ranked cross reference to the existing set. If already present, this call is ignored.
  ...
  /**
   * Removes a ranked cross reference from the existing set. If not present, this call is ignored.
   * Null values are not acceptable.
   * 
