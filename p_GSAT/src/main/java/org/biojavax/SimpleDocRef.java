...
public class SimpleDocRef extends AbstractChangeable implements DocRef {
  ...
  /**
   * Creates a new document reference from the given immutable authors and location and title. Will
   * throw exceptions if either authors or location are null, but a null title is allowable.
   * 
   ...
   * throw exceptions if either authors or location are null, but a null title is allowable.
   * 
   * @param authors The authors of the referenced document, as a string to be parsed with
   *        {@link DocRefAuthor.Tools#parseAuthorString(String)}.
   ...
   */
  public SimpleDocRef(String authors, String location, String title, String crossRefKey,
      String crossRefValue, Integer crossRefVersion) {
    this(DocRefAuthor.Tools.parseAuthorString(authors), location, title, crossRefKey, crossRefValue,
        crossRefVersion);
  }
   ...
   * @param crossRefVersion
   */
  public SimpleDocRef(List authors, String location, String title, String crossRefKey,
      String crossRefValue, Integer crossRefVersion) {
    this(authors, location, title);
    this.setCrossref((CrossRef) RichObjectFactory.getObject(SimpleCrossRef.class,
        new Object[] {crossRefKey, crossRefValue, crossRefVersion}));
  }
  ...
  /**
   * {@inheritDoc} The string to be checksummed is constructed by concatenating the authors, title,
   * and location in that order, with no space between. If any values are null they are substituted
   * with the text "&lt;undef>".
   * 
   * @see CRC64Checksum
   */
  ...
  /**
   * {@inheritDoc} Document references are compared first by author, then by location, then by
   * title. If Author and location are equal and this title is null, and theirs isn't, then this
   * will return -1. For symmetry if our title is not null and theirs is then we return 1. If both
   * are null then we return 0.
   */
  public int compareTo(Object o) {
    ...
    if (!this.getAuthors().equals(them.getAuthors()))
    ...
    else if (!this.getLocation().equals(them.getLocation()))
      ...
      if (them.getTitle() == null)
        ...
        return -1;
    } else if (this.getTitle() != null && them.getTitle() == null)
      return 1; // other cases are handled above.
    else
      return this.getTitle().compareTo(them.getTitle());
  }

  /**
   * {@inheritDoc} Document references are equal if they have the same author and location and
   * title.
   */
  public boolean equals(Object obj) {
    ...
    return (this.getAuthors().equals(them.getAuthors())
        && this.getLocation().equals(them.getLocation()) && ((this.getTitle() == them.getTitle())
  ...
  /**
   * {@inheritDoc} Form: "authors; location"
   */
