...
public class SimpleRichAnnotation extends AbstractChangeable implements RichAnnotation {
  ...
  /**
   * {@inheritDoc} The map is a copy of the internal structure. It is a map of
   * <code>ComparableTerm</code>s to <code>String</code>s corresponding to the Term and Value of the
   * <code>Note</code>s in the annotation.
   */
  public Map asMap() {
    Map m = new TreeMap();
  ...
  /**
   * {@inheritDoc} In case the note was already here, a call to ChangeEvent.getPrevious() in the
   * firePostChangeEvent method will return a copy of the original note.
   */
    ...
    if (!this.hasListeners(Annotatable.ANNOTATION)) {
      ...
      synchronized (cs) {
        ...
        if (!change) {
          ...
          ce = new ChangeEvent(this, Annotatable.ANNOTATION, current, clone);
        }
  ...
  public boolean containsProperty(Object key) {
    if (key instanceof Term)
      key = RichObjectFactory.getDefaultOntology().getOrImportTerm((Term) key);
    else
  ...
  public Note getNote(Note note) throws NoSuchElementException {
    ...
    throw new NoSuchElementException(
        "No such property: " + note.getTerm() + ", rank " + note.getRank());
  }

  /**
   * {@inheritDoc} Strictly it will return the first <code>Note</code> which matches the
  ...
  /**
   * {@inheritDoc} Strictly it will return all <code>Note</code>s which match the <code>key</code>
   * (or a <code>Term</code> made with a <code>String</code> key)..
   * 
   * @deprecated
   */
  ...
  /**
   * {@inheritDoc} In case the note is not found, a call to ChangeEvent.getPrevious() in the
   * firePostChangeEvent method will return null.
   */
  public void removeNote(Note note) throws ChangeVetoException {
    ...
    if (!this.hasListeners(Annotatable.ANNOTATION)) {
      ...
      synchronized (cs) {
        ...
        if (!removed) ce = new ChangeEvent(this, Annotatable.ANNOTATION, null, null);
        cs.firePostChangeEvent(ce);
      }
    }
  }
  ...
  /**
   * {@inheritDoc} Strictly it will remove the first <code>Note</code> which matches the
   * <code>key</code> (or a <code>Term</code> made with a <code>String</code> key)..
   * 
   * @deprecated
   */
  ...
  public void setProperty(Object key, Object value)
    ...
    if (key instanceof Term)
      ...
      key = RichObjectFactory.getDefaultOntology().getOrCreateTerm(key.toString());
    this.addNote(new SimpleNote((ComparableTerm) key,
        (String) (value == null ? value : value.toString()), 0));
  }
  ...
  /**
   * {@inheritDoc} Form: list of "[note]" values separated by commas
   */
