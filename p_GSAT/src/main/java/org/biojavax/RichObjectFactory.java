...
/**
 * Runs a service that builds rich objects, and provides some default values for things like default
 * ontology, default namespace, etc.
 * 
...
public class RichObjectFactory {
  ...
  /**
   * Sets the builder to use when instantiating new Rich objects. The basic, default, one is a
   * SimpleRichObjectBuilder, which just calls the constructor. Another useful one is
   * BioSQLRichObjectBuilder, which attempts to load objects from the database. The latter is
   * required if you are working with Hibernate as it will not work without it.
   * 
  ...
  /**
   * Delegates to a RichObjectBuilder to construct/retrieve the object, and returns it. To increase
   * efficiency, it keeps a list of recently requested objects. If it receives further requests for
   * the same object, it returns them from the cache. The size of the cache can be altered using
   * setLRUCacheSize(). The default cache size is 20 objects for each type of class requested.
   * 
  ...
  public static synchronized Object getObject(final Class clazz, Object[] params) {
    ...
    if (!cache.containsKey(applicationClass)) {
      ...
      if (LRUcacheSizes.containsKey(applicationClass))
        LRUcacheSize = ((Integer) LRUcacheSizes.get(applicationClass)).intValue();
      else
  ...
  /**
   * Allow application to override the default biojava class created in getObject - subclass
   * restriction is checked in the builder.
   * 
   * @param theBiojavaClass one of the well-known builder classes: SimpleNamespace,
   *        SimpleComparableOntology, SimpleNCBITaxon, SimpleCrossRef, or SimpleDocRef
   * @param theApplicationClass - a subclass of theBiojavaClass
   */
  public final static void setApplicationClass(final Class theBiojavaClass,
      final Class theApplicationClass) {
    if (theApplicationClass == null
        || theBiojavaClass.isAssignableFrom(theApplicationClass) == false)
      throw new IllegalArgumentException(
          "RichObjectFactory.setApplicationClass-theApplicationClass: <" + theApplicationClass
              + "> must be assignable to the biojava class: <" + theBiojavaClass + ">");
  ...
  /**
   * Sets the size of the LRU cache. This is the size per class of object requested, so if you set
   * it to 20 and request 3 different types of object, you will get 20*3=60 entries in the cache.
   * The default cache size is 20. Setting this value will undo any previous changes made using the
   * setLRUCacheSize(Class,int) method below, but will not override future ones.
   * 
  ...
  /**
   * Sets the size of the LRU cache. This is the size for the specific class of object requested, so
   * does not affect the size of caches of other objects. If this method is not called, then the
   * cache size defaults to 20, or whatever value was passed to setLRUCacheSize(int) above.
   * 
   * @param size the size of the cache.
   */
  ...
  /**
   * Sets the default position resolver to use when creating new rich feature locations. Defaults to
   * the AverageResolver
   * 
  ...
  /**
   * Sets the default crossref resolver to use when resolving remote entries. Defaults to the
  ...
  /**
   * Sets the default sequence handler to use when performing sequence manipulation. Defaults to the
  ...
  public static ComparableOntology getDefaultOntology() {
    return (ComparableOntology) getObject(SimpleComparableOntology.class,
        new Object[] {defaultOntologyName});
  }
