...
public class SimpleRankedCrossRef extends AbstractChangeable implements RankedCrossRef {
  ...
  public void setRank(int rank) throws ChangeVetoException {
    ...
    if (!this.hasListeners(RankedCrossRef.RANK)) {
      ...
      ChangeEvent ce =
          new ChangeEvent(this, RankedCrossRef.RANK, new Integer(rank), new Integer(this.rank));
  ...
  /**
   * {@inheritDoc} Ranked cross references are the same if they have the same rank and refer to the
   * same cross reference (cross references are equal).
   */
  ...
  /**
   * {@inheritDoc} Ranked cross references are sorted first by rank, then by cross reference.
   */
