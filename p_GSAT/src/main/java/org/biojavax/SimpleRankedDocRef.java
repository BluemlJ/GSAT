...
public class SimpleRankedDocRef extends AbstractChangeable implements RankedDocRef {
  ...
  /**
   * Constructs a new docref for a given location. If one or the other of start and end are null,
   * only the non-null value is used. If both are null, no value is used for the location.
   * 
  ...
  public void setRank(int rank) throws ChangeVetoException {
    ...
    if (!this.hasListeners(RankedDocRef.RANK)) {
      ...
      ChangeEvent ce =
          new ChangeEvent(this, RankedDocRef.RANK, new Integer(rank), new Integer(this.rank));
  ...
  private void createLocation() {
    if (this.start == null && this.end == null)
      ...
      location = new SimpleRichLocation(null, new SimplePosition(this.end.intValue()), 0);
    else if (this.end == null)
      location = new SimpleRichLocation(new SimplePosition(this.start.intValue()), null, 0);
    else
      location = new SimpleRichLocation(new SimplePosition(this.start.intValue()),
          new SimplePosition(this.end.intValue()), 0);
  }
  ...
  final String getLocationText() {
    return getLocation() == RichLocation.EMPTY_LOCATION
        ? null
        : GenbankLocationParser.writeLocation(getLocation());
  }

  /**
   * {@inheritDoc} Two ranked document references are equal if they have the same rank and refer to
   * the same location and same document reference.
   */
  public boolean equals(Object obj) {
    ...
    return (this.rank == them.getRank() && this.location.equals(them.getLocation())
        && this.docref.equals(them.getDocumentReference()));
  }

  /**
   * {@inheritDoc} Ranked document references are sorted first by rank then location then by actual
   * document reference.
   */
