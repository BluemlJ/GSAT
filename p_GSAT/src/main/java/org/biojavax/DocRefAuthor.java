...
public interface DocRefAuthor extends Comparable, Changeable {
  ...
  /**
   * Returns a textual description of the authors name. This field is immutable so should be set
   * using the constructor of the implementing class.
   * 
  ...
  /**
   * Returns the extended version of the authors name. Form: "name (consortium) (ed.)" where
  ...
  public static class Tools {
    ...
    /**
     * Takes a set of authors and creates a comma-separated string. For the final comma, it replaces
     * it with the word "and".
     * 
     * @param authors set of authors
     * @param useAnd whether or not to do the and thing, otherwise use all commas.
     * @return a comma-separated string with the word "and" in place of the final comma.
     */
    public static String generateAuthorString(List<DocRefAuthor> authors, boolean useAnd) {
      ...
      for (Iterator<DocRefAuthor> i = authors.iterator(); i.hasNext();) {
        ...
        if (i.hasNext()) {
          if (authCount++ == authors.size() - 1 && useAnd)
            sb.append(" and ");
          else
            sb.append(", ");
        }
