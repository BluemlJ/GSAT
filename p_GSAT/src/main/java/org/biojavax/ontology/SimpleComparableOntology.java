...
public class SimpleComparableOntology extends AbstractChangeable implements ComparableOntology {
  ...
  /**
   * Creates a new instance of SimpleComparableOntology with the given, immutable, non-nullable
   * name.
   * 
  ...
  /**
   * {@inheritDoc} If the term has to be created, it is added with the description "auto-generated
   * by biojavax".
   */
    ...
    try {
      if (this.getTriples(subject, object, predicate).size() == 0) {
        return (ComparableTriple) this.createTriple(subject, object, predicate,
            predicate.getName() + "(" + subject.getName() + ", " + object.getName() + ")", null);
      } else
  ...
  /**
   * {@inheritDoc} This particular implementation merely copies the term into this ontology, and
   * returns a pointer to the copied term. Thus the term becomes a part of this ontology instead of
   * a pointer to another ontology.
   * 
   * @see ComparableTerm
   */
  public Term importTerm(Term t, String localName)
    ...
    if (!this.hasListeners(ComparableOntology.TERM)) {
      ...
      ChangeEvent ce =
          new ChangeEvent(this, ComparableOntology.TERM, ct, this.termsMap.get(localName));
      ChangeSupport cs = this.getChangeSupport(ComparableOntology.TERM);
      synchronized (cs) {
        cs.firePreChangeEvent(ce);
        this.termsMap.put(localName, ct);
  ...
  /**
   * {@inheritDoc} If you call this method with plain Terms instead of ComparableTerms, it will
   * import them into the local ontology first. This is done BEFORE the check to see if the triple
   * already exists, as obviously it can't check until it has the right terms to check with. So you
   * may find the terms get imported but the triple does not. Moral of the story: use ComparableTerm
   * objects!
   * 
   * @see ComparableTerm
   */
  public Triple createTriple(Term subject, Term object, Term predicate, String name,
    ...
    if (this.containsTriple(subject, object, predicate)) throw new AlreadyExistsException(
        "Ontology already has triple with this subject/object/predicate combination");
    ComparableTriple ct = new SimpleComparableTriple(this, (ComparableTerm) subject,
        (ComparableTerm) object, (ComparableTerm) predicate);
  ...
  /**
   * {@inheritDoc} If you call this method with a plain Term instead of a ComparableTerm, it may not
   * match any of the terms in the ontology as they are all stored as ComparableTerms. So, use
   * ComparableTerm objects! This method also removes all triples that the term is involved in.
   * 
   * @see ComparableTerm
   */
  public void deleteTerm(Term t) throws ChangeVetoException {
    ...
    for (Iterator i = this.triples.iterator(); i.hasNext();) {
      ...
      if (ct.equals(t) || ct.getSubject().equals(t) || ct.getObject().equals(t)
          || ct.getPredicate().equals(t)) {
        if (!this.hasListeners(ComparableOntology.TRIPLE)) {
          i.remove();
        } else {
  ...
  /**
   * {@inheritDoc} If you call this method with plain Terms instead of ComparableTerms, it may not
   * match any of the triples in the ontology as they are all stored as ComparableTerms. So, use
   * ComparableTerm objects! The set returned is a set of ComparableTriple objects.
   * 
   * @see ComparableTerm
  ...
  /**
   * {@inheritDoc} This will always return a set of ComparableTerm objects. It is not the original
  ...
  public boolean containsTriple(Term subject, Term object, Term predicate) {
    for (Iterator i = this.triples.iterator(); i.hasNext();) {
      ComparableTriple ct = (ComparableTriple) i.next();
      if (ct.getSubject().equals(subject) && ct.getObject().equals(object)
  ...
  public Variable createVariable(String name, String description)
    ...
    throw new ChangeVetoException(
        "BioSQL doesn't know what these are so we cowardly refuse to know too.");
  }
  ...
  public void setDescription(String description) throws ChangeVetoException {
    if (!this.hasListeners(ComparableOntology.DESCRIPTION)) {
      ...
      ChangeEvent ce =
          new ChangeEvent(this, ComparableOntology.DESCRIPTION, description, this.description);
