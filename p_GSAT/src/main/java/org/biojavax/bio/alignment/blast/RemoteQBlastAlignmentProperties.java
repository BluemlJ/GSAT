...
/**
 * 
 * This class implements RemotePairwiseAlignmentProperties by specifying several convenient methods
 * used to wrap the addition of Blast alignment parameters.
 * 
...
public class RemoteQBlastAlignmentProperties implements RemotePairwiseAlignmentProperties {
  ...
  /**
   * This method set the program to be use with blastall. This method does a validation before
   * running on the valid blastall programs: blastn / blastp / blastx / tblastn / tblastx
   * 
   * @param program : one of blastall programs
   * @exception BioException if the named program is not a valid blastall options
   * 
   */
  ...
  /**
   * <p>
   * This method is to be used if a request is to use non-default values at submission. According to
   * QBlast info, the accepted parameters for PUT requests are:
   * </p>
   * 
   * <ul>
   * <li>-G: cost to create a gap. Default = 5 (nuc-nuc) / 11 (protein) / non-affine for
   * megablast</li>
   * <li>-E: Cost to extend a gap. Default = 2 (nuc-nuc) / 1 (protein) / non-affine for
   * megablast</li>
   ...
   * <li>-W: word size. Default = 3 (proteins) / 11 (nuc-nuc) / 28 (megablast)</li>
   * <li>-y: dropoff for blast extensions in bits, using default if not specified. Default = 20 for
   * blastn, 7 for all others (except megablast for which it is not applicable).</li>
   * <li>-X: X dropoff value for gapped alignment, in bits. Default = 30 for blastn/megablast, 15
   * for all others.</li>
   * <li>-Z: final X dropoff value for gapped alignement, in bits. Default = 50 for blastn, 25 for
   * all others (except megablast for which it is not applicable)</li>
   * <li>-P: equals 0 for multiple hits 1-pass, 1 for single hit 1-pass. Does not apply to blastn ou
   * megablast.</li>
   ...
   * <li>-Y: effective length of the search space. Default = 0 (0 represents using the whole
   * space)</li>
   * <li>-z: a real specifying the effective length of the database to use. Default = 0 (0
   * represents the real size)</li>
   ...
   * </p>
   * <p>
   * You have to be aware that at not moment is there any error checking on the use of these
   * parameters by this class.
   * </p>
   * 
   * @param aStr : a String with any number of optional parameters with an associated value.
   * 
   */
  ...
  public String getAlignmentOption(String key) throws BioException {
    if (param.containsKey(key)) {
      ...
      throw new BioException(
          "The key named " + key + " is not set in this RemoteQBlastOutputProperties object");
    }
  }
