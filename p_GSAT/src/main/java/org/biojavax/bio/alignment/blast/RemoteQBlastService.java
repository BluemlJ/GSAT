 ...
 * 
 * <p>
 * NCBI provides a Blast server through a CGI-BIN interface. RemoteQBlastService simply encapsulates
 * an access to it by giving users access to get/set methods to fix sequence, program and database
 * as well as advanced options.
 * </p>
 * 
 * <p>
 * The philosophy behind RemoteQBlastService is to disconnect submission of Blast requests from
 * collection of Blast results. This is done so to allow a user to submit multiple Blast requests
 * while allowing recovery of the reports at a later time.
 * </p>
 * 
 * <p>
...
public class RemoteQBlastService implements RemotePairwiseAlignmentService {
   ...
   * 
   * @throws BioException if the NCBI URL is unresponsive
   * 
   */
  public RemoteQBlastService() throws BioException {
    ...
    /*
     * Needed but should never be thrown since the URL is static and known to exist
     */
  ...
  /**
   * This class is the actual worker doing all the dirty stuff related to sending the Blast request
   * to the CGI_BIN interface. It should never be used as is but any method wanting to send a Blast
   * request should manage to use it by feeding it the right parameters.
   * 
   * @param str : a <code>String</code> representation of a sequence from either of the three
   *        wrapper methods
   ...
   */
  private String sendActualAlignementRequest(String str, RemotePairwiseAlignmentProperties rpa)
      throws BioException {
    ...
    String cmd = "CMD=Put&SERVICE=plain" + "&" + seq + "&" + prog + "&" + db + "&"
        + "FORMAT_TYPE=HTML" + "&TOOL=" + getTool() + "&EMAIL=" + getEmail();
  ...
  /**
   * This method is a wrapper that executes the Blast request via the Put command of the CGI-BIN
   * interface with the specified parameters and a string representing the sequence. It gets the
   * estimated time of completion by capturing the value of the RTOE variable and sets a loop that
   * will check for completion of analysis at intervals specified by RTOE.
   * 
   * <p>
   * It also capture the value for the RID variable, necessary for fetching the actual results after
   * completion.
   * </p>
   * 
  ...
  /**
   * This method is a wrapper that executes the Blast request via the Put command of the CGI-BIN
   * interface with the specified parameters and a RichSequence. It gets the estimated time of
   * completion by capturing the value of the RTOE variable and sets a loop that will check for
   * completion of analysis at intervals specified by RTOE.
   * 
   * <p>
   * It also capture the value for the RID variable, necessary for fetching the actual results after
   * completion.
   ...
   * @throws BioException if it is not possible to sent the BLAST command
   */
  public String sendAlignmentRequest(RichSequence rs, RemotePairwiseAlignmentProperties rpa)
      throws BioException {
  ...
  /**
   * This method is a wrapper that executes the Blast request via the Put command of the CGI-BIN
   * interface with the specified parameters and a GenBank GID. It gets the estimated time of
   * completion by capturing the value of the RTOE variable and sets a loop that will check for
   * completion of analysis at intervals specified by RTOE.
   * 
   * <p>
  ...
  /**
   * <p>
   * This method is used only for the executeBlastSearch method to check for completion of request
   * using the NCBI specified RTOE variable
   * </p>
   * 
  ...
  public boolean isReady(String id, long present) throws BioException {
    ...
    if (holder.containsKey(id)) {
      /*
       * If present time is less than the start of the search added to step obtained from NCBI, just
       * do nothing ;-)
       * 
       * This is done so that we do not send zillions of requests to the server. We do the waiting
       * internally first.
       */
      if (present < start) {
        ready = false;
      ...
      /*
       * If we are at least step seconds in the future from the actual call sendAlignementRequest()
       */
      else {
        try {
          uConn = setQBlastServiceProperties(aUrl.openConnection());
          ...
          while ((line = rd.readLine()) != null) {
            if (line.contains("READY")) {
              ...
              /*
               * Else, move start forward in time... for the next iteration
               */
              start = present + step;
              holder.put(id, start);
            }
      ...
      throw new BioException(
          "Impossible to check for request ID named " + id + " because it does not exists!\n");
    }
    return ready;
  }
  ...
  /**
   * <p>
   * This method extracts the actual Blast report for this request ID. It uses an object
   * implementing the RemotePairwiseAlignmentOutputProperties interface which will specify output
   * formatting options.
   * </p>
   * 
   * @param id :a valid request ID
   * @param rb : a <code>RemotePairwiseAlignmentOutputProperties</code> that will specify specific
   *        output formatting commands
   * @return an <code>InputStream</code> that can be use any way one might desire
   * @throws BioException if it is not possible to recover the results.
   */
  public InputStream getAlignmentResults(String id, RemotePairwiseAlignmentOutputProperties rb)
      throws BioException {
    if (holder.containsKey(id)) {
      String srid = "CMD=Get&RID=" + id + "&" + rb.getOutputOption("FORMAT_TYPE") + "&"
          + rb.getOutputOption("ALIGNMENT_VIEW") + "&" + rb.getOutputOption("DESCRIPTIONS") + "&"
          + rb.getOutputOption("ALIGNMENTS") + "&TOOL=" + getTool() + "&EMAIL=" + getEmail();
      ...
      throw new BioException(
          "Impossible to get output for request ID named " + id + " because it does not exists!\n");
    }
  }
