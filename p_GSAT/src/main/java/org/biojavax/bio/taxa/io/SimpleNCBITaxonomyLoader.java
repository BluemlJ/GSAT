...
/**
 * Loads NCBI taxon information from names.dmp and nodes.dmp, which are two of the files in the
 * archive downloadable at ftp://ftp.ncbi.nih.gov/pub/taxonomy/ . This simple implementation makes
 * no attempt to process deletions or merges - it merely creates instances as it goes along, reusing
...
public class SimpleNCBITaxonomyLoader implements NCBITaxonomyLoader {
  ...
  public NCBITaxon readNode(BufferedReader nodes) throws IOException, ParseException {
    ...
    if ((line = nodes.readLine()) != null) {
      /*
       * separated by '\t|\t' tax_id -- node id in GenBank taxonomy database parent tax_id -- parent
       * node id in GenBank taxonomy database rank -- rank of this node (superkingdom, kingdom, ...)
       * embl code -- locus-name prefix; not unique division id -- see division.dmp file inherited
       * div flag (1 or 0) -- 1 if node inherits division from parent genetic code id -- see
       * gencode.dmp file inherited GC flag (1 or 0) -- 1 if node inherits genetic code from parent
       * mitochondrial genetic code id -- see gencode.dmp file inherited MGC flag (1 or 0) -- 1 if
       * node inherits mitochondrial gencode from parent GenBank hidden flag (1 or 0) -- 1 if name
       * is suppressed in GenBank entry lineage hidden subtree root flag (1 or 0) -- 1 if this
       * subtree has no sequence data yet comments -- free-text comments and citations
       */
  ...
  public NCBITaxon readName(BufferedReader names) throws IOException, ParseException {
    ...
    if ((line = names.readLine()) != null) {
      /*
       * separated by '\t|\t' tax_id -- the id of node associated with this name name_txt -- name
       * itself unique name -- the unique variant of this name if name not unique name class --
       * (synonym, common name, ...)
       */
      String[] parts = line.split("\\|");
      Integer tax_id = Integer.valueOf(parts[0].trim());
      ...
      NCBITaxon t =
          (NCBITaxon) RichObjectFactory.getObject(SimpleNCBITaxon.class, new Object[] {tax_id});
      // add the name
      try {
