...
public class SimpleNCBITaxonName implements Comparable {
   ...
   *        {@link org.biojavax.bio.taxa.NCBITaxon} (for example
   *        {@link org.biojavax.bio.taxa.NCBITaxon#SCIENTIFIC}).
  ...
  public SimpleNCBITaxonName(String nameClass, String name) {
    ...
    if (name.indexOf('\n') >= 0)
      throw new IllegalArgumentException("NCBI taxonomy names cannot embed new lines - at:"
          + name.indexOf('\n') + ", in name: <" + name + ">");
  ...
  /**
   * {@inheritDoc} Two taxon names are equal if their name and class match.
   */
  ...
  /**
   * {@inheritDoc} Taxon names are sorted by class first, then name.
   */
