...
/**
 * Describes locations, and adds the concepts of circularity, fuzziness, annotations, and
 * cross-references to other databases. Also includes strands.
 * 
...
public interface RichLocation extends Location, RichAnnotatable, Comparable {
  ...
  public static final ChangeType NOTE = new ChangeType("This location's notes have changed",
      "org.biojavax.bio.seq.RichLocation", "NOTE");
  public static final ChangeType TERM = new ChangeType("This location's term has changed",
      "org.biojavax.bio.seq.RichLocation", "TERM");
  public static final ChangeType RANK = new ChangeType("This location's rank has changed",
      "org.biojavax.bio.seq.RichLocation", "RANK");
  public static final ChangeType CIRCULAR = new ChangeType(
      "This location's circularity has changed", "org.biojavax.bio.seq.RichLocation", "CIRCULAR");
  public static final ChangeType FEATURE = new ChangeType(
      "This location's parent feature has changed", "org.biojavax.bio.seq.RichLocation", "FEATURE");
  ...
  /**
   * Sorts the member locations of a compound location. Does nothing for non-compound locations.
   * Sorting depends on the compareTo() method of the member locations - usually they will be sorted
   * by their start position. This might be useful when used with the location returned by a union
   * or intersection, or when setting the term of a compound location to ORDER_TERM.
   */
  public void sort();
  ...
  /**
   * Sets the feature this location is associated with. If null, that's fine, but you won't be able
   * to persist it to the database until you give it a not-null value.
   * 
  ...
  /**
   * Retrieves the circular length of this location. If it is 0, the location is not circular. If it
   * is not zero, then the number refers to the wrapping length of the location. eg. 100 would
   * signify that a position of 112 would actually be a position of 112-100 = 12.
   * 
   * @return the position.
   */
  ...
  /**
   * Sets the circular length of this location. If it is 0, the location is not circular. If it is
   * not zero, then the number refers to the wrapping length of the location. eg. 100 would signify
   * that a position of 112 would actually be a position of 112-100 = 12.
   * 
  ...
  /**
   * Sets the cross ref resolver to use when retrieving remote symbols. If none is given, then the
   * default from RichObjectFactory.getDefaultCrossRefResolver() is used.
   * 
  ...
  /**
   * This class represents a strand on which a location may lie. Three strands are defined by
   * default - UNKNOWN, NEGATIVE, and POSITIVE.
   */
  public static class Strand implements Comparable {
    ...
    /**
     * Returns the strand object that matches the number given. Throws an exception if it could not
     * recognise the number. Number is usually 1,-1,0.
     * 
    ...
    /**
     * Returns the strand object that matches the symbol given. Throws an exception if it could not
     * recognise the symbol. Symbol is usually +,-,?.
     * 
  ...
  public static class Tools {
    ...
    /**
     * Constructs a RichLocation object based on the given collection of members. It the collection
     * contains a single location, that is returned. If it contains multiple locations it returns a
     * CompoundRichLocation covering them all, with the default term associated. It returns the
     * empty location if the collection was empty.
     * 
      ...
      for (Iterator<Location> i = members.iterator(); i.hasNext();) {
        ...
        if (previous == null)
          ...
          if ((previous.getCrossRef() == null && rl.getCrossRef() != null)
              ...
              || (previous.getCrossRef() != rl.getCrossRef()
                  && !previous.getCrossRef().equals(rl.getCrossRef())))
            return true;
          if ((previous.getStrand() == null && rl.getStrand() != null)
              || (previous.getStrand() != null && rl.getStrand() == null)
              || (previous.getStrand() != rl.getStrand()
                  && !previous.getStrand().equals(rl.getStrand())))
            return true;
        }
      }
      return false;
    ...
    /**
     * Takes a set of locations and tries to merge all pairs where the union operation results in a
     * simple rich location, not a compound one.
     * 
    ...
    /**
     * Takes a location and returns the set of all members. If any members are compound, it flattens
     * them too.
     * 
    ...
    /**
     * Takes a set of locations and returns the set of all members. If any members are compound, it
     * flattens them too.
     * 
    ...
    public static Collection<Location> flatten(Collection<Location> members) {
      ...
      for (int i = 0; i < flattened.size(); i++) {
        ...
        if (!member.isContiguous()) {
          flattened.remove(i);
          int insertPos = i;
          for (Iterator<Location> j = member.blockIterator(); j.hasNext();)
    ...
    /**
     * Takes a start and end position on a circular location of given length, and shifts them left
     * along the sequence until they sit at the earliest possible point where they still would
    ...
    /**
     * Takes two circular locations of given length, and shifts them left along the sequence until
     * they sit at the earliest possible point where they still would represent the same sequence.
     * The end result ensures that simple overlap calculations will always work on the coordinates
     * returned.
     * 
     * @param a the first location to shift
     * @param b the second location to shift
     * @param seqLength the circular length of the sequence underlying the location
     * @return an integer array where [0] is the translated start and [1] the end of location a, and
     *         [2] and [3] are the translated start and end of location b.
     */
    ...
    /**
     * Takes a point on a circular location and moves it left until it falls at the earliest
     * possible point that represents the same base.
     * 
    ...
    public static RichLocation enrich(Location l) {
      ...
      // Fuzzy single points
        ...
        Position pos = new SimplePosition(f.hasBoundedMin(), f.hasBoundedMax(), f.getMin(),
            f.getMax(), Position.IN_RANGE);
