...
public class SimplePosition implements Position {
  ...
  /**
   * Constructs a point position, with optionally fuzzy start and end. (eg. <1 or 3> or 2 or even
   * <5>).
   * 
  ...
  /**
   * Constructs a range position, with optionally fuzzy start and end. (eg. <1.2 or 1^3> or 2.2 or
   * even <5^6>). The type of the range is given, it should normally be one of the two defined in
   * the Position interface, but its up to you.
   * 
   * @param fs fuzzy start?
   * @param fe fuzzy end?
  ...
  void setFuzzyStart(boolean fs) {
    this.fs = fs;
  }

  // Hibernate requirement - not for public use - futureproofing
  void setFuzzyEnd(boolean fe) {
    this.fe = fe;
  }
  ...
  /**
   * {@inheritDoc} Two positions are equal if they share all parameters in common, eg. fuzzy
