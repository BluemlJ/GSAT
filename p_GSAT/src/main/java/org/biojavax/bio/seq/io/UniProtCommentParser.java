...
public class UniProtCommentParser {
  ...
  /**
   * Parses the comment string from the given comment and populates the internal fields
   * appropriately. If the comment is not a UniProt comment (does not start with -!-) then an
   * exception is thrown.
   * 
  ...
  public void parseComment(String c) throws ParseException {
    ...
    // do the parsing here.
    try {
      c = c.replaceAll("\\s+", " ").trim(); // replace all multi-spaces and newlines with single
                                            // spaces
      ...
      if (this.getCommentType().equalsIgnoreCase(BIOPHYSICOCHEMICAL_PROPERTIES)) {
        /*
         * CC -!- BIOPHYSICOCHEMICAL PROPERTIES: CC Absorption: CC Abs(max)=xx nm; CC
         * Note=free_text; CC Kinetic parameters: CC KM=xx unit for substrate [(free_text)]; CC
         * Vmax=xx unit enzyme [free_text]; CC Note=free_text; CC pH dependence: CC free_text; CC
         * Redox potential: CC free_text; CC Temperature dependence: CC free_text;
         */
        do {
        ...
        /*
         * CC -!- MASS SPECTROMETRY: MW=XXX[; MW_ERR=XX]; METHOD=XX; RANGE=XX-XX[ (Name)];
         * NOTE={Free text (Ref.n)|Ref.n}.
         */
        c = c.substring(0, c.length() - 1); // chomp trailing dot
        String[] parts = c.split(";");
        for (int i = 0; i < parts.length; i++) {
          ...
          if (key.equalsIgnoreCase("MW"))
          ...
          else if (key.equalsIgnoreCase("MW_ERR"))
            ...
            if (value.indexOf(' ') > -1) value = value.substring(0, value.indexOf(' ')); // drop
                                                                                         // name
        ...
        /*
         * CC -!- INTERACTION: CC {{SP_Ac:identifier[ (xeno)]}|Self}; NbExp=n;
         * IntAct=IntAct_Protein_Ac, IntAct_Protein_Ac;
         */
        String[] parts = c.split(";");
        ...
        /*
         * CC -!- ALTERNATIVE PRODUCTS: CC Event=Alternative promoter; CC Comment=Free text; CC
         * Event=Alternative splicing; Named isoforms=n; CC Comment=Optional free text; CC
         * Name=Isoform_1; Synonyms=Synonym_1[, Synonym_n]; CC IsoId=Isoform_identifier_1[,
         * Isoform_identifier_n]; Sequence=Displayed; CC Note=Free text; CC Name=Isoform_n;
         * Synonyms=Synonym_1[, Synonym_n]; CC IsoId=Isoform_identifier_1[, Isoform_identifier_n];
         * Sequence=VSP_identifier_1 [, VSP_identifier_n]; CC Note=Free text; CC Event=Alternative
         * initiation; CC Comment=Free text;
         */
        Event event = null;
        ...
        for (int i = 0; i < parts.length; i++) {
          ...
          if (key.equalsIgnoreCase("Event")) {
            ...
            else if (value.equalsIgnoreCase("Not described"))
        ...
        /*
         * CC -!- SEQUENCE_CAUTION: Sequence=Sequence; Type=Type;[ Positions=Positions;][
         * Note=Note;]
         */
        ...
        for (int i = 0; i < parts.length; i++) {
          ...
          if (key.equalsIgnoreCase("SEQUENCE")) {
            ...
            seqc.setType(value);
          else if (key.equalsIgnoreCase("POSITIONS"))
   ...
   * @throws ParseException if the current settings do not allow the creation of a correct comment
   *         string.
   */
  public String generate() throws ParseException {
    ...
    if (this.getCommentType().equalsIgnoreCase(BIOPHYSICOCHEMICAL_PROPERTIES)) {
      /*
       * CC -!- BIOPHYSICOCHEMICAL PROPERTIES: CC Absorption: CC Abs(max)=xx nm; CC Note=free_text;
       * CC Kinetic parameters: CC KM=xx unit for substrate [(free_text)]; CC Vmax=xx unit enzyme
       * [free_text]; CC Note=free_text; CC pH dependence: CC free_text; CC Redox potential: CC
       * free_text; CC Temperature dependence: CC free_text;
       */
      ...
      /*
       * CC -!- MASS SPECTROMETRY: MW=XXX[; MW_ERR=XX]; METHOD=XX; RANGE=XX-XX[ (Name)]; NOTE={Free
       * text (Ref.n)|Ref.n}.
       */
      sb.append("MW=");
