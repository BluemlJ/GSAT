...
public class SimpleRichLocation extends AbstractChangeable implements RichLocation {
  ...
  /**
   * Creates a new instance of SimpleRichSequenceLocation that points to a single position on the
   * positive strand.
   * 
  ...
  /**
   * Creates a new instance of SimpleRichSequenceLocation that points to a single position on
   * another sequence.
   * 
   * @param pos the location position (a point).
   * @param rank Rank of location.
  ...
  /**
   * Creates a new instance of SimpleRichSequenceLocation that points to a range position on the
   * positive strand.
   * 
  ...
  /**
   * Creates a new instance of SimpleRichSequenceLocation that points to a range position on another
   * sequence.
   * 
   * @param min the minimum bound of the location
   * @param max the maximum bound of the location
  ...
  /**
   * {@inheritDoc} <b>Warning</b> this method gives access to the original Collection not a copy.
   * This is required by Hibernate. If you modify the object directly the behaviour may be
   * unpredictable.
   */
  ...
  public void setCircularLength(int circularLength) throws ChangeVetoException {
    if (!this.hasListeners(RichLocation.CIRCULAR)) {
      ...
      ChangeEvent ce = new ChangeEvent(this, RichLocation.CIRCULAR, new Integer(circularLength),
  ...
  void setStrandNum(int token) {
    this.strand = Strand.forValue(token);
  }
  ...
  public void setRank(int rank) throws ChangeVetoException {
    if (!this.hasListeners(RichLocation.RANK)) {
      ...
      ChangeEvent ce =
          new ChangeEvent(this, RichLocation.RANK, new Integer(rank), new Integer(this.rank));
  ...
  public int getMax() {
    if (this.max.equals(this.min))
      return this.getMin(); // because resolver might resolve differently
    else
  ...
  public boolean contains(int p) {
    ...
    if (this.circularLength > 0) {
      ...
      int[] ourModParts =
          RichLocation.Tools.modulateCircularLocation(modStart, modEnd, this.circularLength);
  ...
  public Location translate(int dist) {
    return new SimpleRichLocation(this.min.translate(dist), this.max.translate(dist), 0,
        this.strand, this.crossRef);
  }

  /**
   * {@inheritDoc} A location contains another location if it overlaps it, and the coordinates
   * enclose those of the other location at both ends, and they fall on the same strand.
   */
  public boolean contains(Location l) {
    ...
    if (l instanceof EmptyRichLocation)
      ...
      if (this.circularLength > 0) {
        ...
        int parts[] =
            RichLocation.Tools.modulateCircularLocationPair(this, rl, this.circularLength);
  ...
  /**
   * {@inheritDoc} A location overlaps another location if it is on the same sequence, and the
   * coordinates overlap, and both are of the same circularity.
   */
  public boolean overlaps(Location l) {
    ...
    if (l instanceof EmptyRichLocation)
      ...
      if (this.circularLength != rl.getCircularLength()) return false; // Diff circularLength
                                                                       // location sizes = not
                                                                       // overlapping
  ...
  /**
   * {@inheritDoc} A merged SimpleRichLocation is returned if the two locations overlap and are on
   * the same strand. Otherwise, a CompoundRichLocation is returned containing the two locations as
   * members.
   */
    ...
    if (l instanceof EmptyRichLocation)
      ...
      if (this.overlaps(rl) && this.getStrand().equals(rl.getStrand())) {
        ...
        if (this.circularLength > 0) {
          ...
          RichLocation newloc =
              new SimpleRichLocation(startPos, endPos, 0, this.strand, this.crossRef);
          newloc.setCircularLength(this.circularLength);
          return newloc;
        } else {
          // Union of Overlapping non-circular locations
          return new SimpleRichLocation(this.posmin(this.min, rl.getMinPosition()),
              this.posmax(this.max, rl.getMaxPosition()), 0, this.strand, this.crossRef);
        }
      }
  ...
  /**
   * {@inheritDoc} If the locations overlap and are on the same strand, the intersection is
   * returned. If they overlap but are on different strands, a CompoundRichLocation of the
   * overlapping portions is returned. If they do not overlap, the empty sequence is returned.
   */
  public Location intersection(Location l) {
    RichLocation rl = RichLocation.Tools.enrich(l);
    if (rl instanceof EmptyRichLocation)
      return rl;
    else if (rl instanceof CompoundRichLocation)
      return rl.intersection(this);
    else if (circularLength != rl.getCircularLength())
      return rl;
    else {
      if (this.overlaps(rl)) {
        if (this.getStrand().equals(rl.getStrand())) {
          ...
          if (circularLength > 0) {
            ...
            return new SimpleRichLocation(this.posmax(this.min, rl.getMinPosition()),
                this.posmin(this.max, rl.getMaxPosition()), 0, this.strand, this.crossRef);
          }
        }
        // We can do the one-v-one different-strand overlapping intersection here
        Collection members = new ArrayList();
        members.add(new SimpleRichLocation(this.posmax(this.min, rl.getMinPosition()),
            this.posmin(this.max, rl.getMaxPosition()), 0, this.strand, this.crossRef));
        members.add(new SimpleRichLocation(this.posmax(this.min, rl.getMinPosition()),
            this.posmin(this.max, rl.getMaxPosition()), 0, rl.getStrand(), this.crossRef));
        if (RichLocation.Tools.isMultiSource(members))
          return new MultiSourceCompoundRichLocation(members);
        else
          return new CompoundRichLocation(members);
  ...
  /**
   * {@inheritDoc} If the location is circular but the sequence is not, or they are both circular
   * but of different circular lengths, an exception is thrown. The symbol list passed in is the
   * sequence used to obtain symbols if the cross reference for this location has not been set. If
   * the cross reference has been set, then the symbol list passed in is only used if it has the
   * same accession, namespace and version as the cross reference on this location. Otherwise, the
   * cross referenced symbol list is looked up and used instead.
   */
  public SymbolList symbols(SymbolList seq) {
    ...
    if (seq instanceof RichSequence) {
      ...
      if (this.getCircularLength() > 0) {
        if (!rs.getCircular()) throw new IllegalArgumentException(
            "Attempt to apply circular location to non-circular sequence");
        if (rs.length() != this.getCircularLength()) throw new IllegalArgumentException(
            "Attempt to apply circular location to circular sequence of different length");
      }
    }
    ...
    if (this.getCrossRef() != null) {
      ...
      if (seq instanceof RichSequence) {
        ...
        // It's assumed to be remote because we can't tell what the sequence we were passed really
        // is
  ...
  /**
   * {@inheritDoc} Locations are equal if their term, min, max, strand, and crossref are the same,
   * and if their rank is the same too.
   */
  ...
  /**
   * {@inheritDoc} Locations are sorted first by rank, then crossref, then strand, then term, then
   * min, then max.
   */
  public int compareTo(Object o) {
    ...
    if (this.strand.equals(RichLocation.Strand.POSITIVE_STRAND)
        || this.strand.equals(RichLocation.Strand.UNKNOWN_STRAND)) {
  ...
  /**
   * {@inheritDoc} Form: "start..end" or just "point" for point locations
   */
  ...
  void setLocationText(final String theLocation) throws ParseException {
    ...
    if (theLocation == null) {
      ...
      final RichLocation location = GenbankLocationParser
          .parseLocation(RichObjectFactory.getDefaultNamespace(), null, theLocation);
  ...
  String getLocationText() {
    // System.out.println("SimpleRichLocation.getLocationText-returns:
    // ["+GenbankLocationParser.writeLocation(new SimpleRichLocation(getMinPosition(),
    // getMaxPosition(), getRank()))+"], this: ["+this+"]");
    return GenbankLocationParser
        .writeLocation(new SimpleRichLocation(getMinPosition(), getMaxPosition(), getRank()));
  }
