...
/**
 * This is purely for debugging purposes. Use it to wrap an input stream then pass it as a parameter
 * to some format to listen for sequence generation events. It will dump out char-by-char what it
 * reads, followed event-by-event the events it receives from the format.
 * 
...
public class DebuggingRichSeqIOListener extends BufferedInputStream implements RichSeqIOListener {
  ...
  public void addSymbols(Alphabet alpha, Symbol[] syms, int start, int length)
    ...
    this.message("addSymbols: (alpha) " + alpha + " (syms) " + syms + " (start) " + start
        + " (length) " + length);
  }
