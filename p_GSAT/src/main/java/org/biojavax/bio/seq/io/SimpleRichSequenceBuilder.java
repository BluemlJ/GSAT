...
public class SimpleRichSequenceBuilder extends RichSeqIOAdapter implements RichSequenceBuilder {
  ...
  /**
   * Creates a new instance of SimpleRichSequenceBuilder using a SimpleSymbolListFactory with a
   * threshold of zero.
   */
  ...
  /**
   * Creates a new instance of SimpleRichSequenceBuilder with the desired symbollistfactory and
   * threshold of zero.
   * 
   ...
   * @param threshold the threshold at which the specified symbollistfactory should come into use.
   *        If <=0, it will be used from the start.
   */
  ...
  public void startFeature(Feature.Template templ) throws ParseException {
    try {
      ...
      if (this.featureStack.size() == 0)
        ...
        parent.addFeatureRelationship(new SimpleRichFeatureRelationship(parent, f,
      ...
      if (!this.taxon.equals(taxon)) {
        System.err.println(
            "Warning: attempted to set taxon twice with different values. Keeping first value. "
                + "old value (retained): " + this.taxon + " new value: " + taxon + ", accession: <"
                + accession + ">, version:" + version);
      }
    }
    this.taxon = taxon;
  }
  ...
  public Sequence makeSequence() throws BioException {
    ...
    RichSequence rs = new SimpleRichSequence(this.namespace, this.name, this.accession,
        this.version, syms, new Double(this.seqVersion));
    // set misc stuff
    try {
      // set features
