...
public class UniProtLocationParser {
  ...
  public static RichLocation parseLocation(String loc) throws ParseException {
    try {
      ...
      if ((startPos == null) && (endPos == null)) {
        return new SimpleRichLocation(new SimplePosition(0), new SimplePosition(0), 1,
            Strand.POSITIVE_STRAND, null);
      } else if (endPos == null) {
        return new SimpleRichLocation(startPos, new SimplePosition(0), 1, Strand.POSITIVE_STRAND,
            null);
      } else if (startPos == null) {
        return new SimpleRichLocation(new SimplePosition(0), endPos, 1, Strand.POSITIVE_STRAND,
            null);
      }
  ...
  private static String _writePosition(Position p, boolean useMax) {
    ...
    if (s != e) {
      ...
      if (useMax)
        a = RichObjectFactory.getDefaultPositionResolver().getMax(p);
      else
        a = RichObjectFactory.getDefaultPositionResolver().getMin(p);
    } else {
      a = s;
    }
    if (fs) sb.append("<");
