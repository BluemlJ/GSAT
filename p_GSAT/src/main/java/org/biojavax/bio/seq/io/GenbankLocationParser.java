...
public class GenbankLocationParser {
   ...
   */
  public static RichLocation parseLocation(Namespace featureNS, String featureAccession,
      String locationString) throws ParseException {
    /*
     * 
     * FROM GENBANK FEATURE TABLE DOCS
     * 
     * 3.5.3 Location examples The following is a list of common location descriptors with their
     * meanings: Location Description
     * 
     * 467 Points to a single base in the presented sequence
     * 
     * 340..565 Points to a continuous range of bases bounded by and including the starting and
     * ending bases
     * 
     * <345..500 Indicates that the exact lower boundary point of a feature is unknown. The location
     * begins at some base previous to the first base specified (which need not be contained in the
     * presented sequence) and con- tinues to and includes the ending base
     * 
     * <1..888 The feature starts before the first sequenced base and continues to and includes base
     * 888
     * 
     * (102.110) Indicates that the exact location is unknown but that it is one of the bases
     * between bases 102 and 110, in- clusive
     * 
     * (23.45)..600 Specifies that the starting point is one of the bases between bases 23 and 45,
     * inclusive, and the end point is base 600
     * 
     * (122.133)..(204.221) The feature starts at a base between 122 and 133, inclusive, and ends at
     * a base between 204 and 221, inclusive
     * 
     * 123^124 Points to a site between bases 123 and 124
     * 
     * 145^177 Points to a site between two adjacent bases anywhere between bases 145 and 177
     * 
     * join(12..78,134..202) Regions 12 to 78 and 134 to 202 should be joined to form one contiguous
     * sequence
     * 
     * complement(1..23) Complements region 1 to 23
     * 
     * complement(join(2691..4571,4918..5163) Joins regions 2691 to 4571 and 4918 to 5163, then
     * complements the joined segments (the feature is on the strand complementary to the presented
     * strand)
     * 
     * join(complement(4918..5163),complement(2691..4571)) Complements regions 4918 to 5163 and 2691
     * to 4571, then joins the complemented segments (the feature is on the strand complementary to
     * the presented strand)
     * 
     * complement(34..(122.126)) Start at one of the bases complementary to those between 122 and
     * 126 on the presented strand and finish at the base complementary to base 34 (the feature is
     * on the strand complementary to the presented strand)
     * 
     * J00194:100..202 Points to bases 100 to 202, inclusive, in the entry (in this database) with
     * primary accession number 'J00194'
     * 
     */

    rank = 0;
    return parseLocString(featureNS, featureAccession, null, Strand.POSITIVE_STRAND,
        locationString);
  }
  ...
  private static RichLocation parseLocString(Namespace featureNS, String featureAccession,
      CrossRef parentXref, Strand parentStrand, String locStr) throws ParseException {
    // First attempt to find the group enclosing everything we've been passed
    Matcher gm = gp.matcher(locStr);
    if (!gm.matches()) {
      String message = ParseException.newMessage(GenbankLocationParser.class, featureAccession,
          "unknown", "Bad location string found", locStr);
    ...
    if (xrefName != null) {
      ...
      if (!xm.matches()) {
        String message = ParseException.newMessage(GenbankLocationParser.class, featureAccession,
            "unknown", "Bad location xref found", locStr);
        throw new ParseException(message);
      }
      String xrefAccession = xm.group(1);
      String xrefVersion = xm.group(3);
      if (xrefAccession.equals(featureAccession))
        crossRef = null;
      else {
        // Found an accession, but does it have a version?
        if (xrefVersion != null) {
          crossRef = (SimpleCrossRef) RichObjectFactory.getObject(SimpleCrossRef.class,
              new Object[] {featureNS.getName(), xrefAccession, Integer.valueOf(xrefVersion)});
        } else {
          crossRef = (SimpleCrossRef) RichObjectFactory.getObject(SimpleCrossRef.class,
              new Object[] {featureNS.getName(), xrefAccession, new Integer(0)});
        }
      }
    }
    ...
    if (groupType != null) {
      ...
      if (groupType.equalsIgnoreCase("complement")) {
        ...
        if (parentStrand == Strand.NEGATIVE_STRAND)
          strand = Strand.POSITIVE_STRAND;
        else
          strand = Strand.NEGATIVE_STRAND;

        // Get the parsed contents of the complement 'group'
        List resultBlocks = new ArrayList(RichLocation.Tools
            .flatten(parseLocString(featureNS, featureAccession, crossRef, strand, subLocStr)));
      ...
      // Otherwise, it's a compound location
        ...
        if (groupType.equalsIgnoreCase("order"))
          ...
          String message = ParseException.newMessage(GenbankLocationParser.class, featureAccession,
              "unknown", "Unknown group type found", locStr);
          throw new ParseException(message);
        }
        ...
        try {
          for (int i = 0; i < chars.length; i++) {
            ...
            if (c == ',' && bracketCount == 0) {
              ...
              members
                  .add(parseLocString(featureNS, featureAccession, crossRef, parentStrand, subStr));
          ...
          String message = ParseException.newMessage(GenbankLocationParser.class, featureAccession,
              "unknown", "Problem with location format", locStr);
          throw new ParseException(message);
        }
        if (sb.length() > 0) members.add(
            parseLocString(featureNS, featureAccession, crossRef, parentStrand, sb.toString()));
    ...
    if (!rm.matches()) {
      String message = ParseException.newMessage(GenbankLocationParser.class, featureAccession,
          "unknown", "Bad location description found", subLocStr);
      throw new ParseException(message);
    }
  ...
  private static Position parsePosition(String position) throws ParseException {
    ...
    boolean endStartsFuzzy =
        ((startfuzz != null && startfuzz.equals("<")) || (endfuzz != null && endfuzz.equals("<")));
    boolean endEndsFuzzy =
        ((endfuzz != null && endfuzz.equals(">")) || (startfuzz != null && startfuzz.equals(">")));
    ...
    if (endRangeType != null) {
      ...
      return new SimplePosition(endStartsFuzzy, endEndsFuzzy, Integer.parseInt(endStart),
          Integer.parseInt(endEnd), endRangeType);
    } else {
      // less fuzzy
      return new SimplePosition(endStartsFuzzy, endEndsFuzzy, Integer.parseInt(endStart));
    }
  ...
  private static String _writePosition(Position p) {
    ...
    // System.out.println("GenbankLocationParser._writePosition-p: ["+p+"], s:"+s+", e:"+e+", t:
    // ["+t+"], fs? "+fs+", fe? "+fe);
    if (s != e) {
  ...
  private final static boolean isAnyLocationOnDifferentStrand(
      final CompoundRichLocation theLocation) {
    final Iterator c = theLocation.blockIterator();
