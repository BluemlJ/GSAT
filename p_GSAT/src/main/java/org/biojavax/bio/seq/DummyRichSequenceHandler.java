...
public class DummyRichSequenceHandler implements RichSequenceHandler {
  ...
  public List<Symbol> toList(RichSequence seq) {
    return seq.getInternalSymbolList().toList();
  }
  ...
  public SymbolList subList(RichSequence seq, int start, int end) throws IndexOutOfBoundsException {
    if (seq.getCircular()) {
      try {
        ...
        ChunkedSymbolListFactory symsf =
            new ChunkedSymbolListFactory(new PackedSymbolListFactory());
        if (modEnd > seqLength) {
          // add it in chunks
          int remaining = modLength;
          int chunkSize = (seqLength - modStart) + 1;
          // add modStart -> seqLength
          symsf.addSymbols(seq.getAlphabet(), (Symbol[]) seq.getInternalSymbolList()
              .subList(modStart, seqLength).toList().toArray(new Symbol[0]), 0, chunkSize);
          remaining -= chunkSize;
          // repeat add seqLength
          while (remaining > seqLength) {
            chunkSize = seqLength;
            symsf.addSymbols(seq.getAlphabet(), (Symbol[]) seq.getInternalSymbolList()
                .subList(1, seqLength).toList().toArray(new Symbol[0]), 0, chunkSize);
            remaining -= chunkSize;
          }
          // add 0 -> remaining
          chunkSize = remaining;
          symsf.addSymbols(seq.getAlphabet(), (Symbol[]) seq.getInternalSymbolList()
              .subList(1, chunkSize).toList().toArray(new Symbol[0]), 0, chunkSize);
        } else {
          // add modStart->modEnd
          symsf.addSymbols(seq.getAlphabet(), (Symbol[]) seq.getInternalSymbolList()
              .subList(modStart, modEnd).toList().toArray(new Symbol[0]), 0, modLength);
        }
      ...
      return seq.getInternalSymbolList().subList(start, end);
  }
  ...
  public String seqString(RichSequence seq) {
    return seq.getInternalSymbolList().seqString();
  }
  ...
  public Iterator<Symbol> iterator(RichSequence seq) {
    return seq.getInternalSymbolList().iterator();
  }
}
