...
public interface RichSequenceFormat extends SequenceFormat {
  /**
   * Check to see if a given file is in our format. Some formats may be able to determine this by
   * filename, whilst others may have to open the file and read it to see what format it is in.
   * 
  ...
  /**
   * On the assumption that the file is readable by this format (not checked), attempt to guess
   * which symbol tokenization we should use to read it. For formats that only accept one
   * tokenization, just return it without checking the file. For formats that accept multiple
  ...
  /**
   * On the assumption that the stream is readable by this format (not checked), attempt to guess
   * which symbol tokenization we should use to read it. For formats that only accept one
   * tokenization, just return it without checking the stream. For formats that accept multiple
   * tokenizations, its up to you how you do it.
   * 
  ...
  /**
   * Informs the writer that we want to start writing. This will do any initialisation required,
   * such as writing the opening tags of an XML file that groups sequences together.
   * 
   * @throws IOException if writing fails.
   */
  ...
  /**
   * Informs the writer that are done writing. This will do any finalisation required, such as
   * writing the closing tags of an XML file that groups sequences together.
   * 
   * @throws IOException if writing fails.
   */
  ...
  /**
   * Reads a sequence from the given buffered reader using the given tokenizer to parse sequence
   * symbols. Events are passed to the listener, and the namespace used for sequences read is the
   * one given. If the namespace is null, then the default namespace for the parser is used, which
   * may depend on individual implementations of this interface.
   * 
   ...
   * @throws IllegalSymbolException if the tokenizer couldn't understand one of the sequence symbols
   *         in the file.
  ...
  public boolean readRichSequence(BufferedReader reader, SymbolTokenization symParser,
      RichSeqIOListener listener, Namespace ns)
      throws BioException, IllegalSymbolException, IOException;

  /**
   * Writes a sequence out to the outputstream given by beginWriting() using the default format of
   * the implementing class. If namespace is given, sequences will be written with that namespace,
   * otherwise they will be written with the default namespace of the implementing class (which is
   * usually the namespace of the sequence itself). If you pass this method a sequence which is not
   * a RichSequence, it will attempt to convert it using RichSequence.Tools.enrich(). Obviously this
   * is not going to guarantee a perfect conversion, so it's better if you just use RichSequences to
   * start with!
   * 
  ...
  /**
   * Use this method to toggle reading of comments data. Will also ignore remarks lines in
  ...
  /**
   * Is the format going to emit events when comments data or remarks from bibliographic references
   * are read?
   * 
   * @return true if it is <em>not</em> otherwise false (false is default).
   */
