...
/**
 * Reference implementation of a BioEntry object which has no features or sequence. Equality is the
 * combination of namespace, name, accession and version.
 * 
...
public class SimpleBioEntry extends AbstractChangeable implements BioEntry {
  ...
  /**
   * Creates a new bioentry representing the sequence in the given namespace with the given name,
   * accession and version. These properties are all immutable and non-nullable.
   * 
  ...
  /**
   * {@inheritDoc} Two bioentries are equal if they share the same namespace, name, accession and
   * version.
   */
  public boolean equals(Object obj) {
    ...
    return (this.ns.equals(them.getNamespace()) && this.name.equals(them.getName())
        && this.accession.equals(them.getAccession()) && this.version == them.getVersion());
  }

  /**
   * {@inheritDoc} Bioentries are ordered first by namespace, then name, accession, and finally
   * version.
   */
  ...
  public String toString() {
    return this.getNamespace() + ":" + this.getName() + "/" + this.getAccession() + "."
        + this.getVersion();
  }
  ...
  // Hibernate requirement - not for public use.
  void setRelationships(Set<BioEntryRelationship> relationships) {
  ...
  // Hibernate requirement - not for public use.
  void setRankedDocRefs(Set<RankedDocRef> docrefs) {
