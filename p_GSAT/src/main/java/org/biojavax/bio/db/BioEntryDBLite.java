...
/**
 * A database of BioEntrys. This may have several implementations with rich behaviour, but basically
 * most of the time you will just use the interface methods to do stuff. A BioEntry database
 * contains a finite number of BioEntrys stored under unique keys.
 *
...
public interface BioEntryDBLite {
  /**
   * Signals that sequences are being added to or remove from the database. The sequences being
   * removed should be listed in the previous field by id, either as a single String, an array or a
   * Set. The sequences being added should be listed in the change field as either an array Object[]
   * { id, seq}, or a Map of id->seq.
   */
  public static final ChangeType BIOENTRYS =
      new ChangeType("BioEntrys have been added or removed from the database",
          "org.biojavax.bio.db.BioEntryDB", "BIOENTRYS");
  ...
  /**
   * Retrieve multiple BioEntry into a specific sequence database. If that database is null, a new
   * HashBioEntryDB is used.
   *
   * @param ids a set of ids to retrieve by
   ...
   * @throws ChangeVetoException if either the database does not allow BioEntrys to be added or the
   *         modification was vetoed
   */
   ...
   * @throws BioException if something failed while removing the BioEntry for that ID
   * @throws ChangeVetoException if either the database does not allow BioEntrys to be removed or
   *         the modification was vetoed
   */
