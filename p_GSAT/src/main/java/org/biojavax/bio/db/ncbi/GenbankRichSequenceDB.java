...
/**
 * This class contains functions accessing DNA sequences in Genbank format. It adds methods to
 * return RichSequences instead of plain Sequences.
 *
 * @author Lei Lai
...
public class GenbankRichSequenceDB extends AbstractRichSequenceDB implements RichSequenceDBLite {
  ...
  /**
   * The default constructor delegates to the parent class. The constructor refers to
   * RichObjectFactory.getDefaultNamespace() so make sure your factory is initialised before calling
   * this constructor. Sets the default factory to THRESHOLD.
   */
  ...
  protected URL getAddress(String id) throws MalformedURLException {
    ...
    String url = baseurl + db + "&id=" + id + "&rettype=" + type + "&retmode=" + mode + "&tool="
   ...
   * @throws Exception if the sequence could not be retrieved for reasons other than the identifier
   *         not being found.
   */
  public RichSequence getRichSequence(String id) throws BioException, IllegalIDException {
    try {
      ...
      SymbolTokenization rParser = DNATools.getDNA().getTokenization("token"); // get
                                                                               // SymbolTokenization
  ...
  /**
   * Given the appropriate Genbank ID, return the matching RichSequence object. Additionally define
   * a new Namespace for the received RichSequence object.
   * 
   * @param id the Genbank ID to retrieve.
   ...
   *         not being found.
   */
  public RichSequence getRichSequence(String id, Namespace nsp)
    ...
    try {
      ...
      RichSequenceIterator seqI =
          RichSequence.IOTools.readGenbank(reader, rParser, seqFactory, nsp);
   ...
   * @return The rich database object (HashSequenceDB) with downloaded rich sequences. You will need
   *         to cast the sequences you get from this database object into RichSequence objects if
   *         you want to access their full features.
   */
   ...
   *
   * @param list List of NCBI sequence number (GI), accession, accession.version, fasta or seqid.
   * @param database Where to store rich sequences. If database is null, use an HashSequenceDB
   *        Object.
   * @return The database object with downloaded rich sequences. You will need to cast the sequences
   *         you get from this database object into RichSequence objects if you want to access their
   *         full features.
   */
  public RichSequenceDB getRichSequences(Set list, RichSequenceDB database)
      throws BioException, IllegalIDException {
    try {
      ...
      RichSequenceIterator seqI =
          RichSequence.IOTools.readGenbank(fromServer, rParser, seqFactory, ns);
