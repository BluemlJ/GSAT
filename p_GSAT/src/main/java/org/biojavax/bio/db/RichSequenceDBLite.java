...
/**
 * A database of RichSequences. This may have several implementations with rich behaviour, but
 * basically most of the time you will just use the interface methods to do stuff. A RichSequence
 * database contains a finite number of RichSequences stored under unique keys.
 *
...
public interface RichSequenceDBLite extends BioEntryDBLite, SequenceDBLite {
  ...
  /**
   * Retrieve multiple RichSequence into a specific sequence database. If that database is null, a
   * new HashRichSequenceDB is used.
   *
   * @param ids a set of ids to retrieve by
   ...
   * @throws ChangeVetoException if either the database does not allow RichSequences to be added or
   ...
   * @throws BioException if something failed while removing the RichSequence for that ID
   * @throws ChangeVetoException if either the database does not allow RichSequences to be removed
   *         or the modification was vetoed
   */
