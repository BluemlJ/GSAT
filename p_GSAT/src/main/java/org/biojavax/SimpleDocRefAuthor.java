...
public class SimpleDocRefAuthor extends Unchangeable implements DocRefAuthor {
  ...
  public SimpleDocRefAuthor(String name, boolean consortium, boolean editor) {
    ...
    if (editor == true) {
      ...
      if (idx != -1 && idx == (name.length() - 6)) this.name = name.substring(0, name.length() - 6);
    }
  ...
  public SimpleDocRefAuthor(String s) {
    ...
    if (parts.length == 3) {
      ...
      if (parts[1].equals("ed.") || parts[2].equals("ed."))
        this.editor = true;
      else
        this.editor = false;
      if (parts[1].equals("consortium") || parts[2].equals("consortium"))
      ...
      parts[1] = parts[1].trim();
      parts[1] = parts[1].substring(0, parts[1].length() - 1); // chomp bracket
      if (parts[1].equals("ed."))
        this.editor = true;
      else
        this.editor = false;
      if (parts[1].equals("consortium"))
        this.consortium = true;
      else
        this.consortium = false;
    } else {
  ...
  /**
   * {@inheritDoc} Document authors are compared first by name, then consortium status, then editor
   * status.
   */
  ...
  /**
   * {@inheritDoc} Document references are equal if they have all fields the same.
   */
  public boolean equals(Object obj) {
    if (this == obj) return true;
    if (obj == null || !(obj instanceof DocRefAuthor)) return false;
    DocRefAuthor them = (DocRefAuthor) obj;
    return (this.name.equals(them.getName()) && this.consortium == them.isConsortium()
  ...
  /**
   * {@inheritDoc} Form: "name (consortium) (ed.)" where sections in brackets are optional.
   */
