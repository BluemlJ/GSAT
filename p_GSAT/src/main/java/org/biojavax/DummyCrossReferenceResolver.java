...
public class DummyCrossReferenceResolver implements CrossReferenceResolver {
  ...
  /**
   * {@inheritDoc} All responses are instances of InfinitelyAmbiguousSymbolList.
   */
  public SymbolList getRemoteSymbolList(CrossRef cr, Alphabet a) {
    if (!(a instanceof FiniteAlphabet)) throw new IllegalArgumentException(
        "Cannot construct dummy symbol list for a non-finite alphabet");
    return new InfinitelyAmbiguousSymbolList((FiniteAlphabet) a);
  }

  /**
   * {@inheritDoc} All responses are null.
   */
