 ...
 *
 * <p>
 * This class provides common constants representing access modifiers and types. There are also some
 * utility methods for munging data.
 * </p>
 *
 * @author Thomas Down
...
public class CodeUtils {
   ...
   *
   * <p>
   * The names of each class in classes will be seperated by a comma and a space and will use
   * CodeClass.getName() to produce strings for each one. Their names will be present in the return
   * value in the same order they are found in the classes array
   * </p>
   *
   ...
   *
   * <p>
   * Longs and doubles require 2 words (64 bits), where as everything else needs 1 word (32 bits).
   * Void needs no words. This just hides that knowledge.
   * </p>
   *
   ...
   *
   * <p>
   * All numeric types are integer (whole number) types, except for the floating point types. All
   * other classes are not integer types.
   * </p>
   *
   * @param cc the class to check
  ...
  public static boolean isIntegerType(CodeClass cc) {
    return cc == TYPE_LONG || cc == TYPE_BOOLEAN || cc == TYPE_BYTE || cc == TYPE_CHAR
        || cc == TYPE_LONG || cc == TYPE_SHORT;
  }
}
