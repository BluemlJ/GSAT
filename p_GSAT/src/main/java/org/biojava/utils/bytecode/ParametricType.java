 ...
 *
 * <p>
 * Template types are resolved at code-generation type rather than at Instruction generation type.
 * They let you bind the concrete type for opcodes at the last minute, so the same max conditional
 * could be used for all primative types, with the type only being bound at the last moment.
 * </p>
 *
 * <p>
...
public class ParametricType {
  ...
  /**
   * Create a new ParametricType that claims to resolve to an array type. All array types are object
   * types.
   *
   * @param name the name given to this type
   * @return a new ParametricType instance with that name
  ...
  /**
   * Create a new ParametricType that claims to be castable to all the classes in a list. Since
   * neither Java nor bytecode support multiple inheritance, the classes must either be interfaces,
   * or classes that fall into an inheritance path.
   *
   * @param name the name given to this type
   * @param classes an array of Class objects that any bound type must be castable to
