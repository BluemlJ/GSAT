...
/**
 * Factory for objects which encapsulate individual Java bytecode instructions. Most methods in this
 * class are auto-generated.
 *
 * <p>
 * There are two classes of methods. The first ones are for creating objects that directly represent
 * opcodes. These effectively wrap one of the opcode constants. The others do something more clever.
 * For example, make_if emits something that is equivalent to a normal Java if statement.
 * </p>
 *
 * <p>
...
public class ByteCode {
  ...
  public static Instruction make_iload(LocalVariable lv) throws CodeException {
    ...
    if (cc != CodeUtils.TYPE_INT && cc != CodeUtils.TYPE_SHORT && cc != CodeUtils.TYPE_CHAR
        && cc != CodeUtils.TYPE_BYTE && cc != CodeUtils.TYPE_BOOLEAN) {
  ...
  public static Instruction make_return(CodeClass clazz) {
    if (false) {} else if (CodeUtils.TYPE_VOID.equals(clazz)) {
      return make_return();
    } else if (CodeUtils.TYPE_BYTE.equals(clazz) || CodeUtils.TYPE_SHORT.equals(clazz)
        || CodeUtils.TYPE_CHAR.equals(clazz) || CodeUtils.TYPE_BOOLEAN.equals(clazz)
   ...
   *
   * <p>
   * Constructors and private methods are invoked by invokespecial. Static methods are invoked by
   * invokestatic. Methods referred to by interface are invoked by invokeinterface, and methods
   * referred to by classes are invoked by invokeabstract.
   * </p>
   *
  ...
  /**
   * Synchronize the processing of an entire block of code on a local variable. The local variable
   * must refer to a Java object. It is safe to replace the value of the local variable within the
   * block.
   *
   * @param lockVar Label for the local variable to lock on
   * @param code the CodeGenerator that will make the body of the synchronized block
   * @throws CodeException if locVar holds a primative value
   */
  public static CodeGenerator make_synchronizedBlock(LocalVariable lockVar, CodeGenerator code)
   ...
   *
   * <p>
   * Obcodes for IF jump to the label if the condition is true, or execute the next instruction
   * ontherwise. In this case, op will be used to compare the top of the stack and if the condition
   * is true, will cause a jump to the label. Otherwise, the next instruction in the stream will be
   * used.
   * </p>
   *
   * @param op if opcode
   ...
   * @return a ParametricCodeGenerator that will load the correct type.
   */
  public ParametricCodeGenerator make_load(final ParametricType type, final LocalVariable lv)
      throws CodeException {
    return new ParametricCodeGenerator() {
      public ParametricType getType() {
        return type;
   ...
   */
  public ParametricCodeGenerator make_save(final ParametricType type, final LocalVariable lv)
      throws CodeException {
    return new ParametricCodeGenerator() {
      public ParametricType getType() {
        return type;
  ...
  public PParametricCodeGenerator make_cast(final ParametricType from, final ParametricType to)
      throws CodeException {
    if (from.isObject()) {
