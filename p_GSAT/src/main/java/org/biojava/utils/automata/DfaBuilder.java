...
public class DfaBuilder {
  private class DfaNode {
    ...
    /**
     * checks whether this closure set has a terminal Node in it.
     */
  ...
  /**
   * Given a DFA node representing a Set of NFA nodes, construct other DFA nodes that transit from
   * it.
   */
  private void constructSubsets(DfaNode dfaNode) throws AutomatonException {
    ...
    for (Iterator symI = symbolSet.iterator(); symI.hasNext();) {
      ...
      for (Iterator closI = closureSet.iterator(); closI.hasNext();) {
        org.biojava.utils.automata.Nfa.Node node =
            (org.biojava.utils.automata.Nfa.Node) closI.next();
        ...
        // System.out.println("closure set for " + currSymbol.getName() + " from NFA node " +
        // node.getID() + " is " + currNodeSet.toString());
  ...
  /**
   * get the DFA Node associated with the closure Set of NFA nodes. If it does not exist, create it.
   */
