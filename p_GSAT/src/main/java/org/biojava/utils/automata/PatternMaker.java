...
public class PatternMaker {
  ...
  private static class Tokenizer {
    ...
    private char getToken() throws IndexOutOfBoundsException {
      if (hasNext())
        ...
        throw new IndexOutOfBoundsException(
            "text length: " + packedTxt.length() + " index: " + ptr);
    }
    ...
    private int nextTokenType() {
      ...
      if (Character.isLetter(nextCh)) return SYMBOL_TOKEN;
      if (Character.isDigit(nextCh)) return NUMERIC;
      // now check for specific chars
      if (nextCh == '{') return LEFT_BRACE;
      if (nextCh == '}') return RIGHT_BRACE;
      if (nextCh == ',') return COMMA;
      if (nextCh == '(') return LEFT_BRACKET;
      if (nextCh == ')') return RIGHT_BRACKET;
      if (nextCh == '+') return PLUS;
      if (nextCh == '*') return ASTERISK;
      if (nextCh == '|') return VERT_BAR;
      return UNKNOWN;
    }
  ...
  /**
   * Compiles the regex described by patternString into a DFA. The DFA can subsequently be converted
   * into a state state machine for searching. <b>This is the main work method for this class.</b>
   */
  ...
  private NfaSubModel parse(NfaBuilder delegate) throws AutomatonException {
    /**
     * The model is that the pattern here can have alternative patterns so a single pattern ends up
     * being a pattern of branching 1.
     */
    ...
    while (toke.hasNext()) {
      ...
      switch (tokenType) {
        ...
        case Tokenizer.VERT_BAR:
          ...
          System.out
              .println(returnSubModel.getStart().getID() + " " + branchSubModel.getStart().getID());
        ...
        case Tokenizer.RIGHT_BRACKET:
          ...
          // link current branch into return value
          if (!gotContent) throw new AutomatonException("no content in this branch!");
          // System.out.println(returnSubModel.getStart().getID() + " " +
          // branchSubModel.getStart().getID());
          returnSubModel.addEpsilonTransition(returnSubModel.getStart(), branchSubModel.getStart());
          returnSubModel.addEpsilonTransition(branchSubModel.getEnd(), returnSubModel.getEnd());
  ...
  NfaSubModel parseSymbol(NfaBuilder delegate) throws AutomatonException {
    ...
    if (sym instanceof AtomicSymbol) {
      ...
      for (Iterator symI = ((FiniteAlphabet) sym.getMatches()).iterator(); symI.hasNext();) {
      ...
      switch (tokenType) {
        ...
        default:
          throw new AutomatonException(
              toke.getToken() + " is not valid in an iteration specifier.");
      }
    }
