 ...
 * <p>
 * This implementation has epsilon and lambda transitions. Both transitions are silent but the
 * former is intended to be optimised away while the latter must be retained during optimisation.
 * This is necessary to implement limited closure for the REs that one may want to build with this
 * NFA.
 *
 * @author David Huen
 * @since 1.4
...
public class Nfa extends FiniteAutomaton implements NfaBuilder {
  ...
  /**
   * Retrieve all Nodes reachable from the specified node by emissionless lambda transitions.
   */
  // FIXME: take into consideration cycles of lambda transitions!
  NodeSet getLambdaClosure(Node node) throws AutomatonException {
    return _getLambdaClosure(node, createNodeSet());
  }

  private NodeSet _getLambdaClosure(Node node, NodeSet visitedNodes) throws AutomatonException {
  ...
  /**
   * goes thru data structures replacing every instance of old Node with new Node. Duplicate entries
   * that arise from the process are removed too.
   * <p>
   * The Nfa version of this method also strips epsilon self-transitions.
   */
  private void replaceNode(Set oldNodes, Node newNode) {
    ...
    for (int i = 0; i < j; i++) {
      ...
      if ((currTransition.sym != EPSILON) || (currTransition.source != currTransition.dest))
