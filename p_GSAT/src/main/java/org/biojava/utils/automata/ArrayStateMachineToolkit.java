...
public class ArrayStateMachineToolkit implements StateMachineToolkit {
  /**
   * A StateMachine implementation in which the transitions are maintained in an array. While every
   * ArrayStateMachine is initialised with a FiniteAlphabet, symbol data is provided as an
   * AlphabetIndex int value for effiiciency reasons and callers are responsible for validating all
   * such data prior to passing it to an instance of this class. It is not recommended that this
   * class is used with Alphabets with a large number of Symbols as it uses a dense State matrix
   * implementation.
   *
   * @author David Huen
   * @since 1.4
  ...
  private static class ArrayStateMachine implements StateMachineFactory {
    ...
    class Instance implements StateMachineInstance {
       ...
       * @return true if the symbol is valid and this state machine should continue to receive
       *         input.
       */
      ...
      /**
       * Two Instances are equal if they are children of the same ArrayStateMachine instance and
      ...
      public boolean equals(Object o) {
        ...
        if (other.parent() != ArrayStateMachine.this) return false;
    ...
    class GreedyInstance implements StateMachineInstance {
      ...
      /**
       * Two GreedyInstances are equal if they are children of the same ArrayStateMachine instance
       * and have the same start value.
       */
      public boolean equals(Object o) {
        if (!(o instanceof Instance)) return false;
    ...
    /**
     * maps a Node ID to the index in the array where its transition data is stored.
     */
    private int[] entryPoints;

    /**
     * transition matrix. Organised as:- [source nodeID * symbol idx] -&gt; dest nodeID.
     */
    private byte[] transitions;

    /**
     * Creates an ArrayStateMachine from a FiniteAutomaton instance. The FiniteAutomaton must be in
     * its most compact form (node IDs in contiguous running order, no duplicate/surplus
     * nodes/transitions).
     */
    ...
    private void convert(FiniteAutomaton fa) {
      try {
        ...
        for (Iterator transI = transitionSet.iterator(); transI.hasNext();) {
          ...
          // System.out.println(currTransition.getSymbol() + " " + symIdx + " " +
          // currTransition.getSource().getID() + " " + currTransition.getDest().getID());
    ...
    /**
     * Return a StateMachineInstance if the Symbol represented by the symbol index is valid as the
     * initial symbol of the pattern. This method must remain package private as it does no alphabet
     * checks at all.
     *
     * @param symIdx alphabet index value for specified symbol.
     * @param greedy should greedy regex semantics be used?
     * @return an instance of StateMachineInstance if symbol is valid otherwise null.
     */
  ...
  public StateMachineFactory getFactory(String factoryName, FiniteAutomaton fa)
    ...
    if (alfa != fa.getAlphabet()) throw new IllegalAlphabetException(
        "The FiniteAutomaton is defined on an Alphabet incompatible with this ArrayStateMachineToolKit.");
