...
public class PatternBlitz {
  ...
  private void scanPatterns(Symbol sym, int start) throws IllegalSymbolException {
    ...
    for (ListIterator instanceI = instanceStore.listIterator(); instanceI.hasNext();) {

      // remove all terminal entries
      StateMachineInstance instance = (StateMachineInstance) instanceI.next();
      if (!instance.transit(symIdx)) instanceI.remove();
    }

    // now traverse the pattern store to initiate new instances
    for (Iterator patternI = patternStore.iterator(); patternI.hasNext();) {
  ...
  public void search(SymbolList sl) throws IllegalAlphabetException {
    ...
    if (sl.getAlphabet() != alfa) throw new IllegalAlphabetException("incompatible alphabets");
