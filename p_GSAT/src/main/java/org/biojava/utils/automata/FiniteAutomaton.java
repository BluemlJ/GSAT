 ...
 * <p>
 * This class models basic FA behaviour. More specialised behaviour is implemented by subclassing
 * this.
 *
 * @author David Huen
 * @since 1.4
...
public class FiniteAutomaton {
  ...
  class Node {
    ...
    /**
     * Nodes are assigned nodeIDs sequentially. Terminal nodes are assigned negative Node IDs.
     */
    ...
    /**
     * Two Nodes are equal if they share the same parent and have the same node ID.
     */
   ...
   */
  class NodeSet extends HashSet {
    /**
     * Adds a Node to this NodeSet. The Node and the NodeSet to which it is added must be from the
     * same FiniteAutomaton instance.
     */
    ...
    /**
     * Equality implies that both Objects are NodeSets, both from the same FiniteAutomaton instance
     * and have the same Nodes within them.
     */
    public boolean equals(Object o) {
    ...
    /**
     * Returns the parental FiniteAutomaton that this NodeSet is a child of.
     */
  ...
  class Transition {
    ...
    /**
     * Two Transitions are equal if they are both defined on the same FiniteAutomaton instance and
     * their source and destination Nodes and associated Symbols are equal.
     */
    public boolean equals(Object o) {
      ...
      if ((source.equals(other.source)) && (dest.equals(other.dest)) && (sym == other.getSymbol()))
        return true;
      else
        return false;
    }
    ...
    public int hashCode() {
      try {
        return (source.getID() << 20) + (dest.getID() << 10) + alphaIndex(sym);
      } catch (IllegalSymbolException ise) {
        throw new BioError(
            "Fatal error: Unexpected IllegalSymbolException on computing indexForSymbol.");
      }
    }
  ...
  /**
   * What is the Set of Nodes reachable from source Node by transitions associated with Symbol.
   */
  NodeSet getClosure(Node source, Symbol sym) {
    ...
    for (Iterator transI = getTransitions(source).iterator(); transI.hasNext();) {
      ...
      try {
        if (currTransition.sym == sym) closureSet.addNode(currTransition.dest);
  ...
  private Set getTransitions(Node source) {
    ...
    if ((transitionSet = (Set) transitionMap.get(source)) == null) {
      ...
      for (Iterator transI = transitions.iterator(); transI.hasNext();) {
        Transition currTransition = (Transition) transI.next();
        if (currTransition.source == source) transitionSet.add(currTransition);
      }
  ...
  /**
   * dumps internal data of Nodes and Transitions that describe this FiniteAutomaton. It is not
   * possible to dump it as a regex as there are FA that cannot be expressed as a regex.
   */
  public String toString() {
    StringBuffer output = new StringBuffer();
    ...
    /*
     * } catch (IllegalSymbolException ise) { throw new AssertionError(ise); } catch (BioException
     * be) { throw new AssertionError(be); }
     */
    return output.toString();
  }
