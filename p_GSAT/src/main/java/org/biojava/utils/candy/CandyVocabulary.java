...
/**
 * <p>
 * This interface defines functionality of a controlled vocabulary. The implementation is supposed
 * to behave as a Java bean (regarding accessing vocabulary properties).
 * </p>
 *
 * <p>
 * Each vocabulary consists of (usually many) vocabulary entries which are represented by
 * {@link CandyEntry CandyEntries}.
 * </p>
 *
 * @version $Id$
...
public interface CandyVocabulary extends PropertyChangeListener {
  ...
  /**
   * A property name. Its boolean value is <tt>true</tt> if the vocabulary entries names should be
  ...
  /**
   * A property name. Its type is {@link CandyVocabulary} and it can be used to set an entire
   * vocabulary. An implementation may use it together with an empty constructor.
   */
  ...
  /**
   * It returns a name of this vocabulary. The name should be unique within a {@link CandyFinder}
  ...
  /**
   * It returns <tt>true</tt> if the vocabulary entries should be considered as case-sensitive.
   */
   ...
   *
   * <p>
   * An implementation may use this boolean property to make sure that returned vocabulary entries
   * are in the same order as they were read from its original source.
   * </p>
   */
