...
/**
 * This class is analogous to java.util.Matcher except that it works on SymbolLists instead of
 * Strings. All coordinates are in the 1-based coordinate system used by SymbolLists.
 *
 * @author David Huen
 * @since 1.4
 */
public class Matcher {
  ...
  /**
   * Returns the index of the last Symbol, plus one, of the subsequence captured by the given group
   * during the previous match operation.
   * <p>
   * Capturing groups are indexed from left to right, starting at one. Group zero denotes the entire
   * pattern, so the expression m.end(0) is equivalent to m.end().
   * 
   * @param group The index of a capturing group in this matcher's pattern.
   * @return The index of the last Symbol captured by the group, plus one, or -1 if the match was
   ...
   * method was successful and the matcher has not since been reset, at the first Symbol not matched
   * by the previous match. If the match succeeds then more information can be obtained via the
   ...
   * methods, and subsequent invocations of the find() method will start at the first Symbol not
   ...
   * <p>
   * For a matcher m, input sequence s, and group index g, the expressions m.group(g) and
   ...
   * <p>
   * Any non-negative integer smaller than the value returned by this method is guaranteed to be a
   ...
   * <p>
   * Like the matches method, this method always starts at the beginning of the input sequence;
   * unlike that method, it does not require that the entire input sequence be matched. If the match
   * succeeds then more information can be obtained via the start, end, and group methods.
   * 
   ...
   * Resets this matcher.
   * <p>
   * Resetting a matcher discards all of its explicit state information and sets its append position
   * to zero.
   * 
   * @return this matcher.
   */
   ...
   * <p>
   * Capturing groups are indexed from left to right, starting at one. Group zero denotes the entire
   * pattern, so the expression m.start(0) is equivalent to m.start().
   * 
   * @param group The index of a capturing group in this matcher's pattern.
   * @return The index of the first character captured by the group, or -1 if the match was
   *         successful but the group itself did not match anything.
   */
