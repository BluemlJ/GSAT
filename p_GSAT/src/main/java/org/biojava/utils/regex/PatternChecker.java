...
class PatternChecker {
  private static class Tokenizer {
    ...
    private int nextTokenType() {
      ...
      if (nextCh == '[') return LEFT_SQBRACKET;
      if (nextCh == ']') return RIGHT_SQBRACKET;
      if (nextCh == '?') return Q_MARK;
      if (nextCh == '^') return CARET;
      if (nextCh == '$') return DOLLAR;
      return UNKNOWN;
    }
  ...
  String parse(String patternString) throws RegexException, IllegalAlphabetException {
    ...
    try {
      ...
      throw new IllegalAlphabetException(be,
          "Attempt to get a tokenization for this alphabet failed.");
    }
  ...
  void parseSymbol() throws IllegalSymbolException {
    ...
    while (toke.nextTokenType() == Tokenizer.SYMBOL_TOKEN) {
      ...
      if (sym instanceof AtomicSymbol) {
        ...
        SortedSet sortedAmbigs = new TreeSet(new Comparator() {
        ...
        output.append("[");
        for (Iterator symI = sortedAmbigs.iterator(); symI.hasNext();) {
          Symbol atomicSym = (Symbol) symI.next();
  ...
  void parseVariantSymbols() throws RegexException {
    ...
    if (toke.nextTokenType() == Tokenizer.RIGHT_SQBRACKET)
      output.append(toke.getToken());
    else
      throw new RegexException(
          "missing right square bracket while specifying variants. Encountered " + toke.getToken()
              + " instead.");
  }
