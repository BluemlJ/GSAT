...
public class PatternFactory {
  ...
  private void fetchTokenizer() {
    ...
    try {
      ...
      if (toke.getTokenType() == SymbolTokenization.CHARACTER) gotCharTokenizer = true;
    ...
    if (!gotCharTokenizer) {
      ...
      cToke.bindSymbol(AlphabetManager.getAllAmbiguitySymbol((FiniteAlphabet) alfa), '\uF8FF');
      // add terminal gap
  ...
  /**
   * Returns a Pattern object that applies the specified regex against SymbolLists in the Alphabet
   * that this PatternFactory was defined against. The String returned by getName() is set to
   * pattern.
   */
  ...
  /**
   * Returns a Pattern object that applies the specified regex against SymbolLists in the Alphabet
   * that this PatternFactory was defined against.
   *
   * @param pattern regex pattern expressed as a String.
   * @param label A String label assigned to the Pattern object. Can be retrieved later with
   *        getName().
   */
  ...
  /**
   * Returns the character that represents the specified Symbol in the Alphabet that this
   * PatternFactory was defined for.
   * <p>
   * The character will be ASCII in Alphabets that define a Character tokenization. In Alphabets
   * that don't a Unicode character in the private range is returned instead and this can be used to
   * assemble the String that is the argument for the compile method.
   */
