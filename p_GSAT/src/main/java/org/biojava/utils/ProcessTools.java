 ...
 *
 * This code may be freely distributed and modified under the terms of either the BSD licence or the
 * GNU Lesser General Public Licence. These should be distributed with the code. If you do not have
 * copies see:
 *
 * http://www.opensource.org/licenses/bsd-license.php http://www.gnu.org/copyleft/lesser.html
 *
...
/**
 * Convenience methods for running external processes. This class offers wrappers around the
 * <code>java.lang.Process</code> API, but hides away the details of managing threads and process
 * I/O.
 *
 * <h3>Example</h3>
 *
 ...
 * @since 1.4
 * @deprecated preferable to use org.biojava.utils.ExecRunner or the org.biojava.utils.process
 *             package.
 */
...
public class ProcessTools {
   ...
   * @param input data to present to the process' standard input, or <code>null</code> if the
   *        process does not require input.
   * @param stdout a <code>Writer</code> which will be filled with data from the process' output
   *        stream, or <code>null</code> to ignore output.
   * @param stderr a <code>Writer</code> which will be filled with data from the process' error
   *        stream, or <code>null</code> to ignore output.
   * @return the process' return code.
   */

  public static int exec(String[] args, Reader input, Writer stdout, Writer stderr)
      throws IOException {
    try {
  ...
  /**
   * Execute the specified command and wait for it to return, or kill it if the specified timeout
   * expires first.
   *
   * @param args the command line to execute.
   * @param envp environment variables for the child process, or <code>null</code> to inherit the
   *        current set.
   * @param dir working directory for the child process, or <code>null</code> to inherit the current
   *        directory.
   * @param input data to present to the process' standard input, or <code>null</code> if the
   *        process does not require input.
   * @param stdout a <code>Writer</code> which will be filled with data from the process' output
   ...
   * @param stderr a <code>Writer</code> which will be filled with data from the process' error
   *        stream, or <code>null</code> to ignore output.
   * @param timeout maximum run-time (in milliseconds) for the child process. A value of 0 indicates
   *        no limit.
   * @return the process' return code.
   * @throws IOException if an error occurs while starting or communicating with the process
   * @throws ProcessTimeoutException if the child process was killed because its timeout had
   *         expired.
   */

  public static int exec(String[] args, String[] envp, File dir, Reader input, Writer stdout,
      Writer stderr, long timeout) throws IOException, ProcessTimeoutException {
  ...
  /**
   * Execute the specified command and wait for it to return. This is the simplified version that
   * tries to be nice and make your life easier. If you know exactly what you want, you might want
   * to use exec(String[],...) instead.
   *
   ...
   * @throws IOException if an error occurs while starting or communicating with the process
   */
  public static int exec(String command, Reader input, Writer stdout, Writer stderr)
      throws IOException {
    try {
   ...
   *         expired.
   */
  public static int exec(String command, String[] envp, File dir, Reader input, Writer stdout,
      Writer stderr, long timeout) throws IOException, ProcessTimeoutException {
    String[] cmd = null;
  ...
  /**
   * Check the status of a Pump and re-throw any exception which may have occured during its
   * lifecycle
   *
   * 
   * private static void checkException(Pump p, String msg) throws IOException { IOException ioe =
   * p.getException(); if (ioe != null) { throw new IOException("Exception processing " + msg); } }
   */
  ...
  /**
   * Thread which will kill the specified process if it is not defused before the timeout expires.
   */
  ...
  /**
   * Base class for threads which pump bytes from a source to a sink. Subclasses must implement
   * sourceData and sinkData. They may also wish to override shutdownHook, a dummy method which is
   * called when the pump finishes (usually because the end of the data source has been reached).
   *
   * 
   * private static abstract class Pump extends Thread { private IOException err = null;
   * 
   ...
   * 
   * public void run() { try { byte[] buf = new byte[256]; int cnt; do { cnt = sourceData(buf); if
   * (cnt > 0) { sinkData(buf, cnt); } } while (cnt >= 0); shutdownHook(); } catch (IOException e) {
   * this.err = e; } }
   * 
   * public IOException getException() { return err; } }
   */
  ...
  /*
   * private static final class PumpStreamToStringBuffer extends Pump { private final InputStream
   * is; private final StringBuffer sb;
   * 
   * public PumpStreamToStringBuffer(InputStream is, StringBuffer sb) { super(); this.is = is;
   * this.sb = sb; }
   * 
   * protected int sourceData(byte[] buf) throws IOException { return is.read(buf); }
   * 
   * protected void sinkData(byte[] buf, int len) { sb.append(new String(buf, 0, len)); } }
   * 
   * private static final class PumpStreamToNull extends Pump { private final InputStream is;
   * 
   * public PumpStreamToNull(InputStream is) { super(); this.is = is; }
   * 
   * protected int sourceData(byte[] buf) throws IOException { return is.read(buf); }
   * 
  ...
  /*
   * private static final class PumpStreamToStream extends Pump { private final InputStream is;
   * private final OutputStream os;
   * 
   * public PumpStreamToStream(InputStream is, OutputStream os) { this.is = is; this.os = os; }
   * 
   * protected int sourceData(byte[] buf) throws IOException { return is.read(buf); }
   * 
   * protected void sinkData(byte[] buf, int len) throws IOException { os.write(buf, 0, len);
   * os.flush(); }
   * 
   * protected void shutdownHook() throws IOException { os.close(); } }
   */
}
