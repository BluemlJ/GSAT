...
/**
 * A wrapper around {@link java.io.InputStream} that provides in-memory caching of the input data.
 * This allows it to provide a {@link #seek} method, which lets the user use an
 * {@link java.io.InputStream} like a {@link java.io.RandomAccessFile} (with appropriate caveats
 * about memory footprint, security, and performance).
 * <p>
...
public class CachingInputStream extends InputStream implements Seekable {
  ...
  /**
   * The 0-based index into cache of the _next_ byte to return. If ptr == validLen, data must be
   * read from the stream into the cache.
   */
  protected int ptr;
  /**
   * A count of the number of bytes in {@link #cache} that contain data read from the stream.
   */
  ...
  public void seek(long pos) throws IOException {
    if (pos > Integer.MAX_VALUE || pos < 0) {
      throw new IllegalArgumentException(
          "Cannot seek to " + pos + ": can only do 0 <= seek < " + Integer.MAX_VALUE);
    }
    int newPtr = (int) pos;
    if (newPtr <= validLen) {
      ptr = newPtr;
  ...
  public long skip(long num) throws IOException {
    if (ptr + num > Integer.MAX_VALUE) return 0;
    int n = (int) num;
  ...
  /**
   * Expands the cache to hold some number of <code>additionalBytes</code>. Expansion is done
   * multiplicatively for efficiency. Immediately after calling this method, you must fill the
   * additional bytes from the stream because this method also updates validLen.
   */
