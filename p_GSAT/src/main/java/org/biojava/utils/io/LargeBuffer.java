...
public class LargeBuffer {
  /*
   * We will set up MappedByteBuffers that are responsible for PAGE_SIZE bytes. Unfortunately, word
   * boundaries are not aligned, so someone could try to write a double to the last byte in a
   * buffer. So,
   */
  ...
  public LargeBuffer(FileChannel channel, FileChannel.MapMode mode, long pos, long size)
      throws IOException {
  ...
  private MappedByteBuffer getBuffer(int index) throws IOException {
    if (index != lastBufferIndex) {
      ...
      lastBuffer =
          channel.map(mode, pos + offset, Math.min(size - offset, PAGE_SIZE + PAGE_OVERLAP));
