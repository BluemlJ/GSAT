 ...
 *
 * In addition, probably the most useful feature of ExecRunner is using it to run a command-line
 * program and obtain its stdout and stderr results in two strings. This is done with exec(String) -
 * see that method for an example.
 *
 * With acknowledgements to Michael C. Daconta, author of "Java Pitfalls, Time Saving Solutions, and
 * Workarounds to Improve Programs." and his article in JavaWorld "When Runtime.exec() Won't".
 * </P>
 *
...
public class ExecRunner {
   ...
   *
   * @param command The program or command to run
   * @param arguments An array of Strings in which every single String is exactly one of the
   *        program's arguments. So these arguments are allowed to contain white spaces and are
   *        marked as Strings by the system.
   * @throws ExceptionInInitializerError thrown if a problem occurs
   * @since 1.5
   */
  public ExecRunner(String command, String[] arguments) throws ExceptionInInitializerError {
    this();
    try {
  ...
  /**
   * The <B>exec(String)</B> method runs a process inside of a watched thread. It returns the
   * client's exit code and feeds its STDOUT and STDERR to ExecRunner's out and err strings, where
   * you then use getOutString() and getErrString() to obtain these values. If the command String
   * contains arguments which are Strings containing white spaces, the method
   * <pre>exec(program, arguments)</pre> should be used instead, where <code>arguments</code> is a
   * <code>String[]</code> array containing the arguments of the program.
   * 
   * Example:
   *
   * <pre>
  ...
  /**
   * Sometimes special cases may occur that the arguments of an external program are Strings
   * containing white spaces. Another example are external processes needing the String arguments to
   * be characterized in a special way. In this cases it is sensefull to write every single argument
   * in an String array. The system itselfe will mark these arguments as Strings so that these
   * special encoding can be omitted.
   * 
   * @param command The external program to be executed.
   * @param arguments An array of Strings. Every entry of this array is an argument of the external
   ...
   * @since 1.5
   */
  public int exec(String command, String[] arguments) throws IOException, InterruptedException {
   ...
   * @throws InterruptedException thrown if a problem occurs
   **/
  public int exec(String command, OutputStream stdoutStream, OutputStream stderrStream)
   ...
   * @since 1.5
   **/
  public int exec(String command, String[] arguments, OutputStream stdoutStream,
  ...
  /**
   * The <code>exec(String, PrintWriter, PrintWriter)</code> method runs a process inside of a
   * watched thread. It returns the client's exit code and feeds its STDOUT and STDERR to the
   * passed-in streams.
   *
   * @return The command's return code
   * @param command The program or command to run
   ...
   * @throws InterruptedException thrown if a problem occurs
   **/
  public int exec(String command, PrintWriter stdoutWriter, PrintWriter stderrWriter)
      throws IOException, InterruptedException {
   ...
   * The <code>exec(String, PrintWriter, PrintWriter)</code> method runs a process inside of a
   * watched thread. It returns the client's exit code and feeds its STDOUT and STDERR to the
   * passed-in streams. The arguments for the external program or command are passed as an array of
   * Strings. This has the advantage that the arguments are allowed to contain white spaces and are
   * marked to be Strings by the System itselfe, which is sometimes important. Every single argument
   * has to be exactly one dimension of the arguments array.
   *
   * @return The command's return code
   * @param command The program or command to run
   ...
   * @since 1.5
   **/
  public int exec(String command, String[] arguments, PrintWriter stdoutWriter,
  ...
  /**
   * We override the <code>readObject</code> method here to prevent deserialization of our class for
   * security reasons.
   *
  ...
  /**
   * Sets the maximum run time in seconds. If you do not want to limit the executable's run time,
   * simply pass in a 0 (which is also the default).
   *
  ...
  /**
   * We override the <code>writeObject</code> method here to prevent serialization of our class for
   * security reasons.
   *
...
class StreamGobbler extends Thread {
  ...
  /**
   * A more explicit constructor for StreamGobbler where you can tell it exactly where to relay the
   * output to. Creation date: (9/23/2001 8:48:01 PM)
   *
   * @param in InputStream
  ...
  /**
   * Tells the StreamGobbler to quit it's operation. This is safer than using stop() since it uses a
   * semophore checked in the main wait loop instead of possibly forcing semaphores to untimely
   * unlock.
   */
  public void quit() {
