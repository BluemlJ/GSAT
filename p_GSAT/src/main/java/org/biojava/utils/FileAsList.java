...
/**
 * <code>FileAsList</code> creates a writable <code>List</code> implementation backed by a random
 * access file. There is a restriction on the record length that the string representation of that
 * integer may not be longer than 4 bytes. This is because a fixed 4 byte leader is used to encode
 * the record length in the file.
 *
...
public abstract class FileAsList extends AbstractList implements Commitable {
  ...
  /**
   * Creates a new <code>FileAsList</code> and corresponding backing file.
   *
   * @param mappedFile a <code>File</code> used to back the list. This file must not already exist.
  ...
  /**
   * Creates a new <code>FileAsList</code> instance from an existing backing file.
   *
   * @param mappedFile a <code>File</code> used to back the list. This file must already exist.
  ...
  /**
   * <code>rawGet</code> reads the record at the specified index as a raw byte array.
   *
   * @param indx an <code>int</code> list index.
   *
   * @return a <code>byte []</code> array containing the raw record data.
   */
  ...
  public void rollback() {
    try {
      ...
      throw new AssertionFailure(
          "Could not roll back. " + "The index store will be in an inconsistent state "
