...
/**
 * This is a ChangeListener that is designed to adapt events of one type from one source to events
 * of another type emitted by another source. For example, you could adapt events made by edits in a
 * database to being events fired by a sequence implementation.
 *
...
public class ChangeForwarder implements ChangeListener {
  ...
  /**
   * Return the underlying <code>ChangeSupport</code> instance that can be used to fire
   * <code>ChangeEvent</code>s and mannage listeners.
   *
   ...
   *
   * <p>
   * The returned ChangeEvent is the event that will be fired, and should be built from information
   * in the original event. If it is null, then no event will be fired.
   * </p>
   *
   * <p>
   * The default implementation just constructs a ChangeEvent of the same type that chains back to
   * ce.
   * </p>
   *
  ...
  public void postChange(ChangeEvent ce) {
    try {
      ...
      throw new AssertionFailure(
          "Assertion Failure: Change was vetoed after it had been accepted by preChange", cve);
    }
  }
