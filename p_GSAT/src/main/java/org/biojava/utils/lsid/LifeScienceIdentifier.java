 ...
 * <p>
 * TODO: <br/>
 * Should this class support the spec definition of &quot;lexical equivalence&quot; in the
 * equals(Object) method, or by the introduction of a new method
 * <code>boolean lexicallyEquivalent(LifeScienceIdentifier)</code>?
 * </p>
 * <p>
 * From the spec: <br/>
 * For various purposes such as caching and replication, it's often desirable to determine if two
 * LSIDs are the same without resolving them. The general purpose means of doing so is by testing
 * for "lexical equivalence" as defined: LSIDs are lexically equivalent if the AuthorityID,
 * NamespaceID, ObjectID, and RevisionID are all identical (case-insensitive comparison).
 * </p>
 *
...
public final class LifeScienceIdentifier implements Serializable {
   ...
   *
   * @throws IllegalArgumentException if any of <code>authorityId</code>, <code>namespaceId</code>,
   *         or <code>objectId</code> are null
   */
  private LifeScienceIdentifier(String authorityId, String namespaceId, String objectId,
      String revisionId) {
    if (authorityId == null) throw new IllegalArgumentException("authority must not be null");
    if (namespaceId == null) throw new IllegalArgumentException("namespace must not be null");
  ...
  public boolean equals(Object value) {
    ...
    if (!(value instanceof LifeScienceIdentifier)) return false;

    LifeScienceIdentifier lsid = (LifeScienceIdentifier) value;

    return (authorityId.equals(lsid.getAuthorityId()) && namespaceId.equals(lsid.getNamespaceId())
        && objectId.equals(lsid.getObjectId()) && (revisionId == null
            ? lsid.getRevisionId() == null
            : revisionId.equals(lsid.getRevisionId())));
  }
  ...
  /**
   * Create a new LifeScienceIdentifier parsed from the properly formatted string <code>lsid</code>.
   *
   * @param lsid formatted LifeScienceIdentifier string
   *
   * @throws LifeScienceIdentifierParseException if <code>lsid</code> is not properly formatted
   */
  public static LifeScienceIdentifier valueOf(String lsid)
      throws LifeScienceIdentifierParseException {
    if (lsid.length() < 9) throw new LifeScienceIdentifierParseException(
        "couldn't parse: " + lsid + ", didn't contain urn prefix");

    String urnPrefix = lsid.substring(0, 9);
    lsid = lsid.substring(9);

    if (!("urn:lsid:".equalsIgnoreCase(urnPrefix))) throw new LifeScienceIdentifierParseException(
        "couldn't parse: " + lsid + ", incorrect urn prefix");
    ...
    if (count >= 5) {
      ...
      if (st.hasMoreTokens()) throw new LifeScienceIdentifierParseException(
          "couldn't parse: " + lsid + ", too many tokens");

      return valueOf(authorityId, namespaceId, objectId, revisionId);
    } else {
      throw new LifeScienceIdentifierParseException(
          "couldn't parse: " + lsid + ", badly formatted lsid");
    }
  }
   ...
   *
   * @throws NullPointerException if any of <code>authorityId</code>, <code>namespaceId</code>, or
   *         <code>objectId</code> are null
   */
  public static LifeScienceIdentifier valueOf(String authorityId, String namespaceId,
      String objectId, String revisionId) {
    if ((authorityId == null) || (namespaceId == null) || (objectId == null))
      throw new NullPointerException(
          "all of authorityId, namespaceId, objectId " + "must not be null");
