/**
 * Factory for generating walkers that are customised to particuar feature visitors.
 *
...
public class WalkerFactory {
  ...
  /**
   * Register a type as being a 'container' class. Container classes will be scanned for methods for
   * retrieving child instances that can be walked to.
   *
   * 
  ...
  public synchronized Walker getWalker(Visitor visitor) throws BioException {
    ...
    if (walkerClass == null) {
      walkers.put(visitor.getClass(), walkerClass = generateWalker(visitor.getClass()));
    }
  ...
  private Class generateWalker(Class visitorClass) throws BioException {
    try {
      ...
      // make a class
      GeneratedCodeClass walkerClass = new GeneratedCodeClass(walkerClassName, c_WalkerBase,
          new CodeClass[] {c_Walker}, CodeUtils.ACC_PUBLIC | CodeUtils.ACC_SUPER);
      ...
      for (int mi = 0; mi < methods.length; mi++) {
        ...
        if (args.length > 0) {
          ...
          arg0Name = arg0Name.substring(0, 1).toLowerCase() + arg0Name.substring(1);
          ...
          if (arg0Name.equals(methName)) {
            ...
            if (typeClazz.isAssignableFrom(arg0)) {
              ...
              if (retClass == null) {
                ...
                if (retClass != ret) {
                  throw new BioException(
                      "Return type of all methods must agree. " + "We were expecting: "
                          + retClass.getName() + " but found: " + ret.getName());
                }
              }
              ...
              for (int ai = 1; ai < args.length; ai++) {
                ...
                if (argI != retClass) {
                  throw new BioException(
                      "The first argument to a handler method must be a " + typeClazz.toString()
                          + "All subsequent arguments must match the return type.  In: " + method);
                }
              }
      ...
      GeneratedCodeMethod doWalk = walkerClass.createMethod("doWalk", c_retClass, walkParams,
          new String[] {"target", "visitor"}, CodeUtils.ACC_PUBLIC);
      ...
      for (Iterator fwpi = typesWithParents.iterator(); fwpi.hasNext();) {
        ...
        for (int mi = 0; mi < filtMeth.length; mi++) {
          ...
          if (m.getParameterTypes().length == 0 && typeClazz.isAssignableFrom(m.getReturnType())) {
        ...
        wrapperIV.add(new IfExpression(ByteCode.op_ifne, wfiv, CodeUtils.DO_NOTHING));
      }
      ...
      for (Iterator mi = visitorMeths.iterator(); mi.hasNext();) {
        ...
        walkIV.add(new IfExpression(ByteCode.op_ifne, bodyIV, CodeUtils.DO_NOTHING));
      }
      ...
      if (c_retClass != CodeUtils.TYPE_VOID) {
        f_value = walkerClass.createField("value", CodeUtils.TYPE_OBJECT, CodeUtils.ACC_PRIVATE);
      }

      { // protect us from leakey locals
        GeneratedCodeMethod walkImpl =
            walkerClass.createMethod("walk", CodeUtils.TYPE_VOID, walkParams, CodeUtils.ACC_PUBLIC);
      ...
      { // protect us from leakey locals
        GeneratedCodeMethod getValue = walkerClass.createMethod("getValue", CodeUtils.TYPE_OBJECT,
      ...
      { // protect us from leaky locals
        ...
        GeneratedCodeMethod init = walkerClass.createMethod("<init>", CodeUtils.TYPE_VOID,
            CodeUtils.EMPTY_LIST, CodeUtils.ACC_PUBLIC);
