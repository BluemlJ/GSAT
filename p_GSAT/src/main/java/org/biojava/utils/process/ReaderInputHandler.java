...
/**
 * Reader {@linkplain org.biojava.utils.process.InputHandler input handler} that reads the input for
 * an external process from a {@linkplain org.biojava.utils.process.ReaderWriterPipe#getReader()
 * reader}. The {@linkplain org.biojava.utils.process.StreamPipe#getOutput() output stream} for the
 * input of the external process is closed after the reader is read to its end.
 * 
...
public class ReaderInputHandler extends ReaderWriterPipe implements InputHandler {
   ...
   * 
   * @param reader the reader from which to read the input for the external process. May be
   *        <code>null</code>.
