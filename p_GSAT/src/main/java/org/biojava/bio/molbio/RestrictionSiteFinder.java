...
/**
 * <code>RestrictionSiteFinder</code>s do the work of finding sites for one
 * <code>RestrictionEnzyme</code> in a target <code>Sequence</code>. Instances are passed to a
 * <code>ThreadPool</code> in order to perform several concurrent searches.
 *
 * @author Keith James
 * @since 1.3
 */
class RestrictionSiteFinder implements Runnable {
   ...
   *
   * @param enzyme a <code>RestrictionEnzyme</code> for which to find sites.
   * @param findAll a <code>boolean</code> indicating whether all sites should be found, including
   *        those which have recognition sites within the sequence, but cut outside it.
  ...
  public void run() {
    ...
    try {
      ...
      while (m.find()) {
        ...
        if (!findAll && (idx + usOffset < 0 || idx + dsOffset > seqLen)) continue;
      ...
      throw new BioRuntimeException("Failed to complete search for " + enzyme, e);
    }
  }
}
