 ...
 *
 * <p>
 * <b>WARNING</b> All binary operations on CircularLocations are currently assumed to be performed
 * on Locations from the same sequence, or sequences of the same length. To enforce this binary
 * operations will currently only accept circular locations of the same length.
 * </p>
 *
...
final class CircularLocationTools {
  /**
   * Translates coordinates from circular into linear where base 1 is the first base and base 0 is
   * the base before 1 or the last base of the sequence base -1 is the penultimate base of the
   * sequence etc.
   *
  ...
  protected static CircularLocation makeCircLoc(int min, int max, int seqLength) {
    if (min == max) return new CircularLocation(new PointLocation(min), seqLength);
  ...
  static Location intersection(Location locA, Location locB) {
    ...
    if (circularityA != circularityB) {
      throw new BioRuntimeException(
          "Can't find intersection of locations on circular sequences of non-equal length");
    }
  ...
  /**
   * Creates a union of two <code>Locations</code> in a Circular context. This is effectively a join
   * operation. If <code>locA</code> and <code>locB</code> overlap they will be merged.
   * <p>
   * The order in which <code>Location</code>s are joined is important in a circular context. It
   * will be assumed that the 5' end of the resulting join will be the 5' end of <code>locA</code>
   * <p>
   * If the union event merges two locations such that it is not sensible to maintain the original
   * 5' end of <code>locA</code> the 5' end will become the minimum value of the resulting merged
   * Location eg the value returned by <code>getMin()</code>.
   *
  ...
  /**
   * Tests a location to see if it overlaps the origin of the circular molecule to which it belongs.
   * The origin is taken to be position one.
   *
  ...
  protected static boolean overlapsOrigin(CircularLocation loc) {
    ...
    } else if (loc.getMin() == 1 && loc.getMax() == loc.getLength())
      return true;
    else
      return false;
  }
  ...
  protected static boolean isCircular(Location loc) {
    ...
    try {
      ...
      throw new org.biojava.bio.BioError(
          "class org.biojava.bio.symbol.BetweenLocation could not be loaded");
    }
    return toReturn;
  }
  ...
  public static boolean safeOperation(Location locA, Location locB) {
    if (isCircular(locA) && isCircular(locB)) {
      if (((CircularLocation) locA).getLength() == ((CircularLocation) locB).getLength()) {
        return true;
      } else {
        return false;
      }
