...
/**
 * This implementation of GappedSymbolList wraps a SymbolList, allowing you to insert gaps. Please
 * note that this is a view onto another SymbolList. Gaps created and removed are only in the view
 * not the underlying original. This means that any gaps present in the original cannot be
 * manipulated in this view. To manipulate the original you would need to use Edit objects.
 * 
 * @author Matthew Pocock
 * @since 1.3
 */
public class SimpleGappedSymbolList extends AbstractSymbolList
    implements
      GappedSymbolList,
      Serializable {
  /**
   * An aligned block.
   * <p>
   * The alignment is actualy stoored as a list of these objects. Each block is contiguous with the
   * next in the source fields, but may be gapped in the view fields.
   * 
   * @author Matthew Pocock
   */
  protected static final class Block implements Serializable {
    ...
    public String toString() {
      return "Block: source=" + sourceStart + "," + sourceEnd + " view=" + viewStart + ","
          + viewEnd;
    }
  }
   ...
   * <p>
   * The gapped symbol list can be represented as a list of ungapped regions. Given a list of
   * start-stop pairs in the ungapped coordinate system each with a corresponding pair of start-stop
   * pairs in the gapped view, the entire gapped list can be reconstructed.
   * 
  ...
  public void dumpBlocks() {
    for (Iterator<Block> i = blocks.iterator(); i.hasNext();) {
      Block b = i.next();
      System.out
          .println(b.sourceStart + ".." + b.sourceEnd + "\t" + b.viewStart + ".." + b.viewEnd);
    }
  }
  ...
  /**
   * Translates a Location from the gapped view into the underlying sequence. End points that are in
   * gaps are moved 'inwards' to shorten the location.
   * 
   * @since 1.3
   */
  ...
  public void removeGap(int pos) throws IndexOutOfBoundsException, IllegalSymbolException {
    ...
    if (i == -2) {
      throw new IllegalSymbolException("Attempted to remove a gap at a non-gap index: " + pos
  ...
  public void removeGaps(int pos, int length)
    ...
    if (pos < 1 || length < 1 || end > length()) {
      throw new IndexOutOfBoundsException("Attempted to remove gap outside of this sequence (1.."
          + length() + ") at (" + pos + ".." + end + ")");
    }
    int i = findViewGap(pos);
    if (i == -2) {
    ...
    if (i == -1) { // removing track at the beginning
      ...
      int gap = r.viewStart - l.viewEnd;
      if (gap < length) {
        throw new IllegalSymbolException("Removing " + length + " gaps from + " + i
    ...
    assert isSane() : "Data corrupted: removeGaps(" + pos + "," + length + ") " + blocks;
  }
  ...
  public Symbol symbolAt(int indx) throws IndexOutOfBoundsException {
    ...
    if (i < 0) {
      ...
      try {
        ...
        throw new AssertionFailure("Internal book-keeping error fetching index: " + indx + " of "
            + length() + " blocks: " + blocks, e);
      }
    }
  }
  ...
  protected final int findSourceBlock(int indx) {
    ...
    throw new BioError("Something is screwed. Could not find source block containing index " + indx
        + " in sequence of length " + source.length());
  }
   ...
   * <p>
   * If indx is not within a view block, then it is the index of a gap. The method will return
   * -(indx+1) where indx is the block emediately following the gap.
   * 
   * @param indx the index to find within a view range.
   * @return the index of the block containing index or one less than the negative of the index of
   *         the block following the gap
   */
   ...
   * <p>
   * If indx is in-fact a real symbol, then there will be no Block before it. In this case, the
   * method returns -2. It returns -1 if indx is within the leading gaps and blocks.size()-1 if it
   * is within the trailing gaps.
   * 
   * @param indx the index to find within a view range
   ...
   * <p>
   * This adjusts viewStart and viewEnd to be += delta for each block i->blocks.size(), and sets the
   * total length to += delta.
   * 
   * @param i the first
   */
