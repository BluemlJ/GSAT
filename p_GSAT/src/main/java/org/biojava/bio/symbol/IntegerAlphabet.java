 ...
 *
 * <p>
 * This class can be used to represent lists of integer numbers as a SymbolList with the alphabet
 * IntegerAlphabet. These lists can then be annotated with features, or fed into dynamic-programming
 * algorithms, or processed as per any other SymbolList object.
 * </p>
 *
 * <p>
 * Object identity should be used to decide if two IntegerSymbol objects are the same.
 * IntegerAlphabet ensures that all IntegerSymbol instances are canonicalized.
 * </p>
 *
...
public final class IntegerAlphabet extends Unchangeable implements Alphabet, Serializable {
  ...
  /**
   * Construct a finite contiguous subset of the <code>IntegerAlphabet</code>. Useful for making
   * CrossProductAlphabets with other <code>FiniteAlphabet</code>s.
   *
   ...
   * <p>
   * The returned object is a view onto the underlying array, and does not copy it. Changes made to
   * the original array will alter the symulting SymbolList.
   *
   * @param iArray the array of integers to view
   * @return a SymbolList over the IntegerAlphabet that represent the values in iArray
   */
   ...
   *
   * @param name Currently only "token" is supported. You may also use "default" as a synonym of
   *        "token"
  ...
  /**
   * A light-weight implementation of SymbolList that allows an array to appear to be a SymbolList.
   *
   * @author Matthew Pocock
   */
  ...
  public static class SubIntegerAlphabet extends AbstractAlphabet {
    ...
    public IntegerSymbol getSymbol(int val) throws IllegalSymbolException {
      if (val < min || val > max) {
        throw new IllegalSymbolException("Could not get Symbol for value " + val
            + " as it is not in the range " + min + " : " + max);
      }
    ...
    protected AtomicSymbol getSymbolImpl(List symL) throws IllegalSymbolException {
      ...
      if (symL.size() != 1) {
        throw new IllegalSymbolException(
            "SubIntegerAlphabet is one-dimensional: " + this.getName() + " : " + symL);
      }
      ...
      return new Iterator() {
        ...
        public Object next() {
          try {
            ...
            throw new BioError("Assertion Failure: symbol " + indx
                + " produced by iterator but not found in " + getName(), ise);
          }
        }
