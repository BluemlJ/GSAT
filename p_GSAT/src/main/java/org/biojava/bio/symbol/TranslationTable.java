 ...
 *
 * <p>
 * A TranslationTable is in effect a map or function with the source domain being
 * getSourceAlphabet() and the target domain being getTargetAlphabet(). The method translate() maps
 * a single symbol from source to target.
 * </p>
 *
 * <p>
 * It is presumed that there will be some explicit declaration of the mapping for attomic symbols,
 * and that the mapping for all other symbols will be infered from these.
 * </p>
 *
 * <p>
 * If you wish to translate every symbol in a symbol list then use TranslatedSymbolList to automate
 * the job. If you want to translate windowed regions then first construct a WindowedSymbolList from
 * the original sequence and then build a TranslatedSymbolList from this windowed view.
 * </p>
 *
 * <p>
 * There are a range of named tables available. Their names are specified by the public static final
 * fields in this interface. Each table can be retrieved by calling the static method
 * RNATools.getGeneticCode(name) which will return the appropriate TanslationTable instance.
 * </p>
 *
...
public interface TranslationTable {
   ...
   * @throws IllegalSymbolException if sym is not a member of the source alphabet
   */
