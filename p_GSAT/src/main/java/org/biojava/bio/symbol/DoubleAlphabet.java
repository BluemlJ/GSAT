 ...
 *
 * <p>
 * This class can be used to represent lists of floating-point numbers as a SymbolList with the
 * alphabet DoubleAlphabet. These lists can then be annotated with features, or fed into
 * dynamic-programming algorithms, or processed as per any other SymbolList object.
 * </p>
 *
 * <p>
 * Object identity should be used to decide if two DoubleResidue objects are the same.
 * DoubleAlphabet ensures that all DoubleAlphabet instances are canonicalized.
 * </p>
 *
...
public final class DoubleAlphabet extends Unchangeable implements Alphabet, Serializable {
   ...
   * the original array will alter the resulting SymbolList.
   * </p>
   *
   * @param dArray the array of doubles to view
   * @return a SymbolList over the DoubleAlphabet that represent the values in dArray
   */
  ...
  public static class SubDoubleAlphabet extends Unchangeable implements Alphabet, Serializable {
     ...
     * method was written as protected so that subclasses even from other packages will inherit it.
     * It should only be overridden with care.
     */
    ...
    public Symbol getSymbol(List rl) throws IllegalSymbolException {
      if (rl.size() != 1) {
        throw new IllegalSymbolException(
            "SubDoubleAlphabet is one-dimensional: " + this.getName() + " : " + rl);
      }
    ...
    public void validate(Symbol sym) throws IllegalSymbolException {
      if (!contains(sym)) {
        throw new IllegalSymbolException(
            "This alphabet " + this.getName() + " does not contain the symbol " + sym);
      }
    }
