...
/**
 * <code>FuzzyPointLocation</code> represents two types of EMBL-style partially-defined locations.
 * These are the '(123.567)' type, which represent a single residue somewhere between these
 * coordinates and the '<123' or '>123' type, which represent an unbounded location, not including
 * the residue at that coordinate.
 *
...
public class FuzzyPointLocation extends AbstractLocation {
  ...
  /**
   * Creates a new <code>FuzzyPointLocation</code> object. If the minimum value is equal to the
   * maximum value, it is interperted as a swissprot point location such as "?24". If the minimum is
   * Integer.MIN_VALUE and the maximum is Integer.MAX_VALUE, it is interperted as a swissprot
   * location like '?'.
   *
   * @param min an <code>int</code> value for the minimum boundary of the location,
   *        Integer.MIN_VALUE if unbounded.
   * @param max an <code>int</code> value for the minimum boundary of the location,
   *        Integer.MAX_VALUE if unbounded.
   * @param resolver a <code>PointResolver</code> which defines the policy used to calculate the
  ...
  public boolean contains(Location loc) {
    ...
    return (hasBoundedMin() && hasBoundedMax()) && (resolver.resolve(this) == loc.getMin())
  ...
  public boolean contains(int point) {
    ...
    return (hasBoundedMin() && hasBoundedMax()) && resolver.resolve(this) == point;
  }
  ...
  /**
   * Determines how a <code>FuzzyPointLocation</code> should be treated when used as a normal
   * <code>Location</code>.
   *
   * Use one of the implementations of this interface when creating a
   * <code>FuzzyPointLocation</code> to specify how the fuzzy (inner/outer) properties are
   * translated into the standard Location min and max properties.
   *
  ...
  private static class AveragePointResolver implements PointResolver {
    public int resolve(FuzzyPointLocation loc) {
      ...
      // Swissprot range ?
      else if ((loc.hasBoundedMin() == false) && (loc.hasBoundedMax() == false)) {
        return 0;
      }
