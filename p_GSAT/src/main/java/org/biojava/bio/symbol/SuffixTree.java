...
/**
 * Suffix tree implementation. The interface is a bit strange, as it needed to be as space-efficient
 * as possible. More work could be done on the space issue.
 *
 * <p>
 ...
 * <code>org.biojava.stats.svm.tools.SuffixTreeKernel</code>.
 * </p>
 *
...
public class SuffixTree implements Serializable {
  ...
  /**
   * Return the node object which is the root of this suffix tree. This represents the set of all
   * motifs found in this tree.
   */
  ...
  /**
   * Get a child of a SuffixTree.SuffixNode, constructing a new one if need be. This method is here
   * due to memory optimisations.
   */
  ...
  /**
   * Add a count for all motifs with length of up to <code>window</code> to this tree.
   *
   * @param sList a SymbolList whose motifs should be added to the tree.
   ...
   *
   * @param alphabet The alphabet of this SuffixTree (must be finite).
   */
   ...
   * <p>
   * This class is realy stupid & delegates most work off to a SuffixTree so that it is as small (in
   * memory-per-object terms) as possible.
   *
   * @author Matthew Pocock
   */
  public static abstract class SuffixNode implements Serializable {
    ...
    /**
     * Return a number (usually, but not always, a motif count) associated with this node of the
     * tree.
     */
  ...
  private static class SimpleNode extends SuffixNode {
    ...
    private SuffixNode[] childArray(SuffixTree tree) {
      if (child == null) child = new SuffixNode[tree.getAlphabet().size()];
      return child;
    }
