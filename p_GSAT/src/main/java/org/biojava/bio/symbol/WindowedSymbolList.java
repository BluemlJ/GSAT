 ...
 */
class WindowedSymbolList extends AbstractSymbolList implements Serializable {
  /**
  ...
  public WindowedSymbolList(SymbolList source, int width) throws IllegalArgumentException {
    if ((source.length() % width) != 0) {
      throw new IllegalArgumentException("The source length must be divisible by the window width: "
  ...
  public Symbol symbolAt(int index) throws IndexOutOfBoundsException {
    if (index < 1 || index > length()) {
      throw new IndexOutOfBoundsException(
          "index must be within (1 .. " + length() + "), not " + index);
    }
