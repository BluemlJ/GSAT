...
/**
 * Produced by LocationTools as a result of union operations. It's a <code>RangeLocation</code> and
 * can be used as such but it also retains knowledge of which individual components made it up. None
 * of the methods of RangeLocation are overridden only new methods have been added to get the
 * subcomponents.
 * </p>
 *
 * <p>
 * For example a union operation between the following locations [1,20],[27,45],[30-70] will produce
 * a <code>CompoundLocation</code> like this: {[1,20],[27,70]}, the last two locations have been
 * merged into a <code>MergeLocation</code> which contains the two subcomponents.
 * </p>
 *
 * <p>
...
public class MergeLocation extends RangeLocation {
  ...
  /**
   * Private constructor, use static method mergeLocations to make an instance of this class
   * 
   ...
   *
   * @param recurse if true the method lists the component locations of all nested
   *        <code>MergedLocation</code>s.
   *
  ...
  /**
   * @return A <code>ListIterator</code> over the component locations. The iterator does not recurse
   *         nested MergeLocations.
   */
   ...
   * @throws BioException if the list contains objects that are not <code>Locations</code> or if the
   *         locations don't represent a contiguous block. Use <code>
