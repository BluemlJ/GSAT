...
/*
 * Implementatoin of AlphabetIndex that stores the symbols in an array and does a linear shearch
 * through the list for a given symbol to find its index.
 *
 * @author Matthew Pocock
 * 
 * @since 1.1
 */
class LinearAlphabetIndex extends AbstractChangeable
    implements
      AlphabetIndex,
  ...
  public LinearAlphabetIndex(FiniteAlphabet alpha) {
    ...
    alpha.addChangeListener(indexBuilder = new IndexRebuilder(), Alphabet.SYMBOLS);

    this.addChangeListener(adapter = new ChangeAdapter() {
  ...
  protected class IndexRebuilder extends ChangeForwarder {
    public IndexRebuilder() {
      super(LinearAlphabetIndex.this,
    ...
    public ChangeEvent generateEvent(ChangeEvent ce) throws ChangeVetoException {
      ...
      /*
       * Object change = ce.getChange(); Object previous = ce.getPrevious();
       * 
       * if( (change == null) || (previous != null) ) { throw new ChangeVetoException( ce,
       * "Can not update index as either a symbol is being removed, " +
       * "or the alphabet has substantialy changed" ); }
       * 
       * if(! (change instanceof AtomicSymbol) ) { throw new ChangeVetoException( ce,
       * "Can not update index as the symbol being added is not atomic" ); }
       */

      return new ChangeEvent(getSource(), AlphabetIndex.INDEX,
          // fixme: buildIndex should be called using the proposed new alphabet
          LinearAlphabetIndex.this.buildIndex((FiniteAlphabet) ce.getSource()), symbols, ce);
    }
  }
}
