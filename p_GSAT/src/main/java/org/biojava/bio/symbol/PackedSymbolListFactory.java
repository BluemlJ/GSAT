...
/**
 * This class makes PackedSymbolLists. It could be refactored into the PackedSymbolList class
 * eventually.
 *
 * @author David Huen
 */
public class PackedSymbolListFactory
  ...
  /**
   * Create a factory for PackedSymbolLists. The use of ambiguity packing is determined
   ...
   * @deprecated the argumentless constructor creates a SymbolListFactory that will autoselect the
   ...
   * @return a packed SymbolList with the Symbols in symbolArray and the Alphabet in alfa
   * @exception IllegalAlphabetException if alfa and the Symbols in symbolArray disagree.
   */
    ...
    switch (opMode) {
      case AUTO_SELECT:
        ...
        for (int i = 0; i < size; i++) {
          if (!(symbolArray[i] instanceof AtomicSymbol)) {
            ambiguity = true;
            break;
          }
        }
      case USER_SELECT:
      default:
        return new PackedSymbolList(PackingFactory.getPacking((FiniteAlphabet) alfa, ambiguity),
            symbolArray, size, alfa);
    }
  }
}
