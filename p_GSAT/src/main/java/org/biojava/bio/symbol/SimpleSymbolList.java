 ...
 * License. This should be distributed with the code. If you do not have a copy, see:
 *
 ...
 * <p>
 * SimpleSymbolList is now editable. edit() has been implemented in a way that edits are relatively
 * inefficient, but symbolAt() is very efficient.
 * </p>
 * <p>
 * A new constructor SimpleSymbolList(SymbolParser,String) has been added so you can now simply turn
 * a String into a SymbolList. This is mostly to provide a simple way to create a SymbolList for
 ...
 * </code>
 * <p>
 * with the right parser you should be able to make a protein sequence from the String
 * "AspAlaValIleAsp"
 * </p>
 * <p>
 * subList() is implemented such that subLists are views of the original until such time as the
 * underlying SymbolList is edited in a way that would modify the subList, at which point the
 * subList gets its own array of Symbols and does not reflect the edit to the original. When
 * subList() is called on another subList (which is a veiw SimpleSymbolList) the new
 * SimpleSymbolList is a view of the original, not the subList.
 * </p>
 *
 * @author Thomas Down
 * @author David Waring
...
public class SimpleSymbolList extends AbstractSymbolList implements ChangeListener, Serializable {
  ...
  private int length;
  private boolean isView; // this is for subList which returns a view onto a SimpleSymbolList until
                          // either is edited
  private int viewOffset; // offset of the veiw to the original
  private SymbolList referenceSymbolList; // the original SymbolList subLists of views become
                                          // sublists of original
  ...
  /**
   * Construct a SimpleSymbolList given the Symbol array that backs it. Used primarily with the
   * chunked SymbolList builder but could be used elsewhere too.
   */
  ...
  /**
   * Construct construct a SimpleSymbolList that is a veiw of the original. this is used by
   * subList();
   *
  ...
  /**
   * create a subList of the original, this will be a view until either the original symbolList or
   * the sublist is edited
   */
  ...
  public synchronized void edit(Edit edit)
    ...
    if ((edit.replacement.getAlphabet() != alphabet)
        && (edit.replacement != SymbolList.EMPTY_LIST)) {
      throw new IllegalAlphabetException();
    }

    // give the listeners a change to veto this
    // create a new change event ->the EDIT is a static final variable of type ChangeType in
    ...
    synchronized (cs) {
      ...
      int posRightFragInDestArray5 =
          posRightFragInSourceArray5 + edit.replacement.length() - edit.length;
      int posReplaceFragInDestArray5 = edit.pos - 1;
      int replaceFragLength = edit.replacement.length();
      int totalLength = length + replaceFragLength - edit.length + INCREMENT; // What is this
                                                                              // increment for?

      // extend the array
      dest = new Symbol[totalLength];
      // copy symbols before the edit and make sure we are not editing the edit at the same time
      // (hoops!)
      System.arraycopy(symbols, 0, dest, 0, (edit.pos - 1));

      // copy the symbols after the edit
      if (rightFragLength > 0) {
        System.arraycopy(symbols, posRightFragInSourceArray5, dest, posRightFragInDestArray5,
            rightFragLength);
      }
  ...
  /**
   * On preChange() we convert the SymolList to a non-veiw version, giving it its own copy of
   * symbols
   */
  ...
  /**
   * Converts a view symbolList to a real one that means it gets its own copy of the symbols array
   */
  ...
  /**
   * Return the Java Symbol[] array that backs this object. primarily used to accelerate
   * reconstruction of symbol lists in the packed chunked symbol list implementation.
   */
