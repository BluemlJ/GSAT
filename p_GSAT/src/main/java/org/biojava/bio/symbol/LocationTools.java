 ...
 *
 * <p>
 * Most of the methods in this class are simple set-wise binary operators: for example, calculate
 * the intersection of two locations.
 * </p>
 *
...
final public class LocationTools {
   ...
   *
   * <p>
   * The union will be a Location instance that contains every index contained by either locA or
   * locB.
   * </p>
   *
   ...
   * <p>
   * The intersection will be a Location instance that contains every index contained by both locA
   * and locB.
   * </p>
   *
   * @param locA the first Location
  ...
  /**
   * Return an ordered list of non-overlapping blocks of a location (singleton list for contiguous
   * locations, empty list for empty locations)
   *
   * Note that from this point of view, contiguous circular locations aren't necessarily contiguous
   * :-(.
   *
  ...
  /**
   * Return the index into <code>bl</code> of a block containing <code>point</code>, or -1 if it is
   * not found.
   */
  ...
  /**
   * Return the index of the block containing <code>point</code>, if it exists, or the block
   * preceeding it.
   */
  ...
  /**
   * Determines whether the locations are touching or not (if they could be merged in a single
   * Location.
   * <p>
   * Two locations can merge if they contain at least one index of one beside one index of the
   * other.
   * </p>
   *
   * @param locA the first Location
  ...
  public static boolean canMerge(Location locA, Location locB) {
    if (overlaps(locA, locB) || overlaps(locA.translate(1), locB)
  ...
  /**
   * Return <code>true</code> iff all indices in <code>locB</code> are also contained by
   * <code>locA</code>.
   *
   ...
   * <p>
   * They are equal if both a contains b and b contains a. Equivalently, they are equal if for every
   * point p, locA.contains(p) == locB.contains(p).
   * </p>
   *
   * @param locA the first Location
   * @param locB the second Location
  ...
  public static boolean areEqual(Location locA, Location locB) {
    ...
    while (i1.hasNext() && i2.hasNext()) {
      ...
      if ((l1.getMin() != l2.getMin()) || (l1.getMax() != l2.getMax())) {
  ...
  /**
   * Create a Location instance from the list of contiguous locations in locList.
   * <p>
   * If the list is empty then Location.empty will be produced. If it is just one element long, then
   * this will be returned as-is. If there are multiple locations then they will be sorted and then
  ...
  /**
   * Create a Location instance from the sorted list of contiguous locations in locList.
   * <p>
   * If the list is empty then Location.empty will be produced. If it is just one element long, then
   * this will be returned as-is. If there are multiple locations then they will be sorted and then
   * used to construct a CompoundLocation.
  ...
  /**
   * The n-way union of a Collection of locations. Returns a Location which covers every point
   * covered by at least one of the locations in <code>locs</code>
   *
  ...
  /**
   * A simple method to generate a RangeLocation wrapped in a CircularLocation. The method will cope
   * with situtations where the min is greater than the max. Either of min or max can be negative,
   * or greater than the underlying sequence length. If min and max are equal a wrapped point
   * location will be made.
   *
   * @param min the "left" end of the location
   * @param max the "right" end of the location
   * @param seqLength the lenght of the sequence that the location will be applied to (for purposes
  ...
  /**
   * Currently CircularLocations are handled if and only if CircularLocationTools.safeOperation
   * returns true. For this to be true the locations must have the same sequence length.
   *
   * BetweenLocations are handled in all cases. Overlap cases, such as between, circular locations
   * have indeterminate behavior.
   *
   * The default behavior is to not handle decorators. Any new decorators will have to re-visit this
   * method
   */
  private static void handleDecorations(Location locA, Location locB) {
    if (CircularLocationTools.isCircular(locA) || CircularLocationTools.isCircular(locB)) {
      if (CircularLocationTools.safeOperation(locA, locB) == false) {
        throw new UnsupportedOperationException("Binary operations between Circular and"
      ...
      throw new ClassCastException("Decorated locations are not handled in this version: "
          + locA.getClass().getName() + ", " + locB.getClass().getName());
    }
  }
   ...
   *
   * <p>
   * It is very common in biological sequences to represent locations on a sequence and then reverse
   * that sequence. This method allows locations in the original coordinate space to be transformed
   * int locations in the reverse one.
   * </p>
   *
  ...
  /**
   * Subtract one location from another. This methods calculates the set of points which are
   * contains in location <code>keep</code> but not in <code>remove</code>.
   *
  ...
  /**
   * Return a contiguous location running from the minimum to the maximum points of the specified
   * location.
   * 
   * @param loc a location
