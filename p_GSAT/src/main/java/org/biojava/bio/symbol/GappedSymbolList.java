 ...
 * <p>
 * You could make a SymbolList that contains gaps directly. However, this leaves you with a nasty
 * problem if you wish to support gap-edit operations. Also, the original SymbolList must either be
 * coppied or lost.
 * <p>
 * GappedSymbolList solves these problems. It will maintain a data-structure that places gaps. You
 * can add and remove the gaps by using the public API.
 * <p>
 * For gap-insert operations, the insert index is the position that will become a gap. The symbol
 * currently there will move to a higher index. To insert leading gaps, add gaps at index 1. To
 * insert trailing gaps, add gaps at index length+1.
 * 
...
public interface GappedSymbolList extends SymbolList {
   ...
   * <p>
   * If the index can be projected onto the source, the index it projects onto is returned. If it
   * falls within a gap, then the index of the first symbol after the run of gaps is negated and
   * returned. If the index is after the last block of symbols (and therefore in the trailing list
   * of gaps), then it returns -(length + 1).
   * 
   ...
   * <p>
   * this.symbolAt(pos) will then return gap. Adding a gap at 1 will prepend gaps. Adding a gap at
   * (length+1) will append a gap.
   * 
   ...
   * <p>
   * this.symbolAt(i) will then return gap for i = (pos .. pos+count-1). Adding gaps at 1 will
   * prepend gaps. Adding gaps at (length+1) will append gaps.
   * 
   * @param pos the position to add a gap before
   ...
   * @throws IndexOutOfBoundsException if pos is not within 1->source.length()
   */
  public void addGapsInSource(int pos, int length) throws IndexOutOfBoundsException;
   ...
   * @throws IndexOutOfBoundsException if pos is not within 1..length() or if some of the Symbols
   ...
   * <p>
   * All symbols before firstNonGap are leading gaps. firstNonGap is effectively the index in the
   * view of symbol 1 in the original sequence.
   * 
   ...
   * <p>
   * All symbols after lastNonGap untill length are trailing gaps. lastNonGap is effectively the
   * index in the view of symbol length in the original sequence.
   * 
   ...
   * 
   * <p>
   * This will be a Location that contains every symbol in the underlying ungapped sequence. Every
   * symbol not in the Location is not from the underlying sequence and is a gap.
   * </p>
   * 
