...
final class BetweenLocationTools {
  ...
  protected static Location unwrap(Location theLoc) {
    ...
    try {
      ...
      throw new org.biojava.bio.BioError(
          "Unwrapping not possible: decorated by other than between decorator");
    }
  ...
  /**
   * Tests if the location passed in is an instance of a between location or not.
   *
  ...
  protected static boolean isBetween(Location theLocation) {
    ...
    try {
      if (theLocation
          .getDecorator(Class.forName("org.biojava.bio.symbol.BetweenLocation")) != null) {
        toReturn = true;
      }
    } catch (Exception e) {
      throw new org.biojava.bio.BioError(
