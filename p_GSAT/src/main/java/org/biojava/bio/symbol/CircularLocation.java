...
/**
 * Circular view onto an underlying Location instance. If the location overlaps the origin of the
 * sequence the underlying location will be a CompoundLocation Note that in this case isContiguous()
 * will return false. This behaviour is desirable for proper treatment of the location with
 * LocationTools. To find if a location overlaps the origin use the overlapsOrigin() method
 * <p>
 * Note also that as a location that overlaps the origin is a compound location it's min will be 1
 * and its max will be length (by default). In these cases it is imperative to use the block
 * iterator if you wish to know the 'true' min and max (bearing in mind that there is no logical
 * value for a min or max on a circular sequence).
 * </p>
 * <p>
...
public class CircularLocation
  ...
  /**
   * Constructs a CircularLocation by wrapping another Location <strong>It is preferable to use
   * LocationTools to make CircularLocations</strong>
   * 
   ...
   * <code>fivePrimeEnd</code> parameter must match one of the minimum coordinates of the wrapped
   * <code>Location</code>. This allows the creation of Locations whose polarity can be specified.
   * This method is used internally by <code>LocationTools.union()</code> and by the other
   * constructor.
   *
   ...
   * @throws IllegalArgumentException if the 5' end doesn't match the min coordinate of either
   *         <code>wrapped</code> or one of its blocks.
   */
  ...
  public String toString() {
    if (fivePrimeSortedBlocks.size() > 1) {
      ...
      sb.append(
          get5PrimeEnd() + ", " + ((Location) fivePrimeSortedBlocks.getLast()).getMax() + " {");
  ...
  /**
   * Delegates to the wrapped location. Currently as locations that cross the origin are wrapped
   * CompoundLocations they are not considered contiguous. This is desirable from the point of view
   * of logical operations as it greatly simplifies the calculations of things such as contains,
   * overlaps etc.
   * 
  ...
  /**
   * The point at which indicates the 5' end of the Location. This is needed as compound circular
   * locations have polarity. For example (18..30, 1..12) is not the same as (1..12, 18..30). The 5'
   * coordinate is derived during construction of the Location. In particular during a union
   * operation the 5' coordinate is generally the 5' coordinate of the first location in the union.
   * In the case where this cannot be logically maintained the 5' coordinate will revert to
   * <code>getMin()</code>
   *
  ...
  /**
   * This will give you the coordinate of the minimum point contained by this Location. Generally
   * this will be the leftmost end however if the Location crosses the origin then it will actually
   * be the origin that is the minimum point and the method will return 1. If you want to be
   * guarenteed of getting the leftmost coordinate then call <code>get5PrimeEnd()</code>
   * 
  ...
  /**
   * This will give you the coordinate of the maximum point contained by this Location. Generally
   * this will be the rightmost end however if the Location crosses the origin then it will actually
   * be the point before the origin that is the maximum point and the method will return
   * <code>getLength()</code>. If you want to be guarenteed of getting the rightmost coordinate then
   * call <code>get3PrimeEnd()</code>
   * 
