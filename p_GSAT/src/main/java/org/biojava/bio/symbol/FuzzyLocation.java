 ...
 * <p>
 * Fuzzy locations have propreties that indicate that they may start before min and end after max.
 * However, this in no way affects how they interact with other locations.
 * </p>
 *
...
public class FuzzyLocation extends AbstractRangeLocation implements Serializable {
   ...
   * @param resolver a RangeResolver object which defines the policy used to calculate the
   *        location's min and max properties.
   */

  public FuzzyLocation(int outerMin, int outerMax, int innerMin, int innerMax,
    ...
    this.initializeVariables(outerMin, outerMax, innerMin, innerMax, isMinFuzzy, isMaxFuzzy,
        resolver);
  }

  /**
   * Create a new FuzzyLocation with endpoints (outerMin.innerMin) and (innerMax.outerMax). This
   * constructor allows you to explicitly mark an endpoint as fuzzy, even if there is no other
   * information about it. For example, a valid swissprot location "?5 10" would be a fuzzy location
   * 5 to 10 where the min is fuzzy and the max is not.
   * <p>
   * Note that it is not logical to specify inner and outer values that clearly denote fuzzy
   * boundaries and the set the <code>isMinFuzzy</code> or <code>isMaxFuzzy</code> value to false.
   * This object makes no specific check of your logic so be careful.
   *
   * @param outerMin the lower bound on the location's min value. Integer.MIN_VALUE indicates
   *        unbounded.
   * @param outerMax the upper bound on the location's max value. Integer.MAX_VALUE indicates
  ...
  public FuzzyLocation(int outerMin, int outerMax, int innerMin, int innerMax, boolean isMinFuzzy,
      boolean isMaxFuzzy, RangeResolver resolver) {
    this.initializeVariables(outerMin, outerMax, innerMin, innerMax, isMinFuzzy, isMaxFuzzy,
        resolver);
  }

  public Location translate(int dist) {
    return new FuzzyLocation(outerMin + dist, outerMax + dist, innerMin + dist, innerMax + dist,
        resolver);
  }
   ...
   *
   * @return the Location instance that stores all of the Location interface data
   */
  ...
  public String toString() {
    return "[" + (hasBoundedMin() ? Integer.toString(getMin()) : "<" + Integer.toString(getMin()))
        + ", " + (hasBoundedMax() ? Integer.toString(getMax()) : ">" + Integer.toString(getMax()))
        + "]";
  }

  /**
   * Determines how a <code>FuzzyLocation</code> should be treated when used as a normal
   * <code>Location</code>.
   *
   * Use one of the implementations of this interface when creating a <code>FuzzyLocation</code> to
   * specify how the fuzzy (inner/outer) properties are translated into the standard Location min
   * and max properties.
   *
   ...
   *        location's min and max properties.
   */
  protected void initializeVariables(int outerMin, int outerMax, int innerMin, int innerMax,
      boolean isMinFuzzy, boolean isMaxFuzzy, RangeResolver resolver) {
