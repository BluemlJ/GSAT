...
public class CodonPrefTools {
  ...
  private static void loadCodonPreferences() {
    try {
      ...
      InputStream prefStream = ClassTools.getClassLoader(CodonPrefTools.class)
          .getResourceAsStream("org/biojava/bio/symbol/CodonPrefTables.xml");
  ...
  /**
   * returns an RNA dinucleotide alphabet. Used to represent the non-wobble bases in
   * WobbleDistribution
   */
    ...
    try {
      ...
      for (int i = 0; i < children.getLength(); i++) {
        ...
        for (int j = 0; j < freqs.getLength(); j++) {
          ...
          AtomicSymbol codon =
              (AtomicSymbol) RNATools.getCodonAlphabet().getSymbol(codonSL.toList());
  ...
  /**
   * reads in a file in Codon Usage Database format and translate it into our XML format These can
   * be obtained from the <a href="http://www.kazusa.or.jp/codon/">Codon Usage Database</a>.
   * <p>
   * Note that the output assumes that the universal genetic code is used as that is not encoded in
   * the CUD files. Edit the output appropriately to modify the genetic code if necessary.
   */
  ...
  private static AtomicSymbol getCodon(String codonString) throws IllegalSymbolException {
    return (AtomicSymbol) RNATools.getCodonAlphabet()
        .getSymbol(RNATools.createRNA(codonString).toList());
  }
  ...
  static void dumpToXML(CodonPref codonPref, XMLWriter xw, boolean writeWrapper)
    ...
    if ((codonPref == null) || (xw == null)) throw new NullPointerException();
  ...
  private static void readFromCUD(BufferedReader rdr, CodonPrefFilter filter) {
    ...
    } catch (IOException ioe) {} catch (IllegalSymbolException ise) {} catch (IllegalAlphabetException iae) {} catch (ChangeVetoException cve) {} catch (BioException be) {}
  }
}
