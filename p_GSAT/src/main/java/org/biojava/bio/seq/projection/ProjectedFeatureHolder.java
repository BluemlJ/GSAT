...
/**
 * Helper class for projecting Feature objects into an alternative coordinate system. This class
 * offers a view onto a set of features, projecting them into a different coordinate system, and
 * also changing their <code>parent</code> property. The destination coordinate system can run in
 * the opposite direction from the source, in which case the <code>strand</code> property of
 * StrandedFeatures is flipped.
 *
 * <p>
 * The projected features returned by this class are small proxy objects. Proxy classes are
 * autogenerated on demand for any sub-interface of <code>Feature</code> by the
 * <code>ProjectionEngine</code> class.
 * </p>
 *
 * @author Thomas Down
...
public final class ProjectedFeatureHolder extends AbstractFeatureHolder
    implements
      FeatureHolder,
      Serializable {
  ...
  /**
   * Called internally to generate a forwarded version of a ChangeEvent from our underlying
   * FeatureHolder
   */
