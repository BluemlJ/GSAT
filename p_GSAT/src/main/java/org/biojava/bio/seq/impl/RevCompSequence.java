 ...
 * <p>
 * All features of the underlying sequence are reflected onto the RevCompSequence using a
 * ProjectedFeatureHolder
 * </p>
 * calling createFeature() on a RevCompSequence creates a feature on the underlying sequence.
 * Non-Stranded features will return the reverse compemented view of the sequence when getSymbols()
 * is called that is to say if you get what you expect as if your RevCompSequence was a regular
 * Sequence.
 *
 * @author David Waring
 * @author Thomas Down
...
public class RevCompSequence extends SimpleSequence {
  ...
  /**
   * URN, Name and Annotation are copied as is from the original Sequence, unless you use the the
   * other contructor that sets these.
   */
  ...
  public RevCompSequence(Sequence seq, String urn, String name, Annotation annotation)
  ...
  /**
   * containsFeature() will return true if this seq contains the feature in question, or if if the
   * original (non reverse complement) sequence contains the feature;
   */
  ...
  /**
   * createFeature() will call createFeature() on the underlying Sequence. returns the feature as it
   * will be projected onto the reverse complement sequence not the actual feature that was created.
   *
   */
  ...
  /**
   * getFeatureFromOriginal() Since you can not create a feature on a projectedFeature at this time,
   * I am including this method so that you can get the corresponding feature from the original
   * sequence. (which is not projected) and do something with that such as createFeature().
   */
