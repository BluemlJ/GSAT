...
/**
 * <code>SimpleRestrictionSite</code> represents the recognition site of a restriction enzyme.
 *
 * @author Keith James
 * @since 1.3
 */
public class SimpleRestrictionSite extends SimpleStrandedFeature implements RestrictionSite {
   ...
   */
  public SimpleRestrictionSite(Sequence sourceSeq, FeatureHolder parent,
      RestrictionSite.Template template) {
    super(sourceSeq, parent, template);

    this.enzyme = template.enzyme;

    position = template.location.getMin() + template.enzyme.getDownstreamCut()[0];
  }
