...
class DistProjectionContext extends ReparentContext {
  ...
  public DistProjectionContext(FeatureHolder fh, FeatureHolder parent, Annotation annotation) {
    super(parent, fh);
    ...
    if (f instanceof ComponentFeature && getParent() instanceof DistributedSequence) {
      ...
      if (pcf == null) {
        ComponentFeature.Template cft =
            (ComponentFeature.Template) ((ComponentFeature) f).makeTemplate();
        ...
        try {
          pcf = new DistComponentFeature((DistributedSequence) getParent(), cft);
        } catch (Exception ex) {
