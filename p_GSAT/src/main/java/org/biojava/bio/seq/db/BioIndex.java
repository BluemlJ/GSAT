...
public class BioIndex implements IndexStore {
  ...
  public BioIndex(File indexDirectory, String namespace, int idLength)
      throws IOException, BioException {
    if (indexDirectory.exists()) {
      throw new BioException(
          "Can't create new index as directory already exists: " + indexDirectory);
    }
    ...
    PrintWriter fileidsWriter;
    {
      ...
      fileidsWriter = new PrintWriter(new FileWriter(fileids));
    }
    ...
    // create index file
    {
      ...
      indxList = new IndexFileAsList(new RandomAccessFile(unique, "rw"), recordLen);
   ...
   *
   * If indexDirectory does not exist, or is not a bioindex stoore, this will barf.
   */
  public BioIndex(File indexDirectory) throws IOException, BioException {
    ...
    // read in the global config
    {
      ...
      BufferedReader fi =
          new BufferedReader(new FileReader(new File(indexDirectory, "config.dat")));
    ...
    // set up file set
    {
      ...
      BufferedReader fi =
          new BufferedReader(new FileReader(new File(indexDirectory, "fileids.dat")));
      for (String line = fi.readLine(); line != null; line = fi.readLine()) {
        ...
        if (file.length() != fileLength) {
          throw new BioException(
              "File length changed: " + file + " " + file.length() + " vs " + fileLength);
        }
  ...
  public void commit() throws BioException {
    ...
    try {
      ...
      {
        PrintStream fo =
            new PrintStream(new FileOutputStream(new File(indexDirectory, "fileids.dat")));
        for (int i = 0; i < fileCount; i++) {
          fo.print(i);
          fo.print('\t');
  ...
  private abstract class FileAsList extends AbstractList implements /* RandomAccess, */ Commitable {
