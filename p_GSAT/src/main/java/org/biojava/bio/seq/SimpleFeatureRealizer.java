...
/**
 * FeatureRealizer which uses a lookup table to map template classes to implementations. Optionally,
 * this implementation can fall back on another FeatureRealizer if it fails.
 *
 * <p>
 * When searching for a Feature implementation to match a specific Feature.Template class, the
 * following search order is used:
 * <ol>
 * <li>Mappings added to this SimpleFeatureRealizer, in reverse order of addition.</li>
 * <li>Any mappings known to the fallback realizer, if one is installed.</li>
 * <li>If no mapping can be found, a BioException is thrown.</li>
 * </ol>
 * </p>
 *
 * @author Thomas Down
...
public class SimpleFeatureRealizer implements FeatureRealizer, Serializable {
   ...
   * The implementation class MUST have a public constructor of the form (Sequence, FeatureHolder,
   * Feature.Template).
   *
   * <p>
   * A newly added implementation takes precendence over any existing implementations if a template
   * can be realized by more than one implementation.
   * </p>
   *
  ...
  public Feature realizeFeature(Sequence seq, FeatureHolder parent, Feature.Template temp)
      throws BioException {
    for (Iterator i = templateToImpl.iterator(); i.hasNext();) {
      TemplateImpl ti = (TemplateImpl) i.next();
      if (ti.accept(temp)) return ti.realize(seq, parent, temp);
    }

    if (fallBack != null) return fallBack.realizeFeature(seq, parent, temp);

    throw new BioException(
        "Couldn't find realized implementation for template of class " + temp.getClass().getName());
  }
  ...
  private static class TemplateImpl {
    ...
    private TemplateImpl(Class template, Class impl) throws BioException {
      ...
      try {
        ...
        throw new BioException("Class " + impl.getName() + " does not have suitable constructor",
            ex);
      }
    }
    ...
    public Feature realize(Sequence seq, FeatureHolder parent, Feature.Template temp)
        throws BioException {
