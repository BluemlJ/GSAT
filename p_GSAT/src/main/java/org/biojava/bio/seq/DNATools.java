...
public final class DNATools {
  ...
  /**
   * Return a new DNA <span class="type">SymbolList</span> for <span class="arg">dna</span>.
   *
   * @param dna a <span class="type">String</span> to parse into DNA
   * @return a <span class="type">SymbolList</span> created form <span class="arg">dna</span>
   * @throws IllegalSymbolException if <span class="arg">dna</span> contains any non-DNA characters
   */
  ...
  /**
   * Return a new DNA <span class="type">Sequence</span> for <span class="arg">dna</span>.
   *
   * @param dna a <span class="type">String</span> to parse into DNA
  ...
  public static Sequence createDNASequence(String dna, String name) throws IllegalSymbolException {
    ...
    // try {
    return new SimpleSequenceFactory().createSequence(createDNA(dna), "", name,
  ...
  public static GappedSequence createGappedDNASequence(String dna, String name)
  ...
  /**
   * Return an integer index for a symbol - compatible with <code>forIndex</code>.
   *
   * <p>
  ...
  /**
   * Returns a SymbolList that is reverse complemented if the strand is negative, and the origninal
   * one if it is not.
   *
  ...
  /**
   * return a (DNA x DNA) cross-product Distribution with specified DNA contents in each component
   * Alphabet.
   * 
  ...
  /**
   * Converts a <code>SymbolList</code> from the DNA <code>Alphabet</code> to the RNA
   * <code>Alphabet</code>.
   * 
   * @param syms the <code>SymbolList</code> to convert to RNA
   * @return a view on <code>syms</code> where <code>Symbols</code> have been converted to RNA. Most
   *         significantly t's are now u's. The 5' to 3' order of the Symbols is conserved.
   * @since 1.4
  ...
  /**
   * Transcribes DNA to RNA. The method more closely represents the biological reality than
   * <code>toRNA(SymbolList syms)</code> does. The presented DNA <code>SymbolList</code> is assumed
   * to be the template strand in the 5' to 3' orientation. The resulting RNA is transcribed from
   * this template effectively a reverse complement in the RNA alphabet. The method is equivalent to
   * calling <code>reverseComplement()</code> and <code>toRNA()</code> in sequence.
   * <p>
   * If you are dealing with cDNA sequences that you want converted to RNA you would be better off
   * calling <code>toRNA(SymbolList syms)</code>
   * 
   * @param syms the <code>SymbolList</code> to convert to RNA
  ...
  /**
   * Convenience method that directly converts a DNA sequence to RNA then to protein. The translated
   * protein is from the +1 reading frame of the <code>SymbolList</code>. The whole
   * <code>SymbolList</code> is translated although up to 2 DNA residues may be truncated if full
   * codons cannot be formed.
   * 
   * @param syms the sequence to be translated.
   * @return the translated protein sequence.
   * @throws org.biojava.bio.symbol.IllegalAlphabetException if <code>syms</code> is not from the
   *         DNA alphabet.
   * @since 1.5.1
   */
  public static SymbolList toProtein(final SymbolList syms) throws IllegalAlphabetException {
  ...
  /**
   * Convenience method to translate a region of a DNA sequence directly into protein. While the
   * start and end can be specified if the length of the specified region is not evenly divisible by
   * three then the translated region will be truncated until a full terminal codon can be formed.
   * 
