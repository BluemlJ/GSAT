...
/**
 * <p>
 * Implementation of SymbolTokenization which binds symbols to strings of characters. These
 * tokenizations are intented to provide alternate way of writing sequences into Strings. Therefore
 * they cannot be used for parsing files.
 * </p>
 *
 * <p>
 * As this release, alternate tokenizations are available for the built-in DNA alphabet (write
 * symbols as capital letter) and PROTEIN-TERM alphabet (write symbol as triplets of characters with
 * the first one being a capital letter as in "Glu".
 * </p>
 *
 * <p>
 * By convention, instances of AlternateTokenization should have an associated token starting by the
 * word 'alternate'.
 *
...
public class AlternateTokenization extends Unchangeable
   ...
   */
  public int getWidth() {
    if (initiated == false) throw new IllegalStateException("Tokenization not initialize yet");
    return width;
  }
  ...
  public void bindSymbol(Symbol s, String str) {
    if (!initiated) init(str);
    if (str.length() != width) throw new IllegalArgumentException(
        "This tokenization must have all its tokens with the same size");
