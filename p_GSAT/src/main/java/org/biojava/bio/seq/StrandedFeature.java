 ...
 * <p>
 * Strandedness only applies to some types of sequence, such as DNA. Any implementation should blow
 * chunks to avoid being added to a sequence for which strand is a foreign concept.
 *
 * Strand is intrinsicly part of all {@link org.biojavax.bio.seq.RichFeature RichFeatures} We
 * strongly recommend using this interface.
 *
...
public interface StrandedFeature extends Feature {
   ...
   *
   * @exception ChangeVetoException if the strand may not be changed.
   */
   ...
   * <p>
   * If the feature is on the negative strand then the SymbolList will be reverse-complemented as
   * appropriate.
   *
  ...
  /**
   * Template class for parameterizing the creation of a new <code>StrandedFeature</code>.
   *
   * @author Matthew Pocock
   */
   ...
   * <p>
   * Strandedness may be re-used in other situations, but basically what it means is whether the
   * feature has directionality, and if it does, does it travel from its location min to max, or max
   * to min.
   *
   * @author Matthew Pocock
   */
  public static final class Strand implements Serializable {
    ...
    /**
     * Returns the integer label for strandedness. That is, "+1", "-1", or "0" for positive,
     * negative, and unknown strands respectively.
     */
    public int getValue() {
      return value;
    }
    ...
    /**
     * Returns the token for strandedness. That is, "+","-","." for positive, negative and unknown
