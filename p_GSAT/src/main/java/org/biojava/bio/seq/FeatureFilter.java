 ...
 *
 * <p>
 * It is possible to write custom <code>FeatureFilter</code>s by implementing this interface. There
 * are also a wide range of built-in features, and it is possible to build complex queries using
 * <code>FeatureFilter.And</code>, <code>FeatureFilter.Or</code>, and
 * <code>FeatureFilter.Not</code>. Where possible, use of the built-in filters is preferable to
 * writing new filters, since the methods in the <code>FilterUtils</code> class have access to
 * special knowledge about the built-in filter types and how they relate to one another.
 * </p>
 *
 * <p>
 * If the filter is to be used in a remote process, it is recognized that it may be serialized and
 * sent over to run remotely, rather than each feature being retrieved locally.
 * </p>
 *
 * @since 1.0
...
public interface FeatureFilter extends Serializable {
  ...
  final public static class ByType implements OptimizableFilter {
    ...
    public boolean isDisjoint(FeatureFilter filt) {
      return (filt instanceof AcceptNoneFilter)
          || ((filt instanceof ByType) && !getType().equals(((ByType) filt).getType()));
    }
  ...
  public final static class BySource implements OptimizableFilter {
    ...
    public boolean accept(Feature f) {
      return source.equals(f.getSource());
    }
    ...
    public boolean isDisjoint(FeatureFilter filt) {
      return (filt instanceof AcceptNoneFilter)
          || ((filt instanceof BySource) && !getSource().equals(((BySource) filt).getSource()));
    }
  ...
  public final static class ByClass implements OptimizableFilter {
    ...
    public boolean isDisjoint(FeatureFilter feat) {
      if (feat instanceof ByClass) {
        ...
        return !(featC.isAssignableFrom(getTestClass()))
  ...
  public final static class StrandFilter implements OptimizableFilter {
    ...
    /**
     * Accept the Feature if it is an instance of StrandedFeature and matches the value of
     * getStrand().
     *
    ...
    public boolean isDisjoint(FeatureFilter filt) {
      return (filt instanceof AcceptNoneFilter)
          || ((filt instanceof StrandFilter) && ((StrandFilter) filt).getStrand() == getStrand());
    }
  }
   ...
   * @since 1.3
   */
  public final static class BySequenceName implements OptimizableFilter {
    ...
    public boolean equals(Object o) {
      return (o instanceof BySequenceName)
  ...
  public final static class ContainedByLocation implements OptimizableFilter {
    ...
    public boolean equals(Object o) {
      return (o instanceof ContainedByLocation)
  ...
  public final static class OverlapsLocation implements OptimizableFilter {
    ...
    public boolean equals(Object o) {
      return (o instanceof OverlapsLocation)
  ...
  /**
   * A filter that accepts all features whose shadow overlaps a specified <code>Location</code>. The
   * shadow is defined as the interval between the minimum and maximum positions of the feature's
   * location. For features with contiguous locations, this filter is equivalent to
   * <code>FeatureFilter.OverlapsLocation</code>..
   *
   * <p>
   * A typical use of this filter is in graphics code where you are rendering features with
   * non-contiguous locations in a `blocks and connectors' style, and wish to draw the connector
   * even when no blocks fall within the selected field of view
   * </p>
   *
   * @author Thomas Down
   * @since 1.3
  ...
  public final static class ShadowOverlapsLocation implements OptimizableFilter {
    ...
    public boolean equals(Object o) {
      return (o instanceof ShadowOverlapsLocation)
   ...
   * <code>Location</code>. The shadow is defined as the interval between the minimum and maximum
   * positions of the feature's location. For features with contiguous locations, this filter is
   * equivalent to <code>FeatureFilter.ContainedByLocation</code>.
   *
   * @author Thomas Down
   * @since 1.3
   */
  ...
  public final static class ShadowContainedByLocation implements OptimizableFilter {
    ...
    public boolean equals(Object o) {
      return (o instanceof ShadowContainedByLocation)
  ...
  /**
   * A filter that returns all features that have an annotation bundle that is of a given annotation
   * type.
   *
   * @author Matthew Pocock
   * @author Thomas Down
    ...
    public boolean isProperSubset(FeatureFilter filter) {
      if (filter instanceof ByAnnotationType) {
        ...
        for (Iterator i = thatProps.iterator(); i.hasNext();) {
          ...
          if (!thatP.subConstraintOf(thisP)) {
            return false;
          }
        }
   ...
   * @since 1.1
   */
  public final static class ByAnnotation extends ByAnnotationType {
    private Object key;
    private Object value;

    /**
     * Make a new ByAnnotation that will accept features with an annotation bundle containing
  ...
  /**
   * Retrieve features that contain a given annotation, and that the set of values contains the
   * value given.
   *
   * @author Thomas Down
   * @since 1.3
   */
  public final static class AnnotationContains extends ByAnnotationType {
    private Object key;
    private Object value;

    /**
     * Make a new AnnotationContains that will accept features with an annotation bundle where the
     * value-set assosiated with the property <code>key</code> contains a member equal to
     * <code>value</code>.
     *
     * @param key the Object used as a key in the annotation
     * @param value the Object associated with key in the annotation
    ...
    public AnnotationContains(Object key, Object value) {
      ...
      type.setConstraint(key, new CollectionConstraint.Contains(
          new PropertyConstraint.ExactValue(value), CardinalityConstraint.ONE));
      setType(type);
    }
   ...
   * @since 1.1
   */
  public final static class HasAnnotation extends ByAnnotationType {
    private Object key;

    /**
     * Make a new ByAnnotation that will accept features with an annotation bundle containing any
     * value associated with 'key'.
     *
     * @param key the Object used as a key in the annotation
     */
  ...
  /**
   * Filter by applying a nested <code>FeatureFilter</code> to the parent feature. Always
   * <code>false</code> if the parent is not a feature (e.g. top-level features, where the parent is
   * a sequence).
   *
   * @author Thomas Down
   * @since 1.2
  ...
  public static class ByParent implements OptimizableFilter, Up {
    ...
    public boolean isDisjoint(FeatureFilter ff) {
      ...
      if (ff instanceof IsTopLevel) {
        ...
        return FilterUtils.areDisjoint(((FeatureFilter.ByParent) ff).getFilter(), getFilter());
      } else if (ff instanceof FeatureFilter.ByAncestor) {
        return FilterUtils.areDisjoint(((FeatureFilter.ByAncestor) ff).getFilter(), getFilter());
      } else {
  ...
  /**
   * Filter by applying a nested <code>FeatureFilter</code> to all ancestor features. Returns
   * <code>true</code> if at least one of them matches the filter. Always <code>false</code> if the
   * parent is not a feature (e.g. top-level features, where the parent is a sequence).
   *
   * @author Thomas Down
   * @since 1.2
   */
        ...
        if (childFilter != null) {
          ...
          if (FilterUtils.areProperSubset(childFilter, leaf)) {
            ...
            return FilterUtils.areDisjoint(
                new FeatureFilter.Or(childFilter, new FeatureFilter.ByAncestor(childFilter)), ff);
          }
        }
      }
  ...
  /**
   * Accepts features where all immediate children meet the supplied filter. This will be
   * <code>true</code> in the case where no child features exist. Mainly useful for defining schemas
   * of feature-trees.
   *
   * @author Thomas Down
   * @since 1.3
  ...
  public static class OnlyChildren implements OptimizableFilter, ByHierarchy {
    ...
    public boolean isProperSubset(FeatureFilter ff) {
      if (ff == FeatureFilter.all) {
        ...
        return FilterUtils.areProperSubset(getFilter(), ((OnlyChildren) ff).getFilter());
      } else if (ff instanceof OnlyDescendants) {
        return FilterUtils.areProperSubset(getFilter(), ((OnlyDescendants) ff).getFilter());
      } else {
        return false;
      }
    }
    ...
    public boolean isDisjoint(FeatureFilter ff) {
      if (ff instanceof ByChild) {
        return FilterUtils.areDisjoint(getFilter(), ((ByChild) ff).getFilter());
      } else {
        return false;
      }
    }
  ...
  /**
   * Accepts features where all descendants meet the supplied filter. This will be <code>true</code>
   * in the case where no child features exist. Mainly useful for defining schemas of feature-trees.
   *
   * @author Thomas Down
   * @since 1.3
   */
  ...
  public static class OnlyDescendants implements OptimizableFilter, ByHierarchy {
    ...
    public boolean isDisjoint(FeatureFilter ff) {
      if (ff instanceof ByChild) {
        ...
        return FilterUtils.areDisjoint(getFilter(), ((ByDescendant) ff).getFilter());
      } else {
        return false;
      }
    }
  ...
  /**
   * Filter by applying a nested <code>FeatureFilter</code> to the child features. Always
   * <code>false</code> if there are no children.
   *
   * @author Matthew Pocock
   * @author Thomas Down
   * @since 1.3
  ...
  public static class ByChild implements OptimizableFilter, Down {
    ...
    public boolean isDisjoint(FeatureFilter ff) {
      if (ff instanceof OnlyChildren) {
        return FilterUtils.areDisjoint(getFilter(), ((OnlyChildren) ff).getFilter());
      } else if (ff instanceof OnlyDescendants) {
        return FilterUtils.areDisjoint(getFilter(), ((OnlyDescendants) ff).getFilter());
      } else {
        return false;
      }
    }
  ...
  /**
   * Filter by applying a nested <code>FeatureFilter</code> to all descendant features. Returns
   * <code>true</code> if at least one of them matches the filter. Always <code>false</code> if the
  ...
  public final static class FrameFilter implements OptimizableFilter {
    ...
    public boolean isDisjoint(FeatureFilter filt) {
      return (filt instanceof AcceptNoneFilter)
          || ((filt instanceof FrameFilter) && ((FrameFilter) filt).getFrame() == getFrame());
    }
  }

  /**
   * <code>ByPairwiseScore</code> is used to filter <code>SimilarityPairFeature</code>s by their
   * score. Features are accepted if their score falls between the filter's minimum and maximum
   * values, inclusive. Features are rejected if they are not <code>SimilarityPairFeature</code>s.
   * The minimum value accepted must be less than the maximum value.
   *
   * @author Keith James
   * @since 1.3
   */
  public static final class ByPairwiseScore implements OptimizableFilter {
    ...
    /**
     * Accept a Feature if it is an instance of SimilarityPairFeature and its score is <= filter's
     * minimum score and >= filter's maximum score.
     *
    ...
    /**
     * <code>getMinScore</code> returns the minimum score accepted.
     *
    ...
    /**
     * <code>getMaxScore</code> returns the maximum score accepted.
     *
     * @return a <code>double</code>.
     */
    ...
    public boolean equals(Object o) {
      if (o instanceof ByPairwiseScore) {
        ...
        if (psf.getMinScore() == minScore && psf.getMaxScore() == maxScore) {
          return true;
        }
      }
      return false;
    ...
    public boolean isProperSubset(FeatureFilter sup) {
      if (sup instanceof ByPairwiseScore) {
        ...
        return (psf.getMinScore() <= minScore && psf.getMaxScore() >= maxScore);
      }
      return false;
    }

    public boolean isDisjoint(FeatureFilter filt) {
      if (filt instanceof AcceptNoneFilter) return true;

      if (filt instanceof ByPairwiseScore) {
        ByPairwiseScore psf = (ByPairwiseScore) filt;
        return (psf.getMaxScore() < minScore || psf.getMinScore() > maxScore);
      }
      return false;
    }
  ...
  public final static class ByComponentName implements OptimizableFilter {
    ...
    public boolean equals(Object o) {
      return (o instanceof ByComponentName)
          && ((ByComponentName) o).getComponentName().equals(cname);
    }
  ...
  /**
   * A filter which accepts only top-level Features. This is true is <code>getParent()</code>
   * returns a <code>Sequence</code> instance.
   *
   * @since 1.3
   */
