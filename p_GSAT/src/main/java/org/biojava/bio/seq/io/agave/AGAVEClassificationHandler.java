 ...
 */
public class AGAVEClassificationHandler extends StAXFeatureHandler
    implements
  ...
  public void addIdAlias(AGAVEIdAlias id) {
    if (idAliases == null) idAliases = new ArrayList(1);
    idAliases.add(id);
  }
  /*
   ...
   * 
   * // assume feature set to describe a transcript st.type = "classification"; st.strand =
   * StrandedFeature.UNKNOWN; // set up annotation bundle st.annotation = annot; st.location = new
   * Location.EmptyLocation(); if( staxenv != null ) staxenv. subFeatures .add( this ) ;
   * 
   * return st; }
   */
  ...
  public void endElementHandler(String nsURI, String localName, String qName,
    ...
    try {
      if (idAliases != null) annot.setProperty("id_alias", idAliases);
    } catch (Exception e) {
      throw new SAXException(e.getMessage());
    }
  }
