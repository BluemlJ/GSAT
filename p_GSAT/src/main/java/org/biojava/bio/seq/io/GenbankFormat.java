...
/**
 * Format reader for GenBank files. Converted from the old style io to the new by working from
 * <code>EmblLikeFormat</code>.
 *
 * @author Thomas Down
 * @author Thad Welch Added GenBank header info to the sequence annotation. The ACCESSION header tag
 *         is not included. Stored in sequence.getName().
 * @author Greg Cox
 * @author Keith James
...
public class GenbankFormat
  ...
  /**
   * Reads a sequence from the specified reader using the Symbol parser and Sequence Factory
   * provided. The sequence read in must be in Genbank format.
   *
   * @return boolean True if there is another sequence in the file; false otherwise
   */
  ...
  /**
   * Use this method to toggle reading of sequence data. If you're only interested in header data
   * set to true.
   * 
