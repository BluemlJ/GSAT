 ...
 * @author Greg Cox
 */
public class AGAVEContigHandler extends StAXFeatureHandler
    implements
  ...
  /*
   * protected Feature.Template createTemplate() { // create Gene Template for this
   * StrandedFeature.Template st = new StrandedFeature.Template();
   * 
   * // assume feature set to describe a transcript st.type = "contig"; st.strand =
   * StrandedFeature.UNKNOWN; // set up annotation bundle st.annotation = annot; //new
   * SmallAnnotation(); st.location = new Location.EmptyLocation(); if( staxenv != null ) staxenv.
   * subFeatures .add( this ) ;
   * 
   * return st; }
   */
  ...
  public void endElementHandler(String nsURI, String localName, String qName,
    ...
    try {
      if (sequenceSet.size() == 0) {
        if (dna == null) throw new SAXException("dna sequence must offered ");
        ...
        int contig_len =
            new Integer((String) UtilHelper.getProperty(featureTemplate.annotation, "length"))
                .intValue();
