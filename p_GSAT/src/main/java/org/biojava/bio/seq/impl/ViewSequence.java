...
/**
 * A view onto another Sequence object. This class allows new features and annotations to be
 * overlaid onto an existing Sequence without modifying it.
 *
 * You will almost certainly want to be calling
 * {@link org.biojava.bio.seq.SequenceTools#view(Sequence seq)} instead of instantiating this class
 * directly.
 *
 * @author Thomas Down
...
public class ViewSequence extends Unchangeable
    implements
      Sequence,
  ...
  public ViewSequence(Sequence seq, String name) {
    ...
    try {
      exposedFeatures
          .addFeatureHolder(new ProjectedFeatureHolder(new ReparentContext(this, seqDelegate)));
  ...
  /**
   * Remove a feature from this sequence. <strong>NOTE:</strong> This method will only succeed for
   * features which were added to this ViewSequence. Trying to remove a Feature from the underlying
   * sequence will cause an IllegalArgumentException. I think this is the correct behaviour.
   */
  ...
  public Feature createFeature(Feature.Template template) throws BioException, ChangeVetoException {
    ...
    if (loc.getMin() < 1 || loc.getMax() > this.length()) {
      throw new BioException(
          "Failed to create a feature with a location " + loc + " outside the sequence: name '"
              + getName() + "', URN '" + getURN() + "' length " + length());
    }
