...
public class ReparentContext implements ProjectionContext, Serializable {
  ...
  public final Sequence getSequence(Feature f) {
    ...
    if (!(fh instanceof Sequence)) {
      throw new BioRuntimeException(
          "Chasing up parents to get sequence: actually hit: " + fh.toString());
    }
  ...
  public final Feature createFeature(Feature.Template projTempl)
    ...
    synchronized (wrapped) {
      f = wrapped.createFeature(ProjectionEngine.DEFAULT.revertTemplate(projTempl, this));
    }
  ...
  private class ProjectionSet extends Unchangeable implements FeatureHolder, Serializable {
    ...
    public boolean containsFeature(Feature f) {
      if (!(f instanceof Projection)) {
        ...
        return p.getProjectionContext() == ReparentContext.this
            && baseSet.containsFeature(p.getViewedFeature());
      }
    }
  ...
  private class PFChangeForwarder extends ChangeSupport implements ChangeListener {
    ...
    protected ChangeEvent forwardFeatureChangeEvent(Feature f, ChangeEvent cev) {
      return new ChangeEvent(projectFeature(f), cev.getType(), cev.getChange(), cev.getPrevious(),
          cev);
