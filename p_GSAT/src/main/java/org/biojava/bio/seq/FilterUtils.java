...
public class FilterUtils {
  ...
  /**
   * Determines if the set of features matched by sub can be <code>proven</code> to be a proper
   * subset of the features matched by sup.
   * <p>
   * If the filter sub matches only features that are matched by sup, then it is a proper subset. It
   * is still a proper subset if it does not match every feature in sup, as long as no feature
   * matches sub that is rejected by sup.
   * </p>
   *
   ...
   * <p>
   * They are disjoint if there is no element that is matched by both filters - that is, they have
   * an empty intersection. Order of arguments to this method is not significant.
   * </p>
   *
   ...
   * @return the minimal location which any features matching <code>ff</code> must overlap, or
   *         <code>null</code> if no proof is possible (normally indicates that the filter has
   ...
   * <code>f1.accept(feature) == f2.accept(feature)</code> for all values of <code>feature</code>.
   * If areEqual returns false, this may indicate that they accept clearly different sets of
   * features. It may also, howerver, indicate that the method was unable to prove that they were
   * equal.
   * </p>
   *
   * <p>
   * Note that given a finite set of features, f1 and f2 may match the same sub-set of those
   * features even if they are not equal.
   * </p>
   *
  ...
  /**
   * Construct a filter which matches features with a specific <code>type</code> value.
   */
  ...
  /**
   * Construct a filter which matches features with a specific <code>source</code> value.
   */
  ...
  /**
   * Construct a filter which matches features which are assignable to the specified class or
   * interface.
   */

  public final static FeatureFilter byClass(Class clazz) throws ClassCastException {
  ...
  /**
   * Construct a filter which matches features with locations contained by or overlapping the
   ...
   * <code>min</code> and <code>max</code> positions are contained by or overlap the specified
   * <code>Location</code>.
   */
  ...
  /**
   * Construct a filter which matches features with locations where the interval between the
   * <code>min</code> and <code>max</code> positions are contained by the specified
   * <code>Location</code>.
   */
  ...
  /**
   * Match features where the annotation property named <code>key</code> is equal to
   * <code>value</code>.
   */
  ...
  /**
   * Match features where the annotation property named <code>key</code> is an instance of
   * <code>valClass</code>.
   */
  ...
  /**
   * Match features where the property <code>key</code> has been defined as having some value,
  ...
  /**
   * Match features where the parent feature matches the specified filter. This cannot match
   * top-level features.
   */
  ...
  /**
   * Match features where at least one of the ancestors matches the specified filter. This cannot
   * match top-level features.
   */
  ...
  /**
   * Match features where at least one child feature matches the supplied filter. This does not
   * match leafFeatures.
   */
  ...
  /**
   * Construct a filter which matches features whose children all match the specified filter. This
   * filter always matches leaf features.
   */
  ...
  /**
   * Construct a filter which matches features whose decendants all match the specified filter. This
   * filter always matches leaf features.
   */
  ...
  /**
   * Construct a filter which matches all features which implement the <code>ComponentFeature</code>
   * interface and have a <code>componentName</code> property equal to the specified value
   */
  ...
  /**
   * Return a filter which matches all top-level features. These are features which are direct
   * children of a <code>Sequence</code> rather than another <code>Feature</code>.
   */
   ...
   *
   * <p>
   * This will attempt to push all leaf constraints as far from the root of the filter expression as
   * possible, in an attept to prove an empty or universal set. It will then propogate these through
   * the logical operators in an attempt to reduce the entire expression to the empty or universal
   * set. If filters can be combined (for example, overlapping constraints), then this will happen
   * on the way.
   * </p>
   *
   * <p>
   * The resulting filter is guaranteed to accept exactly the same set of\ features that is accepted
   * by the argument. In particular, <code>areEqual(filter, optimize(filter))</code> is always true.
   * </p>
   *
      ...
      if (filter instanceof FeatureFilter.And) {
        ...
        if (filters.isEmpty()) {
          if (ors.isEmpty()) {
            ...
            FeatureFilter andedOrs =
                FilterUtils.and((FeatureFilter[]) filters.toArray(new FeatureFilter[] {}));
          ...
          if (ors.isEmpty()) {
            ...
            FeatureFilter res =
                optimize(or((FeatureFilter[]) combs.toArray(new FeatureFilter[] {})));
        ...
        if (filters.isEmpty()) {
          ...
          if (ands.isEmpty()) {
            ...
            FeatureFilter val =
                optimize(and((FeatureFilter[]) combs.toArray(new FeatureFilter[] {})));
  ...
  private static FeatureFilter intersection(FeatureFilter f1, FeatureFilter f2) {
    if (f1 instanceof FeatureFilter.ContainedByLocation
        && f2 instanceof FeatureFilter.ContainedByLocation) {
      Location loc =
          LocationTools.intersection(((FeatureFilter.ContainedByLocation) f1).getLocation(),
    ...
    } else if (f1 instanceof FeatureFilter.OverlapsLocation
        && f2 instanceof FeatureFilter.OverlapsLocation) {
      // can't do much here
    } else if (f1 instanceof FeatureFilter.ByAnnotationType
    ...
    } else if (f1 instanceof ByHierarchy && f2 instanceof ByHierarchy) {
  ...
  private static FeatureFilter union(FeatureFilter f1, FeatureFilter f2) {
    if (f1 instanceof FeatureFilter.ContainedByLocation
        && f2 instanceof FeatureFilter.ContainedByLocation) {
      return containedByLocation(
          LocationTools.union(((FeatureFilter.ContainedByLocation) f1).getLocation(),
              ((FeatureFilter.ContainedByLocation) f2).getLocation()));
    } else if (f1 instanceof FeatureFilter.OverlapsLocation
        && f2 instanceof FeatureFilter.OverlapsLocation) {
      return overlapsLocation(
          LocationTools.intersection(((FeatureFilter.OverlapsLocation) f1).getLocation(),
      ...
      if (f1 instanceof Up && f2 instanceof Up) {
        ...
        if (f1h instanceof FeatureFilter.ByDescendant
  ...
  /**
   * <p>
   * This is a general framework method for transforming one filter into another. This method will
   * handle the logical elements of a query (and, or, not) and delegate all the domain-specific
   * munging to a FilterTransformer object.
   * </p>
   *
   * <p>
   * The transformer could flip strands and locations of elements of a filter, add or remove
   * attributes required in annotations, or systematically alter feature types or sources.
   * </p>
   *
  ...
  public static FeatureFilter transformFilter(FeatureFilter ff, FilterTransformer trans) {
    ...
    if (ff instanceof FeatureFilter.And) {
      ...
      return and(transformFilter(and.getChild1(), trans), transformFilter(and.getChild2(), trans));
    } else if (ff instanceof FeatureFilter.Or) {
      FeatureFilter.Or or = (FeatureFilter.Or) ff;
      return or(transformFilter(or.getChild1(), trans), transformFilter(or.getChild2(), trans));
    } else if (ff instanceof FeatureFilter.Not) {
      return not(transformFilter(((FeatureFilter.Not) ff).getChild(), trans));
    } else {
  ...
  /**
   * An implementation of FilterTransformer that attempts to transform by one transformer, and if
   * that fails, by another.
   *
   * @author Matthew Pocock
   */
