...
public class SimpleFeature extends AbstractChangeable
    implements
      Feature,
  ...
  /**
   * The FeatureHolder that we will delegate the FeatureHolder interface too. This is lazily
   * instantiated.
   */
  ...
  /**
   * The type of this feature - something like Exon. This is included for cheap interoperability
   * with GFF.
   */
  private String type;
  /**
   * The source of this feature - the program that generated it. This is included for cheap
   * interoperability with GFF.
   */
  ...
  /**
   * A utility function to find out if the feature holder delegate has been instantiated yet. If it
   * has not, we may avoid instantiating it by returning some pre-canned result.
   *
   * @return true if the feature holder delegate has been created and false otherwise
   */
  ...
  protected ChangeSupport getChangeSupport(ChangeType ct) {
    ...
    if ((featureForwarder == null) && (ct == null || ct == FeatureHolder.FEATURES)) {
      featureForwarder = new ChangeForwarder(this, cs);
      getFeatureHolder().addChangeListener(featureForwarder, FeatureHolder.FEATURES);
    }
  ...
  public FeatureHolder filter(FeatureFilter ff, boolean recurse) {
    if (featureHolderAllocated()) return getFeatureHolder().filter(ff, recurse);
    return FeatureHolder.EMPTY_FEATURE_HOLDER;
  }
  ...
  /**
   * Create a <code>SimpleFeature</code> on the given sequence. The feature is created underneath
   * the parent <code>FeatureHolder</code> and populated directly from the template fields. However,
   * if the template annotation is the <code>Annotation.EMPTY_ANNOTATION</code>, an empty
   * <code>SimpleAnnotation</code> is attached to the feature instead.
   * 
  ...
  public SimpleFeature(Sequence sourceSeq, FeatureHolder parent, Feature.Template template) {
    if (template.location == null) {
      throw new IllegalArgumentException(
          "Location can not be null. Did you mean Location.EMPTY_LOCATION? " + template.toString());
    }
    if (!(parent instanceof Feature) && !(parent instanceof Sequence)) {
      throw new IllegalArgumentException(
          "Parent must be sequence or feature, not: " + parent.getClass() + " " + parent);
    }

    if (template.location.getMin() < 1 || template.location.getMax() > sourceSeq.length()) {
      // throw new IllegalArgumentException("Location " + template.location.toString() + " is
      // outside 1.." + sourceSeq.length());
    }
    ...
    this.annotation =
        template.annotation != null ? new SimpleAnnotation(template.annotation) : null;
  }

  public String toString() {
    return "Feature " + getType() + " " + getSource() + " " + getLocation();
  }
  ...
  public boolean equals(Object o) {
    ...
    if (!fo.getSequence().equals(getSequence())) return false;
