...
/**
 * Encapsulate state used while reading data from a specific Genbank file.
 *
 * @author Thomas Down
 * @author Greg Cox
...
class GenbankContext
  ...
  /**
   * Constructor that takes the listener and the Symbol parser from the GenbankFormat.
   *
  ...
  /**
   * Processes the line passed in in the context of previous lines processed. This is the method
   * that does the real work of the class.
   *
   * @throws ParseException Thrown when an error occurs parsing the file
   * @throws IllegalSymbolException Thrown when there is an illegal symbol in the file; i.e. there
   *         is a symbol in the file that is not in the alphabet being used
  ...
  /**
   * Private method that processes a line, assuming that it is a sequence line instead of a feature
   * line or header line. Removes whitespace and location numbers and hands the sequence data to
   * theParser to convert to biojava symbols
   *
   * @throws IllegalSymbolException Thrown when there is an illegal symbol in the file; i.e. there
   *         is a symbol in the file that is not in the alphabet being used
   * @param line The line to process
  ...
  private void processSeqLine(String line, StreamParser theParser) throws IllegalSymbolException {
    ...
    while (parseStart < cline.length) {
      while ((parseStart < cline.length)
          && ((cline[parseStart] == ' ') || (Character.isDigit(cline[parseStart])))) {
  ...
  /**
   * Private method to process a line assuming it's a feature line. A feature line is defined to be
   * a line between the FEATURE tag and the ORIGIN tag. The BASE COUNT line is processed here.
   *
   * @throws ParseException Thrown when an error occurs parsing the file
  ...
  /**
   * Private method to process a line assuming it's a header line. A header line is defined to be a
   * line before the FEATURE tag appears in the file.
   *
   * @throws ParseException Thrown when an error occurs parsing the file
   * @param line The line to be processed
   */
  private void processHeaderLine(String line) throws ParseException {
    if (line.startsWith(GenbankFormat.LOCUS_TAG)) {
      ...
      if (lineTokens.hasMoreTokens()) {
        ...
        if (nextToken.startsWith(GenbankFormat.GI_TAG)) {
          ...
          headerTagText = new StringBuffer(nextToken.substring(3));
        }
      }
    } else if (hasHeaderTag(line)) { // line has a header tag
      this.saveSeqAnno();
    ...
    else if (line.length() >= TAG_LENGTH) { // keep appending tag text value
      headerTagText.append(" " + line.substring(TAG_LENGTH));
    }
  }

  /**
   * Checks which version of the locus line format is used. The algorithm switches on the size of
   * the line; <75 means pre-127, otherwise it's 127.
   *
   * @param theLine the line to check the format of.
   * @return TRUE if the line is in Genbank release 126 or earlier format. FALSE otherwise
   */
  ...
  /**
   * Parses the locus line assuming it is in post release 127 format. Will also handle the case
   * where the strand tag is optional. That is the only tag that is supported as optional. Awaiting
   * a response from NCBI if it is or if their data is incorrectly formatted.
   *
   * @param theLine Locus line to parse.
   * @throws ParseException If the line is too short.
   */
  ...
  private void saveSeqAnno() throws ParseException {
    if (!headerTag.equals("")) { // save tag and its text
  ...
  /**
   * @return does the line contain a header tag. Yes, if any of the leading TAG_LENGTH characters
   *         aren't a space
   */
