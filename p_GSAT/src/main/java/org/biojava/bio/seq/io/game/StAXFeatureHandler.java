...
/**
 * StAX handler shamelessly ripped off from Thomas Down's XFFFeatureSetHandler. It was modified for
 * greater generality.
 *
...
public class StAXFeatureHandler extends StAXContentHandlerBase {
  ...
  /**
   * this is the stack of handler objects for the current feature. The base value is the
   * FeatureHandler itself. your feature and property handlers place and remove themselves from this
   * stack.
   *
   * the purpose of all this is to implement context sensitivty for property handlers translucently.
   * Property handlers can pop the stack for other handlers that implement interfaces that process
   * that element. This way the context code is within the object that defines that context rather
   * than in a child property handler.
   */
  ...
  /**
   * return current stack level. Remember that the stack level is incremented/decremented AFTER the
   * push()/pop() calls and superclass startElement()/StopElement calls.
   */
  ...
  private void fireStartFeature() throws ParseException {
    if (startFired) throw new ParseException("startFeature event has already been fired");

    if (featureTemplate == null) featureTemplate = createTemplate();
  ...
  private void fireEndFeature() throws ParseException {
    if (!startFired) throw new ParseException("startFeature has not yet been fired!");

    if (endFired) throw new ParseException("endFeature event has already been fired!");
  ...
  public void startElement(String nsURI, String localName, String qName, Attributes attrs,
    ...
    // sanity check
    if (!setOnceFired) throw new SAXException("StAXFeaturehandler not initialised before use!");
    ...
    if (!inFeature) {
      ...
      if (hasCallback) {
        ...
        // System.out.println("StAXFeatureHandler startElement called, localName, level: " +
        // localName + " " + stackLevel);
  ...
  public void endElement(String nsURI, String localName, String qName, StAXContentHandler handler)
    ...
    // System.out.println("StAXFeatureHandler endElement called, localName, level: " + localName + "
    // " + stackLevel);
    ...
    // System.out.println("StAXFeatureHandler endElement-leaving, localName, level: " + localName +
    // " " + stackLevel);
  }
}
