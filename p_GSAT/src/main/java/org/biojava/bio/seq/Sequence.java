 ...
 *
 * <p>
 * This interface is a symbol list, so it contains symbols. It is annotatable so that you can add
 * annotation to it, and it is a FeatureHolder so that you can add information about specific
 * regions.
 * </p>
 *
 * <p>
 * It is expected that there may be several implementations of this interface, each of which may be
 * fairly heavy-weight. It takes the SymbolList interface that is nice mathematically, and turns it
 * into a biologically useful object.
 * </p>
 * The {@link org.biojavax.bio.seq.RichSequence RichSequence} interface offers considerably more
 * functionality and better persitence to BioSQL than it's super interface Sequence. We would
 * recommend using it wherever possible.
 * 
...
public interface Sequence extends SymbolList, FeatureHolder, Annotatable {
  /**
   * A <a href="http://www.rfc-editor.org/rfc/rfc2396.txt">Uniform Resource Identifier</a> (URI)
   * which identifies the sequence represented by this object. For sequences in well-known database,
   * this may be a URN, e.g.
   *
   * <pre>
