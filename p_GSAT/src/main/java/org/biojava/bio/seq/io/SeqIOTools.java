...
public final class SeqIOTools {
   ...
   * @return a <CODE>SequenceIterator</CODE> that iterates over each <CODE>Sequence</CODE> in the
   *         file
   */
  public static SequenceIterator readEmbl(BufferedReader br) {
    return new StreamReader(br, new EmblLikeFormat(), getDNAParser(), getEmblBuilderFactory());
  }
  ...
  public static SequenceIterator readEmblRNA(BufferedReader br) {
    return new StreamReader(br, new EmblLikeFormat(), getRNAParser(), getEmblBuilderFactory());
  }
  ...
  public static SequenceIterator readEmblNucleotide(BufferedReader br) {
    return new StreamReader(br, new EmblLikeFormat(), getNucleotideParser(),
  ...
  public static SequenceIterator readGenbank(BufferedReader br) {
    return new StreamReader(br, new GenbankFormat(), getDNAParser(), getGenbankBuilderFactory());
  }
  ...
  public static SequenceIterator readGenbankXml(BufferedReader br) {
    return new StreamReader(br, new GenbankXmlFormat(), getDNAParser(), getGenbankBuilderFactory());
  }
  ...
  public static SequenceIterator readGenpept(BufferedReader br) {
    return new StreamReader(br, new GenbankFormat(), getProteinParser(),
  ...
  public static SequenceIterator readSwissprot(BufferedReader br) {
    return new StreamReader(br, new EmblLikeFormat(), getProteinParser(),
  ...
  public static SequenceIterator readFasta(BufferedReader br, SymbolTokenization sTok) {
    return new StreamReader(br, new FastaFormat(), sTok, getFastaBuilderFactory());
  }

  /**
   * Read a fasta file using a custom type of SymbolList. For example, use
   * SmartSequenceBuilder.FACTORY to emulate readFasta(BufferedReader, SymbolTokenization) and
   * SmartSequenceBuilder.BIT_PACKED to force all symbols to be encoded using bit-packing.
   * 
   * @param br the BufferedReader to read data from
   * @param sTok a SymbolTokenization that understands the sequences
  ...
  public static SequenceIterator readFastaDNA(BufferedReader br) {
    return new StreamReader(br, new FastaFormat(), getDNAParser(), getFastaBuilderFactory());
  }
  ...
  public static SequenceIterator readFastaRNA(BufferedReader br) {
    return new StreamReader(br, new FastaFormat(), getRNAParser(), getFastaBuilderFactory());
  }
  ...
  public static SequenceIterator readFastaProtein(BufferedReader br) {
    return new StreamReader(br, new FastaFormat(), getProteinParser(), getFastaBuilderFactory());
  }

  /**
   * Create a sequence database from a fasta file provided as an input stream. Note this somewhat
   * duplicates functionality in the readFastaDNA and readFastaProtein methods but uses a stream
   * rather than a reader and returns a SequenceDB rather than a SequenceIterator. If the returned
   * DB is likely to be large then the above mentioned methods should be used.
   * 
   * @return a <code>SequenceDB</code> containing all the <code>Sequences</code> in the file.
   * @since 1.2
  ...
  public static SequenceDB readFasta(InputStream seqFile, Alphabet alpha) throws BioException {
    ...
    for (SequenceIterator seqI =
        new StreamReader(seqFile, fFormat, alpha.getTokenization("token"), sbFact); seqI
            .hasNext();) {
      ...
      try {
        ...
        throw new AssertionFailure(
            "Could not successfully add sequence " + seq.getName() + " to sequence database", cve);
      }
    }
    return db;
  }
  ...
  /**
   * Writes sequences from a SequenceIterator to an OutputStream in Fasta Format. This makes for a
   * useful format filter where a StreamReader can be sent to the StreamWriter after formatting.
   * 
   * @since 1.2
   ...
   * @param os the OutputStream.
   * @param in a SequenceIterator.
   * @throws org.biojava.bio.BioException if the <CODE>Sequence</CODE> cannot be converted to
   *         SwissProt format
   * @exception IOException if there was an error while writing.
   */
   ...
   * @param os the OutputStream.
   * @param seq the Sequence.
   * @throws org.biojava.bio.BioException if the <CODE>Sequence</CODE> cannot be written to
   *         SwissProt format
   * @throws IOException if there was an error while writing.
   */
   ...
   * @param os the OutputStream.
   * @param in a SequenceIterator.
   * @throws org.biojava.bio.BioException if the <CODE>Sequence</CODE> cannot be written to Genpept
   *         format
   * @exception IOException if there was an error while writing.
   */
  ...
  /**
   * <code>identifyFormat</code> performs a case-insensitive mapping of a pair of common sequence
   * format name (such as 'embl', 'genbank' or 'fasta') and alphabet name (such as 'dna', 'rna',
   * 'protein', 'aa') to an integer. The value returned will be one of the public static final
   * fields in <code>SeqIOConstants</code>, or a bitwise-or combination of them. The method will
   * reject known illegal combinations of format and alphabet (such as swissprot + dna) by throwing
   * an <code>IllegalArgumentException</code>. It will return the
   * <code>SeqIOConstants.UNKNOWN</code> value when either format or alphabet are unknown.
   *
  ...
  public static int identifyFormat(String formatName, String alphabetName) {
    ...
    } else if (formatName.equalsIgnoreCase("swissprot") || formatName.equalsIgnoreCase("swiss")) {
      if (alphabetName.equalsIgnoreCase("aa") || alphabetName.equalsIgnoreCase("protein")) {
        return SeqIOConstants.SWISSPROT;
      } else {
        throw new IllegalArgumentException(
            "Illegal format and alphabet " + "combination " + formatName + " + " + alphabetName);
      }
  ...
  /**
   * <code>getSequenceFormat</code> accepts a value which represents a sequence format and returns
   * the relevant <code>SequenceFormat</code> object.
   *
   * @param identifier an <code>int</code> which represents a binary value with bits set according
   *        to the scheme described in <code>SeqIOConstants</code>.
   *
  ...
  public static SequenceFormat getSequenceFormat(int identifier) throws BioException {
    ...
    if (alphaType == 0) throw new IllegalArgumentException("No alphabet was set in the identifier");

    // Mask alphabet bytes
    int formatType = identifier & (~0xffff0000);
    if (formatType == 0) throw new IllegalArgumentException("No format was set in the identifier");
    ...
    switch (identifier) {
      ...
      default:
        throw new BioException(
            "No SequenceFormat available for " + "format/alphabet identifier '" + identifier + "'");
    }
  }

  /**
   * <code>getBuilderFactory</code> accepts a value which represents a sequence format and returns
   * the relevant <code>SequenceBuilderFactory</code> object.
   *
   * @param identifier an <code>int</code> which represents a binary value with bits set according
   *        to the scheme described in <code>SeqIOConstants</code>.
   *
    ...
    switch (identifier) {
      ...
      default:
        throw new BioException("No SequenceBuilderFactory available for "
            + "format/alphabet identifier '" + identifier + "'");
    }
  }

  /**
   * <code>getAlphabet</code> accepts a value which represents a sequence format and returns the
   * relevant <code>FiniteAlphabet</code> object.
   *
   * @param identifier an <code>int</code> which represents a binary value with bits set according
   *        to the scheme described in <code>SeqIOConstants</code>.
   *
  ...
  public static FiniteAlphabet getAlphabet(int identifier) throws BioException {
    ...
    switch (alphaType) {
      ...
      default:
        throw new BioException(
            "No FiniteAlphabet available for " + "alphabet identifier '" + identifier + "'");
    }
  }
  ...
  /**
   * Attempts to guess the filetype of a file given the name. For use with the functions below that
   * take an int fileType as a parameter. EMBL and Genbank files are assumed to contain DNA
   * sequence.
   * 
   * @deprecated because there is no standard file naming convention and guessing by file name is
   *             inherantly error prone and bad.
  ...
  /**
   * Attempts to retrieve the most appropriate <code>SequenceBuilder</code> object for some
   * combination of <code>Alphabet</code> and <code>SequenceFormat</code>
   *
   * @param format currently supports <code>FastaFormat</code>, <code>GenbankFormat</code>,
   *        <code>EmblLikeFormat</code>
   ...
   *             <code>identifyBuilderFactory</code>.
   */
  public static SequenceBuilderFactory formatToFactory(SequenceFormat format, Alphabet alpha)
      throws BioException {
    ...
    if ((format instanceof FastaFormat)
      ...
      return getFastaBuilderFactory();
    } else if (format instanceof GenbankFormat && alpha == DNATools.getDNA()) {

      return getGenbankBuilderFactory();
    } else if (format instanceof GenbankFormat && alpha == ProteinTools.getAlphabet()) {
      return getGenpeptBuilderFactory();
    } else if (format instanceof EmblLikeFormat && alpha == DNATools.getDNA()) {
      return getEmblBuilderFactory();
    } else if (format instanceof EmblLikeFormat && alpha == ProteinTools.getAlphabet()) {
      return getSwissprotBuilderFactory();
    } else {
   ...
   * @since 1.3
   */
  public static Object fileToBiojava(String formatName, String alphabetName, BufferedReader br)
      throws BioException {
  ...
  /**
   * Reads a file and returns the corresponding Biojava object. You need to cast it as an Alignment
   * or a SequenceIterator as appropriate.
   * 
   ...
   * @return either a <code>SequenceIterator</code> if the file type is a sequence file, or a
   *         <code>Alignment</code> if the file is a sequence alignment.
   */
  ...
  /**
   * Writes a Biojava <code>SequenceIterator</code>, <code>SequenceDB</code>, <code>Sequence</code>
   * or <code>Aligment</code> to an <code>OutputStream</code>
   *
   ...
   */
  public static void biojavaToFile(String formatName, String alphabetName, OutputStream os,
      Object biojava) throws BioException, IOException, IllegalSymbolException {
    int fileType = identifyFormat(formatName, alphabetName);
   ...
   * @param biojava a <code>SequenceIterator</code>, <code>SequenceDB</code>, <code>Sequence</code>,
   *        or <code>Alignment</code>
   * @throws org.biojava.bio.BioException if <code>biojava</code> cannot be converted to that
   *         format.
   * @throws java.io.IOException if the output cannot be written to <code>os</code>
   * @throws org.biojava.bio.symbol.IllegalSymbolException if <code>biojava
   * </code> contains a <code>Symbol</code> that cannot be understood by the parser.
   */
   ...
   */
  private static void alignToFile(int fileType, OutputStream os, Alignment align)
   ...
   */
  private static void seqToFile(int fileType, OutputStream os, SequenceIterator seq)
      throws IOException, BioException {
    switch (fileType) {
      case SeqIOConstants.FASTA_DNA:
      case SeqIOConstants.FASTA_AA:
