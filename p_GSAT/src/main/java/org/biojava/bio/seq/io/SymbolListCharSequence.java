...
/**
 * <code>SymbolListCharSequence</code> is a <code>CharSequence</code> implementation which wraps a
 * <code>SymbolList</code>. It is present primarily to support regular expression matching over
...
public class SymbolListCharSequence implements CharSequence {
  ...
  /**
   * Creates a new <code>SymbolListCharSequence</code> wrapping a <code>SymbolList</code>.
   *
  ...
  public SymbolListCharSequence(SymbolList syms) {
    ...
    if (!(alphabet instanceof FiniteAlphabet)) throw new IllegalArgumentException(
        "Only SymbolLists using a FiniteAlphabet are supported by SymbolListCharSequence");

    SymbolTokenization sToke = getTokenizer(alphabet, "token");
    if (sToke == null) sToke = getTokenizer(alphabet, "unicode");
    if (sToke == null) throw new BioError(
        "unable to get a character tokenization for alphabet " + alphabet.getName());
  ...
  public CharSequence subSequence(int start, int end) {
    return new SymbolListCharSequence(syms.subList(start + 1, end), alphaTokens);
  }
