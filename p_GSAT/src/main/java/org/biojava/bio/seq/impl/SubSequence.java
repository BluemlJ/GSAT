...
/**
 * View a sub-section of a given sequence object, including all the features intersecting that
 * region.
 *
 * <p>
 * All features entirely contained within the region are projected by just translating their
 * locations. The features that overlap the region are replaced by RemoteFeature instances with
 * fuzzy locations that are trunchated to fit inside the sub-section. All features not contained by
...
public class SubSequence implements Sequence, java.io.Serializable {
  ...
  /**
   * Construct a new SubSequence of the specified sequence. Generally you would use the
   * SequenceTools.subSequence() methods to get an instance of this class.
   * 
   ...
   */
  public SubSequence(Sequence seq, final int start, final int end, final String name) {
    this.parent = seq;
  ...
  protected ProjectedFeatureHolder getFeatures() {
    if (features == null) {
      FeatureHolder clipped =
          new ProjectedFeatureHolder(new SubProjectedFeatureContext(parent, parentLocation));
      features = new ProjectedFeatureHolder(
          new TranslateFlipContext(this, clipped, -parentLocation.getMin() + 1));
    }
    return features;
  }
  ...
  public void addChangeListener(ChangeListener cl, ChangeType ct) {
    ...
    if (annotationForwarder == null && ct == Annotatable.ANNOTATION) {
      annotationForwarder = new ChangeForwarder.Retyper(this, changeSupport, Annotation.PROPERTY);
   ...
   *
   * <p>
   * This context is public because all contexts must be public, and not because it is usefull to
   * you or part of the API. Don't use it directly.
   * </p>
   *
   * <p>
   * The context extends TranslateFlipContext so that it can translate all features within the
   * parent location to being from index 1 in the projection. It also transforms and reverts
   * locations so that locations falling outside the parent location are trunchated and replaced by
   * fuzzy locations. Lastly, features with fuzzy locations are replaced by RemoteFeature instances.
   * </p>
   *
   * @author Matthew Pocock
   * @author Thomas Down
  ...
  public static class SubProjectedFeatureContext extends ReparentContext {
    ...
    private SubProjectedFeatureContext(FeatureHolder wrapped, RangeLocation parentLocation) {
      super(FeatureHolder.EMPTY_FEATURE_HOLDER,
          new LazyFilterFeatureHolder(wrapped, FilterUtils.overlapsLocation(parentLocation)));

      this.parentLocation = parentLocation;
      this.remoteLocationFilter =
          new FeatureFilter.Not(FilterUtils.containedByLocation(parentLocation));
    ...
    public Feature projectFeature(Feature origFeat) {
      if (remoteLocationFilter.accept(origFeat)) {
        ...
        List regions = makeRegions(origFeat.getLocation(), origFeat.getSequence().getName());
    ...
    public FeatureHolder projectChildFeatures(Feature f, FeatureHolder parent) {
      return new LazyFilterFeatureHolder(super.projectChildFeatures(f, parent), clippingFilter);
    }
        ...
        if (LocationTools.overlaps(parentLocation, l)) {
          ...
          if (fuzzyLeft || fuzzyRight) {
            locList.add(new FuzzyLocation(Math.min(l.getMin(), parentLocation.getMin()),
                ...
                Math.min(l.getMax(), parentLocation.getMax()), fuzzyLeft, fuzzyRight,
    ...
    protected FilterUtils.FilterTransformer getReverter() {
      ...
      return new FilterUtils.FilterTransformer() {
        public FeatureFilter transform(FeatureFilter filt) {
          ...
          filt = new FeatureFilter.And(filt, FilterUtils.overlapsLocation(parentLocation));
      ...
      public void setStrand(StrandedFeature.Strand strand) throws ChangeVetoException {
        if (wrapped instanceof StrandedFeature) {
          ...
          throw new ChangeVetoException(
              "Can't set strand. The underlying feature is not stranded: " + wrapped);
        }
      }
