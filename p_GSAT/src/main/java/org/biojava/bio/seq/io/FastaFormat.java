...
/**
 * Format object representing FASTA files. These files are almost pure sequence data. The only
 * `sequence property' reported by this parser is PROPERTY_DESCRIPTIONLINE, which is the contents of
 * the sequence's description line (the line starting with a '>' character). Normally, the first
 * word of this is a sequence ID. If you wish it to be interpreted as such, you should use
 * FastaDescriptionLineParser as a SeqIO filter.
 *
 * If you pass it a RichSeqIOListener, you'll get RichSequence objects in return. Likewise, if you
 * write RichSequence objects, you'll get absolutely correct FASTA formatted output.
 *
 * @author Thomas Down
...
public class FastaFormat
  ...
  /**
   * Reads information from a flatfile to a <code>SeqIOListener</code> using a
   * <code>SymbolTokenizer</code> to convert sequence strings to <code>Symbol</code> objects.
   * 
   * @param reader The reader that is the source of the information
   * @param symParser converts text seqeunce to biojava objects
   * @param siol The listener that listens for event callbacks from this class. The listener can be
   *        a <code>RichSeqIOListener</code>.
   * @throws org.biojava.bio.symbol.IllegalSymbolException if <code>symParser</code> doesn't know
   *         how to convert the text sequence into biojava <code>Symbol</code>s
  ...
  private boolean readSequenceData(BufferedReader r, SymbolTokenization parser,
      ...
      if (bytesRead < 0) {
        ...
        if (parseStart < bytesRead && cache[parseStart] == '>') {
          try {
            ...
            throw new IOException("Can't reset: " + ioe.getMessage() + " parseStart=" + parseStart
                + " bytesRead=" + bytesRead);
          }
  ...
  /**
   * Writes a <code>Sequence</code> or <code>RichSequence</code> to a <code>PrintStream</code> in
   * FASTA format. If the sequence is a <code>RichSequence</code> the format of the header will be
   * in line with the NCBI standard.
   * 
   * @param seq the sequence to format
   * @param os the stream to write the sequence to. To print to screen use <code>System.out</code>
