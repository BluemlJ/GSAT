 ...
 * documents. It uses XMLAnnotationTypeHandler to parse <code>byAnnotationType</code> elements. To
 * handle an individual XML filter, you should call the getStAXContentHandler method
 * <p>
 *
 * <p>
...
public class XMLFilterHandler {
  ...
  /**
   * StAXContentHandler for a particular type of FeatureFilter. Implement this interface to add
   * parsing support for new types of FeatureFilter.
   *
   * @author Thomas Down
   */
  ...
  /**
   * Factory of StAXContentHandlers for a particular type of FeatureFilter. Implement this interface
   * to add parsing support for new types of FeatureFilter.
   *
   * @author Thomas Down
   */
  ...
  public FilterHandler getHandler(String nsURI, String localName) throws SAXException {
    FilterHandlerFactory factory =
        (FilterHandlerFactory) handlerFactories.get(new QName(nsURI, localName));
    if (factory != null) {
  ...
  public XMLFilterHandler() {
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "byType", new CDATAHandlerFactory() {
      protected FeatureFilter stringToFilter(String s) {
        return new FeatureFilter.ByType(s);
      }
    });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "bySource", new CDATAHandlerFactory() {
      protected FeatureFilter stringToFilter(String s) {
        return new FeatureFilter.BySource(s);
      }
    });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "byClass", new CDATAHandlerFactory() {
    ...
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "byStrand", new CDATAHandlerFactory() {
      protected FeatureFilter stringToFilter(String s) throws SAXException {
        s = s.trim();
    ...
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "byFrame", new CDATAHandlerFactory() {
      protected FeatureFilter stringToFilter(String s) throws SAXException {
    ...
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "all", new CDATAHandlerFactory() {
      protected FeatureFilter stringToFilter(String s) {
        return FeatureFilter.all;
      }
    });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "none", new CDATAHandlerFactory() {
      protected FeatureFilter stringToFilter(String s) {
        return FeatureFilter.none;
      }
    });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "isTopLevel", new CDATAHandlerFactory() {
      protected FeatureFilter stringToFilter(String s) {
        return FeatureFilter.top_level;
      }
    });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "isLeaf", new CDATAHandlerFactory() {
      protected FeatureFilter stringToFilter(String s) {
        return FeatureFilter.leaf;
      }
    });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "bySequenceName",
        new CDATAHandlerFactory() {
          protected FeatureFilter stringToFilter(String s) {
            return new FeatureFilter.BySequenceName(s);
          }
        });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "byComponentName",
        new CDATAHandlerFactory() {
          protected FeatureFilter stringToFilter(String s) {
            return new FeatureFilter.ByComponentName(s);
          }
        });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "not", new SingleFilterHandlerFactory() {
      protected FeatureFilter filterToFilter(FeatureFilter ff) {
        return new FeatureFilter.Not(ff);
      }
    });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "byParent",
        new SingleFilterHandlerFactory() {
          protected FeatureFilter filterToFilter(FeatureFilter ff) {
            return new FeatureFilter.ByParent(ff);
          }
        });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "byAncestor",
        new SingleFilterHandlerFactory() {
          protected FeatureFilter filterToFilter(FeatureFilter ff) {
            return new FeatureFilter.ByAncestor(ff);
          }
        });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "byChild",
        new SingleFilterHandlerFactory() {
          protected FeatureFilter filterToFilter(FeatureFilter ff) {
            return new FeatureFilter.ByChild(ff);
          }
        });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "byDescendant",
        new SingleFilterHandlerFactory() {
          protected FeatureFilter filterToFilter(FeatureFilter ff) {
            return new FeatureFilter.ByDescendant(ff);
          }
        });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "onlyChildren",
        new SingleFilterHandlerFactory() {
          protected FeatureFilter filterToFilter(FeatureFilter ff) {
            return new FeatureFilter.OnlyChildren(ff);
          }
        });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "onlyDescendants",
        new SingleFilterHandlerFactory() {
          protected FeatureFilter filterToFilter(FeatureFilter ff) {
            return new FeatureFilter.OnlyDescendants(ff);
          }
        });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "and", new FiltersHandlerFactory() {
    ...
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "or", new FiltersHandlerFactory() {
      protected FeatureFilter filtersToFilter(List l) {
        FeatureFilter ff;
        Iterator i = l.iterator();
        ff = (FeatureFilter) i.next();
    ...
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "overlapsLocation",
    ...
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "containedByLocation",
        new LocationHandlerFactory() {
          protected FeatureFilter locationToFilter(Location loc) {
            return new FeatureFilter.ContainedByLocation(loc);
          }
        });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "shadowOverlapsLocation",
        new LocationHandlerFactory() {
          protected FeatureFilter locationToFilter(Location loc) {
            return new FeatureFilter.ShadowOverlapsLocation(loc);
          }
        });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "shadowContainedByLocation",
        new LocationHandlerFactory() {
          protected FeatureFilter locationToFilter(Location loc) {
            return new FeatureFilter.ShadowContainedByLocation(loc);
          }
        });
    registerHandlerFactory(XMLFilterWriter.XML_FILTER_NS, "byAnnotationType",
    ...
    private class AnnotationTypeHandler extends StAXContentHandlerBase implements FilterHandler {
      ...
      public void endElement(String nsURI, String localName, String qName,
        ...
        if (delegate instanceof XMLAnnotationTypeHandler) {
          filter = new FeatureFilter.ByAnnotationType(
              ((XMLAnnotationTypeHandler) delegate).getAnnotationType());
        }
        --depth;
      }
