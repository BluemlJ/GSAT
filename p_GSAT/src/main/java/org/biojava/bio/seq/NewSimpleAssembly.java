...
/**
 * A Sequence which is assembled from other sequences contained in a set of ComponentFeature
 * objects.
 * <p>
 * The class has been extended to handle overlapping component sequences. It is assumed that the
 * overlaps are consistent between components: this will not be checked.
 * <p>
...
public class NewSimpleAssembly extends AbstractChangeable
    implements
      Sequence,
  ...
  /**
   * Construct a new SimpleAssembly using the DNA alphabet. Initially, the sequence will just
   * contain a list of `N's. Sequence data can be added by adding one or more ComponentFeatures.
   *
  ...
  /**
   * Construct a new NewSimpleAssembly using the DNA alphabet. Initially, the sequence will just
   * contain a list of `N's. Sequence data can be added by adding one or more ComponentFeatures.
   *
   * @param name The name of the sequence (returned by getName())
   * @param uri The identifier of the sequence (returned by getURN());
  ...
  public Feature createFeature(Feature.Template temp) throws BioException, ChangeVetoException {
    if (temp.location.getMin() < 1) throw new BioException("Coordinates out of range");
    ...
    if (temp instanceof ComponentFeature.Template) {
      if (fh != this) {
        throw new BioException(
            "ComponentFeatures can only be attached directly to SimpleAssembly objects");
      }
      ...
      while (gopher instanceof Feature) {
        if (gopher instanceof ComponentFeature) {
          throw new BioException(
              "Cannot [currently] realize features on components of SimpleAssemblies");
        }
  ...
  protected ChangeSupport getChangeSupport(ChangeType ct) {
    ...
    if (annotationForwarder == null && (ct == null || ct == Annotatable.ANNOTATION)) {
