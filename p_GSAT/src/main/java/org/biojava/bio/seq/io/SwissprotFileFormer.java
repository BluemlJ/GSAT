...
public class SwissprotFileFormer extends AbstractGenEmblFileFormer implements SeqFileFormer {
  ...
  /**
   * Creates a new <code>SwissprotFileFormer</code> using <code>System.out</code> stream.
   */
  ...
  /**
   * Creates a new <code>SwissprotFileFormer</code> using the specified stream.
   *
  ...
  /**
   * Start the processing of a sequence. This method exists primarily to enforce the life-cycles of
   * SeqIOListener objects.
   */
  ...
  /**
   * The name is printed out as part of the identifier line. It will be replaced if an ID keyword
   * exists in the annotations.
   *
   * @param theName the String that should be returned by getName for the sequence being parsed
   */
  ...
  /**
   * Null implementation. This object formats and prints a sequence. The URI alone cannot be printed
   * in Swissprot format. Therefore, it's easiest to ignore it.
   * 
  ...
  /**
   * Prints out the sequences properties in order. Prints out the symbol array passed in in lines of
   * 60, blocks of 10
   *
   ...
   *
   * @throws IllegalAlphabetException if we can't cope with this alphabet.
   */
  public void addSymbols(Alphabet theAlphabet, Symbol[] theSymbols, int theStart, int theLength)
  ...
  /**
   * Formats sequence properties into form suitable for printing to file.
   *
  ...
  private String sequenceBufferCreator(Object key, Object value) {
    ...
    if (value == null) {
      ...
      while (iter.hasNext()) {
        ...
        if (iter.hasNext()) temp.append(nl);
      }
    } else {
  ...
  /**
   * Notify the listener of a sequence-wide property. This might be stored as an entry in the
   * sequence's annotation bundle. Checks for possible known properties to be shown in the file.
   *
  ...
  public void addFeatureProperty(Object key, Object value) throws ParseException {
    ...
    if (key.equals(Feature.PROPERTY_DATA_KEY)) return;
    ...
    if (Collection.class.isInstance(value)) {
      for (Iterator vi = ((Collection) value).iterator(); vi.hasNext();) {
        ...
        ub.setLength(0);
        StringBuffer fb =
            formatQualifierBlock(qb, formatQualifier(ub, key, vi.next()).toString(), leader, 80);
        ftb.append(fb + nl);
      }
    } else {
      qb.setLength(0);
      ub.setLength(0);
      StringBuffer fb =
          formatQualifierBlock(qb, formatQualifier(ub, key, value).toString(), leader, 80);
      ftb.append(fb + nl);
    }
  }
  ...
  /**
   * <code>getPrintStream</code> returns the <code>PrintStream</code> to which an instance of
   * SwissprotFileFormer will write the formatted data. The default is System.out
   *
  ...
  /**
   * <code>setPrintStream</code> informs an instance which <code>PrintStream</code> to use.
   *
  ...
  /**
   * <code>formatLocation</code> creates a String representation of a <code>Location</code>. Strand
   * information is ignored, as Swissprot files represent proteins. An alternative form of this
   * function does not take a Strand; that form is available only on SwissprotFileFormer; it is not
   * part of the SeqFileFormer interface.
   *
   * @param theBuffer a <code>StringBuffer</code> to append the location to.
   * @param theLocation a <code>Location</code> to format.
   * @param theStrand a <code>StrandedFeature.Strand</code> indicating nothing of relevance
   *
  ...
  // Public methods
  /**
   * <code>formatLocation</code> creates a String representation of a <code>Location</code>. The
   * stringbuffer returned represents columns 15-27 of the Swissprot feature table entry. An
   * alternative form of this function takes a Strand; that form is part of the SeqFileFormer
   * interface.
   *
   * @param theBuffer a <code>StringBuffer</code> to append the location to.
   * @param theLocation a <code>Location</code> to format.
  ...
  public StringBuffer formatLocation(StringBuffer theBuffer, Location theLocation) {
    ...
    if ((theLocation instanceof PointLocation) || (theLocation instanceof RangeLocation)) {
      ...
      startPoint = this.formatPoint(tempLocation.getOuterMin(), tempLocation.getInnerMin(),
          tempLocation.isMinFuzzy());
      endPoint = this.formatPoint(tempLocation.getInnerMax(), tempLocation.getOuterMax(),
          tempLocation.isMaxFuzzy());
    }
   ...
   * @throws IllegalAlphabetException if we can't cope with this alphabet.
   */
  protected void printOutSequenceHeaderLine(Alphabet theAlphabet, Symbol[] theSymbols, int theStart,
      int theLength) throws IllegalAlphabetException {
    this.getPrintStream().println("SQ   SEQUENCE   " + theLength + " AA;   ");
  }

  /**
   * Converts the symbol list passed in into an array of strings. The strings will be blocks of ten,
   * with six blocks on a line.
   *
   * @param theAlphabet The alphabet of the symbol data
   * @param theSymbols An array containing symbols
   * @param theStart The start offset of valid data within the array
   * @param theLength The number of valid symbols in the array
   * @return The symbol list passed in broken into blocks of ten characters, six to a string.
   *
   * @throws IllegalAlphabetException if we can't cope with this alphabet.
   */
  protected List breakSymbolArray(Alphabet theAlphabet, Symbol[] theSymbols, int theStart,
      int theLength) throws IllegalAlphabetException {
  ...
  /**
   * Simple method that adds spaces onto the buffer passed in. This method exists to refactor some
   * code used in location formatting. It isn't intended to be generally used.
   *
   ...
   * this.formatPoint(FuzzyLocation.getInnerMax(), FuzzyLocation.getOuterMax(),
   * FuzzyLocation.isFuzzyMax())
   *
