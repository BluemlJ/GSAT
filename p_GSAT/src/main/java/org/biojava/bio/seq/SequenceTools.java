...
public final class SequenceTools {
   ...
   *
   * <p>
   * The sub-sequence will be indexed from 1 through to (end-start+1). An index of i in the
   * sub-sequence corresponds to (i+start-1) in the original. All features from the original
   * sequence will be projected down into this co-ordinate system. All features overlapping the
   * edges will be given fuzzy locations.
   * </p>
   *
   ...
   * @throws IndexOutOfBoundsException if start or end are not in seq, or if end < start
   */
   ...
   *
   * <p>
   * The sub-sequence will be indexed from 1 through to (end-start+1). If the strand is NEGATIVE,
   * all features will be flipped in the same manner as the reverseComplement method. If it is
   * UNKNOWN or POSITIVE, then this is identical to the other subSequence methods.
   * </p>
   *
   * @param seq the sequence to sub-sequence
   * @param start the first index to include in the sub-sequence
   * @param end the last index to include in the sub-sequence
   * @param name a new name to give to this sub-sequence
   * @param strand a StrandedFeature.Strand indicating which strand the sub-sequence should be on
   * @return a view Sequence for this region
   * @throws IndexOutOfBoundsException if start or end are not in seq, or if end < start
   */
  public static Sequence subSequence(Sequence seq, int start, int end, String name,
      StrandedFeature.Strand strand) throws IndexOutOfBoundsException, IllegalAlphabetException {
   ...
   * @throws IllegalAlphabetException if the symbols in the sequence can not be complemented
   */
  ...
  /**
   * Create a new sequence that has all of the data in the original, but allows new features and
   * top-level annotations to be added independantly. Use this as a scratch-space.
   *
  ...
  /**
   * Creates a new Sequence with the data of the old but with a different FeatureRealizer that will
   * be applied to new Features.
   *
   ...
   *
   * <p>
   * The gapped sequence can be used to insert gaps. The features on the underlying sequence will be
   * projected onto the view taking the gaps into account.
   * </p>
   *
   * @param seq
   ...
   *
   * <P>
   * This will return a view of a sequence where everything outside loc is dropped. This includes
   * all symbols, which become gaps, and all features, which behave in a similar manner to those
   * produced by subSequence().
   * </p>
   *
  ...
  public static Sequence maskSequence(Sequence seq, RangeLocation loc)
    ...
    GappedSequence gSeq = gappedView(
        subSequence(seq, loc.getMin(), loc.getMax(), seq.getName() + ":" + loc.toString()));
   ...
   *
   * Instantiate this if an API requres a sequence, but you can't be bothered or are not able to
   * provide full sequence information.
   * 
   ...
   * @since 1.4
   */
  public static Sequence createDummy(Alphabet alpha, int length, Symbol sym, String uri,
      String name) throws IllegalSymbolException {
    return createSequence(new DummySymbolList(alpha, length, sym), uri, name,
        new SmallAnnotation());
  }
