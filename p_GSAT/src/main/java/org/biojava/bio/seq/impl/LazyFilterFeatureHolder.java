...
/**
 * FeatureHolder which lazily applies a specified filter to another FeatureHolder. This means that
 * when you use a second filter to query the LazyFilterFeatureHolder, the underlying holder receives
 * a single <code>filter</code> request with the two queries ANDed together.
 *
 * @author Thomas Down
 * @since 1.3
 */
...
public class LazyFilterFeatureHolder implements FeatureHolder {
  ...
  /**
   * Only forward events concerning features which are either accepted by our filter, or are parents
   * of a feature which is.
   *
   * In future this maybe ought to look further down the event chain to reject forwarded events from
   * uninterested features.
   */
  ...
  private ChangeEvent getForwardedEvent(ChangeEvent cev) {
    ...
    if (change == null && previous == null) {
      ...
      forward = isInterestingFeature((Feature) previous) || isInterestingFeature((Feature) change);
    }
    if (forward) {
  ...
  public FeatureFilter getSchema() {
    return new FeatureFilter.And(featureFilter, new FeatureFilter.Or(featureHolder.getSchema(),
