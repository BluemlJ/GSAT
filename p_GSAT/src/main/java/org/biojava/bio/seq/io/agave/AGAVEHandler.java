...
public class AGAVEHandler extends StAXFeatureHandler implements AGAVECallbackItf {
  ...
  /**
   * @param sequence from sub-tag &lt;bio_sequence&gt;/&lt;contig&gt;/&lt;chromosome&gt; <pre>
  ...
  /**
   * get all the top level sequences bio_sequence --> SimpleSequence contig --> SimpleAssembly -->
   * SimpleSequence( if only one sequence ) chromosome -> SimpleAssembly -> SimpleSequence( if only
   * one sequence)
   */
