...
/**
 * SequenceDB implementation which lazily applies a SequenceAnnotator to sequences retrieved from a
 * SequenceDB.
 *
 * @author Thomas Down
  ...
  protected Sequence doAnnotation(Sequence seq) throws BioException {
    try {
      ...
      throw new BioException(
          "Couldn't apply annotator " + annotator.toString() + " to " + seq.getURN(), ex);
    } catch (ChangeVetoException cve) {
      throw new BioException(
          "Couldn't apply annotator " + annotator.toString() + " to " + seq.getURN(), cve);
    }
  }
}
