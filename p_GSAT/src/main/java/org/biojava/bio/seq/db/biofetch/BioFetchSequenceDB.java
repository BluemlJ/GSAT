...
public class BioFetchSequenceDB extends Unchangeable implements SequenceDBLite {
  ...
  public void addSequence(Sequence seq) throws ChangeVetoException {
    throw new ChangeVetoException(
        "Failed to add sequence." + " Sequences may not be added" + " to a biofetch database");
  }

  public void removeSequence(String id) throws ChangeVetoException {
    throw new ChangeVetoException(
        "Failed to add sequence." + " Sequences may not be removed" + " from a biofetch database");
  }

  public Sequence getSequence(String id) throws BioException, IllegalIDException {
    String format = "";

    if (dbName.equals("embl")) format = "embl";
    if (dbName.equals("genbank"))
      format = "genbank";
    else if (dbName.equals("swiss"))
      format = "swissprot";
    else if (dbName.equals("refseq"))
      throw new BioException("Sequence database " + dbName + " is not supported");
    ...
    try {
      ...
      else if (dbName.equals("swiss")) si = SeqIOTools.readSwissprot(data);
