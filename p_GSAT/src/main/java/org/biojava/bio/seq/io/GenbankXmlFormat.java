...
public class GenbankXmlFormat extends GenbankFormat {
  ...
  public boolean readSequence(BufferedReader reader, SymbolTokenization symParser,
    ...
    GenbankXmlSequence sequence = (GenbankXmlSequence) sequences.get(m_sequenceIndex++);
  ...
  private void generateSequenceForListener(GenbankXmlSequence sequence, SeqIOListener listener,
      SymbolTokenization symParser) throws ParseException, IllegalSymbolException {
    ...
    if (strandedness != null)
      listener.addSequenceProperty(STRAND_NUMBER_TAG, convertToStrandednessName(strandedness));

    String moltype = sequence.getMolType();
    if (moltype != null) listener.addSequenceProperty(TYPE_TAG, convertToMolTypeName(moltype));
    String topology = sequence.getTopology();
    if (topology != null)
      listener.addSequenceProperty(CIRCULAR_TAG, convertToTopologyName(topology));
    ...
    if (primaryAccession != null) listener.addSequenceProperty(ACCESSION_TAG, primaryAccession);

    String accessionVersion = sequence.getAccessionVersion();
    if (accessionVersion != null) listener.addSequenceProperty(VERSION_TAG, accessionVersion);
    ...
    if (comment != null) listener.addSequenceProperty(COMMENT_TAG, comment);
  ...
  private void handleOtherSequenceIds(GenbankXmlSequence sequence, SeqIOListener listener)
    ...
    while (iter.hasNext()) {
      ...
      if (seqId.startsWith("gi|")) listener.addSequenceProperty(GI_TAG, seqId.substring(3));
    }
  }
  ...
  private void handleReferences(GenbankXmlSequence sequence, SeqIOListener listener)
      throws ParseException {
    ...
    while (iter.hasNext()) {
      ...
      while (authorIter.hasNext()) {
        ...
        if (authorIter.hasNext()) theAuthors.append(", ");
      }
      listener.addSequenceProperty(AUTHORS_TAG, theAuthors.toString());

      String title = reference.getTitle();
      if (title != null) listener.addSequenceProperty(TITLE_TAG, title);
  ...
  private void handleFeatures(GenbankXmlSequence sequence, SeqIOListener listener)
      throws ParseException {
    ...
    while (iter.hasNext()) {
      ...
      String featureString =
          feature.getKey() + createBlankString(16 - keyLength) + feature.getLocation();
      ...
      while (qualifiersIter.hasNext()) {
        ...
        String qualifierString =
            "                /" + qualifier.getName() + "=" + "\"" + qualifier.getValue() + "\"";
  ...
  private static class GenbankXmlHandler extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes) {
      ...
      else if (qName.equals("GBInterval")) m_currentSequence.getCurrentFeature().addNewInterval();
    }
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
      if (qName.equals("GBSet"))
        ...
        m_currentSequence.getCurrentFeature().getCurrentInterval()
            .setFrom(m_currentString.toString());
      else if (qName.equals("GBInterval_to"))
        m_currentSequence.getCurrentFeature().getCurrentInterval()
            .setTo(m_currentString.toString());
      else if (qName.equals("GBInterval_point"))
        m_currentSequence.getCurrentFeature().getCurrentInterval()
            .setPoint(m_currentString.toString());
      else if (qName.equals("GBInterval_accession"))
        m_currentSequence.getCurrentFeature().getCurrentInterval()
            .setAccession(m_currentString.toString());
      else if (qName.equals("GBFeature_quals"))
        return; // do nothing
      else if (qName.equals("GBQualifier"))
        return; // do nothing
      else if (qName.equals("GBQualifier_name"))
        m_currentSequence.getCurrentFeature().getCurrentQualifier()
            .setName(m_currentString.toString());
      else if (qName.equals("GBQualifier_value"))
        m_currentSequence.getCurrentFeature().getCurrentQualifier()
            .setValue(m_currentString.toString());
