 ...
 * <p>
 * This object implements SequenceIterator, so you can loop over each sequence produced. It consumes
 * a stream, and uses a SequenceFormat to extract each sequence from the stream.
 * <p>
 * It is assumed that the stream contains sequences that can be handled by the one format, and that
 ...
 * being guaranteed that StreamReader will not require you to keep any of the sequences in memory.
 * 
 * <p>
 * More functionality is offered by {@link org.biojavax.bio.seq.io.RichStreamReader
...
public class StreamReader implements SequenceIterator, org.biojava.utils.ParseErrorListener {
   ...
   * <p>
   * This method will delegate parsing from the stream to a SequenceFormat object, and then return
  ...
  public StreamReader(InputStream is, SequenceFormat format, SymbolTokenization symParser,
  ...
  public StreamReader(BufferedReader reader, SequenceFormat format, SymbolTokenization symParser,
      SequenceBuilderFactory sf) {
    this.reader = reader;
    this.format = format;
    this.symParser = symParser;
