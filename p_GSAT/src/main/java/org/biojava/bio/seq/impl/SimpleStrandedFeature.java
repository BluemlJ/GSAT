...
public class SimpleStrandedFeature extends SimpleFeature implements StrandedFeature {
  ...
  public SymbolList getSymbols() {
    ...
    if (getStrand() == NEGATIVE) {
      try {
        ...
        throw new BioError(
            "Could not retrieve symbols for feature as " + "the alphabet can not be complemented.",
            iae);
      }
    }
    return symList;
  ...
  public void setStrand(Strand strand) throws ChangeVetoException {
    if (hasListeners()) {
      ...
      synchronized (cs) {
        ChangeEvent ce = new ChangeEvent(this, STRAND, strand, this.strand);
  ...
  public SimpleStrandedFeature(Sequence sourceSeq, FeatureHolder parent,
      StrandedFeature.Template template) throws IllegalArgumentException
