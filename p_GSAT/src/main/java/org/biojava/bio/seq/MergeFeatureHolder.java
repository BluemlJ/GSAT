...
/**
 * FeatureHolder which exposes all the features in a set of sub-FeatureHolders. This is provided
 * primarily as a support class for ViewSequence. It may also be useful for other applications, such
 * as simple distributed annotation systems.
 *
 * @author Thomas Down
...
public class MergeFeatureHolder extends AbstractFeatureHolder implements Serializable {
  ...
  /**
   * Add an extra FeatureHolder to the set of FeatureHolders which are merged. This method is
   * provided for backward compatibility, and is equivalent to:
   *
   * <pre>
   ...
   *
   * <p>
   * You should always use the two-arg version in preference if you are able to define the
   * membership of a FeatureHolder.
   * </p>
   */
  ...
  /**
   * Iterate over all the features in all child FeatureHolders. The Iterator may throw
   * ConcurrantModificationException if there is a change in the underlying collections during
   * iteration.
   */
  ...
  private class MFHIterator implements Iterator {
    ...
    public boolean hasNext() {
      if (fIterator.hasNext()) return true;
      if (fhIterator.hasNext()) {
        fIterator = ((FeatureHolder) fhIterator.next()).features();
        return hasNext();
      }
    ...
    public Object next() {
      if (fIterator.hasNext()) return fIterator.next();
      if (fhIterator.hasNext()) {
        fIterator = ((FeatureHolder) fhIterator.next()).features();
        return next();
      }
