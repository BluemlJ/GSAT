...
public class GAMESpanPropHandler extends StAXPropertyHandler {
  ...
  /*
   * public void startElementHandler( String nsURI, String localName, String qName, Attributes
  ...
  public void endElementHandler(String nsURI, String localName, String qName,
    ...
    if (start < stop) {
      ...
      if (isStrandedTemplate) ((StrandedFeature.Template) templ).strand = StrandedFeature.POSITIVE;
    } else if (start > stop) {
      staxenv.featureTemplate.location = new RangeLocation(stop + 1, start);
      if (isStrandedTemplate) ((StrandedFeature.Template) templ).strand = StrandedFeature.NEGATIVE;
    } else {
      staxenv.featureTemplate.location = new PointLocation(start);
      if (isStrandedTemplate) ((StrandedFeature.Template) templ).strand = StrandedFeature.UNKNOWN;
    }
  }
}
