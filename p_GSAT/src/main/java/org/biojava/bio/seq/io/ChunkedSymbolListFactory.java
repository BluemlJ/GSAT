 ...
 * <p>
 * You can build a SequenceBuilderFactory to create a packed chunked sequence from an input file
 * without making an intermediate symbol list with:- <pre>
 ...
 * Blend to suit taste.
 * <p>
 * Alternatively, you can input Symbols to the factory with addSymbols make the sequence eventually
 * with makeSymbolList.
 * <p>
 * <b>NOTE:</b> An improvement has been introduced where an internal default SymbolList factory is
 * used for small sequences. This implementation allows for faster SymbolList creation and access
 * for small sequences while allowing a more space-efficient implementation to be selected for large
 * sequences.
 * <p>
 * <b>NOTE:</b> This class is inherantly not threadsafe. You should create one instance for each
 * symbol list you wish to manufacture, and then you should throw that instance away.
 *
 * @author David Huen
 */
public class ChunkedSymbolListFactory {
  ...
  /**
   * @param symListFactory class which produces the SymbolLists that are used to store the chunked
   *        symbols.
   */
  ...
  /**
   * @param userSymListFactory User-supplied class which produces the SymbolLists that are used to
   *        store the chunked symbols (only used when the chunked list to be created is larger than
   *        threshold.
   * @param threshold the size of the SymbolList beyond which the userSymListFactory is used. Below
   *        that, the internal default SymbolList factory is used.
   */
  ...
  /**
   * tool to construct the SymbolList by adding Symbols. Note that this class is not thread-safe.
   * Also, it can only assemble one SymbolList at a time. And the composite formed by adding Symbols
   * must not have interstitial missing Symbols.
   */
  public void addSymbols(Alphabet alfa, Symbol[] syms, int pos, int len)
    ...
    if ((opMode == AUTO_SELECT) && (currSymListFactory != userSymListFactory)
        && (symCount > threshold))
