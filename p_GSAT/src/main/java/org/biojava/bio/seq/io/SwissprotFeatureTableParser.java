...
/*
 * Thomas Down refactored to remove dependancy on FeatureTableParser, since we weren't actually
 * reusing very much code, and FeatureTableParser changed quite a bit when it went fully-newio.
 */
...
class SwissprotFeatureTableParser {
    ...
    if (descBuf.length() > 0) {
      ...
      try {
        featureTemplate.annotation.setProperty(
            SwissprotProcessor.PROPERTY_SWISSPROT_FEATUREATTRIBUTE, descBuf.substring(0));
  ...
  private Location getLocation(StringTokenizer theTokens) throws BioException {
    ...
    if (startIndex.isFuzzy() || endIndex.isFuzzy()) {
      ...
      if (startIndex.equals(endIndex)) {
        theLocation = new FuzzyPointLocation(startIndex.getMinValue(), startIndex.getMaxValue(),
            FuzzyPointLocation.RESOLVE_AVERAGE);
      } else {
        theLocation = new FuzzyLocation(startIndex.getMinValue(), endIndex.getMaxValue(),
            startIndex.getMaxValue(), endIndex.getMinValue(), startIndex.isFuzzy(),
   ...
   *
   * @exception BioException Thrown if a non-number token is passed in (fuzzy locations are handled)
  ...
  /**
   * This inner class is a struct to pass back the information contained in a Swissprot point
   */
  private class Index {
    ...
    public boolean equals(Index theIndex) {
      ...
      if ((this.mMinValue == theIndex.getMinValue()) && (this.mMaxValue == theIndex.getMaxValue())
