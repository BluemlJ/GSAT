 ...
 *
 * <p>
 * Use the constructor to create a new index store. Use the static factory method open() to load an
 * existing store.
 * </p>
 *
 * The tab-delimited file looks like: <pre>
...
public class TabIndexStore implements IndexStore, Serializable {
   ...
   *
   * <p>
   * The store file and index file must not exist. This is to prevent you from accidentally
   * destroying an existing index.
   * </p>
   *
   ...
   * @param name the name that will be used by the database backed by this index
   ...
   */
  public TabIndexStore(File storeFile, File indexFile, String name, SequenceFormat format,
      SequenceBuilderFactory sbFactory, SymbolTokenization symbolParser)
  ...
  public void commit() throws BioException {
    try {
      PrintWriter out = new PrintWriter(new FileWriter(indexFile.toString(), true));
      for (Iterator i = uncommited.values().iterator(); i.hasNext();) {
        Index indx = (Index) i.next();

        out.println(getFileIndex(indx.getFile()) + "\t" + indx.getStart() + "\t" + indx.getID());
      }
  ...
  protected void initialize() throws IOException {
    if (indexFile.exists()) {
      ...
      for (String line = reader.readLine(); line != null; line = reader.readLine()) {
        ...
        SimpleIndex index = new SimpleIndex(seqFileIndex[fileNum], start, -1, id);
