...
/**
 * Encapsulate a stream of Symbols being parsed from some input stream. This allows SymbolList
 * creation to be fully decoupled from stream parsing.
 *
 * @author Thomas Down
...
public interface SymbolReader {
   ...
   * <strong>NOTE:</strong> SymbolList implementations are expected to perform any necessary
   * validation of returned Symbols. Client code should not need to perform any extra validation.
   */
   ...
   *
   * @throws IOException if an error occured on the stream, or the end of the stream has already
   *         been reached.
  ...
  public int readSymbols(Symbol[] buffer, int start, int length)
