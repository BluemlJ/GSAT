...
/**
 * <code>EmblLikeLocationParser</code> parses EMBL/Genbank style locations. Supported location
 * forms:
 *
 * <pre>
 *   123
...
public class EmblLikeLocationParser {
  ...
  /**
   * <code>parseLocation</code> creates a <code>Location</code> from the String and returns a
   * stranded location.
   *
   * @param location a location <code>String</code>.
   * @param theTemplate the template to be filled with the parsed out location information.
   *
  ...
  public Feature.Template parseLocation(String location, Feature.Template theTemplate)
    ...
    if (joinType != null) {
      try {
        ...
        theTemplate.annotation.setProperty(Feature.PROPERTY_DATA_KEY, dat);
  ...
  /**
   * <code>processCoords</code> uses the coordinate data in the start/endCoords Lists to create a
   * Location and add to the subLocations List. As this code will require further modification to
   * support fuzzy point locations, please keep any changes well-commented.
   *
   * @exception BioException if an error occurs.
   */
  private void processCoords() throws BioException {
    ...
    if (startCoords.isEmpty() && endCoords.isEmpty()) return;
    ...
    // Range of form: 123
      ...
      if (unboundMin || unboundMax) {
        createdLocation = new FuzzyPointLocation(unboundMin ? Integer.MIN_VALUE : innerMin,
            unboundMax ? Integer.MAX_VALUE : innerMax, FuzzyPointLocation.RESOLVE_AVERAGE);
      } else if (isPointLoc) {
        createdLocation = new PointLocation(innerMin);
      } else {
        throw new BioException(
            "Internal error in location parsing; parser became confused: " + location);
      }
    }
    // Range of form: (123.567)
      ...
      createdLocation =
          new FuzzyPointLocation(innerMin, innerMax, FuzzyPointLocation.RESOLVE_AVERAGE);
    }
    // Range of form: 123..567 or <123..567 or 123..>567 or <123..>567
      ...
      if (unboundMin || unboundMax) {
        createdLocation = new FuzzyLocation(unboundMin ? Integer.MIN_VALUE : outerMin,
            unboundMax ? Integer.MAX_VALUE : outerMax, innerMin, innerMax,
    ...
    // Range of form: (123.567)..789
      ...
      createdLocation =
          new FuzzyLocation(outerMin, outerMax, innerMin, innerMax, FuzzyLocation.RESOLVE_INNER);
    }
    // Range of form: 123..(567.789)
  ...
  /**
   * <code>processInstructs</code> pops an instruction off the stack and applies it to the
   * sub(locations).
   *
  ...
  /**
   * <code>LocationLexer</code> is based on the <code>LocationLexer</code> class in the Artemis
   * source code by Kim Rutherford.
   *
  ...
  private class LocationLexer {
    ...
    /**
     * <code>getNextToken</code> returns the next token. A null indicates no more tokens.
     *
     * @return an <code>Object</code> value.
     */
    Object getNextToken() {
      while (true) {
        if (nextCharIndex == location.length()) return null;
    ...
    /**
     * <code>followInteger</code> returns single sequence coordinate.
     *
    ...
    private Integer followInteger() {
      ...
      while (Character.isDigit(thisChar)) {
        ...
        if (nextCharIndex >= location.length()) break;
    ...
    private String followText() {
      ...
      if (!Character.isLetter(thisChar)) return "";
