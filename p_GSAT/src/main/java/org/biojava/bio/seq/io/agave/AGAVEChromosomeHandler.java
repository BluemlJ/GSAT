 ...
 */
public class AGAVEChromosomeHandler extends StAXFeatureHandler
    implements
  ...
  public void endElementHandler(String nsURI, String localName, String qName,
    ...
    try {
      for (ListIterator i = sequenceSet.listIterator(); i.hasNext();) {
        ...
        if (chromNum != null) seq.getAnnotation().setProperty("chromosome_number", chromNum);
        if (chromLen != null) seq.getAnnotation().setProperty("chromosome_length", chromLen);
