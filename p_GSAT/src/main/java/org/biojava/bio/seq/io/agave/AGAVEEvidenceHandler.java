...
{
  public static final StAXHandlerFactory AGAVE_EVIDENCE_HANDLER_FACTORY = new StAXHandlerFactory() {
  ...
  public void addElementId(String id) {
    if (element_ids == null) element_ids = new ArrayList(1);
    element_ids.add(id);
  }

  /*
   * protected Feature.Template createTemplate() { Feature.Template st = new Feature.Template();
   * st.type = "evidence"; st.annotation = annot; if( staxenv != null ) staxenv. subFeatures .add(
   * this ) ;
   * 
   * return st; }
   */
  public void endElementHandler(String nsURI, String localName, String qName,
    ...
    try {
      if (element_ids != null) annot.setProperty("element_ids", element_ids);
    } catch (Exception e) {
