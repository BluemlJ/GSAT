...
/**
 * Simple implementation of SymbolTokenization which uses the `name' field of the symbols. This
 * class works with any non-cross-product FiniteAlphabet, and doesn't need any extra data to be
 * provided.
 *
 * @author Thomas Down
 * @since 1.2
  ...
  public Symbol parseToken(String token) throws IllegalSymbolException {
    ...
    if (sym == null) {
      ...
      if (c == '[') {
        ...
        throw new IllegalSymbolException("Token `" + token
            + "' does not appear as a named symbol in alphabet `" + getAlphabet().getName() + "'");
      }
    }
    return sym;
  }
