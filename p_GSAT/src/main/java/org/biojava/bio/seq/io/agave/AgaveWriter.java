...
/**
 * Writes Sequence into AGAVE XML document. The AGAVE format is defined in agave.dtd which can be
 * downloaded from http://www.agavexml.org.
 *
 * @author Hanning Ni
 * @author Brian King
 */
public class AgaveWriter {
   ...
   * 
   * @param seq maybe the or simple sequence <pre>
  ...
  protected void writeContig(Annotatable seq) throws IOException {
     ...
     * , sequence? , sequence_map* , map_location* )> <!ATTLIST contig length NMTOKEN #REQUIRED >
     */
    mIndent.indent();
    mOut.print(mIndent);
    mOut.print("<contig");
  ...
  private void writeClassification2(Annotatable f) throws IOException {
    /**
     * <!ELEMENT classification (description? , id_alias* , evidence? )> <!ATTLIST classification
     * system CDATA #REQUIRED id CDATA #REQUIRED type CDATA #REQUIRED assigned_by CDATA #IMPLIED >
     */
    mOut.print(mIndent);
  ...
  protected void writeSequenceMap2(Annotatable f) throws IOException {
    /*
     * <!ELEMENT sequence_map (note? , computation? , annotations? )> <!ATTLIST sequence_map label
     * CDATA #IMPLIED >
     */
    mIndent.indent();
    mOut.print(mIndent);
   ...
   *
   */
  protected void writeAnnotations(FeatureHolder f) throws IOException // changed to FeatureHolder
                                                                      // THOMASD
  {
    // write annotation
    ...
    /*
     * for (Iterator i = f.features(); i.hasNext(); ) { Feature feature = (Feature) i.next(); if(
     * feature.getSource().equalsIgnoreCase("annotations") ) { f = feature ; break; } }
     */
  ...
  protected void writeGene(Annotatable f) throws IOException {
    /**
     * <!ELEMENT gene (classification* , note? , seq_location , xrefs? , evidence? , qualifier* ,
     * seq_feature* , related_annot* , transcript*)> <!ATTLIST gene element_id ID #IMPLIED label
     * CDATA #IMPLIED >
     **/
    mIndent.indent();
    mOut.print(mIndent);
  ...
  protected void writeTranscript(Annotatable f) throws IOException {
    ...
    for (Iterator i = ((FeatureHolder) f).features(); i.hasNext();) {
      Feature subf = (Feature) i.next();
      if (subf.getSource().equalsIgnoreCase("transcript")) writeTranscript2(subf);
    }
  }
  ...
  private void writeTranscript2(Annotatable f) throws IOException {
    ...
    for (Iterator i = ((FeatureHolder) f).features(); i.hasNext();) {
      ...
      if (type.equalsIgnoreCase("cds"))
        ...
        writeMrna(subf);
      else if (type.equalsIgnoreCase("predicted_protein")) writePredictedProtein(subf);
  ...
  protected void writeSeqFeature(Annotatable f) throws IOException {
    /*
     * <!ELEMENT seq_feature (classification* , note? , seq_location , xrefs? , evidence? ,
     * qualifier* , seq_feature* , related_annot*)> <!ATTLIST seq_feature element_id ID #IMPLIED
     * feature_type CDATA #REQUIRED label CDATA #IMPLIED >
     */
    mIndent.indent();
    mOut.print(mIndent);
  ...
  private void writeEvidence(Annotatable f) throws IOException {
    for (Iterator i = ((FeatureHolder) f).features(); i.hasNext();) {
      Feature subf = (Feature) i.next();
      if (subf.getSource().equalsIgnoreCase("evidence")) writeEvidence2(subf);
    }
  }
  ...
  /*
   * <!ELEMENT comp_result (note? , match_desc? , match_align? , query_region? , match_region? ,
   * result_property* , result_group* , related_annot*)> <!ATTLIST comp_result element_id ID
   * #IMPLIED result_id NMTOKEN #IMPLIED group_order NMTOKEN #IMPLIED result_type CDATA #REQUIRED
   * feature_type CDATA #IMPLIED on_complement_strand (true | false ) 'false' confidence NMTOKEN
   * #IMPLIED align_length NMTOKEN #IMPLIED align_units (bp | AA) #IMPLIED >
   */
  ...
  /**
   * <!ELEMENT result_group (comp_result+ )> <!ATTLIST result_group group_order NMTOKEN '0' >
   */
  ...
  private void writeSeqLocation(Annotatable f) throws IOException {
    ...
    if (f instanceof StrandedFeature) {
      ...
      else if (type == -1) on_complement_strand = true;
    }
    mIndent.indent();
    mOut.print(mIndent);
