...
/**
 * <code>RecordParser</code> is a general purpose parser for arrays of bytes read from any the the
...
class RecordParser {
  ...
  /**
   * <code>parseInt4</code> creates a long from Little-endian. Named after the EMBOSS Ajax function
  ...
  long parseInt4(byte[] int4) {
    ...
    for (int i = 4; --i >= 0;) {
      if (int4[i] != 0) result += ((int4[i] & 0xff) << (8 * i));
    }
  ...
  /**
   * <code>parseInt2</code> creates an int from Little-endian. Named after the EMBOSS Ajax function
   * which wrote the data.
   *
  ...
  int parseInt2(byte[] int2) {
    ...
    for (int i = 2; --i >= 0;) {
      if (int2[i] != 0) result += ((int2[i] & 0xff) << (8 * i));
    }
  ...
  /**
   * <code>parseDate</code> parses a <code>String</code> from an array of bytes. The date is stored
   * in 4 bytes: 0, unused; 1, year; 2, month; 3, day. With a 1 byte year it's not very much use and
   * I'm not sure that the EMBOSS programs set the value correctly anyway.
   *
  ...
  /**
   * <code>parseString</code> parses a <code>String</code> from an array of bytes, skipping the
   * empties.
   *
   * @param sb a <code>StringBuffer</code>.
  ...
  /**
   * <code>parseDivRecord</code> parses the raw bytes into an array of Objects.
   *
  ...
  /**
   * <code>parseEntryNamRecord</code> parses the raw bytes into an array of Objects.
   *
   * @param divRecord a <code>byte []</code> array.
   *
   * @return an <code>Object []</code> array.
  ...
  /**
   * <code>parseAcnumTrgRecord</code> parses the raw bytes into an array of Objects.
   *
   * @param divRecord a <code>byte []</code> array.
   *
   * @return an <code>Object []</code> array.
  ...
  /**
   * <code>parseAcnumHitRecord</code> parses the raw bytes into an array of Objects.
   *
   * @param divRecord a <code>byte []</code> array.
   *
   * @return an <code>Object []</code> array.
