...
/**
 * <code>SeqIOEventEmitter</code> is a utility class which scans a <code>Sequence</code> object and
 * sends events describing its constituent data to a <code>SeqIOListener</code>. The listener should
 * be able to reconstruct the <code>Sequence</code> from these events.
 *
 * @author Keith James
 * @since 1.2
...
public class SeqIOEventEmitter {
  ...
  /**
   * <code>getSeqIOEvents</code> scans a <code>Sequence</code> object and sends events describing
   * its data to the <code>SeqIOListener</code>.
   *
  ...
  public void getSeqIOEvents(Sequence seq, SeqIOListener listener) {
    try {
      ...
      // Add symbols
      listener.addSymbols(seq.getAlphabet(), (Symbol[]) seq.toList().toArray(symProto), 0,
          seq.length());
  ...
  /**
   * <code>getSubFeatures</code> is a recursive method which returns a list of all
   * <code>Feature</code>s within a <code>FeatureHolder</code>.
   *
