...
/**
 * <p>
 * A circular view onto another Sequence object. The class allows for reinterpretation of locations
 * and indices onto the sequence to allow for overlapping of the origin. The origin is assumed to be
 * the first symbol. Future versions may support changing the origin.
 * </p>
 * In biojavax {@link org.biojavax.bio.seq.RichSequence RichSequences} intrinsicly know about
 * circularity. No view is required. We strongly recommend using RichSequence if possible.
 *
 *
...
public class CircularView extends ViewSequence {
  ...
  /**
   * <p>
   * Over rides ViewSequence. Allows any integer index, positive or negative to return a symbol via
   * the equation <CODE>val = (val % length);</CODE> Note that base zero is the base immediately
   * before base 1 which is of course the last base of the sequence.
   * </p>
   * 
   ...
   * <p>
   * Over rides ViewSequence. Allows any integer index, positive or negative to return a symbol via
   * the equation <CODE>val = (val % length);</CODE>
   * </p>
   *
   * <p>
  ...
  /**
   * Over rides ViewSequence to allow the use of locations that have coordinates outside of the
   * sequence length (which are needed to describe locations that overlap the origin of a circular
   * sequence).
   *
   * @since 1.2
   * @throws BioException if a non circular location is added that exceeds the 'boundaries' of the
   *         sequence.
   * @throws ChangeVetoException if the sequence is locked.
   * @param template the template of the feature to be created.
   * @return the feature created you can use the template of the returned feature to create another
   *         of the same type.
   */
   ...
   * <p>
   * Over rides ViewSequence. Allows any integer index, positive or negative to return a symbol via
   * the equation <CODE>index = ((index -1) % length)+1</CODE>
   * </p>
   *
   * <p>
   ...
   * @return a <code>SymbolList </code>containing the <code>Symbols</code> from <code>start</code>
