...
/*
 * Greg Cox: Changed private fields and methods to protected so that SwissProtFeatureTableParser
 * could subclass and snag the implementation.
 *
 * Thomas Down: Post 1.1, finally got round to refactoring this to be a `nice' player in the newio
 * world. Needless to say, this simplified things quite a bit.
 *
 * Keith James: Added support for reading fuzzy i.e. (123.567) locations in addition to unbounded
 * i.e. <123..567 locations.
 */
...
public class FeatureTableParser {
    ...
    switch (featureStatus) {
      ...
      case WITHIN:
        if (line.charAt(0) == '/') {
          ...
          int eq = line.indexOf("=");
          if (line.charAt(eq + 1) != '"' || line.charAt(line.length() - 1) == '"') {
      ...
      case ATTRIBUTE:
        ...
        if (((featureBuf.toString().indexOf(" ") >= 0) || (line.toString().indexOf(" ") >= 0))
        ...
        if (featureBuf.charAt(eq + 1) != '"' || featureBuf.charAt(featureBuf.length() - 1) == '"') {
  ...
  /**
   * Process the a string corresponding to a feature-table attribute, and fire it off to our
   * listener.
   */
  private void processAttribute(String attr) throws BioException {
    ...
    if (eqPos == -1) {
      ...
      if (attr.charAt(eqPos) == '"') ++eqPos;
      int max = attr.length();

      if (attr.charAt(max - 1) == '"') --max;
