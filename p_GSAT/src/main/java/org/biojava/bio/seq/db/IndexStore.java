...
/**
 * This defines the objects that IndexedSequenceDB uses to store all of the database state, such as
 * name, format, sequence builder and the actual file offsets.
 *
 * You should only ever have to instantiate an implementation of this. Don't call any of the
 * inerface methods directly. They are intended for IndexedSequenceDB, not you.
 *
 * In general, these objects should be transactional. Calls to store should add the index to
 * temporary storage. When commit is invoked, these indices should all be added to the permanent
 * storage. When rollback is invoked, these indexes should be discarded. If commit fails for any
 * reason, it should leave the permanent storage in the pre-commit status.
 * 
...
public interface IndexStore {
   ...
   * <p>
   * This method should be transactional. If the store fails, the IndexStore should be left in its
   * original state.
   * <p>
   * If the file of the Index is not known yet, it is the responsibility of the IndexStore to add it
   * to the set returned by getFiles.
   *
