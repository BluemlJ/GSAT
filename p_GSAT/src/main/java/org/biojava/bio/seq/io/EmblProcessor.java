...
/**
 * Simple filter which handles attribute lines from an EMBL file. This class delegates creation of
 * <code>Feature</code>s to a <code>FeatureTableParser</code>, which in turn delegates creation of
 * <code>Locations</code> to an <code>EmblLikeLocationParser</code> which is shared with the
 * <code>GenbankProcessor</code>.
 *
 * An <code>EmblLikeLocationParser</code> parses EMBL/Genbank style locations. Supported location
 * forms:
 *
 * <pre>
 *   123
 ...
 * </pre>
 *
 * The only EMBL header information retained over a read/write cycle is the accession number (all
 * numbers).
 *
 * @author Thomas Down
 * @author Greg Cox
    ...
    try {
      if (mBadFeature) {
        ...
        if (value != null) {
          ...
          if ((key.equals(EmblLikeFormat.FEATURE_TABLE_TAG)) && (featureLine.charAt(0) != ' ')) {
        ...
        if (features.inFeature() && !key.equals(EmblLikeFormat.FEATURE_TABLE_TAG)) {
          features.endFeature();
        }
      ...
      ParseErrorEvent offendingLineEvent =
          new ParseErrorEvent(this, "This line could not be parsed: " + value.toString());
      this.notifyParseErrorEvent(offendingLineEvent);
    } catch (IndexOutOfBoundsException ex) {
      // This occurs when for some line min > max
      mBadFeature = true;
      ParseErrorEvent offendingLineEvent =
          new ParseErrorEvent(this, "From must be less than To: " + value.toString());
      this.notifyParseErrorEvent(offendingLineEvent);
    }
  }
