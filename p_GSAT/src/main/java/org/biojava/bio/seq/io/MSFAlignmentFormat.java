...
public class MSFAlignmentFormat implements AlignmentFormat {
    ...
    try {
      ...
      if (DEBUGPRINT) {
        for (currSeqCount = 0; currSeqCount < sequenceNames.size(); currSeqCount++) {
          System.out
              .println((String) sequenceNames.get(currSeqCount) + ":" + sequenceData[currSeqCount]);
        }
      }
      ...
      for (currSeqCount = 0; currSeqCount < sequenceNames.size(); currSeqCount++) {
        ...
        sequenceDataMap.put((String) sequenceNames.get(currSeqCount),
            alph == ProteinTools.getTAlphabet()
                ? ProteinTools.createGappedProteinSequence(sd,
  ...
  public void write(OutputStream os, Alignment align, int fileType)
    ...
    for (int i = 0; i < numSeqs; i++) {
      seqIts[i] = align.symbolListForLabel(labels[i].toString()).iterator();
    ...
    if (align.symbolListForLabel(labels[0].toString()).getAlphabet() == DNATools.getDNA()) {
      fileType = DNA;
    ...
    for (int i = 0; i < numSeqs; i++) {
      ...
      out.print("  Len: " + align.length() + " 	Check: " + 0 + "	Weight: " + 0 + nl); // this
                                                                                         // really
                                                                                         // should
                                                                                         // be seq
      // length?
  ...
  public void writeDna(OutputStream os, Alignment align)
      throws BioException, IllegalSymbolException {
    write(os, align, DNA);
  }
