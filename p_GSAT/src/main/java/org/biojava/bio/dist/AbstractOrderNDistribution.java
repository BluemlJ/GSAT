...
public abstract class AbstractOrderNDistribution extends AbstractDistribution
  ...
  /**
   * The listener that will forward events from the underlying distributions to listeners for this
   * distribution.
   */
  ...
  protected ChangeSupport getChangeSupport(ChangeType ct) {
    ...
    if (((Distribution.WEIGHTS.isMatchingType(ct)) || (ct.isMatchingType(Distribution.WEIGHTS)))
        && weightForwarder == null) {
      weightForwarder = new ChangeForwarder.Retyper(this, changeSupport, Distribution.WEIGHTS);
  ...
  /**
   * Set a weight in one of the conditioned distributions. It is the callers responsibility to
   * ensure that all the conditioned distributions have total weights which sum to 1.0.
   *
  ...
  public void registerWithTrainer(DistributionTrainerContext dtc) {
    ...
    dtc.registerTrainer(this, new IgnoreCountsTrainer() {
      public void addCount(DistributionTrainerContext dtc, AtomicSymbol sym, double count)
