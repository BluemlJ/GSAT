...
public class PairDistribution extends AbstractChangeable implements Serializable, Distribution {
   ...
   * @return a Distribution that is a uniform distribution over the product of first and second
   */
  ...
  public void setWeight(Symbol sym, double weight) throws ChangeVetoException {
    throw new ChangeVetoException("Can't set the weight directly in a PairDistribution. "
        + "You must set the weights in the underlying distributions.");
  }

  /**
   * Create a new PairDistribution that represents the product of two other distributions. The
   * alphabet will be the product of the first and seccond distribution's alphabets, and the weights
   * will be the products of the weights for the first and seccond distributions given the first and
   * second component of the symbol respectively.
   *
  ...
  public PairDistribution(Distribution first, Distribution second) {
    ...
    this.alphabet = AlphabetManager.getCrossProductAlphabet(
        Arrays.asList(new Alphabet[] {first.getAlphabet(), second.getAlphabet()}));
  }
  ...
  private class PairTrainer extends IgnoreCountsTrainer implements Serializable {
    public double getCount(DistributionTrainerContext dtc, AtomicSymbol as)
      ...
      return (dtc.getCount(first, f) + dtc.getCount(second, s)) * 0.5;
    ...
    public void addCount(DistributionTrainerContext dtc, Symbol sym, double times)
      ...
      if (!(sym instanceof AtomicSymbol)) {
        throw new IllegalSymbolException(
            "Can't add counts for ambiguity symbols. Got: " + sym.getName());
      }
  ...
  public Symbol sampleSymbol() {
    try {
      return getAlphabet()
          .getSymbol(Arrays.asList(new Symbol[] {first.sampleSymbol(), second.sampleSymbol()}));
