 ...
 * <p>
 * This requires the distribuiton being trained to have a working setWeight method that doesn't
 * throw an UnsupportedOperationExcepiton.
 * </p>
 *
...
public final class SimpleDistributionTrainer implements DistributionTrainer, Serializable {
  ...
  public void addCount(DistributionTrainerContext dtc, AtomicSymbol sym, double count)
    ...
    if (d == null) {
      throw new IllegalSymbolException(
          "Symbol " + sym.getName() + " not found in " + dis.getAlphabet().getName());
    }
    if (count < 0) {
  ...
  public void train(DistributionTrainerContext dtc, double weight) throws ChangeVetoException {
    ...
    double sum = 0.0;
    try {
      for (Iterator i = ((FiniteAlphabet) dis.getAlphabet()).iterator(); i.hasNext();) {
        Symbol s = (Symbol) i.next();
        Double d = (Double) c.get(s);
        sum += d.doubleValue() + nullModel.getWeight(s) * weight;
        // System.out.println(s.getName() + ": sum=" + sum);
      }
      for (Iterator i = ((FiniteAlphabet) dis.getAlphabet()).iterator(); i.hasNext();) {
        Symbol sym = (Symbol) i.next();
        Double d = (Double) c.get(sym);
        dis.setWeight(sym, (d.doubleValue() + nullModel.getWeight(sym) * weight) / sum);
      }
  ...
  public SimpleDistributionTrainer(Distribution dis) throws IllegalAlphabetException {
    ...
    if (!(a instanceof FiniteAlphabet)) {
      throw new IllegalAlphabetException(
          "Can't create a SimpleDistributionTrainer for non-finite alphabet " + a.getName()
              + " of type " + a.getClass());
    }
    this.dis = dis;
  }
