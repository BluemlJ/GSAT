...
public class OrderNDistributionFactory implements DistributionFactory {
  ...
  /**
   * Construct a new OrderNDistributionFactory with a specified factory for conditioned
   * distributions.
   *
   * @param df The DistributionFactory used for construction new conditioned distributions.
   */
   ...
   *
   * @param alpha the Alphabet should be in a form that clearly indicates the conditioning and the
   *        conditioned alphabet unless it is very obvious. For example (DNA x DNA) is obvious,
   *        ((DNA x DNA x DNA) x DNA) indicates that (DNA x DNA x DNA) is the conditioning
   *        <code>Alphabet</code> and DNA is the conditioned <code>Alphabet</code>. (DNA x DNA x DNA
   *        x DNA) doesn't but for compatibility with biojava 1.2 this is allowed in the
   *        constructor. As from biojava 1.2.3 or greater this will be internally converted to ((DNA
   *        x DNA x DNA) x DNA) which was the convention implied by biojava 1.2 Calls to the
   *        returned <code>Distribution</code>s <code>getAlphabet()</code> method will return the
   *        converted <code>Alphabet</code>.
   *
   * @return An OrderNDistribution
   * @throws IllegalAlphabetException if a Distribution cannot be made with that
   *         <code>Alphabet</code>.
   */
