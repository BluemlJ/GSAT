...
public final class DistributionTools {
   ...
   *
   * @param c the count
   * @return a Distrubution over the same <code>FiniteAlphabet</code> as <code>c</code> and trained
   *         with the counts of <code>c</code>
   */
   ...
   * @since 1.2
   * @param a A <code>Distribution</code> with the same <code>Alphabet</code> as <code>b</code>
   * @param b A <code>Distribution</code> with the same <code>Alphabet</code> as <code>a</code>
   */
   ...
   *
   * @return true if alphabets and symbol weights are equal for each pair of distributions. Will
   *         return false if the arrays are of unequal length.
   * @throws BioException if one of the Distributions is over an infinite alphabet.
   * @since 1.3
   * @param a A <code>Distribution[]</code> consisting of <code>Distributions</code> over a
   *        <code>FiniteAlphabet </code>
   * @param b A <code>Distribution[]</code> consisting of <code>Distributions</code> over a
   *        <code>FiniteAlphabet </code>
   */
  public static final boolean areEmissionSpectraEqual(Distribution[] a, Distribution[] b)
      throws BioException {
   ...
   * @since 1.2
   */
  public static final HashMap KLDistance(Distribution observed, Distribution expected,
      double logBase) {
  ...
  /**
   * Calculates the total Entropy for a Distribution. Entropies for individual <code>Symbols</code>
   ...
   * @return an array of Distribution instances representing columns of the alignment
   * @throws IllegalAlphabetException if the alignment alphabet is not compattible
   */
   ...
   * @param countGaps if true gaps will be included in the distributions (NOT YET IMPLEMENTED!!,
   *        CURRENTLY EITHER OPTION WILL PRODUCE THE SAME RESULT)
   ...
   * @since 1.2
   */
  public static final Distribution jointDistOverAlignment(Alignment a, boolean countGaps,
      double nullWeight, int[] cols) throws IllegalAlphabetException {
    ...
    for (int i = 1; i < seqs.size(); i++) {
      FiniteAlphabet test =
          (FiniteAlphabet) ((SymbolList) a.symbolListForLabel(seqs.get(i))).getAlphabet();
      if (test != alpha) {
        throw new IllegalAlphabetException(
            "Cannot Calculate jointDistOverAlignment() for alignments with" + "mixed alphabets");
      }
    }
    ...
    Distribution dist;
    DistributionTrainerContext dtc = new SimpleDistributionTrainerContext();
    dist = DistributionFactory.DEFAULT
   ...
   * @param nullWeight the number of pseudo counts to add to each distribution, pseudo counts will
   *        not affect gaps, no gaps, no gap counts.
   * @return a <code>Distribution[]</code> where each member of the array is a
   *         <code>Distribution </code>of the <code>Symbols </code>found at that position of the
   *         <code>Alignment </code>.
   * @since 1.2
   */
  public static final Distribution[] distOverAlignment(Alignment a, boolean countGaps,
      double nullWeight) throws IllegalAlphabetException {
    ...
    for (int i = 1; i < seqs.size(); i++) {
      ...
      if (test != alpha) {
        throw new IllegalAlphabetException(
            "Cannot Calculate distOverAlignment() for alignments with" + "mixed alphabets");
      }
    }
    ...
    try {
      for (int i = 0; i < a.length(); i++) {// For each position
        ...
        for (Iterator<String> j = seqs.iterator(); j.hasNext();) {// of each sequence
          ...
          /*
           * If this is working over a flexible alignment there is a possibility that s could be
           * null if this Sequence is not really preset in this region of the Alignment. In this
      ...
      if (countGaps) {// need to adjust counts for gaps
        for (int i = 0; i < adjRatios.length; i++) {
          ...
          for (Iterator iter = ((FiniteAlphabet) d.getAlphabet()).iterator(); iter.hasNext();) {
  ...
  /**
   * Creates an array of distributions, one for each column of the alignment. No pseudo counts are
   * used.
   * 
   * @param countGaps if true gaps will be included in the distributions
   * @param a the <code>Alignment </code>to build the <code>Distribution[]</code> over.
   * @throws IllegalAlphabetException if the alignment is not composed from sequences all with the
   *         same alphabet
   * @return a <code>Distribution[]</code> where each member of the array is a
   *         <code>Distribution </code>of the <code>Symbols </code>found at that position of the
   *         <code>Alignment </code>.
   ...
   * @return a <code>Distribution </code>were the weight of each <code>Symbol </code> is the average
   *         of the weights of that <code>Symbol </code>in each <code>Distribution </code>.
   */
  public static final Distribution average(Distribution[] dists) {
    ...
    try {
      ...
      for (int i = 0; i < dists.length; i++) {// for each distribution
        for (Iterator iter = ((FiniteAlphabet) dists[i].getAlphabet()).iterator(); iter
            .hasNext();) {// for each symbol
          Symbol sym = (Symbol) iter.next();
      ...
      dtc.train();
      return average;
    } catch (IllegalAlphabetException iae) {// The following throw unchecked exceptions as they
                                            // shouldn't happen
   ...
   * @param d the distribution to sample. If this distribution is of order N a seed sequence is
   *        generated allowed to 'burn in' for 1000 iterations and used to produce a sequence over
  ...
  private static final SymbolList generateOrderNSymbolList(OrderNDistribution d, int length) {
    ...
    /*
     * When emitting an orderN sequence a seed sequence is required that is of the length of the
     * conditioning alphabet. The emissions will also be allowed to 'burn in' for 1000 emissions so
     * that the 'end effect' of the seed is negated.
     */
   ...
   */
  protected static final Sequence generateOrderNSequence(String name, OrderNDistribution d,
      int length) {
