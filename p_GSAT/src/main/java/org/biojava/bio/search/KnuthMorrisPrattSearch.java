 ...
 *
 * <p>
 * Reference: KNUTH D.E., MORRIS (Jr) J.H., PRATT V.R., 1977, Fast pattern matching in strings, SIAM
 * Journal on Computing 6(1):323-350. </P.
 *
 * <p>
 * <b>USAGE:</b>
 * </p>
 * When the object is constructed the <code>findMatches()</code> method would be called. This will
 * return an int[] giving the offsets (ie the location of the first symbol of each match in the
 * text). The <code>getKMPNextTable()</code> returns the table of border lengths used in the
 * algorithm. This method is protected as it is unlikely it will be needed except for debugging.
 * </p>
 *
 * <p>
 * The algorithm finds exact matches therefore ambiguity symbols will match only themselves. The
 * class cannot perform regular expressions. The class operates on all alphabets thus if searching
 * for a DNA pattern you should compile both the pattern and its reverse complement.
 * </p>
 *
 * <p>
 * <b>WARNING the behaviour of a pattern containing gaps is undefined. It's not recommended that you
 * try it.</b>
 * </p>
 *
 * <p>
...
public final class KnuthMorrisPrattSearch {
  ...
  /**
   * Constructs a KMP matcher to find exact occurances of <code>pattern</code> in <code>text</code>
   * using the Knuth-Morris-Pratt algorithm.
   * <p>
   *
   * A new class should be constructed for each occurance of <code>pattern</code>.
   * <p>
   * 
  ...
  public KnuthMorrisPrattSearch(SymbolList pattern) {
    ...
    /*
     * need to perform this hack to make the kmpNext capable of dealing with overlapping patterns,
     * unfortunately it means the behaviour of a pattern containing a gap is unspecified.
     */
  ...
  /**
   * This will return an int[] giving the offsets of the matches in <code>text</code> (ie the
   * location of the first symbol of each match in the <code>text</code>).
   *
   * @param text the text or sequence to search for the pattern
   *
   * @return an int[] giving the offsets to the matches or a zero length array if there are no
   *         matches.
   */
