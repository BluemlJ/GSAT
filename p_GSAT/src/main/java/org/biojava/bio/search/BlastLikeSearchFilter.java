...
/**
 * A SearchContentHandler class that implements filtering in chains of SearchContentHandler
 * instances.
 * <p>
 * The SearchContentHandler organise Blast-like searches as a hierarchy of search/hit/subhit. Each
 * search is conducted with a single query sequence. Hits of the query sequence are reported against
 * different target sequences. The hit is further subdivided into one of more subhits which
 * represent the positions within the target sequence that alignments of the query sequence were
 * achieved against the query sequence (e.g. HSPs).
 * <p>
 * This implementation depends on the a well ordered use of the SearchContentHandler interface. In
 * particular, it requires that search/hit/subhit properties are reported immediately following the
 * associated startSearch/startHit/startSubHit call. For example, search properties should not be
 * reported following the corresponding startHit() call.
 * <p>
 * <u>Semantics of this interface</u><br>
 * BlastLikeSearchFilters test different levels of the SearchContentHandler property hierarchy and
 * each filter should be seen as being applied when a full set of events from that level is
 * received. So the ByHitProperty filter is applied when endHit() is called and determines whether
 * all events received between startHit() and endHit() are to be passed on or discarded.
 * <p>
...
public interface BlastLikeSearchFilter extends Serializable {
  ...
  /**
   * returns a TriState indicating the current outcome of evaluating this filter. This is usually
   * the outcome saved when evaluate(FilteringContentHandler fch) was called.
   */
  public TriState accept();

  /**
   * computes the outcome of this filter on the specified node and stores it. <b>This method is only
   * exposed to permit it to be included in an interface. Users should not use it.</b>
   */
  public void evaluate(Node fch);

  /**
   * resets the internal state of this filter including any cached evaluations. <b>This method is
   * only exposed to permit it to be included in an interface. Users should not use it.</b>
   */
  public void reset();

  public abstract static class AbstractBlastLikeSearchFilter implements BlastLikeSearchFilter {
    ...
    public void reset() {
      cachedOutcome = TriState.INDETERMINATE;
    }
  ...
  public static final class And {
    ...
    public TriState accept() {
      ...
      if ((outcome0 == TriState.FALSE) || (outcome1 == TriState.FALSE)) return TriState.FALSE;
  ...
  public static final class Or {
    ...
    public TriState accept() {
      ...
      if ((outcome0 == TriState.TRUE) || (outcome1 == TriState.TRUE)) return TriState.TRUE;
  ...
  public static final class Not extends AbstractBlastLikeSearchFilter {
    ...
    public TriState accept() {
      ...
      if (outcome == TriState.INDETERMINATE) return TriState.INDETERMINATE;
