 ...
 *
 * <p>
 * This adapter is in the same spirit as the event handler adapters in java.awt.event, and is
 * intended as a simple base-class for implementations that only want to handle a small number of
 * the possible call-backs. All method implementations are empty except for getMoreSearches() and
 * setMoreSearches(). These two maintain a boolean state between calls. If you over-ride one, you
