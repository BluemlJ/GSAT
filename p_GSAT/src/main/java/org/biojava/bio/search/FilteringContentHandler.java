...
public class FilteringContentHandler extends SearchContentAdapter
    implements
  ...
  public Object getSearchProperty(Object key) {
    return searchProperty.get(key);
  }

  public Object getHitProperty(Object key) {
    return hitProperty.get(key);
  }

  public Object getSubHitProperty(Object key) {
    return subHitProperty.get(key);
  }

  private static final WalkerFactory walkerFactory =
      WalkerFactory.getInstance(BlastLikeSearchFilter.class);
    ...
    if (!skipSubHits) {
      ...
      if (filter.accept() == TriState.TRUE) {
        if (!emittedStartSearch) {
          ...
          for (Iterator searchPropertyI = searchProperty.entrySet().iterator(); searchPropertyI
              .hasNext();) {
        ...
        if (!emittedStartHit) {
          ...
          for (Iterator hitPropertyI = hitProperty.entrySet().iterator(); hitPropertyI.hasNext();) {
        ...
        for (Iterator subHitPropertyI = subHitProperty.entrySet().iterator(); subHitPropertyI
            .hasNext();) {
