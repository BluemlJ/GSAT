...
/**
 * A BioMatcher class returned by MaxMismatchPattern.matcher() that implements searching of a
 * SymbolList.
 * <p>
...
public class MaxMismatchMatcher implements BioMatcher {
  ...
  private final boolean[] ambiguousPosition; // indicates if the corresponding position in
                                             // patternSymbol represents
                                             // an ambiguity
  private boolean hasAmbiguity = false;

  // working numbers
  private final int[] matches; // this is a modulo'd rotating register that keeps track of the match
                               // count
  ...
  private boolean addSymbol() {
    ...
    if (hasAmbiguity) {
      for (int i = 0; i < maxPatternSymbolIdx; i++) {
        ...
        if (ambiguousPosition[i]) {
          if (patternSymbol[i].getMatches().contains(sym)) matches[idx]++;
        } else {
          if (sym == patternSymbol[i]) matches[idx]++;
        }
      }
    } else {
      for (int i = 0; i < maxPatternSymbolIdx; i++) {
    ...
    matches[(pos + maxPatternSymbolIdx) % patLength] =
        (sym == patternSymbol[maxPatternSymbolIdx]) ? 1 : 0;
  ...
  public int mismatchCount() {
    return patLength - matches[(pos - 1) % patLength];
  }
}
