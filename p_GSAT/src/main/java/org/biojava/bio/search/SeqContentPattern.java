 ...
 *
 * <p>
 * Regular expressions can be used to find sequence patterns. However, some things can't be easily
 * expressed as a regular expression. For example, a region of length 10 that contains at least 8 Gs
 * and up to two Ts and no other symbols. A SeqContentPattern can be built that does represent this.
 * <p>
 *
 * <code><pre>
 ...
 *
 * <p>
 * The minimum counts default to 0, and the maximum counts default to the length. If you have not
 * manually set the maximum count for a symbol, it will continue to adjust while you change the
 * length. Once you have set it, it will not vary, even if you do set the length. To re-set a
 * maximum count to track the length, set it to -1.
 * </p>
 *
 * <p>
 * All regions of the defined length for which all constraints are satisfied will potentialy be
 * found. At the moment we have not defined what will happen for multiple regions that overlap all
 * of which satisfy the constraints.
 * </p>
 *
...
public class SeqContentPattern implements BioPattern {
  ...
  public BioMatcher matcher(SymbolList symList) throws IllegalAlphabetException {
    ...
    return new SeqContentMatcher(symList, index, minCounts, maxCounts, length);
  }
}
