 ...
 *
 * <p>
 * This matcher is responsible for searching through a symbol list for regions that match a pattern.
 * It is implemented in a non-thread-safe way. Either have multiple independant matchers, or
 * synchronize externaly.
 * </p>
 *
 * <p>
 * Instances of this class are obtained from SeqContentPattern.matcher(SymbolList) and it can not be
...
class SeqContentMatcher implements BioMatcher {
  ...
  SeqContentMatcher(SymbolList symList, AlphabetIndex index, int[] minCounts, int[] maxCounts,
      int length) {
  ...
  public boolean find() {
    try {
      ...
      if (pos == 0) {
        ...
        for (int i = 1; i <= length; i++) {
          ...
          if (s instanceof AtomicSymbol) {
            ...
            for (Iterator si = ((FiniteAlphabet) s.getMatches()).iterator(); si.hasNext();) {
      ...
      pos++;
      for (int ourMax = symList.length() - length + 1; pos <= ourMax; pos++) {
        Symbol s;
