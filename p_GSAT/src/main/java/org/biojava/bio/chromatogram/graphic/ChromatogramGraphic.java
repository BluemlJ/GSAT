...
/**
 * Encapsulates a configurable method for drawing a {@link Chromatogram} into a graphics context.
 *
...
public class ChromatogramGraphic implements Cloneable {
  ...
  private static SymbolList SINGLE_CALL =
      new SimpleSymbolList(new Symbol[] {DNATools.getDNA().getGapSymbol()}, 1, DNATools.getDNA());
  ...
  /**
   * Creates a new <code>ChromatogramGraphic</code>, initially displaying the given chromatogram.
   *
  ...
  /**
   * Precomputes the {@link java.awt.geom.GeneralPath}s used to draw the traces.
   */
  protected synchronized void generateSubpaths() {
    if (chromat != null && !subpathsValid) {
      if (subpaths == null) subpaths = new GeneralPath[4][];
      ...
      int countSubpaths =
          (int) Math.ceil(horizScaler.scale(chromat, samples[A].length - 1) / subpathLength);
  ...
  /**
   * Precomputes the {@link java.awt.geom.Rectangle2D}s that are the in-memory representation of the
   * callboxes. These rectangles are used for drawing (via generateDrawableCallboxes) as well as
   * queries (e.g., {@link #getCallContaining}).
   */
  protected synchronized void generateCallboxes() {
    if (chromat != null && !callboxesValid) {
      if (chromat.getSequenceLength() < 2) {
        ...
        int leftIdx = 0;
        int rightIdx =
            bcOffsets[0] + (int) Math.floor(((double) (bcOffsets[1] - bcOffsets[0])) / 2) + 1;
        ...
        // System.out.println("[cg.gcb] cb[0] left="+leftIdx+":"+left+" center="+bcOffsets[0]+"
        // right="+rightIdx+":"+right+" box="+callboxes[0]);
        int i = 1;
        while (i < bcOffsets.length - 1) {
          leftIdx = rightIdx;
          rightIdx =
              bcOffsets[i] + (int) Math.floor(((double) (bcOffsets[i + 1] - bcOffsets[i])) / 2) + 1;
          left = right;
          right = horizScaler.scale(chromat, rightIdx);
          callboxes[i] = new Rectangle2D.Float(left, 0, right - left, max);
          // System.out.println("[cg.gcb] cb["+i+"] left="+leftIdx+":"+left+"
          // center="+bcOffsets[i]+" right="+rightIdx+":"+right+" box="+callboxes[i]);
          i++;
        }
        left = right;
        right = horizScaler.scale(chromat, chromat.getTraceLength() - 1);
        callboxes[i] = new Rectangle2D.Float(left, 0, right - left, max);
        // System.out.println("[cg.gcb] cb["+i+"] left="+left+" center="+bcOffsets[i]+"
        // right="+right+" box="+callboxes[i]);
      }
      callboxesValid = true;
   ...
   * 
   * @param shapeTx the transform to apply to the callboxes to move them into screen space.
   */
  protected synchronized void generateDrawableCallboxes(AffineTransform shapeTx) {
    if (!callboxesValid) generateCallboxes();
    if (!shapeTx.equals(drawnCallboxesTx) || !drawableCallboxesValid) {
      // System.out.println("Regen drawableCallboxes with " + shapeTx + " condition:
      // tx="+shapeTx.equals(drawnCallboxesTx)+" valid="+drawableCallboxesValid);
  ...
  public synchronized void setChromatogram(Chromatogram c) {
    ...
    // System.out.println("chromatgfx[w=" + width + "; h=" + height + "; hs=" + horizScale + "; vs="
    // + vertScale + "]");
  }
  ...
  /**
   * Returns the in-use horizontal scale factor. The "units" of this value are (trace samples) /
   * pixel. For example, a horizontal scale of 1.0 means that there will be one pixel horizontally
  ...
  /**
   * Returns the in use vertical scale factor. The "units" of this value are (trace value bins) /
   * pixel. For example, a vertical scale of 1.0 means that there will be one pixel vertically for
   * each value in the range [0, <code>getChromatogram().getMax()</code>].
   *
  ...
  /**
   * Returns the width of the graphic as it will be rendered. This means that the
   * {@link Option#FROM_TRACE_SAMPLE} and {@link Option#TO_TRACE_SAMPLE} bounds are taken into
   * account.
   *
   ...
   * The {@link Option#FROM_TRACE_SAMPLE} and {@link Option#TO_TRACE_SAMPLE} bounds are taken into
   * account.
   *
  ...
  public int getRenderedWidth(float horizontalScale) {
    return (int) Math.ceil(horizontalScale
        * (getFloatOption(Option.TO_TRACE_SAMPLE) - getFloatOption(Option.FROM_TRACE_SAMPLE) + 1));
  }
   ...
   *
   * @param vs the desired vertical scale. See {@link #getVerticalScale} for semantics.
   * @see Option#HEIGHT_IS_AUTHORITATIVE
   */
   ...
   *
   * @param hs the desired vertical scale. See {@link #getHorizontalScale} for semantics.
   * @see Option#WIDTH_IS_AUTHORITATIVE
   */
  ...
  public Color getBaseFillColor(Symbol b) {
    return (Color) fillColors.get(b);
  }

  /**
   * Maps a color to a DNA symbol. The color as specified will be used for to draw the trace for the
   * symbol (if any). The fill color for calls to the symbol will be derived from the trace color.
   * 
   * @param b the symbol
   * @param c the color
   */
  ...
  /**
   * Returns the number of callboxes, regenerating them if necessary. This should almost always
   * equal <code>getChromatogram().getSequenceLength()</code>
   * 
   ...
   * 
   * @param index the callbox for which to get the bounds <b>0-based</b>.
   * @param boundsOnScreen determines the coordinate system of the returned bounds
   * @return the bounds of the callbox in screen coordinates if <code>boundsOnScreen</code> is true,
   *         otherwise the bounds of the callbox in chromatogram coordinates
   */
  public Rectangle2D getCallboxBounds(int index, boolean boundsOnScreen) {
    if (chromat != null && index >= 0 && index < getCallboxCount()) {
      if (!callboxesValid) generateCallboxes();
      if (boundsOnScreen) {
        if (!drawableCallboxesValid) generateDrawableCallboxes(getTransform());
  ...
  /**
   * Returns the <b>0-based</b> index of the call containing a given point. The point may be either
   * in screen space or chromatogram space, scale-wise. If the point is in screen space, the caller
   * must translate the point such that if it is, for instance, from a mouse click, a click on the
   * upper-left corner of the graphic would be (0,0).
   * 
   * @param point the point to search for
   * @param pointOnScreen if true, the point will be treated as though it is in screen space.
   *        Otherwise, it will be considered to be in chromatogram space.
  ...
  public int getCallContaining(Point2D point, boolean pointOnScreen) {
    if (chromat != null) {
      ...
      if (pointOnScreen) getInvTransform().transform(point, trans);
      int i = 0;
  ...
  /**
   * Synonym for {@link #getCallContaining(Point2D, boolean)} with <code>pointOnScreen</code>=true.
   *
  ...
  /**
   * Same as {@link #getCallContaining(Point2D, boolean)}, except that only the x-coordinate of the
   * point is specified.
   *
   * @param x the x-coordinate to search for
   * @param xOnScreen whether the coordinate in screen space or chromatogram space
  ...
  /**
   * Synonym for {@link #getCallContaining(float, boolean)} with <code>pointOnScreen</code>=true.
   *
   * @param x the x-coordinate to search for
   * @return the index of the call containing the position x
   */
  ...
  private int getSubpathContaining(float x) {
    ...
    Rectangle2D bounds;
    if (x < subpaths[A][0].getBounds2D().getX()) return 0;
    for (int i = 0; i < subpaths[A].length; i++) {
      bounds = subpaths[A][i].getBounds2D();
      if (bounds.getX() <= x && x <= bounds.getMaxX()) return i;
    }
    return subpaths[A].length - 1;
  }

  /**
   * Returns a new AffineTransform describing the transformation from chromatogram coordinates to
   * output coordinates.
   */
  ...
  /**
   * Returns a new AffineTransform describing the transformation from output space to chromatogram
   * space. Should be much more efficient than <code>getTransform().createInverse()</code>
   */
  ...
  public void drawTo(Graphics2D g2) {
    ...
    if (usePerShpTx) {
      generateDrawableCallboxes(
          AffineTransform.getScaleInstance(shapeTx.getScaleX(), shapeTx.getScaleY()));
    ...
    // System.out.println("Drawing calls with stroke=" +
    // basicStrokeToString((BasicStroke)g2.getStroke()));
    // System.out.println("Drawing " + (rightCbIdx - leftCbIdx + 1) + " visible calls from " +
    // leftCbIdx + " to " + rightCbIdx);
    SymbolList calls = ChromatogramTools.getDNASequence(chromat);
    // if the chromatogram has no calls, fake it
    if (calls.length() < 1) calls = SINGLE_CALL;
    ...
    for (int i = leftCbIdx; i <= rightCbIdx; i++) {
      ...
      if (drawSep) {
        ...
        if (usePerShpTx)
          sep.setLine(drawableCallboxes[i].getX(), drawableCallboxes[i].getY(),
              drawableCallboxes[i].getX(), drawableCallboxes[i].getMaxY());
        else
          sep.setLine(callboxes[i].x, callboxes[i].y, callboxes[i].x,
              callboxes[i].y + callboxes[i].height);
        // System.out.println("sep["+i+"]=(" + sep.x1 + ", " + sep.y1 + ") -> (" + sep.x2 + ", " +
        // sep.y2 + ")");
        // System.out.println(" " + sep.getBounds2D());
        g2.draw(sep);
      }
    }
    if (usePerShpTx) g2.translate(-1 * shapeTx.getTranslateX(), -1 * shapeTx.getTranslateY());


    if (optionIsTrue(Option.USE_CUSTOM_STROKE))
      g2.setStroke((Stroke) getOption(Option.TRACE_STROKE));
    if (!subpathsValid) generateSubpaths();
    ...
    // System.out.println("Drawing traces with stroke=" +
    // basicStrokeToString((BasicStroke)g2.getStroke()));
    int loSubpath, hiSubpath;
    // loSubpath = (int)Math.floor((clip.getX() / shapeTx.getScaleX() + fromTraceSample) /
    // subpathLength);
    loSubpath = getSubpathContaining((float) (clip.getX() / shapeTx.getScaleX() + fromTraceSample));
    // hiSubpath = (int)Math.floor(Math.min(clip.getMaxX() / shapeTx.getScaleX() + fromTraceSample,
    // toTraceSample) / subpathLength);
    hiSubpath = getSubpathContaining(
        (float) Math.min(clip.getMaxX() / shapeTx.getScaleX() + fromTraceSample, toTraceSample));
    // System.out.println("Drawing subpaths ["+loSubpath+","+hiSubpath+"] of "+subpaths[0].length+"
    // each "+subpathLength+" long");
  ...
  /**
   * Sets a new value for the specified option. Be sure that the value is appropriate per the
   * documentation, or you'll induce a ClassCastException somewhere else.
   *
  ...
  /**
   * Helper method for converting a {@link java.lang.Boolean}-valued option into a
  ...
  /**
   * Helper method for converting a {@link java.lang.Number}-valued option into a <code>float</code>
   * primitive.
   * 
   * @param opt the {@link Option} to convert
  ...
  /**
   * Helper method for converting a {@link java.lang.Number}-valued option into an <code>int</code>
   * primitive.
   * 
   * @param opt the {@link Option} to convert
   * @throws ClassCastException when the option isn't <code>Number</code>-valued
   ...
   * {@link ChromatogramGraphic} instance. The semantics and expected values are described with the
   * enumerated options.
   *
   * @author Rhett Sutphin (<a href="http://genome.uiowa.edu/">UI CBCB</a>)
   * @since 1.3
  ...
  public static class Option {
    ...
    /**
     * Looks up an <code>Option</code> instance based on its string description.
     * 
     * @param desc the description of the desired <code>Option</code>
     * @return the <code>Option</code> with the specified description or null if there isn't one
     */
    ...
    /**
     * Option indicating whether width or horizontal scale is the authoritative measure. If the
     * value is true, then when the Chromatogram displayed by the graphic is changed, the horizontal
     * scale may be changed but the width will stay the same. If the value is false, the width may
     * change but the horizontal scale will stay the same.
     * <p>
     * Value type: {@link java.lang.Boolean}.<br/>
    ...
    /**
     * Option indicating whether height or vertical scale is the authoritative measure. If the value
     * is true, then when the Chromatogram displayed by the graphic is changed, the vertical scale
     * may be changed but the height will stay the same. If the value is false, the height may
     * change but the vertical scale will stay the same.
     * <p>
     * Value type: {@link java.lang.Boolean}.<br/>
     * Default value: <code>Boolean.TRUE</code>.
     * </p>
     ...
     * </p>
     */
    public static final Option TRACE_STROKE = new Option("trace-stroke",
    ...
    /**
     * Option indicating whether to apply scaling and translation transforms to each shape
     * individually or to apply a single transform to the graphics context. For putative performance
     * reasons, the latter is the default. However, setting this property to true may result in more
     * attractive output, particularly when the horizontal and vertical scales are very different.
     * This value must also be set to true if using a custom stroke while drawing into a Swing
     * graphics context on JDK 1.3.1 on Mac OS X, due to a nasty rendering bug on that platform.
     * <p>
     * Value type: {@link java.lang.Boolean}.<br/>
     * Default value: <code>Boolean.FALSE</code>.
     * </p>
     ...
     * Specifically, the traces are stored as a set of {@link java.awt.geom.GeneralPath}s. This
     * option indicates how long (in trace samples) each one of these should be. Ideally, this value
     * would be slightly more than the average number of trace samples visible at once in the
     * application using the graphic. However, constantly changing this value is counterproductive
     * as it forces the recalculation of the subpaths. In general, having a value that is too small
     * should be preferred to one that is too large.
     * <p>
    ...
    /**
     * Option indicating the lowest (leftmost) trace sample that should be drawn. The sample at this
     * (0-based) index will be drawn at x=0 in the output chromatogram. Note that this option is
     * reset to zero every time {@link ChromatogramGraphic#setChromatogram} is called.
     * <p>
     * Value type: {@link java.lang.Integer}.<br/>
    ...
    /**
     * Option indicating the highest (rightmost) trace sample that should be drawn. The sample at
     * this (0-based) index will be the last drawn in the output chromatogram. Note that this option
     * is reset to the length of the new chromatogram every time
     ...
     * {@link ChromatogramNonlinearScaler} object.
     * <p>
  ...
  /*
   * private final static String basicStrokeToString(BasicStroke bs) { StringBuffer sb = new
   * StringBuffer(bs.toString()); sb.append("[width=").append(bs.getLineWidth());
   * sb.append("; EndCap="); switch (bs.getEndCap()) { case BasicStroke.CAP_BUTT:
   * sb.append("CAP_BUTT"); break; case BasicStroke.CAP_ROUND: sb.append("CAP_ROUND"); break; case
   * BasicStroke.CAP_SQUARE: sb.append("CAP_SQUARE"); break; } sb.append("; Join="); switch
   * (bs.getLineJoin()) { case BasicStroke.JOIN_BEVEL: sb.append("JOIN_BEVEL"); break; case
   * BasicStroke.JOIN_MITER: sb.append("JOIN_MITER"); break; case BasicStroke.JOIN_ROUND:
   * sb.append("JOIN_ROUND"); break; }
   * sb.append("; MiterLimit=").append(bs.getMiterLimit()).append(']'); return sb.toString(); }
   */
}
