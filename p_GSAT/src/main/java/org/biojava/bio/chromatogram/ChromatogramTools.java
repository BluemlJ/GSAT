...
public class ChromatogramTools {
  ...
  /**
   * Get the peak offsets for the called bases of a chromatogram. A synonym for
   * <code>chromat.getBaseCalls().symbolListForLabel(Chromatogram.OFFSETS)</code>.
   *
   * @param chromat the Chromatogram to process
  ...
  /**
   * Converts the peak offsets list of the given chromatogram into a new <code>int</code> array.
   * <p>
   * The array is, of course, allocated and initialized at each call, so using this method like
   * this: <pre>
   * for (int i = m ; i < n ; i++)
   *    doSomething(getTraceOffsetArray(c)[i]);
   * </pre> is not recommended.
   *
   * @param chromat the Chromatogram to process
   ...
   * @param which which symbol in the trace offset sequence to get. 1-based index.
   * @return the offset for that peak
   */
  public static final int getTraceOffset(Chromatogram chromat, int which) {
    return getIntFromSymbolList(chromat.getBaseCalls().symbolListForLabel(Chromatogram.OFFSETS),
        which);
  }

  /**
   * Retrieves, unwraps, and returns an <code>int</code> from a SymbolList containing
   * {@link org.biojava.bio.symbol.IntegerAlphabet.IntegerSymbol}s.
   * 
