...
/**
 * A factory that creates {@link Chromatogram} objects from files or streams. In either case, the
 * type of object to create is determined from the first four bytes of the stream (the magic
 * number).
 *
...
public class ChromatogramFactory {
  ...
  public static final int SCF_MAGIC =
      (((byte) '.') << 24) + (((byte) 's') << 16) + (((byte) 'c') << 8) + (((byte) 'f'));
  /**
   * The magic number for ABIF files.
   */
  public static final int ABI_MAGIC =
      (((byte) 'A') << 24) + (((byte) 'B') << 16) + (((byte) 'I') << 8) + (((byte) 'F'));
   ...
   * @throws UnsupportedChromatogramFormatException when the file doesn't contain a chromatogram in
   *         a supported format
   */
    ...
    switch (magic) {
      ...
      default:
        throw new UnsupportedChromatogramFormatException(
            "File " + f + " with magic " + magic + " has an unsupported format");
    }
  }

  /**
   * Creates a new <code>Chromatogram</code> object from the supplied stream. Note that for some
   * chromatogram formats, this can be much more memory-intensive than reading from a file.
   * <p>
   * Note also that if the provided stream is a {@link org.biojava.utils.io.CachingInputStream}, it
   * will be seeked back to 0 before being passed to the parser. This is because the parsers that
   * use <code>CachingInputStream</code> assume that the "file" starts at 0.
   * </p>
   *
  ...
  public static Chromatogram create(InputStream in)
    ...
    switch (magic) {
      ...
      default:
        throw new UnsupportedChromatogramFormatException(
            "The provided input stream with magic " + magic + " has an unsupported format");
    }
