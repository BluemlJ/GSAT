...
/**
 * A basic chromatogram implementation which provides public mutators for setting the various
 * attributes of the chromatogram.
 * <p>
 * In general, <b>new chromatogram implementations</b> should be derived from
 * {@link AbstractChromatogram}, <b>not this class</b>, as it is generally undesirable to allow the
 * internal structures of a {@link Chromatogram} to be manipulated externally. This class could
 * still be useful, however, for programatically generated "chromatograms".
 * </p>
 *
...
public class SimpleChromatogram extends AbstractChromatogram {
  ...
  /**
   * Set the DNA and OFFSETS symbol lists for the basecall alignment. Beware: this method does no
   * consistency checks to be sure that all the offsets are valid indices into the trace arrays.
   *
   * @param dna a symbol list in the DNA alphabet that contains the base calls for this chromatogram
   * @param offsets a symbol list in an integer or sub-integer alphabet that contains the locations
   *        in the chromatogram for the bases called in <code>dna</code>
  ...
  /**
   * Sets the trace array for one of the DNA nucleotides. The provided array <i>will not be
   * copied</i>, so any modifications to it will be reflected in calls to {@link #getTrace}.
   * <p>
   * If you need to set a new set of traces whose length is different from the old set, you must
   * call {@link #clearTraceValues} first, or you will provoke an
   * <code>IllegalArgumentException</code>.
   * </p>
   *
   * @param nuc the nucleotide for which to set the trace
   * @param trace the sampled intensities along the trace
   * @param maxVal the maximum value on the trace, or {@link java.lang.Integer#MIN_VALUE} to force
   *        this method to calculate it
