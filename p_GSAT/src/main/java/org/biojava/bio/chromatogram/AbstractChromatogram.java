...
/**
 * A basic, abstract implementation of {@link Chromatogram}. Provides protected setters so that
 * subclasses may set the value of the various properties of a chromatogram.
 *
 * Chromatograms should be created using {@link ChromatogramFactory} or a parser for a particular
 * file format.
 * 
...
public abstract class AbstractChromatogram implements Chromatogram {
  ...
  public int getMax() {
    try {
      return Math.max(Math.max(getMax(DNATools.a()), getMax(DNATools.c())),
  ...
  private final int nucToIndex(AtomicSymbol nuc) throws IllegalSymbolException {
    if (nuc == DNATools.a())
      return A;
    else if (nuc == DNATools.c())
      return C;
    else if (nuc == DNATools.g())
      return G;
    else if (nuc == DNATools.t())
      return T;
    else
      throw new IllegalSymbolException("The symbol " + nuc.getName() + " ("
          + nuc.getClass().getName() + ") is not in the DNA alphabet");
  }

  /*
   * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * Protected mutators for subclasses to
   * initialize fields * * * * * * * * * * * * * * * * * * * * * * * * * * * *
   */
   ...
   * @throws NoSuchElementException when align doesn't contain alignments with the required DNA and
   *         OFFSETS labels
   * @throws IllegalArgumentException the lists in align aren't all the same length.
   * @throws IllegalAlphabetException if the required lists don't have the correct alphabets. See
   *         the documentation of {@link Chromatogram#getBaseCalls() } for details.
   * @see Chromatogram#getBaseCalls
   */
  protected final void setBaseCallAlignment(Alignment align)
      throws IllegalAlphabetException, IllegalArgumentException, NoSuchElementException {
  ...
  /**
   * Sets the trace data structures to null. If a subclass needs to replace the traces with new
   * traces of a different length, this method must be called first to avoid provoking an
   * IllegalArgumentException from setTrace due to a length mismatch.
   */
  protected final void clearTraces() {
    maxTraceValue[A] = maxTraceValue[C] = maxTraceValue[G] = maxTraceValue[T] = Integer.MIN_VALUE;
    traceSample[A] = traceSample[C] = traceSample[G] = traceSample[T] = null;
  }
   ...
   * @param maxVal the maximum value in the trace array. If this value is {@link Integer#MIN_VALUE},
   *        this method will do a linear search of trace to determine the max.
   * @throws IllegalArgumentException when trace.length is different from any of the existing
   *         (non-null) traces
    ...
    if (trace == null) {
      ...
      for (int i = 0; i < 4; i++) {
        if (traceSample[i] != null && traceSample[i].length != trace.length) {
          throw new IllegalArgumentException("All traces must be the same length.  " + trace.length
   ...
   * 
   * @param bits a non-negative integer indicating the number of significant bits in each trace
   *        sample
  ...
  protected final void setBits(int bits) throws IllegalArgumentException {
    if (bits < 0) throw new IllegalArgumentException("Invalid number of significant bits");
  ...
  public Chromatogram reverseComplement() {
    ...
    try {
      ...
      throw new BioError(
          "Can't happen unless reverseComplementBaseCalls or reverseComplementBaseCallList have been overridden out-of-spec",
          iae);
    } catch (NoSuchElementException nsee) {
      throw new BioError(
          "Can't happen unless reverseComplementBaseCalls or reverseComplementBaseCallList have been overridden out-of-spec",
   ...
   * This is achieved by calling {@link #reverseComplementBaseCallList} for each label in the
   * current base call alignment. When that method returns null, no list will appear in reverse
   * complement base call alignment with the null-provoking label. For this reason, subclasses are
   * encouraged to override {@link #reverseComplementBaseCallList} to handle any additional per-base
   * metadata that they store.
   * <p>
   * This implementation should be safely inheritable for all chromatogram implementations, unless
   * one just doesn't want base calls on its reverse complement output. If this is the case, it
   * should override this method to return null.
   * </p>
   *
   * @return a new {@link Alignment} that is the reverse complement of the one in the current
   *         chromatogram
   */
  protected Alignment reverseComplementBaseCalls() {
    ...
    for (Iterator it = current.getLabels().iterator(); it.hasNext();) {
      ...
      if (revList != null) revBCMap.put(curLabel, revList);
    }
    return createImmutableAlignment(revBCMap);
  }

  /**
   * Return a symbol list containing the reverse complement of the base call data for the given
   * label. The returned list will be stored in the reverse complement's base call alignment under
   * the same label.
   * <p>
   * Implementation note: subclasses which do not use an {@link IntegerAlphabet} for their offsets
   * lists must override this method, at least for the case where
   * <code>label == {@link #OFFSETS}</code>.
   * </p>
   *
   * @param label the label Object
   * @return an appropriately reverse-complemented SymbolList, or null if the label is unhandled.
   */
  protected SymbolList reverseComplementBaseCallList(Object label) {
    if (label == DNA) {
      try {
        return SymbolListViews
            .reverse(DNATools.complement(this.getBaseCalls().symbolListForLabel(DNA)));
      } catch (IllegalAlphabetException iae) {
      ...
      try {
        ...
        throw new BioError(
            "Can't happen -- revOffsets was just created with only symbols from alpha");
      }
    } else {
      return null;
    }
  ...
  /**
   * A factory method for creating new symbol lists with a given alphabet. The default
   * implementation should be fine for nearly all cases, but the option is given in case a developer
   * wishes to use a more memory efficient implementation.
   *
   ...
   * @return a new immutable SymbolList containing all the given symbols using the given Alphabet
   */
  ...
  /**
   * A factory method for creating new immutable alignments, particularly for use as base call
   * alignments. The default implementation should be fine for nearly all cases.
   *
   * @param labelsToSymLists a {@link Map} whose keys are desired labels for the alignment and whose
   *        values are the SymbolLists. All the SymbolLists must be the same length.
   * @return a new Alignment
   * @throws IllegalArgumentException if the lists aren't all the same length
   * @throws ClassCastException if any of the values in the map aren't SymbolLists
   */
   ...
   * @return an array of the same length containing all values in src in reverse order
   */
