...
/**
 * A {@link ChromatogramNonlinearScaler} that scales all the base calls in a chromatogram to the
 * same width in pixels, optionally biasing the peak of the call to the center.
 *
...
public class FixedBaseWidthScaler implements ChromatogramNonlinearScaler {
  ...
  /**
   * Creates a new scaler that will scale bases to the specified width without attempting to center
   * their peaks.
   * 
  ...
  /**
   * Creates a new scaler that will scale bases to the specified width and may or may not bias the
   * peaks to the center.
   * 
   * @param width the desired call width in pixels
   * @param centerPeaks if true, the scaler will try to put the peak of in the center of the scaled
   *        call. Otherwise, the whole call will be scaled using the same factor.
   */
  ...
  public float scale(Chromatogram c, int traceSampleIndex) throws IndexOutOfBoundsException {
    if (traceSampleIndex < 0 || traceSampleIndex >= c.getTraceLength())
      throw new IndexOutOfBoundsException(
          "Requested a scale of a trace sample outside of the chromatogram");
    synchronized (this) {
      calcAllScales(c);
      if (traceSampleIndex >= scales.length) throw new BioError(
          "This shouldn't happen: a valid trace sample is not included in the calculated scales array.  This is probably due to a failure in dirty checking.");
      return scales[traceSampleIndex];
    }
  }

  /**
   * Calculates all the scaled x-coordinates for the given chromatogram, but only if it isn't the
   * one that we already have the scales for.
   *
   * @param c the Chromatogram to calculate the scale for
   */
  private synchronized void calcAllScales(Chromatogram c) {
    if (c == null) return;
    if (scales != null && lastC != null && lastC == c && scales.length == c.getTraceLength()
  ...
  /**
   * Calculate the scaled x coordinates for a base call. The base call consists of the samples in
   * the range <code>[left, right]</code> and the peak is at <code>center</code> (which must be in
   * that same range). The parameter <code>widthSoFar</code> provides the x coordinate for the
   * sample at <code>left</code> and this method must return a similar value for the base call
   * immediately following this one (i.e., the one whose leftmost sample would be
   * <code>right+1</code>).
   * 
   ...
   * @param lastBase true if this is the last base, false otherwise. The last base must be handled
   *        specially so that the last sample is at baseWidth * c.sequenceLength.
   */
  private void scaleBase(int left, int center, int right, float nextX, boolean lastBase) {
    if (left > center) throw new BioError(
        "Assertion failure: left > center ; l=" + left + "; c=" + center + "; r=" + right);
    if (center > right) throw new BioError(
        "Assertion failure: center > right ; l=" + left + "; c=" + center + "; r=" + right);
    if (left < 0) throw new BioError(
        "Assertion failure: left < 0 ; l=" + left + "; c=" + center + "; r=" + right);
    if (center < 0) throw new BioError(
        "Assertion failure: center < 0 ; l=" + left + "; c=" + center + "; r=" + right);
    if (right < 0) throw new BioError(
        "Assertion failure: right < 0 ; l=" + left + "; c=" + center + "; r=" + right);
