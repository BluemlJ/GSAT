...
/**
 * <p>
 * Builder for a data store that is backed by a java.nio.MappedByteBuffer. This has a limitation
 * that the total size of the mapped buffer and therefore the hash table can not exceed 2 gigs.
 * </p>
 *
 * <p>
 * The data store file has the following structure. <pre>
...
public class CompactedDataStoreFactory implements DataStoreFactory {
  ...
  public DataStore buildDataStore(File storeFile, SequenceDB seqDB, Packing packing, int wordLength,
      int threshold) throws IllegalAlphabetException, IOException, BioException {
    return this.buildDataStore(storeFile, new SequenceStreamer.SequenceDBStreamer(seqDB), packing,
        wordLength, 1, threshold);
  }

  public DataStore buildDataStore(File storeFile, SequenceStreamer streamer, Packing packing,
      int wordLength, int stepSize, int threshold)
      throws IllegalAlphabetException, IOException, BioException {
    ...
    final int structDataSize = 6 * Constants.BYTES_IN_INT + packingStream.toByteArray().length;
    ...
    int words = 1 << ((int) packing.wordSize() * (int) wordLength);
    // System.out.println("Words:\t" + words);

    hashTablePos = structDataSize;
    int hashTableSize = (int) Constants.BYTES_IN_INT + // hash table length
        words * (int) Constants.BYTES_IN_INT; // hash table entries

    // System.out.println("Allocated:\t" + hashTableSize);
    if (hashTableSize < words) {
      throw new AssertionFailure(
          "Possible underflow. number of words: " + words + "\tsize of hash table: " + hashTableSize
              + "\tcompared to Integer.MAX_VALUE " + Integer.MAX_VALUE);
    }

    final MappedByteBuffer hashTable_MB =
        channel.map(FileChannel.MapMode.READ_WRITE, hashTablePos, hashTableSize);
    ...
    final MappedByteBuffer nameArray_MB =
        channel.map(FileChannel.MapMode.READ_WRITE, nameArrayPos, nameArraySize);
    ...
    int nameTableSize = Constants.BYTES_IN_INT + fpl.seqCount * Constants.BYTES_IN_INT
        + fpl.nameChars * Constants.BYTES_IN_CHAR;
    // System.out.println("nameTableSize:\t" + nameTableSize);
    final MappedByteBuffer nameTable =
        channel.map(FileChannel.MapMode.READ_WRITE, nameTablePos, nameTableSize);
    ...
    long hitTableSize = (long) Constants.BYTES_IN_INT + // size
    ...
    final MappedByteBuffer hitTable =
        channel.map(FileChannel.MapMode.READ_WRITE, hitTablePos, (int) hitTableSize);
    ...
    SecondPassListener spl = new SecondPassListener(packing, wordLength, stepSize, hashTable,
        nameArray, nameTable, hitTable);
    streamer.reset();
    while (streamer.hasNext()) {
    ...
    final MappedByteBuffer rootBuffer =
        channel.map(FileChannel.MapMode.READ_WRITE, 0, structDataSize);
  ...
  private abstract class PackingListener extends SeqIOAdapter {
    ...
    public abstract void processWord(int word, int pos) throws ParseException;

    public void addSymbols(Alphabet alpha, Symbol[] syms, int start, int length)
        throws IllegalAlphabetException {
      if (alpha != packing.getAlphabet()) {
        throw new IllegalAlphabetException(
            "Alphabet " + alpha.getName() + " doesn't match packing");
      }
  ...
  private class SecondPassListener extends PackingListener {
    ...
    SecondPassListener(Packing packing, int wordLength, int stepSize, IntBuffer hashTable,
        IntBuffer nameArray, MappedByteBuffer nameTable, MappedByteBuffer hitTable) {
      super(packing, wordLength, stepSize);

      if ((hashTable == null) || (nameArray == null) || (nameTable == null) || (hitTable == null)) {
        throw new NullPointerException(
            "Buffers must not be null. " + "\thashTable: " + hashTable + "\tnameArray: " + nameArray
                + "\tnameTable: " + nameTable + "\thitTable: " + hitTable);
      }
  ...
  private void writeRecord(IntBuffer hashTable, MappedByteBuffer hitTable, int offset,
      int seqNumber, int word) {
