...
/**
 * A simple no-frills implementation of the HomologeneBuilder interface. Used to instantiate a
 * in-memory copy of the Homologene data.
 *
 * @author David Huen
 */
public class SimpleHomologeneBuilder implements HomologeneBuilder {
  ...
  public void endOrthologue() {
    ...
    if ((orthologueTmpl.taxonID == -9999) || (orthologueTmpl.homologeneID == null)
        || (orthologueTmpl.accession == null)) {
      // System.out.println(orthologueTmpl.taxonID + " " + orthologueTmpl.homologeneID + " " +
      // orthologueTmpl.accession);
    ...
    if ((taxon = HomologeneTools.getTaxon(orthologueTmpl.taxonID)) == null) {
      try {
        ...
        taxon = HomologeneTools.createTaxon(orthologueTmpl.taxonID,
            "Unknown species " + orthologueTmpl.taxonID);
    ...
    if ((orthologue = db.getOrthologue(orthologueTmpl.homologeneID)) == null) {
      orthologue = new SimpleOrthologue(taxon, orthologueTmpl.locusID, orthologueTmpl.homologeneID,
          orthologueTmpl.accession);
    }
  ...
  public void endOrthoPair() {
    ...
    // validate template
    if ((orthologyTmpl.type == null) || (orthologyTmpl.firstOrtho == null)
        || (orthologyTmpl.secondOrtho == null)) {
      System.out.println(
          orthologyTmpl.type + " " + orthologyTmpl.firstOrtho + " " + orthologyTmpl.secondOrtho);
    ...
    try {
      if (orthologyTmpl.type == SimilarityType.CURATED) {
        ...
        OrthoPair orthology = db.createOrthoPair(orthologyTmpl.firstOrtho,
            orthologyTmpl.secondOrtho, orthologyTmpl.ref);

        group.addOrthoPair(orthology);
      } else {
        OrthoPair orthology = db.createOrthoPair(orthologyTmpl.firstOrtho,
            orthologyTmpl.secondOrtho, orthologyTmpl.type, orthologyTmpl.percentIdentity);
