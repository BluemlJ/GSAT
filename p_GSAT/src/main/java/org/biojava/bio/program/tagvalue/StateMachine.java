 ...
 * </p>
 * <p>
 * Transitions can be specified to occur between States when specific events are encountered. These
 * events can be Tags delivered by startTag as well as the endTag/endRecord events. Events that
 * result in exit from the current State can be specified to be notifiable to the State listener.
 * </p>
 * <p>
 * In addition, tables of transitions can be defined and specified as the fallback when the a
 * corresponding Transition cannot be found for the tag. This is useful for specifying the
 * destination States that are globally applicable for particular tags. As the fallbacks can be
 * chained, you can end up with a hierarchy of Transition Tables, some being State-specific, others
 * applicable to groups of States and finally one being global.
 * </p>
 * 
 * @author David Huen
 */
public class StateMachine implements TagValueWrapper {
  ...
  /**
   * Interface implemented by State listeners that want notification when a transition leaves the
   * State.
   */
   ...
   * <p>
   * Note that you can chain a series of these Transition tables and the lookup will proceed along
   * the chain until it succeeds.
   */
  public class TransitionTable {
    ...
    public void put(Object tag, Transition transition) throws ParserException {
      if (table.containsKey(tag)) throw new ParserException("duplicate tag " + tag);
    ...
    /**
     * set the specified TransitionTable to be looked looked up if the Transition cannot be found in
     * this one.
     */
  ...
  public class BasicState implements State {
    ...
    /**
     * when this constructor is used, a fixed listener is used with this state. When setListener is
     * called on this class, it is the listener of this fixed listener that is changed.
     */
    ...
    /**
     * Find the destination State when the specified tag is encountered. If the destination is
     * successfully obtained, the listener is also notified of exit if required.
     */
    public void transit(Object tag) throws ParserException {
      ...
      if (transition == null) {
        ...
        if (defaultTransitions == null) throw new ParserException(
            "no transition available from " + statePointer.getLabel() + " for tag '" + tag + "'");

        transition = defaultTransitions.get(tag);

        if (transition == null) throw new ParserException(
            "no transition available from " + statePointer.getLabel() + " for tag '" + tag + "'");
      }
      ...
      if (transition.notifyOnExit) {
        // System.out.println("notifyOnExit being checked. this is " + this.getLabel() + " next is "
        // + nextState.getLabel());
        if (nextState != this) {
  ...
  /**
   * a basic listener for a State. It forwards all events to the delegate for the StateMachine.
   * Extend to implement listeners for specific states.
   */
  public class SimpleStateListener implements TagValueListener {
     ...
     * Default is that a ParserException is thrown. i
     */
  ...
  public void endTag() throws ParserException {
    ...
    if ((lstnr = ((BasicState) statePointer).getListener()) != null) lstnr.endTag();
    statePointer.transit(END_TAG);
  }

  public void startRecord() throws ParserException {
    TagValueListener lstnr;
    if ((lstnr = ((BasicState) statePointer).getListener()) != null) lstnr.startRecord();
    statePointer.transit(START_RECORD_TAG);
  }

  public void endRecord() throws ParserException {
    TagValueListener lstnr;
    if ((lstnr = ((BasicState) statePointer).getListener()) != null) lstnr.endRecord();
    statePointer.transit(END_RECORD_TAG);
  }

  public void value(TagValueContext ctxt, Object value) throws ParserException {
    TagValueListener lstnr;
    if ((lstnr = ((BasicState) statePointer).getListener()) != null) lstnr.value(ctxt, value);
  }
}
