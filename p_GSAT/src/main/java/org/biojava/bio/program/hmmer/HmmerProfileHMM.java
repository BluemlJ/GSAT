...
/**
 * This is a class for representing HMMER generated Profile HMM. It differs from the normal
 * ProfileHMM only in the states which are connected: - there are no insert <-> delete transitions
 * allowed - there is no iO initial insert state (between begin and initial match states) - there is
 * not iN final insert state (between final match state and end state)
 *
 * @author Lachlan Coin
 */
public class HmmerProfileHMM extends ProfileHMM {
  protected HmmerProfileHMM(Alphabet alpha, int columns, DistributionFactory matchFactory,
  ...
  protected void connectModel() throws ChangeVetoException, IllegalSymbolException,
    ...
    for (int i = 1; i <= columns(); i++) {
      ...
      if (i < columns()) createTransition(mN, iN);
      createTransition(mO, dN);

      // from an insert state
      if (i < columns()) createTransition(iN, iN);
      ...
      if (i > 1) createTransition(magicalState(), mN);
  ...
  public double transScore(State from, State to, Symbol symFrom, Symbol symTo)
  ...
  protected EmissionState makeNewInsertState(String str, Annotation ann, int[] adv,
      Distribution dis) {
    return new ProfileEmissionState(str, ann, adv, dis);
  }

  protected EmissionState makeNewMatchState(String str, Annotation ann, int[] adv,
      Distribution dis) {
    return new ProfileEmissionState(str, ann, adv, dis);
  }
