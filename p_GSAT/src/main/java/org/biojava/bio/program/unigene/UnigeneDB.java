...
public interface UnigeneDB extends Changeable {
   ...
   * @throws BioException if there is no known cluster by that ID or if there was an error fetching
   *         it
   */
   ...
   * @return a (possibly new) UnigeneCluster that is equivalent to <code>cluster</code> but is
   *         served from this <code>UnigeneDB</code> instance
   */
