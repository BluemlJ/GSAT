 ...
 * <ul>
 * <li>replace a single value with a new single value (e.g. changing the string "1.87" into a Double
 * object)</li>
 * <li>split a single value into multiple values and pass each one individualy on to the delegate
 * e.g. "a, b, c" becomes three values "a", "b", "c".</li>
 * </ul>
 * </p>
 *
 * <p>
 * For a given tag, changers take precendence over splitters, and explicitly registered changers or
 * splitters take precendence over the default handlers. If there is not a specific handler for a
 * tag and there is no default set, then the value is passed on unchanged.
 * </p>
 *
...
public class ValueChanger extends SimpleTagValueWrapper {
  ...
  /**
   * The changer that will be applied to the values of tags not registered explicitly to any changer
   * or splitter instance.
   *
  ...
  /**
   * The splitter that will be applied to the values of tags not registered explicitly to any
   * changer or splitter instance.
   *
