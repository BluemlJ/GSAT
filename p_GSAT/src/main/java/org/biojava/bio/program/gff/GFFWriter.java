 ...
 * <p>
 * This will ignore all exceptions. Perhaps the error-handling needs to move into an error handling
 * interface?
 *
...
public class GFFWriter implements GFFDocumentHandler {
  ...
  /**
   * Prints the comment directly to the <span class="type">PrintWriter</span> after adding a leading
   * '<code>#</code>'.
   */
  ...
  public void recordLine(GFFRecord record) {
    out.print(record.getSeqName() + "\t" + record.getSource() + "\t" + record.getFeature() + "\t"
