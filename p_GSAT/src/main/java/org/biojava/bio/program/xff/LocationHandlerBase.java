...
/**
 * Handler to the XFF location type. To use this, write a simple subclass which takes the location
...
public abstract class LocationHandlerBase extends StAXContentHandlerBase {
    ...
    if (level == 0) {
      ...
      if (localName.equals("span")) {
        try {
          ...
          Location span = new RangeLocation(Integer.parseInt(starts), Integer.parseInt(stops));
  ...
  /**
   * Override this method to do something useful with the location we collect. Maybe we should do
   * this by delegation rather than extension.
   */
