...
/**
 * A class for parsing in Hmmer markov models from HMM_ls files generated by HMMER training note
 * that this class is still currently experimental.
 * 
 * @author Lachlan Coin
 */
public class HmmerProfileParser {
  ...
  class HmmerModel {
    ...
    private void addProbability(Distribution dist, State state, double prob) throws Exception {
      if (Double.isNaN(dist.getWeight(state))) dist.setWeight(state, 0);
      Iterator iter = ((FiniteAlphabet) dist.getAlphabet()).iterator();
      while (iter.hasNext()) {
        Symbol to = (Symbol) iter.next();
    ...
    /**
     * Modifies HMM search for partial hits, by dividing probability by 2 at each point and adding
    ...
    void setProfileHMM() {
      try {
        for (int i = 0; i <= hmm.columns(); i++) {
          ...
          if (i < hmm.columns()) {
            dist.setWeight(hmm.getMatch(i + 1), convertToProb(transitions[i][0]));
            if (i >= 1) {
              dist.setWeight(hmm.getInsert(i), convertToProb(transitions[i][1]));
            }
            dist.setWeight(hmm.getDelete(i + 1), convertToProb(transitions[i][2]));
          } else {
        ...
        for (int i = 1; i < hmm.columns(); i++) {
          ...
          dist.setWeight(hmm.getMatch(i + 1), convertToProb(transitions[i][3]));
          dist.setWeight(insert, convertToProb(transitions[i][4]));
        }
        for (int i = 1; i < hmm.columns(); i++) {
          DotState delete = hmm.getDelete(i);
          Distribution dist = hmm.getWeights(delete);
          dist.setWeight(hmm.getMatch(i + 1), convertToProb(transitions[i][5]));
          dist.setWeight(hmm.getDelete(i + 1), convertToProb(transitions[i][6]));
        }
        setBeginEnd();
        checkTransitionSum();
        ...
        for (int i = 1; i <= hmm.columns(); i++) {
          ...
          if (i > 1 && i < hmm.columns()) hmm.getInsert(i).setDistribution(insertEmission);
          for (int j = 0; j < alphList.length; j++) {
            double prob;
            if (j < alphList.length - 1) {
