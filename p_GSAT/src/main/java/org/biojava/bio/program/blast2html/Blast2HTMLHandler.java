...
public class Blast2HTMLHandler extends DefaultHandler {
   ...
   *
   * @param poRenderer <code>HTMLRenderer</code> - a configured HTMLRenderer.
   */
  ...
  /**
   * This is called when an element is entered. That is, the parser has met the first tag of the tag
   * pair.
   *
   * @param poNameSpace <code>String</code> - the name space.
   * @param poElementName <code>String</code> - the local name of the tag.
   * @param poQName <code>String</code> - the fully qualified name with prefix
   * @param poAtts an <code>Attributes</code> - the tag attributes.
   * @exception SAXException if an error occurs
   */
  public void startElement(String poNameSpace, String poElementName, String poQName,
      Attributes poAtts) throws SAXException {
    ...
    if (inCollection) {
      ...
      if (poElementName.equals("BlastLikeDataSet")) {
        ...
        oHitSummary.numberOfContributingHSPs = poAtts.getValue("numberOfContributingHSPs");
        oHitSummary.smallestSumProbability = poAtts.getValue("smallestSumProbability");
        oHitSummary.readingFrame = poAtts.getValue("readingFrame");
        oHitSummary.numberOfDomains = poAtts.getValue("numberOfDomains");
        ...
        oDetailHit.sequenceLength = poAtts.getValue("sequenceLength");

      } else if (poElementName.equals("HSPSummary")) {

        oHSPSummary.percentageIdentity = poAtts.getValue("percentageIdentity");
        oHSPSummary.score = poAtts.getValue("score");
        oHSPSummary.expectValue = poAtts.getValue("expectValue");
        oHSPSummary.alignmentSize = poAtts.getValue("alignmentSize");
        oHSPSummary.numberOfIdentities = poAtts.getValue("numberOfIdentities");
        ...
        oHSPSummary.numberOfPositives = poAtts.getValue("numberOfPositives");
        oHSPSummary.percentagePositives = poAtts.getValue("percentagePositives");
        ...
        oAlignment.oQuerySeq.startPosition = poAtts.getValue("startPosition");
        oAlignment.oQuerySeq.stopPosition = poAtts.getValue("stopPosition");
        sb.setLength(0);
      } else if (poElementName.equals("MatchConsensus")) {
        sb.setLength(0);
      } else if (poElementName.equals("HitSequence")) {
        oAlignment.oHitSeq.startPosition = poAtts.getValue("startPosition");
        oAlignment.oHitSeq.stopPosition = poAtts.getValue("stopPosition");
        sb.setLength(0);
  ...
  public void endElement(String poNameSpace, String poElementName, String poQName) {
    ...
    if (poElementName.equals("Header")) {
      ...
      oRenderer.writeTitleAndHeader(oProgram, oVersion, oQuery, oDatabase);
  ...
  void getQueryIdAndDatabase() {
    ...
    BufferedReader reader = new BufferedReader(new StringReader(sb.substring(0)));
