 ...
 * Title: ABITrace<br>
 * <br>
 * ABITrace is a class for managing ABI file information, it is capable of opening an ABI file and
 * storing the most important fields, which can be recalled as simple java types. It can also return
 * an image corresponding to the trace. It has three constructors with input types
 * <code>File, URL, and byte[]</code>.<br>
 * <br>
 * ABI files contain two sets of basecall and sequence data, one that was originally created
 * programatically and the other, which is an editable copy. This version of this object only
 * references the original unedited data.<br>
 *
 * Copyright (c) 2001
 * 
...
public class ABITrace {
  ...
  /**
   * Returns an <code>int[]</code> array that represents the basecalls - each int in the array
   * corresponds to an x-coordinate point in the graph that is a peak (a base location).
   */
  ...
  /**
   * Returns one of the four traces - all of the y-coordinate values, each of which correspond to a
   * single x-coordinate relative to the position in the array, so that if element 4 in the array is
   * 972, then x is 4 and y is 972 for that point.
   *
   ...
   * pixels, the width in pixels is determined by the scaling factor times the number of points in
   * the trace (<code>getTraceLength()</code>). The entire trace is represented in the returned
   * image.
   *
   * @param imageHeight is the desired height of the image in pixels.
   * @param widthScale indiates how many horizontal pixels to use to represent a single x-coordinate
   *        (try 2).
   */
  public BufferedImage getImage(int imageHeight, int widthScale) {
    BufferedImage out =
        new BufferedImage(TraceLength * widthScale, imageHeight, BufferedImage.TYPE_BYTE_INDEXED);
    ...
    for (int q = 1; q <= 5; q++) {
      for (int x = 0; x <= TraceLength - 2; x++) {
        ...
        if (q == 5) {
          ...
          if (bc[here] == x) {
            g.drawLine(2 * x, transmute(-2, imageHeight, 1.0), 2 * x,
                transmute(-7, imageHeight, 1.0));
            ...
            // add a number
            {
              ...
              g.drawString(Integer.toString(basenum + 1), 2 * x - 3,
                  transmute(-36, imageHeight, 1.0));
            }
            switch (seq[basenum]) {
              case 'A':
              case 'a':
  ...
  /**
   * Shuffle the pointers to point to the proper spots in the trace, then load the traces into their
   * arrays.
   */
  private void setTraces() {
    ...
    for (int i = 0; i <= 3; i++) {
      ...
      for (int x = 0; x <= TraceLength - 1; x++) {
        ...
        } catch (IOException e)// This shouldn't happen. If it does something must be seriously
                               // wrong.
        {
  ...
  /**
   * Utility method to translate y coordinates from graph space (where up is greater) to image space
   * (where down is greater).
   */
  ...
  /**
   * Get the maximum height of any of the traces. The data is persisted for performance in the event
   * of multiple calls, but it initialized lazily.
   */
  ...
  /**
   * Returns the scaling factor necessary to allow all of the traces to fit vertically into the
   * specified space.
   * 
  ...
  private void setIndex() {
    ...
    for (int record = 0; record <= NumRecords - 1; record++) {
      ...
      if (RecName.equals("FWO_")) FWO = IndexBase + (record * 28) + 20;
  ...
  /**
   * Test to see if the file is ABI format by checking to see that the first three bytes are "ABI".
   * Also handle the special case where 128 bytes were prepended to the file due to binary FTP from
