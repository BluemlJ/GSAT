...
/**
 * <p>
 * Listens to tag-value events and passes on indexing events to an IndexStore. This is an update to
 * Indexer that understands that indexed properties may not be at the top level.
 * </p>
 *
 * <p>
    ...
    if (depth == 0) {
      ...
      for (Iterator i = keyValues.keySet().iterator(); i.hasNext();) {
        ...
        if (ks.keyName.equals(primaryKeyName)) {
          if (ks.values.size() != 1) {
            throw new ParserException("There must be exactly one value for the primary key: "
      ...
      indexStore.writeRecord(file, offset, length, primaryKeyValue, secKeys);
