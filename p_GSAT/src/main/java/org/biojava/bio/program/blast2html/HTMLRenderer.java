 ...
 * <p>
 *
 * Makes an assumption that the gap character is '-', should parameterize.
 *
 *
 * Primary author - Colin Hardman (CAT) Other authors - Tim Dilks (CAT) Simon Brocklehurst (CAT)
 * Stuart Johnston (CAT) Lawerence Bower (CAT) Derek Crockford (CAT) Neil Benn (CAT)
 *
...
public class HTMLRenderer {
   ...
   * <LI>summaryBodyLineEven </UL
   *
   * The Javascript function setStatus overrides status text for some links.
   *
   * The URLGeneratorFactory provides a list of objects that can convert a database ID into a URL
   * for fetching the entry. Obviously this requires the ID to be parsed correctly.<B> These URL's
   * are used in two places, in the summary and detail sections. The first URLGenerator is used for
   * the summary, and all for the detail.
   *
   *
   * @param poPrintWriter - the output stream to write the HTML to.
   * @param poStyleDef - definition of styles, if null uses hard coded.
   * @param piAlignmentWidth width in characters of the alignment regions of output
   * @param poFactory <code>URLGeneratorFactory</code> provides an array of
   *        <code>DatabaseURLGenerator</code>s, null for no links
   * @param poAlignmentMarker - for configurable markup of alignments, for no markup use null.
   * @param poOptions properties of
   */
  public HTMLRenderer(PrintWriter poPrintWriter, String poStyleDef, int piAlignmentWidth,
      URLGeneratorFactory poFactory, AlignmentMarker poAlignmentMarker, Properties poOptions) {
    ...
    if (poFactory != null) {
      ...
      if (!oURLGeneratorList.isEmpty()) {
        oFirstURLGenerator = (DatabaseURLGenerator) oURLGeneratorList.get(0);
      }
    }
  ...
  /**
   * Set the <CODE>PrintWriter</CODE> to output the HTML to.
   *
   ...
   *
   * @return <code>boolean</code> - true if no summary of Hit elements where encountered.
   */
  ...
  void writeTitleAndHeader(String oProgram, String oVersion, String oQuery, String oDatabase) {
    ...
    out.println(
        "    <td class=\"titleLevel1\" rowspan=\"3\"><a name=\"#AnchorTop\"></a>Sequence Similarity Report</td>");
  ...
  void writeCurrentSummary(HitSummary oHitSummary) {
    ...
    if (oFirstURLGenerator != null) {
      ...
      out.print("<a href=\"");
      out.print(oFirstURLGenerator.toURL(oHitSummary.oHitId.id, oOptions));
      out.print(
          "\" onMouseOver=\"setStatus('Retrieve hit from database');return document.retVal\" onMouseOut=\"setStatus(' ');return document.retVal\" class=\"dbRetrieve\">");
    ...
    out.print(oHitSummary.oHitId.id);
    out.print(
        "\" onMouseOver=\"setStatus('Jump to detailed results');return document.retVal\" onMouseOut=\"setStatus(' ');return document.retVal\">");
  ...
  void writeCurrentDetail(DetailHit oDetailHit) {
    ...
    out.println("</span></td>");
    out.println(
        "<td class=\"titleLevel3\" align=\"right\" color=\"#000000\" ><a href=\"#AnchorTop\" onMouseOver=\"setStatus('Jump to top of  document');return document.retVal\" onMouseOut=\"setStatus(' ');return document.retVal\">Top</a></td>");
    out.println("</tr>");
    out.println("<tr align=\"left\" valign=\"top\"> ");
    ...
    if (oURLGeneratorList != null && !oURLGeneratorList.isEmpty()) {
      ...
      for (int i = 0; i < oURLGeneratorList.size(); i++) {
        ...
        DatabaseURLGenerator oURLGenerator = (DatabaseURLGenerator) oURLGeneratorList.get(i);
        out.print(oURLGenerator.toLink(oDetailHit.oHitId.id, oOptions));
      }
      out.println("</p></td>");
      out.println("</tr>");
    }
  ...
  /**
   * Utility for writing out each HSP info item, such as score ot number of identities.
   *
   */
  ...
  void writeCurrentHSP(HSPSummary oHSPSummary, BlastLikeAlignment oAlignment) {
    ...
    out.println("<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"5\">");
    out.println("<tr align=\"left\" valign=\"top\"> ");
    out.println("<td class=\"titleLevel4\">High-scoring segment pair (HSP) group</td>");
    out.println("</tr>");
    out.println("</table>");
   ...
   */
  String drawSubAlignment(String piQueryStart, String piQueryStop, String piHitStart,
  ...
  void drawCurrentAlignment(BlastLikeAlignment oAlignment) {
    ...
    int iQueryMultiplier =
        ((iQStop - iCurrentQueryStart) + queryDirection) / (iQueryLen - iNumberOfQueryGaps);

    int iHitMultiplier =
        ((iHStop - iCurrentHitStart) + hitDirection) / (iHitLen - iNumberOfHitGaps);
    ...
    while (((i + 1) * iAlignLen) < iQueryLen) {
      ...
      String oCurrentQueryString =
          oAlignment.oQuerySeq.seq.substring(i * iAlignLen, (i + 1) * iAlignLen);
      String oCurrentHitString =
          oAlignment.oHitSeq.seq.substring(i * iAlignLen, (i + 1) * iAlignLen);

      iNumberOfQueryGaps = this.countNumberOfGaps(oCurrentQueryString);
      iNumberOfHitGaps = this.countNumberOfGaps(oCurrentHitString);

      iCurrentQueryEnd = iCurrentQueryStart + ((iAlignLen - iNumberOfQueryGaps) * iQueryMultiplier);
      iCurrentHitEnd = iCurrentHitStart + ((iAlignLen - iNumberOfHitGaps) * iHitMultiplier);

      out.println(drawSubAlignment(iCurrentQueryStart + "",
          (iCurrentQueryEnd - queryDirection) + "", iCurrentHitStart + "",
          (iCurrentHitEnd - hitDirection) + "", oCurrentQueryString,
          oAlignment.oConsensus.substring(i * iAlignLen, (i + 1) * iAlignLen), oCurrentHitString));
      i++;
      iCurrentQueryStart += ((iAlignLen - iNumberOfQueryGaps) * iQueryMultiplier);
      iCurrentHitStart += ((iAlignLen - iNumberOfHitGaps) * iHitMultiplier);
    ...
    if (iQStop != iCurrentQueryEnd) {
      ...
      out.println(
          drawSubAlignment(iCurrentQueryStart + "", iCurrentQueryEnd + "", iCurrentHitStart + "",
              iCurrentHitEnd + "", oAlignment.oQuerySeq.seq.substring(i * iAlignLen),
              oAlignment.oConsensus.substring(i * iAlignLen),
              oAlignment.oHitSeq.seq.substring(i * iAlignLen)));
    }
  ...
  void endDetailTable() {
    ...
    out.println("Produced using <a href=\"http://www.biojava.org/\" class=\"footer\">biojava</a> ");
