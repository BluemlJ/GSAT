...
/**
 * Simple implementation for specifying markup styles. Has 3 modes of operation: SHOW_ALL, SHOW_SAME
 * & SHOW_DIFF.
 * <p>
 *
 * SHOW_ALL - returns the default style for all given residues. SHOW_SAME - only returns a markup
 * style if the <B>styles</B> for both characters are the same. SHOW_DIFF - only returns a markup
 * style if the <B>styles</B> for both are different.
 *
 * Styles can be easily defined in two ways.<BR>
 * 
 * 1. Add each style by calling <CODE>addStyle( poChar, poColour )</CODE> For example, <CODE>
 *    	String oRed = "FFA2A2";
 ...
 * <p>
 *
 * 2. Alternatively the styles could be specified in a java properties file and loaded by calling
 * <CODE>readColourMapFromProperties( poFilename )</CODE>, or <CODE>readColourMap()</CODE> and
 * setting the system property 'colourMap' to the correct filename. <BR>
 *
 * This file should be in java properties format, mapping characters to colours, specified in HEX
 * RGB.
 * 
 * For example: <PRE>
 * # set everything red
 ...
 * </PRE>
 *
 * Note this is simply character based, so if you want to colour gaps then you need to specify a
 * colour for the gap character.
 * <p>
 *
...
public class SimpleAlignmentStyler extends AbstractAlignmentStyler {
  ...
  /**
   * Creates a new <CODE>SimpleAlignmentStyler</CODE> instance.
   * <p>
   *
   * The int flag should be one of SimpleAlignmentStyler.SHOW_ALL, SimpleAlignmentStyler.SHOW_SAME
   * or SimpleAlignmentStyler.SHOW_DIFF.
   *
  ...
  public SimpleAlignmentStyler(int piStyle) {
    ...
    if (piStyle != SHOW_DIFF && piStyle != SHOW_ALL && piStyle != SHOW_SAME) {
      throw new IllegalArgumentException(
          "Style flag not one of SimpleAlignmentStyler.SHOW_DIFF, " + " SHOW_ALL or SHOW_SAME");
    }
    iStyle = piStyle;
  }
  ...
  protected void readColourMapFromProperties(String poFileName) {
    ...
    for (Iterator i = oColourProps.entrySet().iterator(); i.hasNext();) {
  ...
  /**
   * Read the the properties file that specifies the character/colour mapping. The location of the
   * property file is specified by the system property 'colourMap'.
   *
   */
