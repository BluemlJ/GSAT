 ...
 *
 * <p>
 * For any particular alignment position, the colouring depends only on the two characters at that
 * position. The decision is made in two stages:-
 * <ol>
 * <li>Whether to colour or not in the <CODE>ColourCommand.isColoured()</CODE> method</li>
 * <li>what the colours are in the <CODE>AlignmentStyler.getStyle()</CODE> method</li>
 * </ol>
 * </p>
 *
 * <p>
 * This allows simple choices to highlight the mismatches, the identities or simply colour up
 * everything.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * As the FONT styles need to be defined before being used in the HTML, it means the all colours to
 * be used have to calculated up front.
 * </p>
 *
 * <p>
 * The position in the alignment is not passed in so position dependent colouring ( say against a
 * HMM profile ) would be involve either changing the interfaces or implementing them such that they
 * knew the required information via another route.
 * </p>
 *
 * <p>
 * <pre>
...
public class AlignmentMarker {
  ...
  /**
   * Single method interface for deciding whether to colour a particular alignment pair.
   */
   ...
   *
   * @param poColourCommand - controls whether a particular alignment pair should be coloured
   * @param poStyler - specifies all possible styles and where each style is used
   */
  ...
  /**
   * Takes three sequences ( of the same length ) as strings and returns three strings with colour
   * markup in HTML.
   *
   * @param poAlignment - three strings representing a pairwise alignment query, hit, consensus
   */
  void alignment2HTML(String[] poAlignment) {
    ...
    for (int i = 0, n = poAlignment[0].length(); i < n; i++) {
      ...
      if (oColourCommand.isColoured(oFirst, oSecond)) {
        oStyler.getStyle(oFirst, oSecond, oNewStyle);
      ...
      System.arraycopy(oNewStyle, 0, oCurrentStyle, 0, oNewStyle.length);
  ...
  /**
   * Simple utility function to call applyStyle on seq1, markup & seq2.
   *
   * @param poCurrentStyle a <code>String[]</code>
   * @param poNewStyle a <code>String[]</code>
   * @param poMarkedUp a <code>StringBuffer[]</code>
   */
  private void applyStyles(String[] poCurrentStyle, String[] poNewStyle,
   ...
   *
   * <p>
   * Takes care of runs of the same style and changing styles.
   * </p>
   *
