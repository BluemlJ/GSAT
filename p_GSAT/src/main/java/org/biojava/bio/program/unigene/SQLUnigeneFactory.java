 ...
 * directly, but rather use UnigeneTools.loadDatabase() with a jdbc URL.
 * </p>
 *
 * <p>
 * This class will store unigene data in a relational database with a schema defined by the resource
 * src/org/biojava/bio/program/unigene/createUnigene.sql and currently only realy supports mysql. To
 * import data to a newly created database, repeatedly call addCluster() on the UnigeneDB you get
 * back.
 * </p>
 *
...
public class SQLUnigeneFactory implements UnigeneFactory {
  ...
  private static String getCreateDBStatement() {
    if (CREATE_DB_STATEMENT == null) {
      /*
       * StringBuffer stmt = new StringBuffer(); BufferedReader stmtIn = new BufferedReader( new
       * InputStreamReader( ClassTools.getClassLoader(SQLUnigeneFactory.class).getResourceAsStream(
