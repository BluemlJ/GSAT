 ...
 *
 * <p>
 * This implementation of the data store factory uses longs as indecies internaly, so can be used
 * with files exceeding 2 gigs in size.
 * </p>
 *
 * <p>
...
public class NIODataStoreFactory implements DataStoreFactory {
  ...
  public DataStore buildDataStore(File storeFile, SequenceDB seqDB, Packing packing, int wordLength,
    ...
    final int structDataSize = 3 * Constants.BYTES_IN_LONG + // positions
    ...
    long hashTableSize = Constants.BYTES_IN_LONG + // hash table length
    ...
    int nameTableSize = Constants.BYTES_IN_INT + // bytes in table
    ...
    long hitTableSize = (long) Constants.BYTES_IN_LONG + // size
    ...
    final LargeBuffer hitTable =
        new LargeBuffer(channel, FileChannel.MapMode.READ_WRITE, hitTablePos, hitTableSize);
    hitTable.position(0);
  ...
  private void writeRecord(LongBuffer hashTable, LargeBuffer hitTable, int offset, int seqNumber,
    ...
    if (kmerPointer != -1) {
      ...
      long pos = kmerPointer + hitCount * (Constants.BYTES_IN_INT + Constants.BYTES_IN_INT)
