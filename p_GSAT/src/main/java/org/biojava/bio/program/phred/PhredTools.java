...
public final class PhredTools {
  ...
  /**
   * Retrieves the PHRED alphabet from the AlphabetManager. The Phred alphabet is a cross product of
   * a subset of the IntegerAlphabet from 0...99 and the DNA alphabet. The Phred alphabet is finite.
   *
   * The Phred Alphabet contains 400 BasisSymbols named, for example, (guanine 47). The BasisSymbols
   * can be fragmented into their component AtomicSymbols using the <code>getSymbols()</code> method
   * of BasisSymbol.
   */
   ...
   * 
   * @throws IllegalSymbolException if the provided symbol is not from the PHRED alphabet.
   */
  ...
  /**
   * Merges a Symbol List from the DNA alphabet with a SymbolList from the [0..99] subset of the
   * IntegerAlphabet into a SymbolList from the PHRED alphabet.
   * 
   ...
   * @throws IllegalSymbolException if a combination of Symbols cannot be represented by the PHRED
   *         alphabet.
   */
  public static SymbolList createPhred(SymbolList dna, SymbolList quality)
    ...
    if (dna.length() != quality.length()) {
      throw new IllegalArgumentException(
          "SymbolLists must be of equal length " + dna.length() + " : " + quality.length());
    }
    if (dna.getAlphabet() != DNATools.getDNA()) {
      throw new IllegalAlphabetException("Expecting SymbolList 'dna' to use the DNA alphabet, uses "
    ...
    if (quality.getAlphabet() != subint && quality.getAlphabet() != IntegerAlphabet.getInstance()) {
      throw new IllegalAlphabetException(
          "Expecting SymbolList quality to use the " + subint.getName() + " alphabet"
              + "or IntegerAlphabet instead uses " + quality.getAlphabet().getName());
    }
  ...
  /**
   * Creates a symbol from the PHRED alphabet by combining a Symbol from the DNA alphabet and a
   * Symbol from the IntegerAlphabet (or one of its subsets).
   * 
   * @throws IllegalSymbolException if there is no Symbol in the PHRED alphabet that represents the
   *         two arguments.
   */
  ...
  public static void writePhredQuality(OutputStream qual, OutputStream seq, SequenceDB db)
    ...
    seqw.writeStream(db.sequenceIterator());// this works as sequence methods act on the underlying
                                            // SimpleSequence
  }

  /**
   * Constructs a StreamReader to read in Phred quality data in FASTA format. The data is converted
   * into sequences consisting of Symbols from the IntegerAlphabet.
   */
  public static StreamReader readPhredQuality(BufferedReader br) {
    return new StreamReader(br, new PhredFormat(), getQualityParser(), getFastaBuilderFactory());
  }
  ...
  /**
   * The quality value is related to the base call error probability by the formula QV = - 10 *
   * log_10( P_e ) where P_e is the probability that the base call is an error.
   * 
   * @return a <code>double</code> value, note that for most Phred scores this will be rounded to
   *         the nearest <code>int</code>
   */
  ...
  /**
   * Calculates the probability of an error from the quality score via the formula P_e =
   * 10**(QV/-10)
   */
  ...
  /**
   * Converts a Phred sequence to an array of distributions. Essentially a fuzzy sequence Assumes
   * that all of the non called bases are equiprobable
   */
  public static Distribution[] phredToDistArray(PhredSequence s) {
    ...
    for (int i = 0; i < s.length(); i++) {// for each symbol in the phred sequence
      ...
      try {
        ...
        throw new AssertionFailure("Sequence " + s.getName() + " contains an illegal alphabet",
            iae);
      } catch (ChangeVetoException cve) {
