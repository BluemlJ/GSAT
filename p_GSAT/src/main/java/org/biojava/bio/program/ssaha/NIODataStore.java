 ...
 * obtain one of these by using the methods in NIODataStoreFactory.
 *
...
class NIODataStore implements DataStore {
  ...
  NIODataStore(File dataStoreFile) throws IOException {
    ...
    MappedByteBuffer rootBuffer =
        channel.map(FileChannel.MapMode.READ_ONLY, 0, 3 * Constants.BYTES_IN_LONG + // positions
    ...
    rootBuffer = channel.map(FileChannel.MapMode.READ_ONLY, 0,
    ...
    MappedByteBuffer hashTable_MB =
        channel.map(FileChannel.MapMode.READ_ONLY, hashTablePos, Constants.BYTES_IN_LONG);
    hashTable_MB.position(0);
    long hashTableSize = hashTable_MB.getLong();
    System.out.println("hashTableSize: " + hashTableSize);
    hashTable = channel.map(FileChannel.MapMode.READ_ONLY, hashTablePos + Constants.BYTES_IN_LONG,
        hashTableSize - Constants.BYTES_IN_LONG).asLongBuffer();

    // map in buffer for names table
    MappedByteBuffer nameTable_MB =
        channel.map(FileChannel.MapMode.READ_ONLY, nameTablePos, Constants.BYTES_IN_INT);
    nameTable_MB.position(0);
    int nameTableSize = nameTable_MB.getInt();
    nameTable = channel.map(FileChannel.MapMode.READ_ONLY, nameTablePos + Constants.BYTES_IN_INT,
        nameTableSize - Constants.BYTES_IN_INT);

    // map in buffer for hit table
    MappedByteBuffer hitTable_MB =
        channel.map(FileChannel.MapMode.READ_ONLY, hitTablePos, Constants.BYTES_IN_LONG);
    hitTable_MB.position(0);
    long hitTableSize = hitTable_MB.getLong();
    hitTable = new LargeBuffer(channel, FileChannel.MapMode.READ_ONLY,
        hitTablePos + Constants.BYTES_IN_LONG, hitTableSize - Constants.BYTES_IN_LONG);
  }
  ...
  public String seqNameForID(int id) throws SearchException {
    ;
    // try {
  ...
  private void fireHits(int word, int offset, SearchListener listener) throws IOException {
    ...
    if (hitOffset != -1) {
      ...
      for (int i = 0; i < hits; i++) {
        listener.hit(hitTable.getInt(), offset, hitTable.getInt(), wordLength);
      }
    }
  }
}
