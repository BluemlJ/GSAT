...
/**
 * An implementation of {@link org.biojava.bio.chromatogram.Chromatogram} to encapulsulate
 * chromatogram data extracted from the files produced by ABI sequencers, such as the the 377 and
 * the 3700. The format was described by Clark Tibbetts in his paper "Raw Data File Formats, and the
 * Digital and Analog Raw Data Streams of the ABI PRISM 377 DNA Sequencer." Available online
...
public class ABIFChromatogram extends AbstractChromatogram implements Serializable {
   ...
   * <p>
   * Due to the non-single-pass design of the ABI format, this method will wrap the InputStream in
   * an {@link org.biojava.utils.io.CachingInputStream}. For this reason, {@link #create(File)}
   * should be preferred.
   * </p>
   * 
  ...
  /**
   * An extension of {@link ABIFParser} that reads the particular fields from the ABIF that contain
   * the chromatogram data and initializes the fields in its enclosing <code>ABIFChromatogram</code>
   * instance.
   */
  protected class Parser extends ABIFParser {
    ...
    private final void parse() throws IOException, UnsupportedChromatogramFormatException {
      ...
      if (fwoRec == null) throw new UnsupportedChromatogramFormatException(
          "No FWO_ (1) record in ABIF file, therefore no trace data");
      ...
      for (int i = 0; i < 4; i++) {
        ...
        try {
          ...
          throw new UnsupportedChromatogramFormatException("An unexpected character (" + fwo_[i]
              + ") was found in the FWO_ tag.  Parsing cannot continue.");
        }
    ...
    private void parseTrace(AtomicSymbol sym, int whichData)
        throws IOException, UnsupportedChromatogramFormatException {
      TaggedDataRecord dataPtr = getDataRecord("DATA", whichData);
      if (dataPtr.numberOfElements > Integer.MAX_VALUE)
        throw new UnsupportedChromatogramFormatException("Chromatogram has more than "
            + Integer.MAX_VALUE + " trace samples -- can't handle it");
      ...
      if (dataPtr.elementLength == 2) {
        ...
        throw new UnsupportedChromatogramFormatException(
            "Only 8- and 16-bit trace samples are supported");
      }
    ...
    private void parseBaseCalls() throws IOException, UnsupportedChromatogramFormatException {
      ...
      if (offsetsPtr.numberOfElements > Integer.MAX_VALUE)
        throw new UnsupportedChromatogramFormatException(
            "Chromatogram has more than " + Integer.MAX_VALUE + " base calls -- can't handle it");
      ...
      if (offsetsPtr.elementLength == 2) {
        ...
        for (int s = 0; s < shortArray.length; s += 2) {
          offsets.add(integerAlphabet
              .getSymbol(((short) ((shortArray[s] << 8) | (shortArray[s + 1] & 0xff))) & 0xffff));
        }
      ...
      try {
        ...
        baseCalls.put(Chromatogram.OFFSETS,
            createImmutableSymbolList(IntegerAlphabet.getInstance(), offsets));
