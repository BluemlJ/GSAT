...
/**
 * <p>
 * A parser that splits a line into tag/value at a given column number. The GENBANK and EMBL
 * constants are parsers pre-configured for genbank and embl style files respectively.
 * </p>
 *
 * <p>
 * There are many properties of the parser that can be set to change how lines are split, and how
 * the tag and value is produced from that split.
 * <ul>
 * <li>endOfRecord - string starting lines that mark record boundaries e.g. "//"</li>
 * <li>splitOffset - column index of the first character of the value, and the length of the raw tag
 * e.g. 5 for EMBL files</li>
 * <li>trimTag - trim white-space from tags</li>
 * <li>trimValue - trim white-space from values</li>
 * <li>continueOnEmptyTag - if the tag is empty, use the previous tag e.g. this is true for GENBANK
 * files and false for EMBL files</li>
 * <li>mergeSameTag - if two consecutive tags have the same value, consider their values to be a
 * continuation of a single value so don't fire start/end tag events e.g. true for EMBL</li>
 * </ul>
 *
...
public class LineSplitParser implements TagValueParser, Cloneable {
  ...
  /**
   * Choose whether to treat empty tags as a continuation of previous tags or as a new tag with the
   * value of the empty string.
   *
   * @param continueOnEmptyTag true to enable empty tags to be treated as a continuation of the
   *        previous tag, false otherwise
   */
  ...
  /**
   * See if empty tags are treated as a continuation of previous tags or as a new tag with the value
   * of the empty string.
   *
  ...
  /**
   * Enable or disable treating runs of identical tags as a single tag start event with multiple
   * values or each as a separate tag start, value, and tag end.
   *
