...
public class GFFParser {
   ...
   * @param handler the <span class="type">GFFDocumentHandler</span> that will listen for 'stuff'
   *
    ...
    for (String line = bReader.readLine(); line != null; line = bReader.readLine()) {
      ...
      try {
        ...
        if (line.startsWith("#")) {
          ...
          if (aList.size() < 7) {
            throw new ParserException("Line doesn't look like GFF", locator, lineNum, line);
          }
   ...
   * <span class="type">GFFRecord</span> and informs <span class="arg">handler</span>.
   *
   * @param handler a <span class="type">GFFDocumentHandler</span> to inform of any parse errors,
   *        and the completed <span class="type">GFFRecord</span>
   ...
   *         correct a parse error
   */
  protected GFFRecord createRecord(GFFDocumentHandler handler, List aList, String rest,
      String comment) throws BioException, ParserException, IgnoreRecordException {
   ...
   * and value lists.
   * <p>
   * The resulting <span class="type">Map</span> will have <span class="type">String</span> keys,
   * with <span class="type">List</span> values. If there are no values associated with a key, then
   * it will have an empty <span class="type">List</span>, not <span class="kw">null</span> as its
   * value.
   *
