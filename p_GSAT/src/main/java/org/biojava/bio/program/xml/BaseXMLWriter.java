...
/**
 *
 * Base XMLWriter class for writing XML representations of Java Value Objects with bespoke
 * architectures.
 *
 * Currently reporting QNames for all attribute values as a fix for needing to defining namespaces
 * by attributes with QNames. This may not be ideal.
 *
 * <p>
...
public class BaseXMLWriter {
  ...
  public String startElement(String string) {
    ...
    oStr.setLength(0);
    if (!tLastWritePCData) oStr.append(nl());
    oStr.append(indent());
    oStr.append("<");
    oStr.append(string);
    oStr.append(">");
  ...
  public String startElement(String string, Attributes attributes) {
    ...
    for (i = 0; i < attributes.getLength() - 1; i++) {
      ...
      oStr.append(" ");
      if (i > 0) oStr.append(oSpaceStore.substring(0));
    ...
    oAttValue = attributes.getValue(i);
    oStr.append(" ");
    if (attributes.getLength() > 1) oStr.append(oSpaceStore.substring(0));
    oStr.append(oAttName);
    oStr.append("=\"");
    oStr.append(oAttValue);
    oStr.append("\"");
