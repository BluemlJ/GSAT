...
/**
 * A {@link org.biojava.bio.chromatogram.Chromatogram} as loaded from an SCF v2 or v3 file. Also
 * loads and exposes the SCF format's "private data" and "comments" sections. The quality values
 * from the SCF are stored as additional sequences on the base call alignment. The labels are the
 * <code>PROB_</code>* constants in this class. The values are
 * {@link org.biojava.bio.symbol.IntegerAlphabet.IntegerSymbol} objects in the range 0 to 255.
 *
 *
...
public class SCF extends AbstractChromatogram {
  ...
  /**
   * Base call alignment sequence label for the substitution probability. In versions of the SCF
   * spec before 3.10, this is called spareQual[0].
   */
  public static final Object PROB_SUBSTITUTION = "substitution-probability";
  /**
   * Base call alignment sequence label for the overcall probability. In versions of the SCF spec
   * before 3.10, this is called spareQual[1].
   */
  public static final Object PROB_OVERCALL = "overcall-probability";
  /**
   * Base call alignment sequence label for the undercall probability. In versions of the SCF spec
   * before 3.10, this is called spareQual[2].
   */
  ...
  /**
   * Overrides {@link AbstractChromatogram#reverseComplementBaseCallList} to support the 7 quality
   * values from the SCF. These are handled thus:
   * <ul>
   * <li><code>PROB_SUBSTITUTION</code>, <code>PROB_OVERCALL</code>, and <code>PROB_UNDERCALL</code>
   * are just reversed &returned.</li>
   * <li><code>PROB_NUC_</code>* returns the reverse of the quality sequence for the complement
   * base.</li>
   * </ul>
   */
  ...
  /**
   * Factory class to create the appropriate parser for the given stream. This decision is based on
   * the version field in the file's header.
   */
    ...
    public static SCF.Parser createParser(DataInputStream din, SCF out, long initOffset)
      ...
      try {
        ...
        throw new UnsupportedChromatogramFormatException(
            "The SCF's version (" + new String(header.version) + ") is not a number");
      }
      if (version < 3.0f && version >= 2.0f) {
        ...
        throw new UnsupportedChromatogramFormatException(
            "Only version 2 and version 3 SCFs are supported (not " + new String(header.version));
      }
      return parser;
    }
  }
  ...
  static interface BaseCallUncertaintyDecoder {
    /**
     * Returns an appropriate Symbol from the DNA alphabet for an encoded byte.
     */
    public Symbol decode(byte call) throws IllegalSymbolException;
  }

  /**
   * A BaseCallUncertaintyDecoder that works for type 0 (default) and type 4 (ABI) code sets.
   */
  ...
  abstract static class Parser {
    ...
    Parser(DataInputStream din, SCF out, SCF.Parser.HeaderStruct header, long initOffset)
        throws UnsupportedChromatogramFormatException {
      if (din == null) throw new IllegalArgumentException("Can't parse a null inputstream");
      this.din = din;
      ...
      if (header.samples > Integer.MAX_VALUE) throw new UnsupportedChromatogramFormatException(
          "Can't parse an SCF with more than " + Integer.MAX_VALUE + " trace samples");

      if (header.bases > Integer.MAX_VALUE) throw new UnsupportedChromatogramFormatException(
    ...
    /**
     * Factory method to create an approriate decoder for the code set. Currently, only a direct
     * interpretation of the encoded byte as an ASCII character is supported (see
     * DefaultUncertaintyDecoder). This interpretation will be used for all code sets, but a warning
     * will be printed if the code set is not known to work with this interpretation.
     */
    private static BaseCallUncertaintyDecoder createDecoder(long codeSet) {
      if (codeSet != 0 && codeSet != 2 && codeSet != 4) System.err.println("Warning: the code set ("
          + codeSet + ") is not specifically supported.  (It may still work, though.)");
    ...
    public void parse() throws IOException, UnsupportedChromatogramFormatException {
      ...
      Integer SAMPLES = new Integer(0), BASES = new Integer(1), COMMENTS = new Integer(2),
      ...
      for (Iterator<Long> it = sectionOrder.keySet().iterator(); it.hasNext();) {
        Integer sect = sectionOrder.get(it.next());
        if (sect == SAMPLES)
          parseSamples();
        else if (sect == BASES)
          parseBases();
        else if (sect == COMMENTS)
          parseComments();
    ...
    protected abstract void parseSamples()
        throws IOException, UnsupportedChromatogramFormatException;
    ...
    protected void parseComments() throws IOException {
      ...
      offset += read;
      BufferedReader r =
          new BufferedReader(new InputStreamReader(new ByteArrayInputStream(raw), "ISO-8859-1"));
    ...
    protected final void skipTo(long newOffset) throws IOException {
      if (newOffset < offset) throw new IllegalArgumentException(
          "Can't skip backwards: (newOffset==" + newOffset + ") < (offset==" + offset + ")");
    ...
    /**
     * Does the grunt work of creating the base call alignment from the given lists of bases,
     * offsets, and probabilities. Catches and "Can't happens" all exceptions.
     */
      ...
      public static HeaderStruct create(DataInputStream din, long initOffset) throws IOException {
        ...
        if (initOffset > 4) {
          throw new IllegalStateException(
              "Can't skip more than four bytes and still have enough info to read header");
  ...
  private static class V3Parser extends Parser {
    V3Parser(DataInputStream din, SCF out, SCF.Parser.HeaderStruct header, long initOffset)
        throws IOException, UnsupportedChromatogramFormatException {
    ...
    protected void parseSamples() throws IOException, UnsupportedChromatogramFormatException {
      ...
      for (int n = 0; n < 4; n++) {
        for (int i = 0; i < trace[n].length; i++) {
          ...
          if (p_sample1[n] >= maxValAllowed) p_sample1[n] = p_sample1[n] - maxValAllowed;
          p_sample2[n] = p_sample1[n] + p_sample2[n];
          if (p_sample2[n] >= maxValAllowed) p_sample2[n] = p_sample2[n] - maxValAllowed;
    ...
    protected void parseBases() throws IOException, UnsupportedChromatogramFormatException {
      ...
      List<IntegerAlphabet.IntegerSymbol> offsets =
          new ArrayList<IntegerAlphabet.IntegerSymbol>(count);
      ...
      try {
        for (int i = 0; i < count; i++) {
          ...
          offset += 4;
          if (tmp > Integer.MAX_VALUE) throw new UnsupportedChromatogramFormatException(
        ...
        try {
          ...
          UnsupportedChromatogramFormatException ue =
              new UnsupportedChromatogramFormatException("Base call decoding failure");
          ue.initCause(ise);
          throw ue;
        }
        // read 'spare' probs
        ...
        throw new BioError(ise,
            "Can't happen unless there's a misdefinition of getProbabilityAlphabet() or IntegerAlphabet");
      }
  ...
  private static class V2Parser extends Parser {
    V2Parser(DataInputStream din, SCF out, SCF.Parser.HeaderStruct header, long initOffset)
        throws IOException, UnsupportedChromatogramFormatException {
      super(din, out, header, initOffset);
    }
    ...
    protected void parseBases() throws IOException, UnsupportedChromatogramFormatException {
      ...
      for (int i = 0; i < count; i++) {
        ...
        try {
          ...
          throw new BioError(ise,
              "Can't happen unless getProbabilityAlphabet() has been misdefined.");
        }
      }
      createAndSetBaseCallAlignment(dna, offsets, probs);
    }
