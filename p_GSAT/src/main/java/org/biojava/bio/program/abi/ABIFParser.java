...
/**
 * A general base parser for files produced by ABI software. This includes chromatograms derived
 * from ABI sequencers and potentially other data files as well. The format was described by Clark
 * Tibbetts in his paper "Raw Data File Formats, and the Digital and Analog Raw Data Streams of the
 * ABI PRISM 377 DNA Sequencer." Available online
 ...
 * <p>
 * Briefly, the format consists of a set of named fixed-length "tagged data records" which may
 * contain data themselves, or pointers to data elsewhere in the file. This class reads these
 * records and exposes them to subclasses through the {@link #getDataRecord} method. The attributes
 * of the records as described in Tibbets' paper are exposed through public (final) fields of
 * {@link TaggedDataRecord} instances.
 * </p>
 * <p>
 * If a record only contains a pointer to the desired data (see
 * {@link TaggedDataRecord#hasOffsetData}, subclasses may get at the raw data by using
 * {@link TaggedDataRecord#offsetData}:
 * </p>
 * <p>
 * This parser provides methods and classes for dealing with the files as streams or local files
 * (local files being more memory-efficient).
 * </p>
 *
...
public class ABIFParser {
  ...
  /**
   * Creates a new ABIFParser for an input stream. Note that the stream will be wrapped in a
   * {@link CachingInputStream} if it isn't one already. If it is, it will be seeked to 0.
   */
  ...
  /**
   * Creates a new ABIFParser for the specified {@link DataAccess} object. If you need to read from
   * something other than a file or a stream, you'll have to implement a
   * {@link DataAccess}-implementing class wrapping your source and then pass an instance to this
   * constructor.
   */
  ...
  /**
   * Decodes a character into a {@link Symbol} in the DNA alphabet. Uses a definition of characters
   * that is compatible with the ABI format.
   * 
   ...
   * @throws IllegalArgumentException if tagName is the wrong length or tagNumber is 0 or negative
  ...
  public ABIFParser.TaggedDataRecord getDataRecord(String tagName, int tagNumber)
    ...
    if (!parsed) throw new IllegalStateException("parsing is not complete");
    if (tagNumber < 1) throw new IllegalArgumentException("tagNumber must be positive");
    if (tagName.length() != 4)
  ...
  /**
   * Obtain all data records. Keys of the map are strings consisting of tag names with tag numbers
   * concatenated immediately afterwards. Values are TaggedDataRecord objects. The map has no
   * particular order and so cannot be relied on to iterate over records in the same order they were
   * read from the file.
   * 
  ...
  /**
   * An aggregate immutable type for an ABIF tagged data record. See the Tibbets paper (referenced
   * in the javadoc for {@link ABIFParser}) for more information.
   */
  public static class TaggedDataRecord {
    ...
    /**
     * Creates a new TaggedDataRecord from the next 28 bytes of <code>din</code>.
     * 
    ...
    /**
     * A very verbose <code>toString</code> that dumps all of the data in this record in a
    ...
    public String toString() {
      ...
      if (recordLength <= 4) {
        switch (dataType) {
          case DATA_TYPE_ASCII_ARRAY:
            if (recordLength > 3) sb.append((char) ((dataRecord >>> 24) & 0xFF));
            if (recordLength > 2) sb.append((char) ((dataRecord >>> 16) & 0xFF));
            if (recordLength > 1) sb.append((char) ((dataRecord >>> 8) & 0xFF));
  ...
  public static interface DataAccess extends Seekable, DataInput {
    /**
     * Called when the parser has finished reading. The access may choose to close itself at this
     * point, e.g. if it is using a RandomAccessFile.
     * 
  ...
  /**
   * Implements DataAccess by delegating to a CachingStream and a DataInputStream
   */
  private static class DataStream implements DataAccess {
    ...
    public String readLine() throws IOException {
      throw new UnsupportedOperationException(
          "DataInputStream#readLine is deprecated.  Use readUTF instead");
    }

    public int skipBytes(int count) throws IOException {
      return din.skipBytes(count);
    }
