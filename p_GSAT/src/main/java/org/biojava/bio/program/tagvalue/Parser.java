 ...
 *
 * <p>
 * Scripts will usually construct a parser object, a BufferedReader, a TagValueParser and
 * TagValueListener, and then set up a loop that processes each record in the reader by calling
 * Parser.read() until it returns false.
 * </p>
 *
 * @since 1.2
 ...
 */
public class Parser {
  public boolean read(BufferedReader reader, TagValueParser parser, TagValueListener listener)
    ...
    for (Object line = reader.readLine(); line != null; line = reader.readLine()) {
      ...
      for (Iterator fi = stack.iterator(); fi.hasNext();) {
        ...
        if (tv.isNewTag() || (tv.getTag() != null && !tv.getTag().equals(frame.tag))) {
  ...
  private static class Frame {
    ...
    public Frame(TagValueParser parser, TagValueListener listener, Object tag) {
      this.parser = parser;
  ...
  private static class Context implements TagValueContext {
    ...
    public void pushParser(TagValueParser subParser, TagValueListener listener) {
