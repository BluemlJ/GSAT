...
public class NcbiDatabaseURLGenerator implements DatabaseURLGenerator {
  ...
  public String toURL(String poID, Properties poProperties) {
    ...
    return "http://www.ncbi.nlm.nih.gov/entrez/query.fcgi?cmd=Search&db="
        .concat(poProperties.getProperty("db")).concat("&term=").concat(poID);
  }

  public String toLink(String poID, Properties poProperties) {
    return "[<A class=\"dbRetrieve\" HREF=\"".concat(this.toURL(poID, poProperties))
        .concat("\">retrieve hit from ncbi</A>]");
  }
}
