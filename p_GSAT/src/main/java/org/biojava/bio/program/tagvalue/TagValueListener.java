 ...
 *
 * <p>
 * This interface is similar in spirit to the SAX interfaces for parsing XML. Many of the methods
 * will always be called in appropriately nested pairs. Entire records will be bracketed by a
 * startRecord and endRecord pair. Within these, any number of startTag and endTag pairs may be
 * called. Within a tag pair, any number of value invocations may be called. If a value is complex
 * and requires parsing as a sub-entry, then the TagValueContext interface can be used to push a new
 * TagValueParser and listener pair onto the parser stack. This will result in the pushed listener
 * recieving a start/end document notification encapsulating the entire sub-set of events generated
 * by the parser using the pushed TagValueParser to process the sub-document.
 * </p>
 *
