...
/**
 * An implementation of DataStore that will map onto a file using the NIO constructs. You should
 * obtain one of these by using the methods in MappedDataStoreFactory.
 *
...
public class CompactedDataStore implements DataStore {
  ...
  CompactedDataStore(File dataStoreFile) throws IOException {
    ...
    MappedByteBuffer rootBuffer = channel.map(FileChannel.MapMode.READ_ONLY, 0, 4 * 6);
    ...
    rootBuffer = channel.map(FileChannel.MapMode.READ_ONLY, 0, 4 * 6 + packingStreamLength);
    ...
    MappedByteBuffer hashTable_MB = channel.map(FileChannel.MapMode.READ_ONLY, hashTablePos, 4);
    hashTable_MB.position(0);
    int hashTableSize = hashTable_MB.getInt();
    hashTable = channel.map(FileChannel.MapMode.READ_ONLY, hashTablePos + 4, hashTableSize - 4)
        .asIntBuffer();

    // map in buffer for hit table
    MappedByteBuffer hitTable_MB = channel.map(FileChannel.MapMode.READ_ONLY, hitTablePos, 4);
    hitTable_MB.position(0);
    int hitTableSize = hitTable_MB.getInt();
    hitTable = channel.map(FileChannel.MapMode.READ_ONLY, hitTablePos + 4, hitTableSize - 4);

    // map in buffer for names array
    MappedByteBuffer nameArray_MB = channel.map(FileChannel.MapMode.READ_ONLY, nameArrayPos, 4);
    ...
    nameArray = channel.map(FileChannel.MapMode.READ_ONLY, nameArrayPos + 4, nameArraySize - 4)
        .asIntBuffer();

    // map in buffer for names table
    MappedByteBuffer nameTable_MB = channel.map(FileChannel.MapMode.READ_ONLY, nameTablePos, 4);
    nameTable_MB.position(0);
    int nameTableSize = nameTable_MB.getInt();
    nameTable = channel.map(FileChannel.MapMode.READ_ONLY, nameTablePos + 4, nameTableSize - 4);
  }
    ...
    if (numSequences == 1) {
      ...
      while (true) {
        ...
        if (pos > offset && pos < endOffset) {
          ...
          throw new Error("Ooops: could not locate seq name for " + "\tpos: " + pos + "\tmid: "
              + mid + "\toffset: " + offset + "\tendOffset: " + endOffset + "\tminBound: "
  ...
  public void fireHits(int word, int offset, SearchListener listener) {
    ...
    if (hitOffset >= 0) {
      ...
      for (int i = 0; i < hits; i++) {
        ...
        listener.hit(id, offset, pos - offsetForID(id), wordLength);
      }
    } else if (hitOffset == -2) {
