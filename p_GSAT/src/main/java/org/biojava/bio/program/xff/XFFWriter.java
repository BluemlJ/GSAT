...
/**
 * Simple XFF writing code, ripped off from Dazzle 0.08. It probably needs a re-write to allow
 * custom types to be used properly, but it has enough hooks for now...
 *
 * @author Thomas Down
 */
...
public class XFFWriter {
  ...
  private void emitFeature_xff(Feature f, XMLWriter xw) throws IOException {
    if (f instanceof ComponentFeature) {
      ...
      xw.closeTag("componentFeature");
        ...
        && ((StrandedFeature) f).getStrand() != StrandedFeature.UNKNOWN) {
      ...
      emitFeature_xff_standardBits(f, xw);
      if (f.countFeatures() > 0) emitFeature_xff_featureSet(f, xw, false);
  ...
  private void emitFeature_xff_featureSet(FeatureHolder fh, XMLWriter xw, boolean explicitNamespace)
      throws IOException {
