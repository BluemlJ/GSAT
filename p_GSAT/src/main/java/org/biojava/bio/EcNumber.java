 ...
 *
 *
 * Implementations of this interface should be imutable. This makes them much more usefull as keys
 * in maps.
 * 
 * it is a good idea to validate that the data being passed in is a sane ec number.
 *
...
public interface EcNumber {
  ...
  /**
   * Constant that represents EC number components that are not defined. This is often represented
   * as a '-' in EC strings.
   */
  public static final int UNDEFINED = -1;

  /**
   * Constant that represents EC number components that are as yet unclassified. This is often
   * represented as 99 in EC strings.
   */
   ...
   *
   * <p>
   * The index can be between 0 and 3 inclusive. 0 correxpons to the top level class, 1 to the
   * sub-class and so on. A return value of UNDEFINED indicates that this field is not populated.
   * </p>
   *
  ...
  public static class Impl implements EcNumber {
    ...
    private Impl(String ecString) {
      ...
      classes = new int[] {process(matcher.group(1)), process(matcher.group(4)),
          process(matcher.group(7)), process(matcher.group(10))};
    }
    ...
    public int hashCode() {
      return getClassNumber(0) * 1000000 + getClassNumber(1) * 10000 + getClassNumber(2) * 100
          + getClassNumber(3);
    }
