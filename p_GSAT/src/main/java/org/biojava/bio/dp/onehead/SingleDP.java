...
public class SingleDP extends DP implements Serializable {
  ...
  /**
   * This method is public for the benefit of training algorithms, and in the future we should look
   * at a better way of exposing the emissions cache.
   */
  ...
  public double[] getEmission(Symbol sym, ScoreType scoreType) throws IllegalSymbolException {
    ...
    if (em == null) {
      ...
      /*
       * System.out.println("Emissions for " + sym); for(int i = 0; i < em.length; i++) {
  ...
  public double forward(SymbolList[] seq, ScoreType scoreType)
    ...
    lockModel();
    DPCursor dpCursor = new SmallCursor(getStates(), seq[0], seq[0].iterator());
  ...
  public double backward(SymbolList[] seq, ScoreType scoreType)
    ...
    lockModel();
    DPCursor dpCursor = new SmallCursor(getStates(), seq[0], new ReverseIterator(seq[0]));
  ...
  protected void forward_initialize(DPCursor dpCursor, ScoreType scoreType)
    ...
    for (int l = getDotStatesIndex(); l < states.length; l++) {
      ...
      int ci = 0;
      while (ci < tr.length && (currentCol[tr[ci]] == Double.NEGATIVE_INFINITY
          || currentCol[tr[ci]] == Double.NaN || currentCol[tr[ci]] == Double.POSITIVE_INFINITY)) {
        ci++;
      }
      ...
      for (int kc = 0; kc < tr.length; kc++) {
        ...
        if (currentCol[k] != Double.NEGATIVE_INFINITY && currentCol[k] != Double.NaN
  ...
  private void forward_recurse(DPCursor dpCursor, ScoreType scoreType)
    ...
    while (dpCursor.canAdvance()) {
      ...
      for (int l = 0; l < getDotStatesIndex(); l++) { // any -> emission
        ...
        if (weight == Double.NEGATIVE_INFINITY) {
          ...
          while (ci < tr.length && (lastCol[tr[ci]] == Double.NEGATIVE_INFINITY
              || lastCol[tr[ci]] == Double.NaN || lastCol[tr[ci]] == Double.POSITIVE_INFINITY)) {
            ci++;
          }
      ...
      for (int l = getDotStatesIndex(); l < states.length; l++) { // all dot states from emissions
        ...
        int ci = 0;
        while (ci < tr.length
            && (currentCol[tr[ci]] == Double.NEGATIVE_INFINITY || currentCol[tr[ci]] == Double.NaN
                || currentCol[tr[ci]] == Double.POSITIVE_INFINITY)) {
          ci++;
        }
        double constant = (ci < tr.length) ? currentCol[tr[ci]] : 0.0;
        // System.out.println("constant: " + constant);
        // System.out.println("read from state: " + ((ci < tr.length) ? states[tr[ci]].getName() :
        // "none"));
        for (int kc = 0; kc < tr.length; kc++) {
          int k = tr[kc];
    ...
    while (dpCursor.canAdvance()) {
      ...
      for (int k = stateCount - 1; k >= 0; k--) {
        ...
        for (int lc = tr.length - 1; lc >= 0; lc--) { // any->dot
          ...
          /*
           * System.out.println( "Processing dot-state transition " + states[k].getName() + " -> " +
           * states[l].getName() );
           */
  ...
  private StatePath viterbi(DPCursor dpCursor, ScoreType scoreType) throws IllegalSymbolException {
      ...
      for (int l = 0; l < states.length; l++) { // don't move from magical state
        ...
        if (emission == Double.NEGATIVE_INFINITY) {
          ...
          for (int kc = 0; kc < tr.length; kc++) {
             ...
             * System.out.println("Old = " + lastCol[k]); System.out.println("New = " +
             * currentCol[k]); System.out.println( "Considering " + states[k].getName() + " -> " +
          ...
          if (prev != -1) {
             ...
             * System.out.println("Old = " + lastCol[l]); System.out.println("New = " +
             * currentCol[l]);
             */
          } else {
    ...
    while (b2.back != b2) {
      lc--;
      // System.out.println("At " + lc + " state=" + b2.state.getName() + ", score=" + b2.score + ",
      // back=" + b2.back);
     ...
     * System.out.println("Added gaps: " + gaps); System.out.println("Gapped view has length " +
     * symView.length());
     */

    unlockModel();
    return new SimpleStatePath(bestScore, symView,
