...
public class SimpleMarkovModel extends AbstractChangeable implements MarkovModel, Serializable {
  ...
  protected ChangeSupport getChangeSupport(ChangeType ct) {
    ...
    if ((MarkovModel.PARAMETER.isMatchingType(ct) || ct.isMatchingType(MarkovModel.PARAMETER))
        && distForwarder == null) {
  ...
  public Distribution getWeights(State source) throws IllegalSymbolException {
    ...
    if (dist == null) {
      throw new BioError("Model does contain " + source.getName()
   ...
   * <p>
   * By default, the distribution P(destination | source) is a totally free distribution. This
   * allows the different probabilities to vary. If you wish to change this behaviour (for example,
   * to make one set of transition probabilities equal to another), then use this method to replace
   * the Distribution with one of your own.
   *
   ...
   * @throws IllegalAlphabetException if dist is not a distribution over the states returned by
   *         model.transitionsFrom(source)
  ...
  public void setWeights(State source, Distribution dist)
    ...
    if (!dist.getAlphabet().equals(ta)) {
      throw new IllegalAlphabetException("Can't set distribution from state " + source.getName()
          + " as the distribution alphabet is not the alphabet of transitions: " + ta.getName()
          + " and " + dist.getAlphabet().getName());
    }
    ...
    if (!hasListeners()) {
      ...
      synchronized (cs) {
        ChangeEvent ce =
            new ChangeEvent(this, MarkovModel.PARAMETER, dist, transWeights.get(source));
  ...
  public void destroyTransition(State from, State to)
    ...
    if (w != 0.0 && !Double.isNaN(w)) {
      throw new ChangeVetoException(ce, "Can't remove transition as its weight is not zero or NaN: "
  ...
  public FiniteAlphabet transitionsFrom(State from) throws IllegalSymbolException {
    ...
    if (s == null) {
      throw new BioError("State " + from.getName() + " is known in states "
          + stateAlphabet().getName() + " but is not listed in the transFrom table");
    }
    return s;
  }
  ...
  public FiniteAlphabet transitionsTo(State to) throws IllegalSymbolException {
    ...
    if (s == null) {
      throw new BioError("State " + to + " is known in states " + stateAlphabet().getName()
  ...
  public void addState(State toAdd) throws IllegalSymbolException, ChangeVetoException {
    ...
    if (toAdd instanceof EmissionState) {
      ...
      if (esh != heads()) {
        throw new IllegalSymbolException("This model " + stateAlphabet().getName() + " has "
            + heads() + " heads, but the state " + toAdd.getName() + " has " + esh + " heads");
      }
    }
    ...
    if (toAdd instanceof ModelInState) {
      ...
      if (esh != heads()) {
        throw new IllegalSymbolException(
            "This model " + stateAlphabet().getName() + " has " + heads()
                + " heads, but the model-in-state " + toAdd.getName() + " has " + esh + " heads");
      }
    }
  ...
  public void removeState(State toGo)
    ...
    if ((t = transitionsFrom(toGo)).size() != 0) {
      throw new IllegalTransitionException(toGo, (State) t.iterator().next(),
          "You can not remove a state untill all transitions to and from it "
              + "have been destroyed");
    }

    if ((t = transitionsTo(toGo)).size() != 0) {
      throw new IllegalTransitionException((State) t.iterator().next(), toGo,
          "You can not remove a state untill all transitions to and from it "
              + "have been destroyed");
    }
  ...
  private void doRemoveState(State toGo) throws IllegalSymbolException, ChangeVetoException {
