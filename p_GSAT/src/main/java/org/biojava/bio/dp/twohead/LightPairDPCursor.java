 ...
 * <p>
 * This object manages memory that is linear on the length of the shortest sequence. It does not
 * maintain any data beyond that necessary for the next round of calcCell invocations.
 *
...
public class LightPairDPCursor implements PairDPCursor {
   ...
   * @param depth1 The number of bases of context required in first sequence to compute DP matrix at
   *        a point (= max advance in first sequence + 1).
   * @param depth2 The number of bases of context required in second sequence to compute DP matrix
   *        at a point (= max advance in second sequence + 1).
   ...
   */
  public LightPairDPCursor(SymbolList seq1, SymbolList seq2,
      // CrossProductAlphabet alpha,
      int depth1, int depth2, int numStates, EmissionCache eCache) throws IllegalSymbolException {
    ...
    if (flip) {
      this.columns = new double[depth[0]][seqs[1].length() + 2][numStates];
      this.emissions = new double[depth[0]][seqs[1].length() + 2][];
      this.bPointers = new BackPointer[depth[0]][seqs[1].length() + 2][numStates];
    } else {
      this.columns = new double[depth[1]][seqs[0].length() + 2][numStates];
      this.emissions = new double[depth[1]][seqs[0].length() + 2][];
      this.bPointers = new BackPointer[depth[1]][seqs[0].length() + 2][numStates];
    }
  ...
  public boolean hasNext() {
    int i = flip ? 0 : 1;
    return pos[i] <= seqs[i].length() + 1;
  }


  /**
   * Returns the minimal context of the DP matrix necessary to compute the value of a single point
   * in that matrix.
   * <p>
   * The Cell [][] array has the origin as the current point to be evaluated and successive
   * rows/columns represent rows/columns backwards within the DP matrix.
   *
  ...
  private void calcEmissions(double[][] em) throws IllegalSymbolException {
    if (flip) {
      ...
      int i = pos[0];
      symL[0] =
          (i < 1 || i > seqs[0].length()) ? AlphabetManager.getGapSymbol() : seqs[0].symbolAt(i);
      for (int j = 0; j <= seqs[1].length() + 1; j++) {
        symL[1] =
            (j < 1 || j > seqs[1].length()) ? AlphabetManager.getGapSymbol() : seqs[1].symbolAt(j);
        em[j] = eCache.getEmissions(symList,
            !((i < 1 && j < 1) || ((i > seqs[0].length()) && (j > seqs[1].length()))));
        // System.out.println("symbol " + symL[0].getName() + ", " + symL[1].getName() + "->" +
        // em[j] + " " + (( i < 1 && j < 1 ) || ((i > seqs[0].length()) && (j > seqs[1].length())) )
        // );
      }
    } else {
      // System.out.println("Calculating emissions at " + pos[0] + ", " + pos[1]);
      ...
      for (int i = 0; i <= seqs[0].length() + 1; i++) {
        symL[0] =
            (i < 1 || i > seqs[0].length()) ? AlphabetManager.getGapSymbol() : seqs[0].symbolAt(i);
        // System.out.println("symbol " + symL[0].getName() + ", " + symL[1].getName() + " " + (( i
        // < 1 && j < 1 ) || ((i > seqs[0].length()) && (j > seqs[1].length())) ) );
