...
public class EmissionCache implements Serializable {
   ...
   * The scores will be computed and cached if not already available.
   * <p>
   * The correct behaviour should be to exclude emission from the MagicalState except at the origin
   * and bottom-right of the DP matrix. As such, the emission vector is only cached when it it is
   * computed for exorcise set to true. The vector is computed afresh every time when exorcise is
   * false. it should be noted that if exorcise is <b>NOT<b> set to false with the emission vector
   * at the ends of the matrix, the model will fail as the start and and end transitions become
   * impossible.
   *
   * @param symList a list of the symbols in each head that require a lookup for emission
   *        probabilities.
  ...
  private double[] computeEmissions(List symList, boolean exorcise) throws IllegalSymbolException {
    ...
    sym[1][1] = alpha
        .getSymbol(Arrays.asList(new Symbol[] {(Symbol) symList.get(0), (Symbol) symList.get(1)}));
    sym[1][0] = alpha.getSymbol(Arrays.asList(new Symbol[] {(Symbol) symList.get(0), gap[1]}));
    sym[0][1] = alpha.getSymbol(Arrays.asList(new Symbol[] {gap[0], (Symbol) symList.get(1)}));
