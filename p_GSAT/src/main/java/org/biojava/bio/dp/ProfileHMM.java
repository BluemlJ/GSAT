...
public class ProfileHMM extends SimpleMarkovModel {
   ...
   * <p>
   * The first match state is at index 1, and the last match state is at column columns(). The
   * states at index 0 and columns()+1 are both the magical state. This is so that the whole model
   * backbone can be addressed without writing lots of special-case code.
   *
   ...
   * <p>
   * Insert_0 is the insert that is accessible directly from the magical state. Insert_1..columns()
   * are 'above' each propper match state. There is no insert state above the magical state at the
   * end of the model, as insert_columns already models trailing inserts.
   *
   ...
   * <p>
   * Delete states are 'above' the match state that they delete. There is no delete state for the
   * magical state at either the beginning or end of the model, so the delete state indx can range
   * within (1..columns()).
   *
   * @param indx the index of the column to retrieve the insert state for
   * @return the insert state for column indx
   ...
   * @deprecated
   */
  public ProfileHMM(Alphabet alpha, int columns, DistributionFactory matchFactory,
   ...
   * <p>
   * The profile will be over the Alphabet alpha. It will have 'columns' match states, 'columns'
   * delete states and 'columns'+1 insert states. The match states will be created by matchFactory,
   * and the insert states will be created by insertFactory. This gives you great freedom for
   * changing how the states are implemented. For example, insertFactory may always return views
   * onto a single underlying probability distribution in which case all insert states will emit the
   * same type of stuff - or it may create an individual state for each insert state, in which case
   * each insert can be a different type of thing. You could make the insert state a view onto your
   * null model, or onto a protein-specific distribution.
   *
   ...
   */
  public ProfileHMM(Alphabet alpha, int columns, DistributionFactory matchFactory,
      DistributionFactory insertFactory, String name)
      throws IllegalSymbolException, IllegalTransitionException, IllegalAlphabetException {
    super(1, alpha, name);
    ...
    try {
      ...
      EmissionState iO = new SimpleEmissionState("i-0", Annotation.EMPTY_ANNOTATION, advance,
      ...
      for (int i = 1; i <= columns; i++) {
        EmissionState mN = new SimpleEmissionState("m-" + i, Annotation.EMPTY_ANNOTATION, advance,
            matchFactory.createDistribution(alpha));
        EmissionState iN = new SimpleEmissionState("i-" + i, Annotation.EMPTY_ANNOTATION, advance,
            insertFactory.createDistribution(alpha));
  ...
  protected void connectModel() throws ChangeVetoException, IllegalSymbolException,
