...
public abstract class AbstractMatrixPairDPCursor implements PairDPCursor {
  ...
  public AbstractMatrixPairDPCursor(SymbolList seq1, SymbolList seq2, int start1, int start2,
      int depth1, int depth2, PairDPMatrix matrix, EmissionCache eCache)
    ...
    for (int i = 0; i <= seq1.length() + 1; i++) {
      symArray[0] =
          (i < 1 || i > seq1.length()) ? AlphabetManager.getGapSymbol() : seq1.symbolAt(i);
      double[][] ei = emissions[i];
      for (int j = 0; j <= seq2.length() + 1; j++) {
        symArray[1] =
            (j < 1 || j > seq2.length()) ? AlphabetManager.getGapSymbol() : seq2.symbolAt(j);
        ei[j] = eCache.getEmissions(symList,
            !((i < 1 && j < 1) || (i > seq1.length() && j <= seq2.length())));
      }
    }
  }
