...
/**
 * Algorithms for dynamic programming (alignments) between pairs of SymbolLists. Based on a
 * single-head DP implementation by Matt Pocock.
 *
 * @author Thomas Down
...
public class PairwiseDP extends DP implements Serializable {
  ...
  public PairwiseDP(MarkovModel mm, CellCalculatorFactoryMaker ccfm)
    ...
    emissionCache.put(ScoreType.PROBABILITY,
        new EmissionCache(alpha, getStates(), getDotStatesIndex(), ScoreType.PROBABILITY));
    emissionCache.put(ScoreType.ODDS,
        new EmissionCache(alpha, getStates(), getDotStatesIndex(), ScoreType.ODDS));
    emissionCache.put(ScoreType.NULL_MODEL,
        new EmissionCache(alpha, getStates(), getDotStatesIndex(), ScoreType.NULL_MODEL));
  ...
  public DPMatrix backwardMatrix(SymbolList[] seqs, ScoreType scoreType)
    ...
    PairDPCursor cursor =
        new BackMatrixPairDPCursor(seqs[0], seqs[1], 2, 2, matrix, getEmissionCache(scoreType));
  ...
  public double forward(SymbolList[] seqs, ScoreType scoreType)
    ...
    lockModel();
    PairDPCursor cursor = new LightPairDPCursor(seqs[0], seqs[1], 2, 2, getStates().length,
  ...
  public DPMatrix forwardMatrix(SymbolList[] seqs, ScoreType scoreType)
    ...
    PairDPMatrix matrix = new PairDPMatrix(this, seqs[0], seqs[1]);
    PairDPCursor cursor =
        new MatrixPairDPCursor(seqs[0], seqs[1], 2, 2, matrix, getEmissionCache(scoreType));
    CellCalculator cc = ccFactory.forwards(scoreType);
    double score = runFB(cursor, cc);
    matrix.setScore(score);
    unlockModel();
  ...
  public StatePath viterbi(SymbolList[] seqs, ScoreType scoreType)
    ...
    PairDPCursor cursor =
        new LightPairDPCursor(seq0, seq1, 2, 2, getStates().length, getEmissionCache(scoreType));
    ...
    for (BackPointer bpi = bp.back; bpi != TERMINAL_BP; bpi = bpi.back) {
      try {
        ...
        /*
         * System.out.print( "Processing " + bpi.state.getName() );
         */
