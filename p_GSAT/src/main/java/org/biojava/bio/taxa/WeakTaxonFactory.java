 ...
 *
 * <p>
 * This implementation holds only weak references to the Taxon instances it knows about. This means
 * that WeakTaxonFactory may not be appropriate for situations where you wish to browse the taxon
 * tree. It does, however, mean that massive taxa can be represented, by effectively reflecting the
...
public class WeakTaxonFactory implements TaxonFactory {
  /**
   * The TaxonFactory that the biojava system should use for storing the taxonomy used by swissprot
   * and embl as in-memory objects.
   */
