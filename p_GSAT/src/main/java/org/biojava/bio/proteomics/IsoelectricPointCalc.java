...
/**
 * Class that computes isoelectric point for denaturated proteins. These pIs are useful for
 * predicting the position of a protein on a 2D gel.
 * <p>
 * The pK values are taken from Bjellqvist B. et al., "Reference points for comparisons of
 * two-dimensional maps of proteins from different human cell types defined in a pH scale where
...
public class IsoelectricPointCalc {
  ...
  public IsoelectricPointCalc() {
    ...
    SymbolPropertyTable PK_NtermTable =
        ProteinTools.getSymbolPropertyTable(SymbolPropertyTable.PK_Nterm);
    SymbolPropertyTable pKTable = ProteinTools.getSymbolPropertyTable(SymbolPropertyTable.PK);
    SymbolPropertyTable PK_CtermTable =
        ProteinTools.getSymbolPropertyTable(SymbolPropertyTable.PK_Cterm);
  ...
  public double getPI(SymbolList peptide, boolean hasFreeNTerm, boolean hasFreeCTerm)
    ...
    if ((peptide.getAlphabet() == ProteinTools.getTAlphabet())
      ...
      try {
        ...
        BioException ex2 = new BioException(
            "Error, the peptide probably contains only positive or negative charges");
        ex2.initCause(ex);
        throw ex2;
      }
      return pI;
  ...
  /**
   * Static public method to compute the pI for a polypeptide in denaturating and reduced conditions
   * with both free ends. Various ambiguity symbols, symbols for which pK data are not available, or
   * illegal symbols are not contributing to the calculated pI.
   * <p>
