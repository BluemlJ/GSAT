...
/**
 * <code>MassCalc</code> calculates the mass of peptides which for our purposes are
 * <code>SymbolList</code>s which contain <code>Symbol</code>sfrom the protein
 * <code>Alphabet</code>. It uses the mono-isotopic and average-isotopic masses identical to those
 * specified at www.micromass.co.uk Note: This class does not handle selenocysteine and pyrrolysine.
 *
...
public class MassCalc {
  ...
  /*
   * If instance methods are being used this will store the isotopically and MH_PLUS correct
   * terminal mass to be created. Need to be able to handle N and C term mods in the future.
   */
   ...
   * 
   * @param isotopicType The type of isotopes to calculate. Either mono isotopic or average
   * isotopic. It realy is the name of SymbolProperty table. Ex. SymbolPropertyTable.AVG_MASS or
   ...
   *
   * @param isotopicType a <code>String</code>. The type of isotopes to calculate. Either mono
   *        isotopic or average isotopic. Acceptable values are
   *        <code>SymbolPropertyTable.AVG_MASS</code> or <code>SymbolPropertyTable.MONO_MASS</code>.
  ...
  /**
   * Use this to set a post translational modification for the <code>Symbol</code> represented by
   * this character. It will only affect the current <code>MassCalc</code> instance and will not
   * affect the static method.
   *
   * @param symbolToken a <code>char</code> representing a <code>Symbol</code>.
   * @param mass a <code>double</code> to be the new mass of the residue.
   *
   * @exception IllegalSymbolException if the <code>Symbol</code> is not recognised.
   */
  ...
  /**
   * Add Variable modifications. If multiple masses are set by this method more then one mass will
   * be returned for a mass calculation. For example if a peptide contains two Mets and the user
   * sets the native and oxidized mass for the Met then the masses returned will be of the peptide
   * with 0, 1 and 2 modified Mets.
   * 
   ...
   * with 0, 1 and 2 modified Mets.
   */
  public void addVariableModification(Symbol residue, double[] masses)
      throws IllegalSymbolException {
  ...
  /**
   * Calculate the molecular weight of a protein, making estimates whenever it is possible like
   * averaging mass values for ambiguity symbols or counting zero when gaps are encountered. The
   * method is tolerant for ambiguity symbols as long as they can be resolved to a series of atomic
   * symbols whose mass is available in the ResidueProperties.xml configuration file or they are
   * gaps. The method returns the same value as getMass(SymbolList proteinSeq,
   * SymbolPropertyTable.AVG_MASS, false) when only atomic symbols are found in the polypeptide.
   *
   * @since 1.5
   */
  public static final double getMolecularWeight(SymbolList proteinSeq)
    ...
    SymbolPropertyTable sPT = ProteinTools.getSymbolPropertyTable(SymbolPropertyTable.AVG_MASS);
  ...
  /**
   * <code>getMass</code> calculates the mass of this peptide. This only works for the values in the
   * ResidueProperties.xml configuration file. It is probably slightly faster than the instance
   * method, but it does not handle post-translational modifications.
   *
   * @param proteinSeq a <code>SymbolList</code> whose mass is to be calculated. This should use the
   *        protein alphabet.
   * @param isotopicType a <code>String</code> The type of isotopes to calculate. Either mono
   *        isotopic or average isotopic. Acceptable values are
   *        <code>SymbolPropertyTable.AVG_MASS</code> or <code>SymbolPropertyTable.MONO_MASS</code>.
   * @param MH_PLUS a <code>boolean</code> true if the value needed is the MH+ mass.
   *
   * @return a <code>double</code> mass of the peptide.
   *
   * @exception IllegalSymbolException if the <code>SymbolList</code> contains illegal
   *            <code>Symbol</code>s.
   */
  ...
  /**
   * Get the Mass of this peptide. Use this if you want to set fixed modifications and have created
   * an instance of MassCalc. The value is calculated using the value of MH_PLUS defined in the
    ...
    for (Iterator it = proteinSeq.iterator(); it.hasNext();) {
      ...
      if (!symbolPropertyMap.containsKey(s)) {
        throw new IllegalSymbolException(s,
            "The mass of the symbol " + s.getName() + " is unknown");
      }
  ...
  /**
   * Get all masses including the variable mass. Allgorythm
   *
   * 1 Get the first residue of the sequence create a list of all the standard and non-standard
   * massses for this reidue for each residue mass goto 1 with the sequence of all residues after
   * the current residue add the residue mass to each mass from 1 to the list
   *
   *
  ...
  /**
   * <code>getTermMass</code> returns the terminal mass being used by the instance methods.
   *
