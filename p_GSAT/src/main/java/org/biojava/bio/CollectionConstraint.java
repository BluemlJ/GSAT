...
/**
 * Used by <code>AnnotationType</code> to represent the constraint on the collection of values in a
 * property-slot. CollectionConstraints usually use a <code>PropertyConstraint</code> to validate
 * the individual elements.
 *
 * 
 * Use one or more of the built-in implementations to build new <code>AnnotationTypes</code>.
 *
 * @since 1.3
 * @author Thomas Down
...
public interface CollectionConstraint {
   ...
   *
   *         powerUser Manually compare items with the CollectionConstraint. Node: this will ususaly
   *         be done for you in an AnnotationType instance
   */
   ...
   * example, <code>CollectionConstraint.ByClass</code> will infer subConstraintOf by looking at the
   ...
   *
   * 
   *         Usefull when attempting to compare two constraints to see if it is necisary to retain
   *         both. You may want to check the more general or the more specific constraint only.
   */
  public boolean subConstraintOf(CollectionConstraint subConstraint);


  /**
   * Return <code>true</code> iff the Collection formed by adding <code>newValue</code> to
   * <code>current</code> would be accepted by this constraint.
   *
   * 
   * Implementations may <em>not</em> assume that <code>current</code> is valid.
   *
   * @param current a Collection containing the current values
   * @param newValue the new value to add
   * @return true if adding the new value will result in an acceptable property
   */

  public boolean validateAddValue(Collection current, Object newValue);

  /**
   * Return <code>true</code> iff the Collection formed by removing <code>newValue</code> from
   * <code>current</code> would be accepted by this constraint.
   *
   * 
   * Implementations may <em>not</em> assume that <code>current</code> is valid. However,
   * <code>current</code> will already have been checked to ensure that it contains
   * <code>victim</code>.
   *
   * @param current a Collection containing the current values
   * @param victim the value to remove
   * @return true if removing the victim will result in an acceptable property value set
   */
  ...
  public static final CollectionConstraint ANY =
      new AllValuesIn(PropertyConstraint.ANY, CardinalityConstraint.ANY);

  /**
   * <code>EMPTY</code> is a constraint which only accepts the empty set.
   * 
   * Use this to indicate that a property must be undefined
   */

  public static final CollectionConstraint EMPTY =
      new AllValuesIn(PropertyConstraint.NONE, CardinalityConstraint.ZERO);

  /**
   * <code>NONE</code> is a constraint which accepts no value for a property under any condition.
   *
   * This value indicates an impossible condition. It may be returned by methods such as
   * <code>AnnotationTools.intersection</code> to indicate that <code>NO</code> values of a property
   * (include undefined) are valid.
   */
  public static final CollectionConstraint NONE = new NoneCollectionConstraint();

  /**
   * CollectionConstraint which validates all members of a Collection. All members must be vaild
   * according to the supplied <code>PropertyConstraint</code>, and the total number of members must
   * be acceptable by the given cardinality constraint.
   *
   * @author Thomas Down
   * @author Matthew Pocock
   */
  ...
  public class AllValuesIn implements CollectionConstraint {
     ...
     * @param card the cardinality constraint restricting the number of values
     */
  ...
  /**
   * CollectionConstraint which validates a portion of a Collection. Accepts only collections where
   * the number of members matching the <code>PropertyConstraint</code> is in the supplied
   * cardinality.
   *
   * <p>
   * A typical application for this would be with Annotations where one property can contain a
   * number of synonyms. <code>CollectionConstraint.Contains</code> could be used as a query to
   * select instances based on one of these synonyms.
   * </p>
   *
   * @author Thomas Down
   */
  ...
  /**
   * A collection constraint that accpepts collections iff they are accepted by both child
   * constraints. This effectively matches the intersection of the items matched by the two
   * constraints.
   *
