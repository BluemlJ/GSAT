...
/**
 * Train a support vector machine using the Sequential Minimal Optimization algorithm. See Kernel
 * Methods book.
 *
 * @author Thomas Down
...
public class SMOTrainer {
  ...
  private boolean takeStep(SMOTrainingContext trainingContext, int i1, int i2) {
    ...
    if (eta < 0) {
      ...
      /*
       * 
       * double gamma = alpha1 + s*alpha2; double v1 = model.classify(model.getVector(i1)) +
       * model.getThreshold() - y1*alpha1*k11 - y2*alpha2*k12; double v2 =
       * model.classify(model.getVector(i2)) + model.getThreshold() - y1*alpha1*k12 - y2*alpha2*k22;
       * 
       * double Lobj = gamma - s * L + L - 0.5*k11*Math.pow(gamma - s*L,2) - 0.5*k22*Math.pow(L,2) -
       * s*k12*(gamma-s*L)*L-y1*(gamma-s*L) - y1*(gamma - s*L)*v1 - y2*L*v2; double Hobj = gamma - s
       * * H + H - 0.5*k11*Math.pow(gamma - s*H,2) - 0.5*k22*Math.pow(H,2) -
       * s*k12*(gamma-s*H)*H-y1*(gamma-s*H) - y1*(gamma - s*H)*v1 - y2*H*v2; if (Lobj >
       * Hobj+epsilon) { a2 = L; } else if (Lobj < Hobj-epsilon) { a2 = H; } else { a2 = alpha2; }
       */
      //// System.out.print("+");
      return false;
    }
    ...
    for (int l = 0; l < trainingContext.size(); ++l) {
      ...
      if (!trainingContext.isBound(trainingContext.getAlpha(l))) {
        trainingContext.updateError(l, y1 * (a1 - alpha1) * trainingContext.getKernelValue(i1, l)
            + y2 * (a2 - alpha2) * trainingContext.getKernelValue(i2, l) + bOLD - b);
      }
    }
  ...
  private int examineExample(SMOTrainingContext trainingContext, int i2) {
    ...
    if ((r2 < -epsilon && alpha2 < C) || (r2 > epsilon && alpha2 > 0)) {
      ...
      for (int l = 0; l < trainingContext.size(); ++l) {
        ...
        if (!trainingContext.isBound(trainingContext.getAlpha(i1))) {
