...
public class SparseVector implements Serializable {
   ...
   * <p>
   * This is the total number of non-zero dimensions. It is not equal to the number of the highest
   * indexed dimension.
   *
   ...
   * <p>
   * This silently handles the case when previously there was no value at dim. It does not contract
   * the sparse array if you set a value to zero.
   *
   ...
   * <p>
   * E.g., if the sparse vector had dimensions 5, 11, 12, 155 set to non-zero values then
   * <code>sv.getDimAtIndex(2)</code> would return 12.
   *
   ...
   * <p>
   * E.g., if the sparse vector contained the data 5->0.1, 11->100, 12->8.5, 155->-10 then
   * <code>sv.geValueAtIndex(2)</code> would return 8.5.
   *
   * @param indx the index
  ...
  public static class NormalizingKernel implements SVMKernel, Serializable {
     ...
     * <code>k(a, b) = sum_i ( a_i * b_i * nv_i )</code> <br>
     * where nv_i is the value of the normalizing vector at index i. This can be thought of as
     * scaling each vector at index i by <code>sqrt(nv_i)</code>.
     */
    public double evaluate(Object o1, Object o2) {
      SparseVector a = (SparseVector) o1;
      SparseVector b = (SparseVector) o2;
     ...
     * <p>
     * It will set up a normalizing vector that has weight that will scale each element so that the
     * average score is 1.
     */
