 ...
 * See "A Tutorial on Support Vector Regression" by Smola and Scholkopf.
 *
 * <p>
 * WARNING: This doesn't work right now -- use and fix (both at own risk...)
 *
...
public class SMORegressionTrainer {
  ...
  private boolean takeStep(int i1, int i2) {
    ...
    while (!finnished) {
      if (!case1 && (alpha1 > 0.0 || (alpha1star == 0.0 && deltaphi > 0.0))
        ...
        case1 = true;
      } else if (!case2 && (alpha1 > 0.0 || (alpha1star == 0.0 && deltaphi > 2.0 * epsilon))
        ...
        case2 = true;
      } else if (!case3 && (alpha1star > 0.0 || (alpha1 == 0.0 && deltaphi < 2.0 * epsilon))
        ...
        case3 = true;
      } else if (!case4 && (alpha1star > 0.0 || (alpha1 == 0 && deltaphi < 0.0))
      ...
      /*
       * 
       * deltaphi = phi2 - phi1 + eta * ( (alpha1 - alpha1star) + (alpha1old - alpha1starold) );
       * 
       */
    ...
    /*
     * 
     * if(!isBound(alpha1)) { b = phi1 + y1*(alpha1 - alpha1old)*k11 + y2*(alpha2 - alpha2old)*k12 +
     * bOld; } else if(!isBound(alpha2)) { b = phi2 + y1*(alpha1 - alpha1old)*k12 + y2*(alpha2 -
     * alpha2old)*k22 + bOld; } else if(!isBound(alpha1star)) { b = phi1 + y1*(alpha1star -
     * alpha1starold)*k11 + y2*(alpha2star - alpha2starold)*k12 + bOld; } else
     * if(!isBound(alpha2star)) { b = phi2 + y1*(alpha1star - alpha1starold)*k12 + y2*(alpha2star -
     * alpha2starold)*k22 + bOld; } else { // no suitable alpha found to infer b - take middle of
     * allowed interval double b1 = phi1 + y1*(alpha1 - alpha1old)*k11 + y2*(alpha2 - alpha2old)*k12
     * + bOld; double b2 = phi2 + y1*(alpha1 - alpha1old)*k12 + y2*(alpha2 - alpha2old)*k22 + bOld;
     * b = (b1 + b2) / 2.0; } model.setThreshold(b); System.out.println("b is " + b);
     * 
     */
    ...
    // Update error cache
    /*
     * E[i1] = 0; E[i2] = 0; for (int l = 0; l < E.length; ++l) { if (l==i1 || l==i2) { continue; }
     * if (!(isBound(model.getAlpha(l)))) { E[l] += y1*(a1-alpha1)*model.getKernelValue(i1, l) +
     * y2*(a2-alpha2)*model.getKernelValue(i2, l) + bOld - b; } }
     */
  ...
  private int examineExample(int i2) {
    ...
    /*
     * 
     * if (Math.abs(phi2) < epsilon) return 0;
     * 
     */

    if ((+phi2 < epsilon && alpha2star < C) || (+phi2 < epsilon && alpha2star > 0.0)
        || (-phi2 > epsilon && alpha2 < C) || (-phi2 > epsilon && alpha2 > 0.0)) {

      /*
       * 
       * int secondChoice = -1; double step = 0.0; for (int l = 0; l < model.size(); ++l) { if
       * (!isBound(model.getAlpha(l))) { double thisStep = Math.abs(getError(l) - phi2); if
       * (thisStep > step) { step = thisStep; secondChoice = l; } } }
       * 
       * if (secondChoice >= 0) { System.out.println("Using secondChoice heuristic for " +
       * secondChoice + ", " + i2); if (takeStep(secondChoice, i2)) { return 1; } }
       * 
       * int randomStart = (int) Math.floor(Math.random() * model.size()); for (int l = 0; l <
       * model.size(); ++l) { int i1 = (l + randomStart) % model.size(); if
       * (!isBound(model.getAlpha(i1))) { System.out.println("Using unbound huristic for " + i1 +
       * ", " + i2); if (takeStep(i1, i2)) { return 1; } } }
       * 
       */
