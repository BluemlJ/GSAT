 ...
 *
 * <p>
 * This is just a set of Term objects, and a set of Triple objects describing relationships between
 * these terms. This class does not itself contain any reasoning functionality. Ontology is a
 * collection of facts, or axioms.
 * </p>
 *
 * @author Thomas Down
...
public interface Ontology extends Changeable {
  public static final ChangeType TERM =
      new ChangeType("A term has been added or removed", "org.biojava.ontology.Ontology", "TERM");

  public static final ChangeType TRIPLE = new ChangeType("A triple has been added or removed",
      "org.biojava.ontology.Ontology", "TRIPLE");
  ...
  /**
   * Return all triples from this ontology which match the supplied pattern. If any of the
   * parameters of this method are <code>null</code>, they are treated as wildcards.
   *
   ...
   *
   * This method should be implemented by ontology implementors to allow OntoTools to get optimized
   * access to some usefull ontology operations. It is not intended that users will ever invoke
   * this. A sensible dumb implementation of this would return a per-ontology instance of
   * DefaultOps.
   *
   ...
   * @throws IllegalArgumentException if either <code>name</code> or <code>description</code> is
   *         <code>null</code>, or violates some other constraint of this implementation.
  ...
  /**
   * Create a view of a term from another ontology. If the requested term has already been imported
   * under that name, this method returns the existing RemoteTerm object. If the term that is being
   * imported is itself a RemoteTerm instance then first unwrap the term back to the orriginal term
   * it represents and then produce a RemoteTerm from that. If the term being imported orriginated
   * from this ontology, then return that term unaltered.
   *
   ...
   * @throws AlreadyExistsException if a triple already exists with the same subject, object and
   *         predicate, regardless of the name and description
   * @throws ChangeVetoException
   * @throws NullPointerException if subject, object or predicate are null
   * @throws IllegalArgumentException if subject, object or predicate are not all from the same
   *         ontology
   */
  ...
  public final class Impl extends AbstractChangeable implements Ontology, java.io.Serializable {
    /**
     * 
     */
    ...
    public Set<Triple> getTriples(Term subject, Term object, Term predicate) {
      if (subject != null && subject.getOntology() != this) {
        throw new IllegalArgumentException(
            "Subject is not in this ontology: " + subject + " " + this);
      }

      if (object != null && object.getOntology() != this) {
        throw new IllegalArgumentException(
            "Object is not in this ontology: " + object + " " + this);
      }

      if (predicate != null && predicate.getOntology() != this) {
        throw new IllegalArgumentException(
            "Predicate is not in this ontology: " + predicate + " " + this);
      }
    ...
    private void addTerm(Term t)
      ...
      if (terms.containsKey(t.getName())) {
        throw new AlreadyExistsException(
            "Ontology " + getName() + " already contains " + t.toString());
      }

      if (!hasListeners()) {
        terms.put(t.getName(), t);
      } else {
        ChangeEvent ce = new ChangeEvent(this, Ontology.TERM, t, null);
    ...
    public void deleteTerm(Term t) throws ChangeVetoException {
      ...
      if (!hasListeners()) {
        ...
        ChangeEvent ce = new ChangeEvent(this, Ontology.TERM, null, t);
        ChangeSupport cs = getChangeSupport(Ontology.TERM);
        synchronized (cs) {
          cs.firePreChangeEvent(ce);
          terms.remove(name);
    ...
    public Triple createTriple(Term subject, Term object, Term predicate, String name,
        String description) throws AlreadyExistsException, IllegalArgumentException,
        ChangeVetoException, NullPointerException, IllegalArgumentException {
      ...
      if (!containsTerm(predicate)) {
        throw new IllegalArgumentException(
            "Ontology " + getName() + " doesn't contain " + predicate);
      }
      ...
      if (!hasListeners()) {
        ...
        ChangeEvent ce = new ChangeEvent(this, Ontology.TRIPLE, t, null);
