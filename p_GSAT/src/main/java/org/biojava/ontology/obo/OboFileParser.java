...
/**
 * A class to parse the content of an OBO file. It delegates handling of the content to the
 * OBOFileEventListener implementation.
 * 
 * This file contains parts of the OBO-Edit file OBOParseEngine, (particularly the encoding and
 * decoding part)
 * 
...
public class OboFileParser {
  ...
  public void parseOBO(BufferedReader oboFile) throws IOException, ParseException {
    ...
    while ((line = oboFile.readLine()) != null) {
      ...
      if (line.charAt(0) == '[') {
        ...
        if (stanzaname.length() < 1) throw new ParseException("Empty stanza: \"" + line + "\"");
        ...
        /*
         * if (nv != null) System.err.println("nv = "+nv+", value = |"+value+"|");
         */
        if (value.length() == 0) throw new ParseException("Tag found with no value " + line);
  ...
  private boolean isSynonym(String key) {
    if (key.equals(OboFileHandler.SYNONYM) || key.equals(OboFileHandler.EXACT_SYNONYM)) return true;
    return false;
  }
  ...
  private Synonym parseSynonym(String key, String value) throws ParseException {
    ...
    if (startIndex == -1) throw new ParseException("Expected \"" + line + " " + linenum);
    SOPair p = unescape(value, '"', startIndex + 1, value.length(), true);
    int defIndex = findUnescaped(value, '[', p.index, value.length());
    if (defIndex == -1) {
      throw new ParseException(
          "Badly formatted synonym. " + "No dbxref list found." + line + " " + linenum);
    }
    ...
    else if (key.equals(OboFileHandler.NARROW_SYNONYM)) scope = Synonym.NARROW_SYNONYM;
    ...
    for (int i = 0; tokenizer.hasMoreTokens(); i++) {
      ...
      if (i == 0) {
        if (token.equals("RELATED"))
          ...
          throw new ParseException("Found unexpected scope " + "identifier " + token + line);
      } else if (i == 1) {
        catID = token;
      } else
        throw new ParseException("Expected dbxref list," + " instead found " + token + line);
    }
    ...
    // System.out.println("SYNONYM: " + p.str +" " + synonym.getCategory() + " " +
    // synonym.getScope());
  ...
  protected Map<String, Object>[] getDbxrefList(String line, int startoffset, int endoffset)
    ...
    while (!stop) {
      ...
      if (trailing) {
        ...
        if (endIndex == -1) {
          throw new ParseException("Badly formatted " + "trailing properties " + line);
        }
        pair.put("nv", nv);
      }
  ...
  public SOPair unescape(String str, char toChar, int startindex, boolean mustFindChar)
      throws ParseException {
    return unescape(str, toChar, startindex, str.length(), mustFindChar);
  }

  public SOPair unescape(String str, char toChar, int startindex, int endindex,
    ...
    for (int i = startindex; i < endindex; i++) {
      ...
      if (c == '\\') {
        i++;
        c = str.charAt(i);
        Character mapchar = escapeChars.get(new Character(c));
        if (mapchar == null)
          throw new ParseException("Unrecognized escape" + " character " + c + " found.");
        out.append(mapchar);
      } else if (c == toChar) {
        endValue = i;
        break;
  ...
  protected SOPair readQuotedString(String value, int startIndex, int stopIndex,
      char terminatingChar, boolean requireQuotes, boolean legalEndOfLine) throws ParseException {
    ...
    for (; i < stopIndex; i++) {
      ...
      // proceed in non-quoted mode
        ...
        if (requireQuotes) throw new ParseException(
            "Expected start of quoted string. " + line + " " + value + " at linenr " + linenum);
        useQuotes = false;
        break;
      } else {
        useQuotes = true;
    ...
    for (; i < stopIndex; i++) {
      if (isEscapeStarter(value.charAt(i))) {
        i++;
        if (i >= value.length()) throw new ParseException("Incomplete escape sequence. " + line);
  ...
  protected int getNestedValue(NestedValue nv, String str, int startIndex) throws ParseException {
    while (startIndex < str.length()) {
      int equalsIndex = findUnescaped(str, '=', startIndex, str.length());
      if (equalsIndex == -1) throw new ParseException("Expected = in trailing modifier " + line);
      ...
      for (; startIndex < str.length(); startIndex++) {
        if (Character.isWhitespace(str.charAt(startIndex)))
          ...
          throw new ParseException(
              "Expected comma in trailing modifier. " + line + " linenr: " + linenum);
        }
      }
    }
    return str.length();
