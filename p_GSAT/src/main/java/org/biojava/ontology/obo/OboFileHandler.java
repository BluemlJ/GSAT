      ...
      if (currentTerm == null) {
        ...
        try {
          Term object = ontology.containsTerm(value)
              ? object = ontology.getTerm(value)
              : ontology.createTerm(value);
          Term predicate =
              ontology.containsTerm(key) ? ontology.getTerm(key) : ontology.createTerm(key);
          ontology.createTriple(currentTerm, object, predicate,
              currentTerm + " " + predicate + " " + object, key + "-relationship");
        } catch (AlreadyExistsException ex) {}
