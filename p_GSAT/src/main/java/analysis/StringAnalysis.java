...
public class StringAnalysis {
    ...
    /**
     * A Map with all possible DNA codons with the matched AminoAcid in short
     * form and the not so short but not the full name form.
     */
    ...
    /**
     * Appends a string to the given length by adding space characters at the
     * end if the String is already long e or longe nothing is done.
     * 
     * @param input
     *            the string we trim
     * @param length
    ...
    /**
     * This method checks if the complementary and/or the reverse gene has a
     * better similarity with the mutated sequence the the original gene. If
     * this so set reference gene of sequence to the best similarity.
     * 
     ...
     *             if complementary can't be build
     * @author jannis blueml
     * 
     */
    public static void checkComplementAndReverse(AnalysedSequence toAnalyse) throws CorruptedSequenceException {
	...
	double toTestcomp = checkSimilarity(toAnalyse.getComplementarySequence(),
		toAnalyse.getReferencedGene().getSequence());
	double toTestrev = checkSimilarity(toAnalyse.getReversedSequence(),
		toAnalyse.getReferencedGene().getSequence());
	double toTestcomprev = checkSimilarity(toAnalyse.getComplementarySequence(toAnalyse.getReversedSequence()),
		toAnalyse.getReferencedGene().getSequence());
    ...
    /**
     * This method is used when the gene is not the referenced gene in the
     * AnalysedSequence Note: This method overrides the old referenced gene with
     * the given gene
     * 
     * @param toAnalyse
     *            the sequence to check
     * @param gene
     *            the gene you reference to.
     * @throws CorruptedSequenceException
     *             If the sequence contains unknown symbols
     * @author jannis blueml
     */
    ...
    /**
     * Changes the sequence representation from nucleotides to aminoacid
     * (shortform).
     * 
     * @param nucleotides
     *            the sequence presented by nucleotides
     ...
     * @return returns the startposition of the HIS-Tags. This is relative to
     ...
     * 
     * @param toAnalyze
     *            The sequence, we compare with the list of genes
     * @param listOfGenes
     *            A list of all genes, we want to compare with the sequence
	...
	for (Gene gene : listOfGenes) {
	    // double similarity = StringAnalysis.checkSimilarity(toAnalyze,
	    // gene);
    ...
    /**
     * this method finds the stopcodon in the given sequence (parameter in form
     * of {@link AnalysedSequence}) and return it.
     * 
     * @param toAnalyze
     *            the {@link AnalysedSequence} we search in
    ...
    /**
     * gets Levenshtein index out of Levenshtein Matrix. The Index represents
     * the maximal disntance between the two Strings wich were used to generate
     * the matrix
     * 
     * @param matrix
     ...
     * Index. The Index represents the maximal distance between the two Strings
     * wich were used to generate the matrix
     * 
     * @param first
     *            The first String
     * @param second
    ...
    /**
     * cuts of the vector of the given sequence (trim vector) and sets the given
     * gene as reference Gene.
     * 
     * @see #trimVector(AnalysedSequence)
     * @param toAlign
     *            The AnalysedSequence to be analysed
     * @param gene
     *            The reference gen (as a template)
     * @author Kevin Otto
     */
    ...
    /**
     * cuts out the Vector off and writes it into the Left vector of the given
     * sequence. Also sets Offset.
     * 
     * @see #findOffset(AnalysedSequence)
     * 
     * @param toAlign
     *            The AnalysedSequence objects to be aligned.
     * 
     * @author Kevin Otto
     */
    ...
    /**
     * calculates the offset and writes it into the sequence. WARNING: does
     * change Offset value! returns false if begin of sequence was not found
     * Returning false may be an indicator for bad sequence, but may also be
     * perfectly fine
     * 
     * @param sequence
     *            The AnalysedSequence object to be analysed
     * 
