...
/**
 * 
 * <h1>This class is from an early version of our system. It only supports the
 * main feature: Finding mutations and writing the results to a .csv file.</h1>
 * <p>
 * 
 * </p>
 * This program version can be started with the console by adding the parameter
 * -c.
 * <p>
 * 
 * 
...
public class ConsoleVersion {
	 ...
	 * 
	 * @param destination
	 *            The destination path (where to store the results).
	 * @param validFiles
	 *            The list of valid AB1/ABI files.
	 * @param badFiles
	 *            The list of invalid files.
	 * @param configReport
	 *            A message on the level of success from reading the
	 *            configuration file.
	 * 
	 * @author Ben Kohr
	 * 
	 */
	public static void reportOnInput(String destination, LinkedList<File> validFiles, LinkedList<File> badFiles,
			String configReport) {
	...
	/**
	 * Starts the console version and runs the general analysis pipeline.
	 ...
	 *            The sequence which will be saved.
	 * @param file
	 *            The file where the sequence has been read. Used to retrieve
	 *            the file name.
	 * @param destinationPath
	 *            The path where the result will be saved.
	 * @author Ben Kohr
	 */
	private static void addLocalEntry(AnalysedSequence activeSequence, File file, String destinationPath) {
		try {
	...
	/**
	 * Asks the User for the path to the AB1 files and returns a list of the
	 * found files Also checks corectness of path and asks again if neccesary
	 * 
	 * @return A pair of all parsed files. The first list contains all correctly
	 *         parsed .abi files and the second list all other files.
	 * @author Kevin Otto
	 */
	...
	/**
	 * Asks user for a comment and sets the comment field of the referenced
	 * analysedSequence.
	 * 
	 * @param sequence
	 * @param file
	...
	private static void askForComment(AnalysedSequence sequence, File file) {
		try {
			sequence.setComments(ConsoleIO
					.readLine("Please enter a comment for file " + file.getName() + " or press ENTER to skip."));
	...
	/**
	 * Asks user for gene sequence and gene name and adds it to the local gene
	 * file.
	 * 
	...
	private static Gene askForGene() {
		...
		while (noGene) {
			try {
				...
				noGene = false;
				strGeneName = ConsoleIO.readLine(
						"Please enter the name of the gene." + System.lineSeparator() + "REFERENCE GENE NAME: ");
	...
	/**
	 * This method asks the user if they want a single output file or a file for
	 * each analyzed sequence.
	 * 
	 * @author Ben Kohr
	 */
	private static void askForOneOrMultipleFiles() {
		String message = "Would you like separate files for each input file?" + System.lineSeparator() + "y/n: ";
	 ...
	 * 
	 * @return Message indicating the level of success for reading the
	 *         configuration.
	 * 
	 * @author Ben Kohr
	 */
	 ...
	 * 
	 * @param sequence
	 *            The sequence which will be analyzed.
	 * @param file
	 *            File of the sequence, used only for the file name.
	 * @author Lovis Heindrich
	 */
	...
	/**
	 * Asks the user for a destination path where the results will be stored.
	 * The path is saved in FileSaver.
	 * 
	...
	/**
	 * Runs the complete analysis pipeline for a single sequence and adds the
	 * sequence to the saved files.
	 * 
	 * @param gene
	 *            The gene the sequence will be compared to.
	 * @param file
	 *            The file which contains the sequence.
	 * @param destinationPath
	...
	/**
	 * Reads genes from file and returns the correct gene. Genes must be in a
	 * .txt file named genes.txt in the same folder as the ab1 files. If
	 * genes.txt can not be found the user will be asked to enter a new gene.
	 * 
	...
	private static Gene readGene() {
		...
		try {
			...
			if (GeneHandler.getNumGenes() == 0) {
				...
				System.out.println();
				int index = Integer.parseInt(ConsoleIO.readLine(
						"Please enter the Index of the gene you want to use." + System.lineSeparator() + "INDEX: "))
						- 1;
	...
	/**
	 * Reads the sequence from a given File and prints error messages if
	 * necessary.
	 * 
	 * @param file
	 *            The file of the sequence which will be parsed.
