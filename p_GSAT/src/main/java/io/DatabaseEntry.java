...
public class DatabaseEntry {
	...
	/**
	 * The id for this entry. It is given by the DatabaseConnection class. Until
	 * the id is given, -1 is the placeholder id.
	 */
	private int id = -1;
	...
	/**
	 * Constructor that sets up all attributes except the id (which is given by
	 * the DatabaseConnection class).
	 * 
	 * @author Ben Kohr
	 */
	public DatabaseEntry(String fileName, int geneID, String sequence, String addingDate, String researcher,
			String comments, String vector, String promotor, boolean manuallyChecked, String mutation, boolean silent) {
	...
	/**
	 * This method converts a given sequence into a list of database entries to
	 * be stored and returns them.
	 * 
	 * @param seq
	 *            The sequence to be converted to database entries
	...
	public static LinkedList<DatabaseEntry> convertSequenceIntoEntries(AnalyzedSequence seq) {
		...
		// TODO mutations auftrennen
		// LinkedList<String> silentMutations = seq.getSilentMutations();

		// Collect normal mutations
		for (String mutation : mutations) {
			DatabaseEntry dbe = new DatabaseEntry(fileName, geneID, sequence, addingDate, researcher, comments, vector,
					promotor, manuallyChecked, mutation, false);
			entries.add(dbe);
		}
	...
	/**
	 * Sets the id of this entry. Typically used be DatabaseConnection to update
	 * the ids correctly.
	 * 
	 * @param id
	 *            The current id
	...
	/**
	 * Returns a boolean indicating whether this entry's mutation is silent or
	 * not
	 * 
