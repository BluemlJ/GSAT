...
/**
 * This class is responsible for storing primer data. Primers get synchronised
 * to a csv file which is stored in the user directory.
 * 
...
public class PrimerHandler {
	...
	/**
	 * Initializes the primer.txt file. Checks if the file exists and creates it
	 * otherwise.
	 * 
	 * @throws IOException
	 *             Error while reading file.
	 ...
	 * @param name
	 *            Name of the primer to be deleted.
	 * @throws IOException
	 *             Error while updating the txt file.
	 * @author Lovis Heindrich
	 */
	 ...
	 * @throws IOException
	 *             Error while writing the local file.
	 * @author Lovis Heindrich
	 */
	...
	/**
	 * Adds a primer to the list if it does not exist yet. Writes all primers to
	 * the txt file afterwards.
	 * 
	 * @param primer
	...
	public static Primer getPrimer(String name) {
		if (name.split(" ")[1].isEmpty()) {
			...
			return getPrimer(name.split(" ")[0], name.split(" ")[1].substring(1, name.split(" ")[1].length() - 1));
		}
		return null;
