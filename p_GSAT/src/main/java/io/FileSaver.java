...
/**
 * This class is used to store the analysis results in local files. It produces
 * comma separated value files (CSV) which can be read with Excel.
 * 
 * @author Ben Kohr
 */
public class FileSaver {
	...
	/**
	 * This is the file name if only one file is desired. If multiple files are
	 * desired, the name of the original AB1 file will be added to this name for
	 * each resulting file. This name can be changed by the user, the name below
	 * is the default name.
	 */
	private static String destinationFileName = "gsat_results";

	/**
	 * This is the class's separator char. It separates the columns of the
	 * resulting CSV file.
	 */
	private static final char SEPARATOR_CHAR = ConfigHandler.SEPARATOR_CHAR;

	/**
	 * Indicating whether this is the first call of the storage method in the
	 * current storage process. This field is necessary to keep up with the
	 * labeling.
	 */
	...
	/**
	 * Indicates whether one or multiple files shall be used for storage. By
	 * default, only one file will be generated.
	 */
	private static boolean separateFiles = false;

	/**
	 * Sets the path where local files shall be created. The String argument is
	 * converted into a File. If null is passed, the path is reset to null.
	 * 
	 * @param pathString
	 *            The path to create local files (as String)
	 * 
	 * @author Ben Kohr
	 */
	...
	/**
	 * The destination file name can be changed by the user. It is passed as a
	 * String argument. If the String is empty, the default name "gsat_results"
	 * will be used.
	 * 
	 * @param destFileName
	...
	/**
	 * Inserts the data of an analysed sequence into one or multiple local
	 * file(s). The name of the AB1 file is given as a parameter. It may be used
	 * to create the CSV file name (if separate files are desired).
	 * 
	 * @param ab1Filename
	 *            the name of the AB1 file the stored entries were obtained
	 *            from. If only one file is desired, then the name will not be
	 *            used.
	 * 
	 ...
	 *             If the path to store the data is not specified.
	 * @throws IOException
	 *             If the writing process fails (due to the used FileWriter).
	 * 
	 * @author Ben Kohr
	 */
	public static void storeResultsLocally(String ab1Filename, AnalysedSequence sequence)
		...
		if (separateFiles) {
			String finalName = localPath.getAbsolutePath() + File.separatorChar + destinationFileName + "_"
					+ ab1Filename + ".csv";
	...
	/**
	 * This methods converts the analysed sequence object passed to it into a
	 * CSV line containing all the information to store.
	 * 
	 * @param sequence
	 *            The analysed sequence object. It is assumed to contain all
	 *            data to be stored.
	 * 
	 * @return a line to insert into a CSV file (with the information of the
	 *         sequence)
	 * 
	 * @author Ben Kohr
	 */
	...
	/**
	 * This method initializes and initially uses a new writer, if only one file
	 * is desired.
	 *
	 * @see #getNewWriter(String, boolean)
	 * 
	 ...
	 * 
	 * @throws IOException
	 *             If the creation or the usage of the FileWriter object fails
	 * 
	 * @author Ben Kohr
	 */
	...
	/**
	 * This method creates a new writer for the current writing situation (one
	 * or several files?). If necessary, also adds the first line. It is
	 * directly called when separate files are needed. It is called via
	 * {@link #getNewWriterForOneFile()} if one file is desired.
	 * 
	 * @param ab1Filename
	 *            The name of the new file
	 * @param append
	 *            Shall content be added to the file (or is it a new file)?
	 * 
	 * @return the writer object, returned to continue writing
	 * 
	 * @throws IOException
	...
	private static FileWriter getNewWriter(String finalName, boolean append) throws IOException {
		...
		if (!append) {
			writer.write("file name" + SEPARATOR_CHAR + " gene" + SEPARATOR_CHAR + " gene organism" + SEPARATOR_CHAR
					+ " mutations (with codons)" + SEPARATOR_CHAR + " HIS tag" + SEPARATOR_CHAR + " manually checked"
					+ SEPARATOR_CHAR + " comments" + SEPARATOR_CHAR + " researcher" + SEPARATOR_CHAR + " date"
					+ SEPARATOR_CHAR + " average quality (percent)" + SEPARATOR_CHAR + " percentage of quality trim"
