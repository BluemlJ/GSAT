...
/**
 * Class to store the analysis results in local files. This class produces comma separated value files (CSV)
 * which can be read with Excel.
 * 
 * @author Ben Kohr
 */
public class FileSaver {
  ...
  /**
   * List of all entries to be stored locally. These are the results of analyzing one or
   * multiple AB1 files.
   */
  ...
  /**
   * This is the standard file name if only one file is desired. (Otherwise, the files are named after the 
   * AB1 source files.)
   */
  ...
  /**
   * This value is needed to keep track of the momentarily used id of the data. One number corresponds to a 
   * single entry (not a single sequence!).
   */
  private static long id = 0;


  /**
   * Inserts the stored data entries into one or multiple local file(s). 
   * The name of the AB1 file is given as a parameter. It can be used to create the CSV file name.
   * 
   * @param filename the name of the AB1 file the stored entries were obtained from. If only one file is desired, then the name will not be used.
   * 
  ...
  /**
   * Puts a single entry in the waiting queue for being written into a file. 
   * It first sets it's id (which is currently not set).
   * 
   ...
   * (each file has it's own number range, starting with zero).
   * 
   * @see #resetIDs()
   * 
   * @author Ben Kohr
   ...
   * This is necessary to start a new writing process.
   * 
   * @author Ben Kohr
   */
  ...
  /**
   * This method creates a new writer for the current writing situation (one or several files?). 
   * If necessary, also adds the first line.
   * It is directly called when separate files are needed. It is called via {@link #getNewWriter()} if one file is desired.
   * 
   ...
   * setting {@link #firstCall} to true. This is necessary to start a completely new analyzing process.
   */
