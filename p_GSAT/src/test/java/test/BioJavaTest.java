...
public class BioJavaTest {
	...
	/**
	 * This test checks whether the BioJavaFramework 4.2 is usable (i.e. it is
	 * configured correctly) by referring to classes from the framework.
	 * 
	...
	public void testABIRead() throws IOException, IllegalSymbolException, IllegalAlphabetException, BioError {
		...
		File("/home/bluemlj/Dropbox/BP_GSAT/Materialien/Dateien/Bsp/AB/93GH02_A04.ab1");
		...
		// The map has no particular order and so cannot be relied on to iterate
		// over records in the
		// same order they were read from the file.
		/*
		 * Map<String,ABIFParser.TaggedDataRecord> dataMap =
		 * myParser.getAllDataRecords(); for
		 * (Map.Entry<String,ABIFParser.TaggedDataRecord> entry :
		 * dataMap.entrySet()) { System.out.println(entry.getKey() + ", " +
		 * entry.getValue()); }
		 */

		// scale has to be 2
		BufferedImage chromaImg = myTrace.getImage(1000, 2);
		File outputfile = new File("/home/bluemlj/Desktop/chroma.png");
	 ...
	 *             the file is not fount
	 * 
	 * @author bluemlj
	 */
	@Test
	public void testABIReadJillionBioJava() throws IOException {
		File myFile = new File("/home/bluemlj/Dropbox/BP_GSAT/Materialien/Dateien/Bsp/AB/93GH02_A04.ab1");
		ABITrace myTrace = new ABITrace(myFile);
		Chromatogram croma = ChromatogramFactory.create(myFile);
